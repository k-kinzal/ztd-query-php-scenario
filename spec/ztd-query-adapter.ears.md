...
- **UPDATE SET col = (subquery) — correlated subqueries fail on SQLite**: Non-correlated scalar subqueries in UPDATE SET clauses (e.g., `SET price = (SELECT MAX(price) FROM t)`) work correctly on all platforms. Correlated subqueries that reference the outer table being updated (e.g., `SET avg = (SELECT AVG(x) FROM t2 WHERE t2.fk = outer.id)`) fail with syntax error on SQLite because the CTE rewriter generates invalid SQL. On MySQL, correlated subqueries in SET clauses may succeed. Workaround: compute the subquery value separately, then UPDATE with a literal value. Verified on SQLite and MySQL.

- **Large dataset (100+ rows) in shadow store**: CTE-based shadow store handles 100+ rows correctly for all operations: INSERT, SELECT with aggregation (SUM, AVG, MIN, MAX), GROUP BY, UPDATE subset, DELETE subset, ORDER BY with LIMIT/OFFSET. No performance degradation or correctness issues observed. Verified on SQLite, MySQL, and PostgreSQL.
