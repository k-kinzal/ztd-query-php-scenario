...
- **UPDATE SET col = (subquery) — correlated subqueries fail**: Non-correlated scalar subqueries in UPDATE SET clauses (e.g., `SET price = (SELECT MAX(price) FROM t)`) work correctly. However, correlated subqueries that reference the outer table being updated (e.g., `SET avg = (SELECT AVG(x) FROM t2 WHERE t2.fk = outer.id)`) fail with syntax error because the CTE rewriter generates invalid SQL. Verified on SQLite. Workaround: compute the subquery value separately, then UPDATE with a literal value.
