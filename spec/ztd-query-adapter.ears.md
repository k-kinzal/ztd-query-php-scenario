...
- **INSERT...SELECT with LEFT JOIN + GROUP BY + aggregation**: The CTE rewriter cannot correctly transfer SELECT result values to inserted rows when the SELECT involves a JOIN with GROUP BY. On **SQLite and PostgreSQL**, the INSERT succeeds (correct affected row count) but all inserted column values are NULL. On **MySQL** (both MySQLi and PDO), the same query throws a column-not-found error because the CTE rewriter loses the JOIN alias references (e.g., "Unknown column 'o.id' in field list"). The workaround on all platforms is to SELECT the aggregated data first, then INSERT the results manually in application code.
