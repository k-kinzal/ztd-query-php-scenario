...
|-----------|----------------|
| PHP | 8.1 - 8.5 |
| MySQL | 5.6 - 9.1 (default: 8.0) |
| PostgreSQL | 14 - 18 (default: 16) |
| SQLite | 3 |
| ztd-query-pdo-adapter | ^0.1 |
| ztd-query-mysqli-adapter | ^0.1 |
...
- `[Unreported]` **Schema-qualified table names (PostgreSQL)**: When using `public.tablename` (or any `schema.tablename`) syntax, INSERT/UPDATE/DELETE work correctly — the mutation resolver strips the schema prefix and resolves to the unqualified table name in the shadow store. However, SELECT with schema-qualified names returns empty results because the CTE rewriter does not recognize `public.tablename` as a shadow table. The workaround is to use unqualified table names in SELECT queries. Mixed usage (INSERT via `public.tablename`, SELECT via `tablename`) works correctly.

- `[Unreported]` **execute_query vs prepare+bind_param for UPSERT/REPLACE (MySQLi)**: MySQLi `prepare()` + `bind_param()` + `execute()` correctly handles UPSERT (`ON DUPLICATE KEY UPDATE`) and `REPLACE INTO` — existing rows are updated/replaced as expected. However, `execute_query()` (PHP 8.2+), which internally calls `prepare()` + `execute($params)`, does NOT update/replace existing rows. This suggests the array-param `execute()` path differs from the `bind_param()` path for mutation processing. UPDATE and DELETE operations work correctly via both paths.

- `[By-Design]` **Multi-table UPDATE/DELETE syntax**: MySQL uses `UPDATE t1 JOIN t2 ON ... SET ...` and `DELETE t1 FROM t1 JOIN t2 ON ...`. PostgreSQL uses `UPDATE t1 SET ... FROM t2 WHERE ...` and `DELETE FROM t1 USING t2 WHERE ...`. Both syntaxes are supported by their respective platform adapters.

- `[Unreported]` **MySQL comma-syntax multi-table UPDATE**: MySQL's alternative comma syntax (`UPDATE t1, t2 SET ... WHERE ...`) is partially supported. The first table listed gets its columns updated, but the secondary table's columns may not be updated depending on the cross-table WHERE conditions. This contrasts with JOIN syntax (`UPDATE t1 JOIN t2 ON ... SET ...`) which works correctly. The `MultiUpdateMutation` processes the CTE-rewritten query and may not properly handle cross-table joins expressed via comma syntax. Users should prefer JOIN syntax for multi-table UPDATE operations.

- `[Issue #26]` **MySQL multi-target DELETE only affects first table**: When using `DELETE t1, t2 FROM t1 JOIN t2 ON ... WHERE ...`, only the first listed table has matching rows deleted. The second table is not affected. This is the same limitation as comma-syntax multi-table UPDATE — the `MultiDeleteMutation` is created but the CTE rewriter only processes the deletion against the primary table. Single-target DELETE with JOIN (`DELETE t1 FROM t1 JOIN t2 ON ... WHERE ...`) works correctly. Verified on both MySQLi and PDO adapters.

- `[Issue #27]` **SQLite ALTER TABLE RENAME TO drops shadow data**: On SQLite, `ALTER TABLE old_name RENAME TO new_name` drops the shadow for the old table name but does NOT create a new entry under the new name. After rename: (1) the old table name falls through to the physical DB, returning only physical rows; (2) the new table name is not accessible (throws PDOException); (3) any ZTD-inserted data is permanently lost. This is because `SqliteMutationResolver::resolveAlterRenameTable()` only creates a `DropTableMutation` for the old name. On MySQL, RENAME TABLE works correctly (shadow store is updated).
...
- `[Issue #32]` **PostgreSQL RETURNING clause not supported**: PostgreSQL's `RETURNING` clause on DML statements (`INSERT ... RETURNING`, `UPDATE ... RETURNING`, `DELETE ... RETURNING`, `INSERT ... ON CONFLICT ... RETURNING`) is not supported under ZTD. The CTE rewriter does not preserve the RETURNING clause, causing all such statements to fail. This affects all DML types. Workaround: execute the DML operation first, then run a separate SELECT to read the affected data.

- `[By-Design]` **TRUNCATE TABLE + re-insert workflow (MySQL)**: On MySQL, `TRUNCATE TABLE t` followed by new INSERTs works correctly in ZTD shadow mode. The `TruncateMutation` clears the shadow store, and subsequent inserts populate it cleanly. Multiple truncate-reinsert cycles work correctly, including re-using the same primary key values.
...
- `[Unreported]` **UNION/EXCEPT/INTERSECT with LIMIT/OFFSET**: UNION ALL and UNION (distinct) work correctly with LIMIT, OFFSET, and ORDER BY in shadow queries across all platforms. UNION reflects shadow mutations (INSERT/DELETE). However, EXCEPT and INTERSECT may return 0 rows with shadow data on some platforms because independently CTE-rewritten branches may have type/format mismatches that prevent row deduplication/matching. UNION ALL (which does not deduplicate) is not affected. Verified on SQLite and PostgreSQL.
...
- `[By-Design]` **DELETE with correlated subqueries**: `DELETE ... WHERE EXISTS (SELECT ...)`, `DELETE ... WHERE NOT EXISTS (SELECT ...)`, `DELETE ... WHERE col IN (SELECT ...)`, and `DELETE ... WHERE col > (SELECT ...)` all work correctly in shadow store. The correlated subquery correctly reflects the current shadow state. Verified on SQLite and MySQL (MySQLi).
...
- `[By-Design]` **ENUM column type in shadow store (MySQL)**: MySQL ENUM columns (`ENUM('active', 'inactive', 'pending')`) work correctly in ZTD shadow store. INSERT with valid ENUM values, UPDATE to different ENUM values, WHERE comparison against ENUM values, and NULL ENUM values all work correctly. Verified on MySQLi.
...
- `[By-Design]` **Prepared UPDATE with self-referencing arithmetic across platforms**: Prepared `UPDATE SET col = col + ?` with parameter binding works correctly on all platforms — increment, decrement, and conditional matching (WHERE counter >= ?) all produce correct results. Verified on SQLite, MySQL (PDO and MySQLi), and PostgreSQL.
...
- `[By-Design]` **Prepared pagination (LIMIT/OFFSET) after mutations**: Prepared SELECT with parameterized LIMIT/OFFSET correctly reflects shadow mutations. INSERT adds rows to pagination results, DELETE reduces total pages, and UPDATE with category change correctly alters filtered pagination. Verified on SQLite.

- `[By-Design]` **ZTD enable/disable cycle data persistence**: Shadow data (INSERT, UPDATE, DELETE) persists through disable/re-enable toggle cycles. Physical INSERT during disabled period is invisible after re-enabling (shadow replaces physical). Multiple toggle cycles correctly accumulate shadow data. Verified on SQLite.

- `[Issue #26]` **Multi-table DELETE (MySQL)**: MySQL multi-table DELETE syntax (`DELETE t1 FROM t1 JOIN t2 ON ... WHERE ...`) works correctly for single-target deletions — the joined table filters which rows to delete from the target. However, multi-target DELETE (`DELETE t1, t2 FROM t1 JOIN t2 ON ... WHERE ...`) only deletes from the **first** table; the second table retains its rows. This is a CTE rewriter limitation — tables are processed independently. Prepared multi-table DELETE with parameter binding also works correctly. Verified on MySQL (PDO and MySQLi).

- `[By-Design]` **HAVING without GROUP BY**: `SELECT COUNT(*) AS cnt FROM t HAVING COUNT(*) > N` (HAVING without GROUP BY) works correctly in shadow queries — returns the aggregate row when the condition matches, empty result when it doesn't. Verified on MySQL (PDO and MySQLi).

- `[By-Design]` **Wide table (20+ columns) in shadow store**: Tables with 20+ VARCHAR columns work correctly for INSERT (literal and prepared), SELECT *, UPDATE subset of columns, and field_count/columnCount reporting. The CTE rewriter handles all columns without truncation or loss. Verified on MySQL (PDO and MySQLi).

- `[By-Design]` **String/numeric edge cases in shadow store**: Empty strings vs NULL are correctly distinguished in shadow queries (`WHERE val IS NOT NULL` filters correctly). Long text values (10,000+ characters via LONGTEXT/TEXT) round-trip correctly through prepared statements. BIGINT boundary values (2,147,483,647, -2,147,483,648) are stored and retrieved correctly. Nested OR/AND conditions in WHERE clauses evaluate correctly. Verified on MySQL (PDO and MySQLi).

- `[By-Design]` **Advanced ORDER BY with CASE WHEN**: `ORDER BY CASE role WHEN 'admin' THEN 1 WHEN 'moderator' THEN 2 ELSE 3 END, name` works correctly in CTE-rewritten shadow queries, both in direct queries and prepared statements with parameters. Multiple interleaved prepared statements (prepare two, execute alternately) maintain independent result sets. Verified on MySQL (PDO and MySQLi).
...
- `[Unreported]` **EXCEPT/INTERSECT on MySQL**: On MySQL (both MySQLi and PDO adapters), `EXCEPT` and `INTERSECT` throw exceptions because the CTE rewriter misparses them as multi-statement queries. UNION works correctly. Workaround: use `NOT IN` subquery for EXCEPT semantics and `IN` subquery for INTERSECT semantics. Verified on MySQL (PDO and MySQLi).
...
- `[By-Design]` **SAVEPOINT commands not supported (cross-platform)**: SAVEPOINT, RELEASE SAVEPOINT, and ROLLBACK TO SAVEPOINT are not supported by the CTE rewriter. On **SQLite** and **MySQL** (both PDO and MySQLi), all three throw exceptions. On **PostgreSQL**, SAVEPOINT/RELEASE/ROLLBACK TO pass through to the physical database without error, but the shadow store does NOT participate in savepoint semantics — shadow data persists even after ROLLBACK TO SAVEPOINT. Shadow data is unaffected by failed SAVEPOINT commands on all platforms. Verified on SQLite, MySQL (PDO and MySQLi), and PostgreSQL.

- `[By-Design]` **Behavior rule configuration**: `ZtdConfig::behaviorRules` accepts `array<string, UnsupportedSqlBehavior>` where keys are patterns. Prefix patterns match case-insensitively. Regex patterns (starting with `/`) use `preg_match()`. Rules are evaluated in order — first matching rule wins. Broad prefix rules should be placed AFTER specific rules to avoid shadowing. Shadow INSERT/SELECT operations continue to work correctly regardless of behavior rules. Verified on MySQL (PDO and MySQLi).

- `[By-Design]` **CREATE TABLE IF NOT EXISTS**: `CREATE TABLE IF NOT EXISTS` on an existing table is a no-op — existing shadow data is preserved. On a new table, it correctly creates the table for subsequent ZTD operations. `CREATE TABLE` (without IF NOT EXISTS) on an existing table throws an error. Verified on SQLite and MySQL (PDO and MySQLi).

- `[Unreported]` **INSERT...SELECT with computed columns (SQLite limitation)**: On SQLite, `INSERT...SELECT` with computed expressions (e.g., `price * 2`) inserts the correct number of rows but computed column values become NULL. Direct column references transfer correctly. Filtered WHERE clauses in the SELECT work correctly. Verified on SQLite.

- `[By-Design]` **LIKE with ESCAPE clause**: LIKE pattern matching with `ESCAPE` character works correctly in CTE-rewritten shadow queries. `LIKE '%!%%' ESCAPE '!'` correctly matches literal `%` characters. `LIKE '%!_%' ESCAPE '!'` correctly matches literal `_` characters. LIKE with prepared statement parameter binding also works. Verified on SQLite.

- `[By-Design]` **Explicit NULL vs omitted columns**: Both explicit `INSERT INTO t (id, name) VALUES (1, NULL)` and column omission `INSERT INTO t (id) VALUES (1)` result in NULL in the shadow store. Database DEFAULT values are NOT applied because ZTD rewrites INSERT to CTE-based SELECT. `IS NULL` / `IS NOT NULL` comparisons work correctly for both cases. Verified on SQLite.

- `[By-Design]` **SELECT...FOR UPDATE locking clauses (cross-platform)**: On **MySQL** (PDO and MySQLi), `SELECT...FOR UPDATE` and `SELECT...FOR SHARE` (or `LOCK IN SHARE MODE`) work correctly — the locking clause is preserved in the CTE-rewritten query but is effectively a no-op since data comes from the CTE shadow. On **PostgreSQL**, additional locking modes `FOR NO KEY UPDATE` and `FOR KEY SHARE` also work. On **SQLite**, `FOR UPDATE` is not valid syntax and throws an exception — regular SELECT should be used instead. Verified on all platforms.

- `[By-Design]` **TRUNCATE TABLE with reinsert cycles (cross-platform)**: `TRUNCATE TABLE` correctly clears the shadow store on **MySQL** (PDO and MySQLi) and **PostgreSQL**. INSERT after TRUNCATE works correctly, and multiple truncate-reinsert cycles produce correct results. Physical isolation is maintained — `disableZtd()` after TRUNCATE + INSERT shows 0 physical rows. Verified on MySQL (PDO and MySQLi) and PostgreSQL.

- `[By-Design]` **DDL mid-session (DROP/CREATE TABLE)**: DROP TABLE during an active ZTD session clears the shadow store for that table. DROP TABLE followed by CREATE TABLE (re-creation) allows fresh shadow operations. Shadow data persists correctly across DDL operations on other tables. Verified on MySQL (MySQLi).

- `[By-Design]` **REST API / controller-layer query patterns**: Common patterns used in REST API controllers — single-record lookup with prepared statements, non-existent record returning NULL, multi-table JOIN with category filtering, LIKE search with parameters, and full CRUD lifecycle (INSERT, UPDATE, SELECT, DELETE) — all work correctly through ZTD shadow store with both query() and prepare()/bind_param()/execute() paths. Verified on MySQL (MySQLi).

- `[By-Design]` **GROUPING SETS / CUBE / ROLLUP (cross-platform)**: On **PostgreSQL**, GROUPING SETS, ROLLUP, and CUBE all work correctly through CTE-rewritten shadow queries. The GROUPING() function also works to distinguish NULL from subtotal rows. On **MySQL** (PDO and MySQLi), `GROUP BY ... WITH ROLLUP` works correctly (MySQL's native syntax); GROUPING SETS and CUBE are not supported by MySQL. On **SQLite**, GROUPING SETS, ROLLUP, and CUBE all throw exceptions (not supported by SQLite). Shadow mutations are correctly reflected in ROLLUP results. Verified on all 3 platforms.

- `[By-Design]` **Standalone VALUES expression not supported**: On PostgreSQL, `VALUES (1, 'a'), (2, 'b')` as a standalone query is valid SQL but the ZTD rewriter treats it as unsupported SQL and throws. VALUES within INSERT statements and VALUES in subqueries/derived tables work correctly. Verified on PostgreSQL.

- `[By-Design]` **VALUES in derived table / subquery**: On PostgreSQL, VALUES used as a derived table (`SELECT * FROM (VALUES (...)) AS v(col1, col2)`) works correctly through ZTD. JOIN with VALUES table expression also works, though the VALUES portion reads from the CTE shadow. Verified on PostgreSQL.

- `[By-Design]` **PostgreSQL :: cast operator**: PostgreSQL-specific double-colon cast syntax (`score::TEXT`, `(expr::NUMERIC * 1.5)::INT`) works correctly in CTE-rewritten shadow queries. Multiple chained casts in a single expression also work. Verified on PostgreSQL.

- `[By-Design]` **CTE MATERIALIZED / NOT MATERIALIZED hints (PostgreSQL)**: PostgreSQL 12+ CTE optimization hints (`AS MATERIALIZED`, `AS NOT MATERIALIZED`) are overwritten by the ZTD CTE rewriter. The user CTE name becomes undefined, resulting in empty result sets or errors. The workaround is to use inline subqueries instead of CTEs with optimization hints. Regular queries without CTE hints work normally. Verified on PostgreSQL.

- `[By-Design]` **PostgreSQL sequences (nextval/currval)**: Sequence functions (`nextval('seq')`, `currval('seq')`) work correctly in standalone SELECT queries through ZTD — they increment the physical sequence. INSERT with `nextval()` for auto-incrementing IDs also works. SERIAL columns (which use sequences internally) may work for basic inserts but DEFAULT values are not applied by the shadow store. Verified on PostgreSQL.

- `[By-Design]` **Composite primary key UPSERT (cross-platform)**: On **PostgreSQL**, `INSERT ... ON CONFLICT (col1, col2) DO UPDATE SET ... = EXCLUDED.x` works correctly with composite primary keys — existing rows are updated, new rows are inserted. `ON CONFLICT DO NOTHING` silently skips duplicates. Multiple sequential upserts produce correct results. On **MySQL** (MySQLi), `INSERT ... ON DUPLICATE KEY UPDATE ... = VALUES(x)` works correctly with composite primary keys, including prepared statements with parameter binding. Verified on PostgreSQL and MySQL (MySQLi).

- `[By-Design]` **Three/four-table JOIN through shadow store**: Complex JOIN topologies involving 3 or 4 shadow tables (e.g., customers → orders → order_items → products) work correctly through CTE rewriting. GROUP BY with aggregation across multi-table JOINs produces correct results. LEFT JOIN with missing data (NULL for unmatched rows) works. Shadow mutations (INSERT into any of the joined tables) are correctly reflected in subsequent JOIN queries. Verified on MySQL (MySQLi).

- `[By-Design]` **Shadow store constraint enforcement**: The ZTD shadow store does NOT enforce database constraints: PRIMARY KEY uniqueness, UNIQUE constraints, NOT NULL constraints, and FOREIGN KEY constraints are all silently allowed. Duplicate PK inserts result in multiple rows with the same key. NULL values are allowed in NOT NULL columns. This applies to all platforms (SQLite, MySQL, PostgreSQL) and both adapters. CREATE TABLE on an existing table DOES throw (table existence is tracked by the shadow registry). Verified on SQLite.

- `[Issue #31]` **PostgreSQL INSERT DEFAULT VALUES and DEFAULT keyword**: Omitted columns in INSERT get NULL in shadow (not database DEFAULT). The `DEFAULT` keyword in VALUES and `INSERT ... DEFAULT VALUES` syntax may throw or produce NULL values. ZTD rewrites INSERT to CTE-based SELECT which doesn't evaluate database defaults. Verified on PostgreSQL.

- `[By-Design]` **Advanced window functions**: NTILE, PERCENT_RANK, CUME_DIST, NTH_VALUE, and FILTER clause on aggregates all work correctly in CTE-rewritten shadow queries. Multiple window functions with different PARTITION BY clauses in a single query work. Window function results correctly reflect shadow mutations (DELETE changes rankings). Verified on SQLite.

- `[Unreported]` **CREATE TABLE AS SELECT (CTAS) on SQLite**: CTAS (`CREATE TABLE new AS SELECT * FROM existing`) creates the table in the shadow registry, but the newly created table is NOT queryable — querying it throws "no such table" because the physical table doesn't exist and the CTE rewriter can't find it. The source table data is preserved. DROP → CREATE → INSERT cycles work correctly because the physical table is created by DDL passthrough. Verified on SQLite.

- `[Unreported]` **INSERT...SELECT with aggregate columns (SQLite limitation)**: On SQLite, `INSERT INTO summary SELECT category, COUNT(*), SUM(amount) FROM orders GROUP BY category` inserts the correct number of rows, but aggregate/computed columns (COUNT, SUM) become NULL. Only direct column references transfer correctly. This is consistent with the known INSERT...SELECT computed columns limitation. HAVING filters and ORDER BY LIMIT in INSERT...SELECT work correctly for row selection. Verified on SQLite.

- `[By-Design]` **NATURAL JOIN and USING clause**: NATURAL JOIN (joining on columns with the same name) and `JOIN ... USING (column)` both work correctly through CTE-rewritten shadow queries. Shadow mutations are correctly reflected in NATURAL JOIN results. CROSS JOIN also works correctly. Verified on SQLite.

- `[By-Design]` **MySQL PARTITION clause**: Partitioned tables work correctly for regular DML (INSERT, UPDATE, DELETE, SELECT) through ZTD. `SELECT FROM table PARTITION (partition_name)` and DML with explicit PARTITION clause may or may not be supported by the CTE rewriter — behavior varies. Regular DML without PARTITION clause works correctly on partitioned tables. Verified on MySQL PDO.

- `[Unreported]` **Multi-table UPDATE with 3+ tables (MySQL)**: Two-table UPDATE (`UPDATE t1 JOIN t2 ON ... SET t1.col = ...`) works correctly. Three-table UPDATE (`UPDATE t1 JOIN t2 JOIN t3 SET ...`) may or may not be supported depending on the CTE rewriter's ability to handle the additional JOINs. Only the first table in the SET clause is modified; other joined tables remain unchanged. Verified on MySQL (MySQLi).

- `[By-Design]` **Reserved SQL keywords as identifiers**: Column names that are reserved SQL keywords (`select`, `from`, `where`, `order`) work correctly when quoted (`"keyword"` in SQLite/PostgreSQL, backtick-quoted in MySQL). INSERT, UPDATE, DELETE, and prepared statements all handle quoted reserved keyword identifiers correctly. Verified on SQLite.

- `[By-Design]` **PostgreSQL UPDATE ... FROM syntax (multi-table update)**: PostgreSQL's multi-table update syntax (`UPDATE t1 SET col = t2.col FROM t2 WHERE t1.id = t2.id`) works correctly through CTE-rewritten shadow queries. Updates from a price lookup table, category-based discount calculations with arithmetic expressions, and unmatched-row preservation all work correctly. Physical isolation is maintained. Verified on PostgreSQL.

- `[By-Design]` **DELETE with aggregate subqueries**: DELETE with subquery conditions using aggregate functions works correctly through CTE shadow: `DELETE WHERE amount > (SELECT AVG(...))`, `DELETE WHERE NOT EXISTS (correlated subquery)`, simple WHERE conditions, and cascade-like multi-step DELETE (delete child rows, then parent). On **SQLite**, `DELETE WHERE col IN (SELECT ... GROUP BY ... HAVING COUNT(*) >= N)` may not be supported (same-table subquery reference limitation). Verified on SQLite.

- `[By-Design]` **CREATE TABLE AS SELECT (MySQL MySQLi)**: CTAS on MySQL via the MySQLi adapter works fully — created shadow table is queryable and populated with shadow data. CTAS with WHERE filter correctly limits copied rows. INSERT into a CTAS-created table works correctly (appends to shadow). Physical isolation is maintained — source table has no physical rows. Verified on MySQL (MySQLi).

- `[By-Design]` **Chained DDL → DML sequences**: Complex sequences of CREATE, DROP, INSERT, UPDATE, DELETE, and SELECT maintain shadow store consistency. INSERT → UPDATE cycles correctly update shadow data. DELETE → reinsert with same ID returns new values. DROP TABLE → CREATE TABLE → INSERT with same or different schema works. Multi-table operation sequences with JOINs across both tables produce correct aggregation results. Verified on SQLite.

- `[By-Design]` **INSERT IGNORE with multi-row and edge cases (MySQL)**: `INSERT IGNORE INTO t VALUES (dup_id, ...)` silently skips duplicate rows while inserting non-duplicate rows. Multi-row INSERT IGNORE correctly inserts new rows and skips duplicates in the same statement. `ON DUPLICATE KEY UPDATE counter = counter + 1` (self-referencing) loses the original value — counter becomes 0 instead of incrementing. `ON DUPLICATE KEY UPDATE name = VALUES(name)` (using VALUES function) works correctly. Verified on MySQL PDO.
...
- `[By-Design]` **PostgreSQL DISTINCT ON**: `SELECT DISTINCT ON (column) ...` works correctly through CTE-rewritten shadow queries, returning the first row per distinct value ordered by the specified ORDER BY. DISTINCT ON correctly reflects shadow mutations (INSERT). Verified on PostgreSQL.

- `[By-Design]` **FETCH FIRST N ROWS ONLY (SQL standard LIMIT)**: PostgreSQL's SQL-standard pagination syntax (`FETCH FIRST N ROWS ONLY`, `OFFSET M ROWS FETCH NEXT N ROWS ONLY`) works correctly through CTE-rewritten shadow queries. Verified on PostgreSQL.
...
- `[Unreported]` **INSERT...SELECT with UNION ALL on MySQL**: `INSERT INTO t SELECT ... UNION ALL SELECT ...` is incorrectly detected as multi-statement SQL by the MySQL CTE rewriter and throws `UnsupportedSqlException`. This is the same parser issue as with EXCEPT/INTERSECT. `INSERT...SELECT` without UNION works correctly. Workaround: perform multiple individual INSERT...SELECT statements. Verified on MySQL (MySQLi).

- `[Unreported]` **PostgreSQL date/time functions through CTE shadow**: `TO_CHAR(date_column::DATE, 'YYYY-MM')` works correctly for date formatting. `EXTRACT(YEAR FROM date_column::DATE)` returns 0 for shadow-stored dates (known limitation — dates stored as strings). Date comparison (`>=`, `<`) with `::DATE` casting works correctly. `CURRENT_DATE` in SELECT alongside shadow data works. Date-based ORDER BY works correctly. Verified on PostgreSQL.
...
- `[By-Design]` **Conditional INSERT patterns (MySQL MySQLi)**: `INSERT...SELECT WHERE NOT EXISTS` (conditional insert) works correctly — inserts when no match, skips when duplicate exists. `INSERT...SELECT` with WHERE condition on source table works correctly for cross-table insertions. Verified on MySQL (MySQLi).

## Test Architecture

Scenario traits (`tests/Scenarios/`) contain platform-independent test logic shared across MySQL, PostgreSQL, SQLite (PDO), and MySQLi. Abstract base classes (`tests/Support/`) handle platform-specific setup and connection management. Platform-specific tests remain in their respective directories.
