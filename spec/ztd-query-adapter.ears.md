...
- **Multi-table DELETE (MySQL)**: MySQL multi-table DELETE syntax (`DELETE t1 FROM t1 JOIN t2 ON ... WHERE ...`) works correctly for single-target deletions — the joined table filters which rows to delete from the target. However, multi-target DELETE (`DELETE t1, t2 FROM t1 JOIN t2 ON ... WHERE ...`) only deletes from the **first** table; the second table retains its rows. This is a CTE rewriter limitation — tables are processed independently. Prepared multi-table DELETE with parameter binding also works correctly. Verified on MySQL (PDO and MySQLi).

- **HAVING without GROUP BY**: `SELECT COUNT(*) AS cnt FROM t HAVING COUNT(*) > N` (HAVING without GROUP BY) works correctly in shadow queries — returns the aggregate row when the condition matches, empty result when it doesn't. Verified on MySQL (PDO and MySQLi).

- **Wide table (20+ columns) in shadow store**: Tables with 20+ VARCHAR columns work correctly for INSERT (literal and prepared), SELECT *, UPDATE subset of columns, and field_count/columnCount reporting. The CTE rewriter handles all columns without truncation or loss. Verified on MySQL (PDO and MySQLi).
...
- **TRUNCATE TABLE with reinsert cycles (cross-platform)**: `TRUNCATE TABLE` correctly clears the shadow store on **MySQL** (PDO and MySQLi) and **PostgreSQL**. INSERT after TRUNCATE works correctly, and multiple truncate-reinsert cycles produce correct results. Physical isolation is maintained — `disableZtd()` after TRUNCATE + INSERT shows 0 physical rows. Verified on MySQL (PDO and MySQLi) and PostgreSQL.

- **DDL mid-session (DROP/CREATE TABLE)**: DROP TABLE during an active ZTD session clears the shadow store for that table. DROP TABLE followed by CREATE TABLE (re-creation) allows fresh shadow operations. Shadow data persists correctly across DDL operations on other tables. Verified on MySQL (MySQLi).
...
- **Standalone VALUES expression not supported**: On PostgreSQL, `VALUES (1, 'a'), (2, 'b')` as a standalone query is valid SQL but the ZTD rewriter treats it as unsupported SQL and throws. VALUES within INSERT statements and VALUES in subqueries/derived tables work correctly. Verified on PostgreSQL.

- **VALUES in derived table / subquery**: On PostgreSQL, VALUES used as a derived table (`SELECT * FROM (VALUES (...)) AS v(col1, col2)`) works correctly through ZTD. JOIN with VALUES table expression also works, though the VALUES portion reads from the CTE shadow. Verified on PostgreSQL.

- **PostgreSQL :: cast operator**: PostgreSQL-specific double-colon cast syntax (`score::TEXT`, `(expr::NUMERIC * 1.5)::INT`) works correctly in CTE-rewritten shadow queries. Multiple chained casts in a single expression also work. Verified on PostgreSQL.
...
- **INSERT...SELECT with aggregate columns (SQLite limitation)**: On SQLite, `INSERT INTO summary SELECT category, COUNT(*), SUM(amount) FROM orders GROUP BY category` inserts the correct number of rows, but aggregate/computed columns (COUNT, SUM) become NULL. Only direct column references transfer correctly. This is consistent with the known INSERT...SELECT computed columns limitation. HAVING filters and ORDER BY LIMIT in INSERT...SELECT work correctly for row selection. Verified on SQLite.

- **NATURAL JOIN and USING clause**: NATURAL JOIN (joining on columns with the same name) and `JOIN ... USING (column)` both work correctly through CTE-rewritten shadow queries. Shadow mutations are correctly reflected in NATURAL JOIN results. CROSS JOIN also works correctly. Verified on SQLite.
...
- **Multi-table UPDATE with 3+ tables (MySQL)**: Two-table UPDATE (`UPDATE t1 JOIN t2 ON ... SET t1.col = ...`) works correctly. Three-table UPDATE (`UPDATE t1 JOIN t2 JOIN t3 SET ...`) may or may not be supported depending on the CTE rewriter's ability to handle the additional JOINs. Only the first table in the SET clause is modified; other joined tables remain unchanged. Verified on MySQL (MySQLi).

- **Reserved SQL keywords as identifiers**: Column names that are reserved SQL keywords (`select`, `from`, `where`, `order`) work correctly when quoted (`"keyword"` in SQLite/PostgreSQL, backtick-quoted in MySQL). INSERT, UPDATE, DELETE, and prepared statements all handle quoted reserved keyword identifiers correctly. Verified on SQLite.
