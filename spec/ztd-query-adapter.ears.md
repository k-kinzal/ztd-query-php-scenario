...
- ZTD toggle error resilience: errors during ZTD-enabled or ZTD-disabled operations do not corrupt the shadow store. Shadow data persists through toggle cycles even after errors. Prepared statements created with ZTD enabled survive toggle cycles (retain their CTE-rewritten form). Shadow-only tables (created via ZTD CREATE TABLE) are not accessible when ZTD is disabled (throw PDOException "no such table"). Verified on SQLite.
