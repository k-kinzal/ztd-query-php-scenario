 ...
 * prepared statement for key lookup, SUM aggregate for usage totals (MySQLi).
 * @spec SPEC-10.2.148
 */
class ApiKeyManagementTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_ak_api_clients (
    ...
    {
        return ['mi_ak_api_usage', 'mi_ak_api_keys', 'mi_ak_api_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Clients
        $this->mysqli->query("INSERT INTO mi_ak_api_clients VALUES (1, 'Acme Corp', 'enterprise', 10000, '2025-01-01')");
        $this->mysqli->query("INSERT INTO mi_ak_api_clients VALUES (2, 'StartupXYZ', 'pro', 1000, '2025-03-15')");
        $this->mysqli->query("INSERT INTO mi_ak_api_clients VALUES (3, 'HobbyDev', 'free', 100, '2025-06-01')");

        // API Keys
        $this->mysqli->query("INSERT INTO mi_ak_api_keys VALUES (1, 1, 'sk_live_acme1', 'active', '2025-01-01', '2026-01-01')");
        $this->mysqli->query("INSERT INTO mi_ak_api_keys VALUES (2, 1, 'sk_live_acme2', 'revoked', '2025-01-01', '2026-01-01')");
        $this->mysqli->query("INSERT INTO mi_ak_api_keys VALUES (3, 2, 'sk_live_start1', 'active', '2025-03-15', '2025-12-31')");
        $this->mysqli->query("INSERT INTO mi_ak_api_keys VALUES (4, 3, 'sk_live_hobby1', 'active', '2025-06-01', '2025-12-31')");
        $this->mysqli->query("INSERT INTO mi_ak_api_keys VALUES (5, 3, 'sk_live_hobby2', 'expired', '2025-06-01', '2025-09-01')");

        // Usage — Key 1 (Acme active), 2025-10-01
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (1, 1, '2025-10-01', '/api/v1/users', 200, 45)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (2, 1, '2025-10-01', '/api/v1/users', 200, 52)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (3, 1, '2025-10-01', '/api/v1/orders', 201, 120)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (4, 1, '2025-10-01', '/api/v1/orders', 200, 88)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (5, 1, '2025-10-01', '/api/v1/health', 200, 12)");

        // Usage — Key 1, 2025-10-02
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (6, 1, '2025-10-02', '/api/v1/users', 200, 38)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (7, 1, '2025-10-02', '/api/v1/orders', 500, 250)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (8, 1, '2025-10-02', '/api/v1/orders', 200, 95)");

        // Usage — Key 3 (Startup active), 2025-10-01
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (9, 3, '2025-10-01', '/api/v1/users', 200, 65)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (10, 3, '2025-10-01', '/api/v1/products', 200, 78)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (11, 3, '2025-10-01', '/api/v1/products', 200, 82)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (12, 3, '2025-10-01', '/api/v1/health', 200, 15)");

        // Usage — Key 3, 2025-10-02
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (13, 3, '2025-10-02', '/api/v1/users', 200, 55)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (14, 3, '2025-10-02', '/api/v1/products', 200, 70)");

        // Usage — Key 4 (Hobby active), 2025-10-01
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (15, 4, '2025-10-01', '/api/v1/users', 200, 110)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (16, 4, '2025-10-01', '/api/v1/users', 200, 105)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (17, 4, '2025-10-01', '/api/v1/health', 200, 20)");

        // Usage — Key 5 (Hobby expired), 2025-09-15 (historical)
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (18, 5, '2025-09-15', '/api/v1/users', 200, 90)");
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (19, 5, '2025-09-15', '/api/v1/health', 200, 25)");
    }
    ...
    public function testKeyStatusSummaryByClient(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, k.status, COUNT(*) AS key_count
             FROM mi_ak_api_clients c
             JOIN mi_ak_api_keys k ON c.id = k.client_id
    ...
    public function testQuotaUtilizationByClient(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, c.daily_quota,
             ...
             FROM mi_ak_api_clients c
             JOIN mi_ak_api_keys k ON c.id = k.client_id
             JOIN mi_ak_api_usage u ON k.id = u.key_id AND u.request_date = '2025-10-01'
    ...
    public function testAverageResponseTimeByTier(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.tier,
             ...
             FROM mi_ak_api_clients c
             JOIN mi_ak_api_keys k ON c.id = k.client_id
             JOIN mi_ak_api_usage u ON k.id = u.key_id
             GROUP BY c.tier
             ORDER BY c.tier"
        );
    ...
    public function testPreparedUsageByKeyPrefix(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT u.id, u.request_date, u.endpoint, u.response_code, u.response_time_ms
             FROM mi_ak_api_usage u
             JOIN mi_ak_api_keys k ON u.key_id = k.id
             WHERE k.key_prefix = ?
             ORDER BY u.id",
            ['sk_live_acme1']
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_ak_api_usage VALUES (20, 1, '2025-10-03', '/api/v1/health', 200, 10)");

        // ZTD sees the new record
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_ak_api_usage");
        $this->assertEquals(20, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_ak_api_usage');
