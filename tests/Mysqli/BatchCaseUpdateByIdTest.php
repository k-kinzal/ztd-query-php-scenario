 ...
 * @spec SPEC-4.2
 */
class BatchCaseUpdateByIdTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_bcu_users (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_bcu_users VALUES (1, 'Alice', 'user', 10)");
        $this->mysqli->query("INSERT INTO mi_bcu_users VALUES (2, 'Bob', 'user', 20)");
        $this->mysqli->query("INSERT INTO mi_bcu_users VALUES (3, 'Carol', 'user', 30)");
        $this->mysqli->query("INSERT INTO mi_bcu_users VALUES (4, 'Dave', 'user', 40)");
    }
    ...
    public function testBatchUpdateSingleColumn(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, name, role FROM mi_bcu_users ORDER BY id");
    ...
    public function testBatchUpdateMultipleColumns(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, role, score FROM mi_bcu_users ORDER BY id");
            $this->assertCount(4, $rows);
    ...
    public function testBatchUpdatePreparedWithParams(): void
    {
        try {
            ...
            $in1 = 1;
            $in2 = 2;
            $stmt->bind_param('isisii', $id1, $role1, $id2, $role2, $in1, $in2);
            $stmt->execute();

            $rows = $this->ztdQuery("SELECT id, role FROM mi_bcu_users ORDER BY id");
    ...
    public function testBatchUpdateWithoutWhere(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, score FROM mi_bcu_users ORDER BY id");
            $this->assertCount(4, $rows);
