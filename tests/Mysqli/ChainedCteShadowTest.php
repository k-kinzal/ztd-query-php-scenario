 ...
 * @spec SPEC-10.2
 */
class ChainedCteShadowTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_ccs_customers (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL
            )',
            'CREATE TABLE mi_ccs_orders (
                id INT PRIMARY KEY,
    ...
    {
        return ['mi_ccs_orders', 'mi_ccs_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Seed customers
        $this->ztdExec("INSERT INTO mi_ccs_customers (id, name) VALUES (1, 'Alice')");
        $this->ztdExec("INSERT INTO mi_ccs_customers (id, name) VALUES (2, 'Bob')");
        $this->ztdExec("INSERT INTO mi_ccs_customers (id, name) VALUES (3, 'Charlie')");

        // Seed orders
        $this->ztdExec("INSERT INTO mi_ccs_orders (id, customer_id, amount, status) VALUES (1, 1, 100.00, 'active')");
        $this->ztdExec("INSERT INTO mi_ccs_orders (id, customer_id, amount, status) VALUES (2, 1, 200.00, 'active')");
        $this->ztdExec("INSERT INTO mi_ccs_orders (id, customer_id, amount, status) VALUES (3, 2, 150.00, 'active')");
        $this->ztdExec("INSERT INTO mi_ccs_orders (id, customer_id, amount, status) VALUES (4, 2, 50.00, 'cancelled')");
        $this->ztdExec("INSERT INTO mi_ccs_orders (id, customer_id, amount, status) VALUES (5, 3, 300.00, 'active')");
    }
    ...
    public function testChainedCteThreeLevel(): void
    {
        try {
            $rows = $this->ztdQuery("
                ...
                FROM high_value hv
                JOIN mi_ccs_customers c ON hv.customer_id = c.id
    ...
    public function testCteAggregateToFilter(): void
    {
        try {
            $rows = $this->ztdQuery("
                ...
                JOIN mi_ccs_customers c ON aa.customer_id = c.id
                ORDER BY c.name
            ");
    ...
    public function testChainedCteAfterUpdate(): void
    {
        try {
            ...
            $this->ztdExec("UPDATE mi_ccs_orders SET status = 'cancelled' WHERE id = 1");
    ...
    public function testChainedCteAfterDelete(): void
    {
        try {
            ...
            $this->ztdExec("DELETE FROM mi_ccs_orders WHERE id = 5");
            ...
            $rows = $this->ztdQuery("
                WITH
                    ...
                    enriched AS (
                        ...
                        FROM active a
                        JOIN mi_ccs_customers c ON a.customer_id = c.id
                        WHERE a.total >= 150
                    )
                SELECT name, total
                FROM enriched
