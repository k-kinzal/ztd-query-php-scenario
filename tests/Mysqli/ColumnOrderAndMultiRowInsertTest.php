...
/**
 * Tests INSERT with non-DDL column ordering and multi-row VALUES on MySQLi.
 *
 ...
 * @spec SPEC-4.1
 */
class ColumnOrderAndMultiRowInsertTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_coi_items (
            id INT PRIMARY KEY,
            name VARCHAR(50),
            price DECIMAL(10,2),
            category VARCHAR(30)
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items (category, price, name, id) VALUES ('electronics', 9.99, 'Widget', 1)");

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items WHERE id = 1');
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items (id, name) VALUES (2, 'Gadget')");

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items WHERE id = 2');
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items (category, id) VALUES ('books', 3)");

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items WHERE id = 3');
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items (id, name, price, category) VALUES (1, 'A', 1.00, 'cat1')");
        $this->ztdExec("INSERT INTO mi_coi_items (category, id, name, price) VALUES ('cat2', 2, 'B', 2.00)");
        $this->ztdExec("INSERT INTO mi_coi_items (price, category, id, name) VALUES (3.00, 'cat3', 3, 'C')");

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items ORDER BY id');
    ...
    public function testPreparedWithReorderedColumns(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            'INSERT INTO mi_coi_items (price, id, name) VALUES (?, ?, ?)',
            [19.99, 10, 'PrepItem']
        );

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items WHERE id = 10');
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items (name, id, category) VALUES ('OldName', 5, 'cat')");
        $this->ztdExec("UPDATE mi_coi_items SET name = 'NewName' WHERE id = 5");

        $rows = $this->ztdQuery('SELECT * FROM mi_coi_items WHERE id = 5');
    ...
    {
        try {
            $this->ztdExec("INSERT INTO mi_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
    ...
    public function testMultiRowInsertThenUpdateOne(): void
    {
        try {
            $this->ztdExec("INSERT INTO mi_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
            $this->ztdExec("UPDATE mi_coi_items SET price = 99.99 WHERE id = 2");
    ...
    public function testMultiRowInsertThenDeleteOne(): void
    {
        try {
            $this->ztdExec("INSERT INTO mi_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
            $this->ztdExec("DELETE FROM mi_coi_items WHERE id = 2");
    ...
    {
        try {
            $this->ztdExec("INSERT INTO mi_coi_items (category, name, id, price) VALUES ('X', 'First', 1, 5.00), ('Y', 'Second', 2, 15.00)");
    ...
    {
        try {
            $this->ztdExec("INSERT INTO mi_coi_items VALUES (1, 'X', 10.00, 'A'), (2, 'Y', 20.00, 'A'), (3, 'Z', 30.00, 'B')");
    ...
    {
        $this->ztdExec("INSERT INTO mi_coi_items VALUES (1, 'Test', 5.00, 'cat')");
        $this->disableZtd();
        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM mi_coi_items');
