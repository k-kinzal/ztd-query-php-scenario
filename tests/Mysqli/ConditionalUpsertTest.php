...
/**
 * Tests upsert behavior on MySQL via MySQLi, documenting the behavioral
 * difference from PostgreSQL/SQLite conditional upsert.
 *
 ...
 * @spec SPEC-4.2
 */
class ConditionalUpsertTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_condup_products (
            id INT PRIMARY KEY,
            name VARCHAR(100),
            price DECIMAL(10,2),
    ...
    public function testOnDuplicateKeyUpdateUpdatesUnconditionally(): void
    {
        try {
            // Seed with version 1
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Widget', 10.00, 1)"
            );

            // Upsert — always updates on conflict (no WHERE guard)
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Widget', 15.00, 2)
                 ON DUPLICATE KEY UPDATE price = VALUES(price), version = VALUES(version)"
            );

            $result = $this->mysqli->query('SELECT id, name, price, version FROM mi_condup_products WHERE id = 1');
    ...
    public function testOnDuplicateKeyUpdateAlwaysUpdatesNoWhereGuard(): void
    {
        try {
            // Seed with version 5
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Widget', 50.00, 5)"
            );
            ...
            $result = $this->mysqli->query('SELECT id, price, version FROM mi_condup_products WHERE id = 1');
            $rows = $result->fetch_all(MYSQLI_ASSOC);
    ...
    public function testPreparedOnDuplicateKeyUpdate(): void
    {
        try {
            // Seed
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Beta', 30.00, 1)"
            );

            $sql = "INSERT INTO mi_condup_products (id, name, price, version) VALUES (?, ?, ?, ?)
                    ON DUPLICATE KEY UPDATE price = VALUES(price), version = VALUES(version)";

            $stmt = $this->mysqli->prepare($sql);
            $stmt->bind_param('isdi', ...[$id = 1, $name = 'Beta', $price = 40.00, $version = 3]);
            $stmt->execute();
    ...
    public function testEmulatedConditionalUpsertWithPriceGuard(): void
    {
        try {
            ...
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Gamma', 100.00, 1)"
            );

            // Attempt with lower price — IF guard should prevent update
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Gamma', 50.00, 2)
                 ON DUPLICATE KEY UPDATE
            ...
            $this->mysqli->query(
                "INSERT INTO mi_condup_products (id, name, price, version) VALUES (1, 'Gamma', 150.00, 3)
                 ON DUPLICATE KEY UPDATE
                     price = IF(VALUES(price) > price, VALUES(price), price),
                     version = IF(VALUES(price) > price, VALUES(version), version)"
            );

            $result2 = $this->mysqli->query('SELECT id, price, version FROM mi_condup_products WHERE id = 1');
