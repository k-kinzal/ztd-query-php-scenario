 ...
 * overrides, and environment-specific settings through ZTD shadow store (MySQLi).
 * Covers cascade resolution, priority-based overrides, and effective config reports.
 * @spec SPEC-10.2.66
 */
class ConfigurationCascadeTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_cc_defaults (
                id INT PRIMARY KEY,
                config_key VARCHAR(255),
                config_value VARCHAR(255),
                description VARCHAR(255)
            )',
            'CREATE TABLE mi_cc_overrides (
                id INT PRIMARY KEY,
                config_key VARCHAR(255),
    ...
    {
        return ['mi_cc_overrides', 'mi_cc_defaults'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 5 default configs
        $this->mysqli->query("INSERT INTO mi_cc_defaults VALUES (1, 'app.name', 'MyApp', 'Application name')");
        $this->mysqli->query("INSERT INTO mi_cc_defaults VALUES (2, 'app.debug', 'false', 'Debug mode flag')");
        $this->mysqli->query("INSERT INTO mi_cc_defaults VALUES (3, 'db.host', 'localhost', 'Database host')");
        $this->mysqli->query("INSERT INTO mi_cc_defaults VALUES (4, 'db.port', '3306', 'Database port')");
        $this->mysqli->query("INSERT INTO mi_cc_defaults VALUES (5, 'cache.ttl', '3600', 'Cache time-to-live in seconds')");

        // 4 overrides
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (1, 'app.debug', 'staging', 'true', 10)");
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (2, 'app.debug', 'production', 'false', 10)");
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (3, 'db.host', 'production', 'db.prod.internal', 10)");
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (4, 'cache.ttl', 'staging', '60', 5)");
    }
    ...
    public function testOverrideResolution(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
             ...
             FROM mi_cc_defaults d
             LEFT JOIN mi_cc_overrides o ON o.config_key = d.config_key AND o.environment = ?
    ...
    public function testEnvironmentSpecificConfig(): void
    {
        $staging = $this->ztdPrepareAndExecute(
            "SELECT config_value FROM mi_cc_overrides
    ...
    public function testPriorityBasedOverride(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (5, 'cache.ttl', 'staging', '30', 20)");

        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
                    COALESCE(
                        (SELECT o.config_value FROM mi_cc_overrides o
    ...
    public function testAddOverride(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_cc_overrides VALUES (5, 'db.port', 'production', '5432', 10)");
    ...
    public function testRemoveOverrideRestoresDefault(): void
    {
        ...
        $this->mysqli->query("DELETE FROM mi_cc_overrides WHERE config_key = 'cache.ttl' AND environment = 'staging'");
