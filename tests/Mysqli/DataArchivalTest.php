...
/**
 * Tests a data archival workflow through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.104
 */
class DataArchivalTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_da_orders (
                id INT PRIMARY KEY,
            ...
            'CREATE TABLE mi_da_archived_orders (
                id INT PRIMARY KEY,
                customer VARCHAR(255),
                product VARCHAR(255),
                amount DECIMAL(10,2),
    ...
    {
        return ['mi_da_archived_orders', 'mi_da_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_da_orders VALUES (1, 'Alice', 'Widget', 29.99, 'completed', '2025-11-01', '2025-11-05')");
        $this->mysqli->query("INSERT INTO mi_da_orders VALUES (2, 'Bob', 'Gadget', 49.99, 'completed', '2025-12-01', '2025-12-10')");
        $this->mysqli->query("INSERT INTO mi_da_orders VALUES (3, 'Charlie', 'Widget', 29.99, 'completed', '2026-01-15', '2026-01-20')");
        $this->mysqli->query("INSERT INTO mi_da_orders VALUES (4, 'Alice', 'Gizmo', 99.99, 'pending', '2026-03-01', NULL)");
        $this->mysqli->query("INSERT INTO mi_da_orders VALUES (5, 'Diana', 'Widget', 29.99, 'shipped', '2026-03-05', NULL)");
    }

    /**
     * Archive old completed orders using INSERT SELECT.
     * On MySQL the computed column (literal '2026-03-09') works correctly in INSERT SELECT.
     */
    public function testArchiveCompletedOrders(): void
    {
        // Archive orders completed before 2026-01-01
        $this->mysqli->query(
            "INSERT INTO mi_da_archived_orders (id, customer, product, amount, status, created_at, completed_at, archived_at)
        ...
        $rows = $this->ztdQuery("SELECT id, customer, archived_at FROM mi_da_archived_orders ORDER BY id");
        $this->assertCount(2, $rows);
        $this->assertSame('Alice', $rows[0]['customer']);
        // MySQL correctly preserves literal values in INSERT SELECT
    ...
    public function testDeleteAfterArchival(): void
    {
        ...
        $this->mysqli->query(
            "DELETE FROM mi_da_orders WHERE status = 'completed' AND completed_at < '2026-01-01'"
        );
        $this->assertSame(2, $this->mysqli->lastAffectedRows());

        // Active table should have 3 remaining orders
        $rows = $this->ztdQuery("SELECT id FROM mi_da_orders ORDER BY id");
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_da_archived_orders");
        $this->assertEquals(2, (int) $rows[0]['cnt']);
    }

    /**
     * Total revenue across active and archived tables using UNION ALL in derived table.
     * MySQL supports UNION ALL in derived tables through ZTD.
     */
    public function testTotalRevenueAcrossTables(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT SUM(amount) AS total_revenue
             FROM (
                SELECT amount FROM mi_da_orders
                UNION ALL
                SELECT amount FROM mi_da_archived_orders
             ) AS combined"
        );
    ...
    public function testArchiveSummaryByProduct(): void
    {
        ...
        $this->mysqli->query("DELETE FROM mi_da_orders WHERE status = 'completed'");

        // Verify active table only has non-completed orders
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_da_orders");
        $this->assertEquals(2, (int) $rows[0]['cnt']);
    ...
    public function testCustomerHistoryAcrossTables(): void
    {
        ...
        $this->mysqli->query("DELETE FROM mi_da_orders WHERE status = 'completed' AND completed_at < '2026-01-01'");

        // Alice's history from both tables
        $rows = $this->ztdPrepareAndExecute(
            "SELECT id, product, amount, status, 'active' AS source
             FROM mi_da_orders WHERE customer = ?
             UNION ALL
             SELECT id, product, amount, status, 'archived' AS source
             FROM mi_da_archived_orders WHERE customer = ?
             ORDER BY id",
            ['Alice', 'Alice']
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_da_archived_orders');
