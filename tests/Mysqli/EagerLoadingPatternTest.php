 ...
 * @spec SPEC-3.1
 */
class EagerLoadingPatternTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_el_categories (id INT PRIMARY KEY, name VARCHAR(50), active TINYINT NOT NULL DEFAULT 1)',
            'CREATE TABLE mi_el_products (id INT PRIMARY KEY, category_id INT, name VARCHAR(50), price DECIMAL(10,2))',
            'CREATE TABLE mi_el_tags (id INT PRIMARY KEY, product_id INT, tag VARCHAR(50))',
        ];
    }

    protected function getTableNames(): array
    {
        return ['mi_el_tags', 'mi_el_products', 'mi_el_categories'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_el_categories VALUES (1, 'Electronics', 1)");
        $this->mysqli->query("INSERT INTO mi_el_categories VALUES (2, 'Books', 1)");
        $this->mysqli->query("INSERT INTO mi_el_categories VALUES (3, 'Clothing', 0)");

        $this->mysqli->query("INSERT INTO mi_el_products VALUES (1, 1, 'Laptop', 999.99)");
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (2, 1, 'Phone', 699.99)");
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (3, 2, 'Novel', 14.99)");
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (4, 2, 'Textbook', 89.99)");
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (5, 3, 'T-Shirt', 19.99)");

        $this->mysqli->query("INSERT INTO mi_el_tags VALUES (1, 1, 'portable')");
        $this->mysqli->query("INSERT INTO mi_el_tags VALUES (2, 1, 'premium')");
        $this->mysqli->query("INSERT INTO mi_el_tags VALUES (3, 2, 'portable')");
        $this->mysqli->query("INSERT INTO mi_el_tags VALUES (4, 3, 'fiction')");
        $this->mysqli->query("INSERT INTO mi_el_tags VALUES (5, 4, 'education')");
    }
    ...
    public function testBasicEagerLoadParentThenChildren(): void
    {
        $categories = $this->ztdQuery("
            SELECT id FROM mi_el_categories WHERE active = 1 ORDER BY id
        ");
        ...
        $products = $this->ztdQuery("
            SELECT id, category_id, name FROM mi_el_products
    ...
    public function testThreeLevelEagerLoad(): void
    {
        ...
        $products = $this->ztdQuery("
            SELECT id, name FROM mi_el_products
        ...
        $tags = $this->ztdQuery("
            SELECT product_id, tag FROM mi_el_tags
    ...
    {
        $this->mysqli->query("INSERT INTO mi_el_categories VALUES (4, 'Toys', 1)");
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (6, 4, 'Puzzle', 9.99)");
        ...
        $catIds = implode(',', array_column($categories, 'id'));
        $products = $this->ztdQuery("
            SELECT name FROM mi_el_products
            WHERE category_id IN ({$catIds})
            ORDER BY name
        ");
    ...
    public function testMutationBetweenEagerLoadSteps(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_el_products VALUES (6, 1, 'Tablet', 499.99)");
    ...
    public function testPreparedInClauseVaryingParamCounts(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name FROM mi_el_products WHERE category_id IN (?) ORDER BY name",
            [1]
        );
    ...
    public function testEagerLoadViaJoin(): void
    {
        $rows = $this->ztdQuery("
            ...
            FROM mi_el_categories c
            JOIN mi_el_products p ON p.category_id = c.id
            WHERE c.active = 1
    ...
    {
        $this->mysqli->query("UPDATE mi_el_categories SET active = 0 WHERE id = 2");
    ...
    public function testEagerLoadViaSubqueryIn(): void
    {
        $rows = $this->ztdQuery("
            ...
            WHERE category_id IN (
                SELECT id FROM mi_el_categories WHERE active = 1
            )
            ORDER BY name
        ");
        $this->assertCount(4, $rows);
    ...
    {
        $this->mysqli->query("DELETE FROM mi_el_categories WHERE id = 1");
