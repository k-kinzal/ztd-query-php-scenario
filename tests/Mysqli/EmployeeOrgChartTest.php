 ...
 * @spec SPEC-10.2.172
 */
class EmployeeOrgChartTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_eoc_employees (
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_eoc_employees VALUES (1, 'Alice', 'CEO', NULL)");
        $this->mysqli->query("INSERT INTO mi_eoc_employees VALUES (2, 'Bob', 'VP Engineering', 1)");
        $this->mysqli->query("INSERT INTO mi_eoc_employees VALUES (3, 'Carol', 'VP Sales', 1)");
        $this->mysqli->query("INSERT INTO mi_eoc_employees VALUES (4, 'Dave', 'Senior Engineer', 2)");
        $this->mysqli->query("INSERT INTO mi_eoc_employees VALUES (5, 'Eve', 'Sales Rep', 3)");
    }
    ...
    public function testSelfJoinEmployeeManager(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name, m.name AS manager_name
             FROM mi_eoc_employees e
             LEFT JOIN mi_eoc_employees m ON e.manager_id = m.id
             ORDER BY e.id"
        );
    ...
    public function testCountDirectReports(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name, COUNT(e.id) AS direct_reports
             FROM mi_eoc_employees m
             LEFT JOIN mi_eoc_employees e ON e.manager_id = m.id
    ...
    {
        $this->ztdExec(
            "INSERT INTO mi_eoc_employees VALUES (6, 'Frank', 'Engineer', 2)"
        );
    ...
    public function testSelfJoinWithPreparedStatement(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT e.name
             FROM mi_eoc_employees e
             JOIN mi_eoc_employees m ON e.manager_id = m.id
             WHERE m.role = ?
             ORDER BY e.name",
            ['CEO']
        );
    ...
    public function testSubqueryReferencesSameTable(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM mi_eoc_employees
             WHERE id IN (SELECT manager_id FROM mi_eoc_employees WHERE manager_id IS NOT NULL)
             ORDER BY name"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_eoc_employees");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_eoc_employees');
