 ...
 * ROUND function, and CASE-based grade boundaries (MySQLi).
 * @spec SPEC-10.2.129
 */
class GradebookWeightedAvgTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_gw_students (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                enrolled_date VARCHAR(20)
            )',
            'CREATE TABLE mi_gw_assignments (
                id INT AUTO_INCREMENT PRIMARY KEY,
                title VARCHAR(200),
                category VARCHAR(50),
                weight DECIMAL(5,2),
    ...
    {
        return ['mi_gw_grades', 'mi_gw_assignments', 'mi_gw_students'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Students
        $this->mysqli->query("INSERT INTO mi_gw_students VALUES (1, 'alice', '2025-09-01')");
        $this->mysqli->query("INSERT INTO mi_gw_students VALUES (2, 'bob', '2025-09-01')");
        $this->mysqli->query("INSERT INTO mi_gw_students VALUES (3, 'charlie', '2025-09-01')");
        $this->mysqli->query("INSERT INTO mi_gw_students VALUES (4, 'diana', '2025-09-01')");

        // Assignments
        $this->mysqli->query("INSERT INTO mi_gw_assignments VALUES (1, 'Midterm Exam', 'exam', 30.00, 100)");
        $this->mysqli->query("INSERT INTO mi_gw_assignments VALUES (2, 'Final Exam', 'exam', 40.00, 100)");
        $this->mysqli->query("INSERT INTO mi_gw_assignments VALUES (3, 'Homework 1', 'homework', 10.00, 50)");
        $this->mysqli->query("INSERT INTO mi_gw_assignments VALUES (4, 'Homework 2', 'homework', 10.00, 50)");
        $this->mysqli->query("INSERT INTO mi_gw_assignments VALUES (5, 'Lab Project', 'project', 10.00, 100)");

        // Grades — Alice (strong student)
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (1, 1, 1, 92.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (2, 1, 2, 88.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (3, 1, 3, 45.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (4, 1, 4, 48.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (5, 1, 5, 95.00)");

        // Grades — Bob (average student)
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (6, 2, 1, 75.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (7, 2, 2, 70.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (8, 2, 3, 35.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (9, 2, 4, 30.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (10, 2, 5, 65.00)");

        // Grades — Charlie (weak student, missing Lab Project)
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (11, 3, 1, 55.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (12, 3, 2, 48.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (13, 3, 3, 20.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (14, 3, 4, 25.00)");

        // Grades — Diana (excellent student)
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (15, 4, 1, 98.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (16, 4, 2, 95.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (17, 4, 3, 50.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (18, 4, 4, 49.00)");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (19, 4, 5, 100.00)");
    }
    ...
    public function testWeightedAverage(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name,
             ...
             FROM mi_gw_students s
             JOIN mi_gw_grades g ON g.student_id = s.id
             JOIN mi_gw_assignments a ON a.id = g.assignment_id
    ...
    public function testMissingAssignments(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name, a.title
             FROM mi_gw_students s
             CROSS JOIN mi_gw_assignments a
             LEFT JOIN mi_gw_grades g ON g.student_id = s.id AND g.assignment_id = a.id
             WHERE g.id IS NULL
            ...
            "SELECT a.category, s.name,
             ...
             HAVING AVG(g.score / a.max_score * 100.0) = (
                 SELECT MAX(sub_avg) FROM (
                     ...
                     FROM mi_gw_grades g2
                     JOIN mi_gw_assignments a2 ON a2.id = g2.assignment_id
    ...
    {
        $this->mysqli->query("INSERT INTO mi_gw_students VALUES (5, 'eve', '2025-09-01')");
        $this->mysqli->query("INSERT INTO mi_gw_grades VALUES (20, 5, 1, 80.00)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_gw_students");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_gw_grades");
        $this->assertEquals(20, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_gw_students');
