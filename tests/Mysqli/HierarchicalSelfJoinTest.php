 ...
 * @spec SPEC-3.3
 */
class HierarchicalSelfJoinTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_hsj_employees (
                id INT PRIMARY KEY,
                name VARCHAR(100),
                manager_id INT,
    ...
    protected function setUp(): void
    {
        ...
        // CEO (no manager)
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (1, 'Alice', NULL, 'Executive', 150000)");
        // VPs (report to CEO)
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (2, 'Bob', 1, 'Engineering', 120000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (3, 'Charlie', 1, 'Sales', 110000)");
        // Managers (report to VPs)
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (4, 'Diana', 2, 'Engineering', 90000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (5, 'Eve', 2, 'Engineering', 85000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (6, 'Frank', 3, 'Sales', 80000)");
        // Individual contributors (report to managers)
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (7, 'Grace', 4, 'Engineering', 70000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (8, 'Heidi', 4, 'Engineering', 72000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (9, 'Ivan', 5, 'Engineering', 68000)");
        $this->mysqli->query("INSERT INTO mi_hsj_employees VALUES (10, 'Judy', 6, 'Sales', 65000)");
    }
    ...
    public function testEmployeeWithManager(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name AS employee, m.name AS manager
             FROM mi_hsj_employees e
             LEFT JOIN mi_hsj_employees m ON e.manager_id = m.id
             ORDER BY e.name"
        );
    ...
    public function testEmployeeWithGrandmanager(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name AS employee, m.name AS manager, gm.name AS grandmanager
             FROM mi_hsj_employees e
             LEFT JOIN mi_hsj_employees m ON e.manager_id = m.id
             LEFT JOIN mi_hsj_employees gm ON m.manager_id = gm.id
    ...
    public function testDirectReportCount(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name AS manager, COUNT(e.id) AS direct_reports
             FROM mi_hsj_employees m
             JOIN mi_hsj_employees e ON e.manager_id = m.id
    ...
    public function testEmployeesEarningMoreThanManager(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_hsj_employees SET salary = 95000 WHERE id = 7");

        $rows = $this->ztdQuery(
            "SELECT e.name AS employee, e.salary AS emp_salary,
                    m.name AS manager, m.salary AS mgr_salary
             FROM mi_hsj_employees e
             JOIN mi_hsj_employees m ON e.manager_id = m.id
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, department FROM mi_hsj_employees WHERE manager_id IS NULL"
        );
    ...
    public function testFindLeafNodes(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name
             FROM mi_hsj_employees e
             WHERE NOT EXISTS (
                 SELECT 1 FROM mi_hsj_employees sub
    ...
    public function testPromoteAndReverify(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_hsj_employees SET manager_id = 1 WHERE id = 4");
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM mi_hsj_employees WHERE manager_id = 1"
        );
    ...
    public function testPreparedDirectReportsLookup(): void
    {
        $stmt = $this->mysqli->prepare(
            'SELECT name FROM mi_hsj_employees WHERE manager_id = ? ORDER BY name'
        );
