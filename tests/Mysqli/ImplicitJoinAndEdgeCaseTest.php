...
/**
 * Implicit comma JOIN and edge-case SQL patterns via MySQLi.
 *
 ...
 * @spec SPEC-3.3
 */
class ImplicitJoinAndEdgeCaseTest extends AbstractMysqliTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO ije_customers (id, name, city) VALUES (1, 'Alice', 'NYC'), (2, 'Bob', 'LA'), (3, 'Carol', 'Chicago')");
        $this->mysqli->query("INSERT INTO ije_orders (id, customer_id, amount, status) VALUES
    ...
    {
        try {
            $this->mysqli->query("INSERT INTO ije_orders (id, customer_id, amount, status) VALUES (6, 3, 125.00, 'completed')");
    ...
    {
        try {
            $this->mysqli->query("DELETE FROM ije_orders WHERE customer_id = 1");
    ...
    {
        $this->disableZtd();
        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM ije_customers");
        $rows = $result->fetch_all(MYSQLI_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical customers table should be empty');

        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM ije_orders");
        $rows = $result->fetch_all(MYSQLI_ASSOC);
