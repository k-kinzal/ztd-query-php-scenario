...
/**
 * Tests an IT incident management scenario through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.160
 */
class IncidentManagementTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_im_incidents (
            ...
            'CREATE TABLE mi_im_agents (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                team VARCHAR(255),
                tier INT
            )',
            'CREATE TABLE mi_im_assignments (
                id INT AUTO_INCREMENT PRIMARY KEY,
                incident_id INT,
                agent_id INT,
                assigned_at TEXT,
    ...
    {
        return ['mi_im_assignments', 'mi_im_agents', 'mi_im_incidents'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 6 incidents
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (1, 'Database connection timeout', 'critical', 'resolved', '2026-02-10', '2026-02-10', 'user_a')");
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (2, 'Login page 500 error', 'high', 'investigating', '2026-02-28', NULL, 'user_b')");
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (3, 'Slow report generation', 'medium', 'open', '2026-03-01', NULL, 'user_c')");
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (4, 'Email notifications delayed', 'low', 'resolved', '2026-02-15', '2026-02-17', 'user_a')");
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (5, 'Payment gateway failure', 'critical', 'closed', '2026-01-20', '2026-01-20', 'user_d')");
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (6, 'Dashboard widget broken', 'medium', 'open', '2026-03-08', NULL, 'user_b')");

        // 4 agents
        $this->mysqli->query("INSERT INTO mi_im_agents VALUES (1, 'Sarah', 'infrastructure', 1)");
        $this->mysqli->query("INSERT INTO mi_im_agents VALUES (2, 'James', 'application', 1)");
        $this->mysqli->query("INSERT INTO mi_im_agents VALUES (3, 'Li', 'infrastructure', 2)");
        $this->mysqli->query("INSERT INTO mi_im_agents VALUES (4, 'Nina', 'application', 2)");

        // 8 assignments
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (1, 1, 1, '2026-02-10 08:30', 'investigate', 'Checking connection pool')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (2, 1, 3, '2026-02-10 09:00', 'escalate', 'DB server OOM - restarted')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (3, 1, 1, '2026-02-10 10:00', 'resolve', 'Connection pool tuned')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (4, 2, 2, '2026-02-28 14:00', 'investigate', 'Reproducing on staging')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (5, 4, 4, '2026-02-15 10:00', 'investigate', 'Checking SMTP config')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (6, 4, 4, '2026-02-17 09:00', 'resolve', 'SMTP timeout increased')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (7, 5, 1, '2026-01-20 03:00', 'investigate', 'Gateway cert expired')");
        $this->mysqli->query("INSERT INTO mi_im_assignments VALUES (8, 5, 3, '2026-01-20 04:00', 'resolve', 'Certificate renewed')");
    }
    ...
    public function testAgentWorkload(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.name, COUNT(asgn.id) AS total_actions,
             ...
             FROM mi_im_agents a
             LEFT JOIN mi_im_assignments asgn ON asgn.agent_id = a.id
    ...
    public function testUnassignedIncidents(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.title, i.severity
             FROM mi_im_incidents i
             WHERE NOT EXISTS (SELECT 1 FROM mi_im_assignments asgn WHERE asgn.incident_id = i.id)
             ORDER BY i.id"
        );
    ...
    public function testPreparedTeamIncidents(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT DISTINCT i.title, i.severity, i.status
             FROM mi_im_assignments asgn
             JOIN mi_im_agents a ON a.id = asgn.agent_id
             JOIN mi_im_incidents i ON i.id = asgn.incident_id
             WHERE a.team = ?
             ORDER BY i.id",
            ['infrastructure']
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_im_incidents VALUES (7, 'Test incident', 'low', 'open', '2026-03-09', NULL, 'user_e')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_im_incidents");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_im_incidents');
