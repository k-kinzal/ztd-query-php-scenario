...
/**
 * Tests INSERT...SELECT with window functions on MySQLi.
 *
 ...
 * @spec SPEC-10.2.372
 */
class InsertSelectWindowTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE mi_isw_employees (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                salary DECIMAL(10,2) NOT NULL,
                dept VARCHAR(30) NOT NULL
            )",
            "CREATE TABLE mi_isw_ranked (
                id INT PRIMARY KEY,
                name VARCHAR(50),
                salary DECIMAL(10,2),
                dept VARCHAR(30),
    ...
    {
        return ['mi_isw_ranked', 'mi_isw_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (1, 'Alice', 90000, 'Engineering')");
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (2, 'Bob', 80000, 'Engineering')");
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (3, 'Carol', 95000, 'Engineering')");
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (4, 'Dave', 70000, 'Sales')");
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (5, 'Eve', 75000, 'Sales')");
        $this->mysqli->query("INSERT INTO mi_isw_employees (id, name, salary, dept) VALUES (6, 'Frank', 85000, 'Marketing')");
    }
    ...
    public function testInsertSelectRowNumberPartitioned(): void
    {
        try {
            $this->mysqli->query("
                INSERT INTO mi_isw_ranked (id, name, salary, dept, rank_in_dept)
                SELECT id, name, salary, dept,
                       ROW_NUMBER() OVER (PARTITION BY dept ORDER BY salary DESC)
                FROM mi_isw_employees
            ");
            $this->assertSame(6, $this->mysqli->lastAffectedRows());

            $rows = $this->ztdQuery("SELECT * FROM mi_isw_ranked ORDER BY dept, rank_in_dept");
    ...
    public function testInsertSelectRankOverSalary(): void
    {
        try {
            // Add a tie in salary
            $this->mysqli->query("UPDATE mi_isw_employees SET salary = 80000 WHERE id = 4");
            ...
            $rows = $this->ztdQuery("SELECT * FROM mi_isw_ranked ORDER BY rank_in_dept, name");
            $this->assertCount(6, $rows);
    ...
    public function testInsertSelectRunningTotalSumOver(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT * FROM mi_isw_ranked WHERE dept = 'Engineering' ORDER BY id");
    ...
    public function testTargetRowCountMatchesSource(): void
    {
        try {
            ...
            $sourceCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_isw_employees");
            $targetCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_isw_ranked");
    ...
    public function testInsertSelectWindowAfterSourceUpdate(): void
    {
        try {
            ...
            $this->mysqli->query("UPDATE mi_isw_employees SET salary = 120000 WHERE name = 'Carol'");
            // Demote Bob
            $this->mysqli->query("UPDATE mi_isw_employees SET salary = 60000 WHERE name = 'Bob'");
            ...
            $rows = $this->ztdQuery("SELECT * FROM mi_isw_ranked WHERE dept = 'Engineering' ORDER BY rank_in_dept");
    ...
    public function testInsertSelectWindowPreparedWithParam(): void
    {
        try {
            ...
            // Insert via prepared statement: first SELECT, then INSERT manually
            // because MySQLi prepared INSERT...SELECT with window is complex
            foreach ($rows as $row) {
                $this->mysqli->query("
                    INSERT INTO mi_isw_ranked (id, name, salary, dept, rank_in_dept)
                    VALUES ({$row['id']}, '{$row['name']}', {$row['salary']}, '{$row['dept']}', {$row['rnk']})
                ");
            }

            $result = $this->ztdQuery("SELECT * FROM mi_isw_ranked ORDER BY rank_in_dept");
            $this->assertCount(3, $result);
            $this->assertSame('Carol', $result[0]['name']);
            $this->assertSame(1, (int) $result[0]['rank_in_dept']);
        } catch (\Throwable $e) {
