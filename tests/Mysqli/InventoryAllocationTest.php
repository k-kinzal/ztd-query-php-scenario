...
/**
 * Tests an inventory allocation workflow through ZTD shadow store (MySQLi).
 * Covers stock reservation, sale conversion, capacity guards, warehouse reporting,
 * and physical isolation.
 * @spec SPEC-10.2.72
 */
class InventoryAllocationTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_ia_warehouses (
                id INT PRIMARY KEY,
                name VARCHAR(255),
                location VARCHAR(255)
            )',
            'CREATE TABLE mi_ia_products (
                id INT PRIMARY KEY,
                warehouse_id INT,
                sku VARCHAR(50),
                name VARCHAR(255),
            ...
            'CREATE TABLE mi_ia_reservations (
                id INT PRIMARY KEY,
                product_id INT,
    ...
    {
        return ['mi_ia_reservations', 'mi_ia_products', 'mi_ia_warehouses'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 warehouses
        $this->mysqli->query("INSERT INTO mi_ia_warehouses VALUES (1, 'Main Warehouse', 'New York')");
        $this->mysqli->query("INSERT INTO mi_ia_warehouses VALUES (2, 'West Coast', 'Los Angeles')");

        // Products: total_stock, reserved, sold
        $this->mysqli->query("INSERT INTO mi_ia_products VALUES (1, 1, 'SKU-001', 'Widget A', 100, 10, 20)");
        $this->mysqli->query("INSERT INTO mi_ia_products VALUES (2, 1, 'SKU-002', 'Widget B', 50, 5, 30)");
        $this->mysqli->query("INSERT INTO mi_ia_products VALUES (3, 2, 'SKU-001', 'Widget A', 80, 0, 10)");
        $this->mysqli->query("INSERT INTO mi_ia_products VALUES (4, 2, 'SKU-003', 'Gadget C', 200, 25, 50)");

        // Existing reservations
        $this->mysqli->query("INSERT INTO mi_ia_reservations VALUES (1, 1, 'Alice', 5, 'active', '2026-03-01 09:00:00')");
        $this->mysqli->query("INSERT INTO mi_ia_reservations VALUES (2, 1, 'Bob', 5, 'active', '2026-03-02 10:00:00')");
        $this->mysqli->query("INSERT INTO mi_ia_reservations VALUES (3, 4, 'Charlie', 25, 'active', '2026-03-03 11:00:00')");
    }
    ...
    public function testReserveStock(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_ia_reservations VALUES (4, 2, 'Diana', 3, 'active', '2026-03-04 09:00:00')");
        $this->mysqli->query("UPDATE mi_ia_products SET reserved = reserved + 3 WHERE id = 2");

        // Verify reservation exists
        $rows = $this->ztdQuery("SELECT quantity, status FROM mi_ia_reservations WHERE id = 4");
        $this->assertEquals(3, (int) $rows[0]['quantity']);
        $this->assertSame('active', $rows[0]['status']);

        // Verify reserved count updated
        $rows = $this->ztdQuery("SELECT reserved, (total_stock - reserved - sold) AS available FROM mi_ia_products WHERE id = 2");
    ...
    public function testConvertReservationToSale(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_ia_reservations SET status = 'converted' WHERE id = 1 AND status = 'active'");
        $this->assertSame(1, $this->mysqli->lastAffectedRows());

        $this->mysqli->query("UPDATE mi_ia_products SET reserved = reserved - 5, sold = sold + 5 WHERE id = 1");

        // Verify reservation converted
        $rows = $this->ztdQuery("SELECT status FROM mi_ia_reservations WHERE id = 1");
        ...
        $rows = $this->ztdQuery(
            "SELECT total_stock, reserved, sold, (total_stock - reserved - sold) AS available
             FROM mi_ia_products WHERE id = 1"
        );
    ...
    {
        $this->mysqli->query("UPDATE mi_ia_reservations SET status = 'cancelled' WHERE id = 2 AND status = 'active'");
        $this->assertSame(1, $this->mysqli->lastAffectedRows());

        $this->mysqli->query("UPDATE mi_ia_products SET reserved = reserved - 5 WHERE id = 1");
    ...
    public function testWarehouseStockReport(): void
    {
        $rows = $this->ztdQuery(
            "SELECT w.name AS warehouse,
             ...
             FROM mi_ia_warehouses w
             LEFT JOIN mi_ia_products p ON p.warehouse_id = w.id
    ...
    public function testStockLookupBySku(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT p.id, w.name AS warehouse, p.total_stock, p.reserved, p.sold,
             ...
             FROM mi_ia_products p
             JOIN mi_ia_warehouses w ON w.id = p.warehouse_id
             WHERE p.sku = ?
             ORDER BY w.name",
            ['SKU-001']
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_ia_reservations VALUES (4, 3, 'Eve', 10, 'active', '2026-03-05 09:00:00')");
        $this->mysqli->query("UPDATE mi_ia_products SET reserved = reserved + 10 WHERE id = 3");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_ia_reservations");
        $this->assertEquals(4, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_ia_reservations');
