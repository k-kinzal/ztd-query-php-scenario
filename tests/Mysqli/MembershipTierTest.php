...
/**
 * Tests a membership tier management scenario through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.161
 */
class MembershipTierTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_mt_tier_rules (
                id INT AUTO_INCREMENT PRIMARY KEY,
                tier_name VARCHAR(255),
                min_spending TEXT,
                benefit_pct TEXT
            )',
            'CREATE TABLE mi_mt_members (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                tier VARCHAR(255),
                joined_at TEXT
            )',
            'CREATE TABLE mi_mt_purchases (
                id INT AUTO_INCREMENT PRIMARY KEY,
                member_id INT,
                amount TEXT,
    ...
    {
        return ['mi_mt_purchases', 'mi_mt_members', 'mi_mt_tier_rules'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 4 tier rules
        $this->mysqli->query("INSERT INTO mi_mt_tier_rules VALUES (1, 'bronze', '0', '2')");
        $this->mysqli->query("INSERT INTO mi_mt_tier_rules VALUES (2, 'silver', '500', '5')");
        $this->mysqli->query("INSERT INTO mi_mt_tier_rules VALUES (3, 'gold', '1500', '10')");
        $this->mysqli->query("INSERT INTO mi_mt_tier_rules VALUES (4, 'platinum', '5000', '15')");

        // 5 members
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (1, 'Alice', 'bronze', '2025-01-15')");
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (2, 'Bob', 'silver', '2025-03-20')");
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (3, 'Carol', 'gold', '2024-06-01')");
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (4, 'Dave', 'bronze', '2025-11-10')");
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (5, 'Eve', 'platinum', '2024-01-01')");

        // 12 purchases
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (1, 1, '150.00', 'electronics', '2025-06-01')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (2, 1, '200.00', 'clothing', '2025-08-15')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (3, 1, '300.00', 'electronics', '2025-12-25')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (4, 2, '600.00', 'home', '2025-07-10')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (5, 2, '250.00', 'electronics', '2025-09-20')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (6, 3, '800.00', 'clothing', '2025-02-14')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (7, 3, '1200.00', 'home', '2025-05-01')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (8, 3, '500.00', 'electronics', '2025-11-11')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (9, 5, '3000.00', 'electronics', '2025-03-15')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (10, 5, '2500.00', 'home', '2025-06-30')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (11, 4, '100.00', 'clothing', '2026-01-05')");
        $this->mysqli->query("INSERT INTO mi_mt_purchases VALUES (12, 2, '180.00', 'clothing', '2025-12-01')");
    }
    ...
    public function testMemberSpendingByCategory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name, p.category, SUM(p.amount) AS total
             FROM mi_mt_members m
             JOIN mi_mt_purchases p ON p.member_id = m.id
    ...
    public function testMemberBenefits(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name, m.tier, COALESCE(t.benefit_pct, 0) AS benefit_pct
             FROM mi_mt_members m
             LEFT JOIN mi_mt_tier_rules t ON t.tier_name = m.tier
             ORDER BY m.name"
        );
    ...
    {
        $this->mysqli->query("UPDATE mi_mt_members SET tier = 'silver' WHERE id = 1");

        $rows = $this->ztdQuery("SELECT name, tier FROM mi_mt_members WHERE id = 1");
    ...
    public function testPreparedPurchaseHistory(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT m.name, p.amount, p.category, p.purchased_at
             FROM mi_mt_purchases p
             JOIN mi_mt_members m ON m.id = p.member_id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_mt_members VALUES (6, 'Frank', 'bronze', '2026-03-01')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_mt_members");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_mt_members');
