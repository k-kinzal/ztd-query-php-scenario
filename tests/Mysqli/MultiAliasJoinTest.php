 ...
 * @spec SPEC-3.3
 */
class MultiAliasJoinTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_maj_employees (
                id INT PRIMARY KEY,
                name VARCHAR(50),
                manager_id INT,
                mentor_id INT,
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (1, 'Alice',   NULL, NULL, 'exec', 200)");
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (2, 'Bob',     1,    NULL, 'exec', 150)");
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (3, 'Charlie', 2,    1,    'eng',  120)");
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (4, 'Diana',   2,    1,    'eng',  110)");
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (5, 'Eve',     3,    2,    'eng',  100)");
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (6, 'Frank',   3,    2,    'eng',   95)");
    }
    ...
    public function testSelfJoinTwoAliases(): void
    {
        $rows = $this->ztdQuery("
            ...
            FROM mi_maj_employees a
            JOIN mi_maj_employees b ON a.manager_id = b.id
            ORDER BY a.name
        ");
    ...
    public function testTripleSelfJoin(): void
    {
        $rows = $this->ztdQuery("
            SELECT
                ...
                m.name AS manager,
                t.name AS mentor
            FROM mi_maj_employees e
            LEFT JOIN mi_maj_employees m ON e.manager_id = m.id
            LEFT JOIN mi_maj_employees t ON e.mentor_id = t.id
            ORDER BY e.id
        ");
    ...
    /**
     * Self-join after UPDATE: change should be reflected through all aliases.
     */
    public function testSelfJoinAfterUpdate(): void
    {
        $this->mysqli->query("UPDATE mi_maj_employees SET name = 'Charles', salary = 160 WHERE id = 3");
    ...
    {
        $this->mysqli->query("INSERT INTO mi_maj_employees VALUES (7, 'Grace', 5, 3, 'eng', 90)");
    ...
    public function testSelfJoinWithAggregation(): void
    {
        $rows = $this->ztdQuery("
            ...
            FROM mi_maj_employees e
            JOIN mi_maj_employees m ON e.manager_id = m.id
    ...
    {
        $this->mysqli->query("DELETE FROM mi_maj_employees WHERE id = 3");
    ...
    public function testQuadrupleAliasSelfJoin(): void
    {
        $rows = $this->ztdQuery("
            ...
            FROM mi_maj_employees e
            LEFT JOIN mi_maj_employees m ON e.manager_id = m.id
            LEFT JOIN mi_maj_employees mm ON m.manager_id = mm.id
            LEFT JOIN mi_maj_employees t ON e.mentor_id = t.id
            WHERE e.id = 5
        ");
        $this->assertCount(1, $rows);
