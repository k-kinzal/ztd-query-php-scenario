...
/**
 * Tests multi-column IN clause with tuple syntax in DML operations on MySQLi.
 *
 ...
 * @spec SPEC-10.2
 */
class MultiColumnInDmlTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return "CREATE TABLE mi_mci_enrollments (
    ...
    protected function setUp(): void
    {
        ...
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (1, 101, 'B+', '2025-01-15')");
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (1, 102, 'A-', '2025-01-16')");
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (2, 101, 'C', '2025-02-01')");
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (2, 102, 'B', '2025-02-02')");
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (3, 101, 'A', '2025-03-01')");
        $this->ztdExec("INSERT INTO mi_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (3, 103, 'B-', '2025-03-10')");
    }
    ...
    {
        try {
            $this->ztdExec("DELETE FROM mi_mci_enrollments WHERE (student_id, course_id) IN ((1, 101), (2, 102))");

            $rows = $this->ztdQuery("SELECT student_id, course_id FROM mi_mci_enrollments ORDER BY student_id, course_id");
    ...
    {
        try {
            $this->ztdExec("UPDATE mi_mci_enrollments SET grade = 'A' WHERE (student_id, course_id) IN ((1, 101))");

            $rows = $this->ztdQuery("SELECT grade FROM mi_mci_enrollments WHERE student_id = 1 AND course_id = 101");
            ...
            $other = $this->ztdQuery("SELECT grade FROM mi_mci_enrollments WHERE student_id = 1 AND course_id = 102");
    ...
    public function testSelectWithMultiColumnInSubquery(): void
    {
        try {
            ...
            $this->ztdExec("UPDATE mi_mci_enrollments SET grade = 'A' WHERE student_id = 3");

            $rows = $this->ztdQuery(
                "SELECT student_id, course_id, grade FROM mi_mci_enrollments
                 WHERE (student_id, course_id) IN (
                     SELECT student_id, course_id FROM mi_mci_enrollments WHERE grade = 'A'
                 )
    ...
    {
        try {
            $stmt = $this->mysqli->prepare("DELETE FROM mi_mci_enrollments WHERE (student_id, course_id) IN ((?, ?))");
            $studentId = 1;
            $courseId = 101;
    ...
    public function testVerifyRemainingRowsAfterMultiColumnInDml(): void
    {
        try {
            ...
            $this->ztdExec("DELETE FROM mi_mci_enrollments WHERE (student_id, course_id) IN ((1, 101), (3, 103))");

            // Update one enrollment
            $this->ztdExec("UPDATE mi_mci_enrollments SET grade = 'A+' WHERE (student_id, course_id) IN ((2, 101))");

            $rows = $this->ztdQuery("SELECT student_id, course_id, grade FROM mi_mci_enrollments ORDER BY student_id, course_id");
