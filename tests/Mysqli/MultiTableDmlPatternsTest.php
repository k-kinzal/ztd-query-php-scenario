 ...
 * referencing other tables through the CTE rewriter.
 *
 ...
 * @spec SPEC-4.3
 */
class MultiTableDmlPatternsTest extends AbstractMysqliTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mtd_warehouses (id, name, active) VALUES
            (1, 'Main', 1), (2, 'Backup', 1), (3, 'Decommissioned', 0)");

        $this->mysqli->query("INSERT INTO mtd_inventory (id, product, stock, min_stock, warehouse_id) VALUES
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM mtd_warehouses");
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt'], 'mtd_warehouses should be empty');

        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM mtd_inventory");
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt'], 'mtd_inventory should be empty');

        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM mtd_restock_log");
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt'], 'mtd_restock_log should be empty');
    }
}
