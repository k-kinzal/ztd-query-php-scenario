 ...
 * @spec SPEC-3.7
 */
class NullInAggregatesTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_nia_scores (id INT PRIMARY KEY, student VARCHAR(255), subject VARCHAR(255), score INT)',
            'CREATE TABLE mi_nia_profiles (id INT PRIMARY KEY, name VARCHAR(255), city VARCHAR(255))',
        ];
    }

    protected function getTableNames(): array
    {
        return ['mi_nia_scores', 'mi_nia_profiles'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (1, 'Alice', 'Math', 90)");
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (2, 'Alice', 'English', NULL)");
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (3, 'Bob', 'Math', 80)");
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (4, 'Bob', 'English', NULL)");
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (5, 'Charlie', 'Math', NULL)");
        $this->mysqli->query("INSERT INTO mi_nia_scores VALUES (6, 'Charlie', 'English', NULL)");

        $this->mysqli->query("INSERT INTO mi_nia_profiles VALUES (1, 'Alice', 'NYC')");
        $this->mysqli->query("INSERT INTO mi_nia_profiles VALUES (2, 'Bob', NULL)");
        $this->mysqli->query("INSERT INTO mi_nia_profiles VALUES (3, 'Dave', 'LA')");
    }
    ...
    {
        $rows = $this->ztdQuery("SELECT COUNT(DISTINCT score) AS distinct_scores FROM mi_nia_scores");
    ...
    {
        $rows = $this->ztdQuery("
            SELECT MIN(score) AS min_score, MAX(score) AS max_score FROM mi_nia_scores
        ");
    ...
    public function testNullInJoinCondition(): void
    {
        ...
        $rows = $this->ztdQuery("
            ...
            FROM mi_nia_scores s
            LEFT JOIN mi_nia_profiles p ON s.student = p.name
    ...
    {
        $rows = $this->ztdQuery("
            SELECT COUNT(*) AS cnt FROM mi_nia_scores WHERE score BETWEEN 70 AND 100
        ");
    ...
    public function testNullComparisonNeverTrue(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_nia_scores WHERE score = NULL");
        $this->assertSame(0, (int) $rows[0]['cnt']);

        // NULL != value should not match either
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_nia_scores WHERE score != 90");
    ...
    public function testNullUpdateThenAggregate(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_nia_scores SET score = NULL WHERE id = 3");

        $rows = $this->ztdQuery("
            SELECT COUNT(score) AS non_null, COUNT(*) AS total FROM mi_nia_scores
        ");
    ...
    public function testPreparedNullInWhereIsNull(): void
    {
        ...
        $rows = $this->ztdPrepareAndExecute(
            "SELECT DISTINCT student FROM mi_nia_scores WHERE score IS NULL ORDER BY student",
            []
        );
