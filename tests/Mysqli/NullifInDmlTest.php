...
/**
 * Tests NULLIF function in DML operations (UPDATE SET, DELETE WHERE)
 ...
 * @spec SPEC-4.3
 */
class NullifInDmlTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_nid_products (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_nid_products VALUES (1, 'Widget', 'electronics', 100.00, 100.00)");
        $this->mysqli->query("INSERT INTO mi_nid_products VALUES (2, 'Gadget', 'electronics', 200.00, 150.00)");
        $this->mysqli->query("INSERT INTO mi_nid_products VALUES (3, 'Gizmo', 'toys', 50.00, 50.00)");
        $this->mysqli->query("INSERT INTO mi_nid_products VALUES (4, 'Doohickey', 'toys', 75.00, NULL)");
    }
    ...
    public function testUpdateSetNullif(): void
    {
        try {
            $this->mysqli->query(
                "UPDATE mi_nid_products SET discount_price = NULLIF(discount_price, price)"
            );

            $rows = $this->ztdQuery(
                "SELECT id, name, discount_price FROM mi_nid_products ORDER BY id"
            );
    ...
    public function testUpdateSetNullifPrepared(): void
    {
        try {
            $stmt = $this->mysqli->prepare(
                "UPDATE mi_nid_products SET discount_price = NULLIF(discount_price, ?) WHERE id = ?"
            );
            $val = 100.00;
            $id = 1;
            ...
            $rows = $this->ztdQuery("SELECT discount_price FROM mi_nid_products WHERE id = 1");
    ...
    public function testDeleteWhereNullifIsNull(): void
    {
        try {
            $this->mysqli->query(
                "DELETE FROM mi_nid_products WHERE NULLIF(discount_price, price) IS NULL"
            );

            $rows = $this->ztdQuery("SELECT name FROM mi_nid_products ORDER BY name");
    ...
    public function testDeleteWhereNullifIsNotNull(): void
    {
        try {
            $this->mysqli->query(
                "DELETE FROM mi_nid_products WHERE NULLIF(discount_price, price) IS NOT NULL"
            );
    ...
    public function testDeleteWhereNullifPrepared(): void
    {
        try {
            $stmt = $this->mysqli->prepare(
                "DELETE FROM mi_nid_products WHERE NULLIF(category, ?) IS NULL"
            );
            $cat = 'toys';
    ...
    public function testUpdateNullifWithWhereClause(): void
    {
        try {
            $this->mysqli->query(
                "UPDATE mi_nid_products
                 SET discount_price = NULLIF(discount_price, price)
                 WHERE category = 'electronics'"
            );

            $rows = $this->ztdQuery("SELECT id, discount_price FROM mi_nid_products ORDER BY id");
            $this->assertCount(4, $rows);
    ...
    public function testMultipleNullifInUpdateSet(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, name, category, discount_price FROM mi_nid_products ORDER BY id");
            $this->assertCount(4, $rows);
