 ...
 * with CASE expressions, SUM CASE cross-tab, and HAVING with arithmetic.
 ...
 * @spec SPEC-10.2.175
 */
class PayrollDeductionTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_pd_employee (
                id INT PRIMARY KEY,
                name VARCHAR(100),
                department VARCHAR(50),
                base_salary DECIMAL(10,2)
            )',
            'CREATE TABLE mi_pd_payroll (
                id INT PRIMARY KEY,
                employee_id INT,
                pay_period TEXT,
    ...
    {
        return ['mi_pd_deduction', 'mi_pd_payroll', 'mi_pd_employee'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Employees
        $this->mysqli->query("INSERT INTO mi_pd_employee VALUES (1, 'Alice', 'Engineering', 8000.00)");
        $this->mysqli->query("INSERT INTO mi_pd_employee VALUES (2, 'Bob', 'Engineering', 7000.00)");
        $this->mysqli->query("INSERT INTO mi_pd_employee VALUES (3, 'Carol', 'Sales', 6000.00)");
        $this->mysqli->query("INSERT INTO mi_pd_employee VALUES (4, 'Dave', 'Sales', 9000.00)");

        // Payroll runs for Jan 2025
        $this->mysqli->query("INSERT INTO mi_pd_payroll VALUES (1, 1, '2025-01', 8000.00, NULL, 'pending')");
        $this->mysqli->query("INSERT INTO mi_pd_payroll VALUES (2, 2, '2025-01', 7000.00, NULL, 'pending')");
        $this->mysqli->query("INSERT INTO mi_pd_payroll VALUES (3, 3, '2025-01', 6000.00, NULL, 'pending')");
        $this->mysqli->query("INSERT INTO mi_pd_payroll VALUES (4, 4, '2025-01', 9000.00, NULL, 'pending')");

        // Deductions: tax (20%), health (500 flat), retirement (5%)
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (1, 1, 'tax', 1600.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (2, 1, 'health', 500.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (3, 1, 'retirement', 400.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (4, 2, 'tax', 1400.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (5, 2, 'health', 500.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (6, 2, 'retirement', 350.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (7, 3, 'tax', 1200.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (8, 3, 'health', 500.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (9, 3, 'retirement', 300.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (10, 4, 'tax', 1800.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (11, 4, 'health', 500.00)");
        $this->mysqli->query("INSERT INTO mi_pd_deduction VALUES (12, 4, 'retirement', 450.00)");
    }

    /**
     * UPDATE SET multiple columns: net_pay = gross_pay * 0.75, status = 'processed'.
     * Tests updating both a computed arithmetic column and a literal in one statement.
     */
    public function testUpdateMultipleColumnsWithArithmetic(): void
    {
        $this->ztdExec(
            "UPDATE mi_pd_payroll SET net_pay = gross_pay * 0.75, status = 'processed'
             WHERE pay_period = '2025-01'"
        );

        $rows = $this->ztdQuery(
            "SELECT employee_id, gross_pay, net_pay, status
             FROM mi_pd_payroll ORDER BY employee_id"
        );
     ...
     * Pivots deduction types into columns using conditional aggregation.
     */
    public function testDeductionBreakdownCrossTab(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
             ...
             FROM mi_pd_employee e
             JOIN mi_pd_payroll p ON p.employee_id = e.id
             JOIN mi_pd_deduction d ON d.payroll_id = p.id
     ...
     * Tests HAVING SUM(col) > other_col * factor.
     */
    public function testHavingWithArithmeticExpression(): void
    {
        ...
        // All employees have deductions ~31-33% of gross, so all should be included
        // Alice: 2500/8000 = 31.3%, Bob: 2250/7000 = 32.1%, Carol: 2000/6000 = 33.3%, Dave: 2750/9000 = 30.6%
        $this->assertGreaterThanOrEqual(3, count($rows));
        // Carol has highest deduction percentage (33.3%)
        $this->assertSame('Carol', $rows[0]['name']);
        $this->assertEqualsWithDelta(33.3, (float) $rows[0]['deduction_pct'], 0.5);
    }

    /**
     * INSERT...SELECT with CASE expression: generate Feb payroll from employee base salary
     * with a CASE-based bonus for Engineering department.
     * Probes near SPEC-11.INSERT-SELECT-COMPUTED (computed columns via CASE).
     */
    public function testInsertSelectWithCaseExpression(): void
    {
        try {
            $this->mysqli->query(
                "INSERT INTO mi_pd_payroll (id, employee_id, pay_period, gross_pay, net_pay, status)
     ...
     * Tests derived table containing aggregate query (no window functions).
     */
    public function testDerivedTableWithGroupByHaving(): void
    {
        try {
            ...
            if (count($rows) === 0) {
                $this->markTestIncomplete(
                    'Derived table with GROUP BY + HAVING returned empty. '
    ...
    {
        $this->ztdExec("UPDATE mi_pd_payroll SET status = 'processed' WHERE id = 1");

        $rows = $this->ztdQuery("SELECT status FROM mi_pd_payroll WHERE id = 1");
        $this->assertSame('processed', $rows[0]['status']);

        $this->mysqli->disableZtd();
        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM mi_pd_payroll");
