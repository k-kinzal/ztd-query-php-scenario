 ...
 * @spec SPEC-10.2.34
 */
class PivotMultiLevelTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_pml_sales (
            id INT PRIMARY KEY,
            region VARCHAR(50),
            category VARCHAR(50),
            month VARCHAR(10),
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (1, 'North', 'Electronics', '2024-01', 1000)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (2, 'North', 'Electronics', '2024-02', 1200)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (3, 'North', 'Clothing', '2024-01', 500)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (4, 'South', 'Electronics', '2024-01', 800)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (5, 'South', 'Clothing', '2024-01', 600)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (6, 'South', 'Clothing', '2024-02', 700)");
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (7, 'North', NULL, '2024-01', 200)");
    }
    ...
    public function testPivotAfterInterleavedMutations(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_pml_sales VALUES (8, 'East', 'Electronics', '2024-01', 900)");

        $rows = $this->ztdQuery("
            SELECT region, SUM(amount) AS total
            FROM mi_pml_sales WHERE category = 'Electronics'
            GROUP BY region ORDER BY region
        ");
        $this->assertCount(3, $rows); // East, North, South

        // Update East's amount
        $this->mysqli->query("UPDATE mi_pml_sales SET amount = 950 WHERE id = 8");
    ...
    public function testPreparedPivotWithCategoryParam(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT region, SUM(amount) AS total FROM mi_pml_sales WHERE category = ? GROUP BY region ORDER BY region",
            ['Electronics']
        );
        $this->assertCount(2, $rows);
        $this->assertSame('North', $rows[0]['region']);
