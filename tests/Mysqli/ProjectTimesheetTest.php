...
/**
 * Tests a project timesheet scenario through ZTD shadow store (MySQLi).
 * Employees log hours on projects. Exercises GROUP BY ... WITH ROLLUP
 ...
 * @spec SPEC-10.2.168
 */
class ProjectTimesheetTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_ts_employees (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                department VARCHAR(255)
            )',
            'CREATE TABLE mi_ts_projects (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                status VARCHAR(255)
            )',
            'CREATE TABLE mi_ts_entries (
                id INT AUTO_INCREMENT PRIMARY KEY,
                employee_id INT,
                project_id INT,
                hours DECIMAL(8,2),
    ...
    {
        return ['mi_ts_entries', 'mi_ts_projects', 'mi_ts_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 4 employees
        $this->mysqli->query("INSERT INTO mi_ts_employees VALUES (1, 'Alice', 'engineering')");
        $this->mysqli->query("INSERT INTO mi_ts_employees VALUES (2, 'Bob', 'engineering')");
        $this->mysqli->query("INSERT INTO mi_ts_employees VALUES (3, 'Carol', 'design')");
        $this->mysqli->query("INSERT INTO mi_ts_employees VALUES (4, 'Dave', 'design')");

        // 3 projects
        $this->mysqli->query("INSERT INTO mi_ts_projects VALUES (1, 'Alpha', 'active')");
        $this->mysqli->query("INSERT INTO mi_ts_projects VALUES (2, 'Beta', 'active')");
        $this->mysqli->query("INSERT INTO mi_ts_projects VALUES (3, 'Legacy', 'maintenance')");

        // 15 time entries
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (1, 1, 1, 8.0, '2025-11-01', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (2, 1, 1, 6.5, '2025-11-02', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (3, 1, 2, 4.0, '2025-11-03', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (4, 1, 3, 2.0, '2025-11-04', 0)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (5, 2, 1, 7.0, '2025-11-01', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (6, 2, 2, 5.5, '2025-11-02', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (7, 2, 2, 3.0, '2025-11-03', 0)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (8, 3, 1, 4.0, '2025-11-01', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (9, 3, 2, 6.0, '2025-11-02', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (10, 3, 3, 3.5, '2025-11-03', 0)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (11, 4, 2, 8.0, '2025-11-01', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (12, 4, 2, 7.0, '2025-11-02', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (13, 4, 3, 2.5, '2025-11-03', 0)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (14, 1, 1, 5.0, '2025-11-05', 1)");
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (15, 2, 1, 4.5, '2025-11-05', 1)");
    }

    /**
     * GROUP BY department, project WITH ROLLUP + COALESCE for NULL labels.
    ...
    public function testHoursByDepartmentAndProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT COALESCE(e.department, '(All)') AS department,
             ...
             FROM mi_ts_entries t
             JOIN mi_ts_employees e ON e.id = t.employee_id
             JOIN mi_ts_projects p ON p.id = t.project_id
             GROUP BY e.department, p.name WITH ROLLUP
             ORDER BY
        ...
        if (count($rows) < 9) {
            $this->markTestIncomplete(
                'GROUP BY WITH ROLLUP returns ' . count($rows) . ' rows instead of 9 through ZTD.'
            );
        }
    ...
    {
        $this->mysqli->query("INSERT INTO mi_ts_entries VALUES (16, 1, 1, 3.0, '2025-11-06', 1)");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_ts_entries");
        $this->assertEquals(16, (int) $rows[0]['cnt']);

        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_ts_entries');
