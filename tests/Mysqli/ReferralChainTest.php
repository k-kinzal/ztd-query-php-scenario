 ...
 * @spec SPEC-10.2.181
 */
class ReferralChainTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mi_ref_users (
            id INT PRIMARY KEY,
            name VARCHAR(50),
            referrer_id INT,
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_ref_users VALUES (1, 'Alice', NULL, 'organic', 100)");
        $this->mysqli->query("INSERT INTO mi_ref_users VALUES (2, 'Bob', 1, 'referral', 50)");
        $this->mysqli->query("INSERT INTO mi_ref_users VALUES (3, 'Carol', 1, 'referral', 75)");
        $this->mysqli->query("INSERT INTO mi_ref_users VALUES (4, 'Dave', 2, 'referral', 30)");
        $this->mysqli->query("INSERT INTO mi_ref_users VALUES (5, 'Eve', NULL, 'organic', 200)");
    }
    ...
    public function testSelfJoinReferrerLookup(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.name, r.name AS referrer
             FROM mi_ref_users u
             LEFT JOIN mi_ref_users r ON u.referrer_id = r.id
             ORDER BY u.id"
        );
    ...
    public function testThreeLevelSelfJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.name, r.name AS referrer, gr.name AS grand_referrer
             FROM mi_ref_users u
             LEFT JOIN mi_ref_users r ON u.referrer_id = r.id
             LEFT JOIN mi_ref_users gr ON r.referrer_id = gr.id
             ORDER BY u.id"
        );
    ...
    public function testNotInWithNullableColumn(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM mi_ref_users
             WHERE id NOT IN (
                 SELECT referrer_id FROM mi_ref_users WHERE referrer_id IS NOT NULL
             )
             ORDER BY id"
        );
    ...
    public function testNotInWithoutNullFilter(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM mi_ref_users
             WHERE id NOT IN (
                 SELECT referrer_id FROM mi_ref_users
             )
             ORDER BY id"
        );
    ...
    {
        $this->ztdExec(
            "INSERT INTO mi_ref_users VALUES (6, 'Frank', 1, 'referral', 0)"
        );
    ...
    public function testUpdateArithmeticOnReferrer(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT name, reward_points FROM mi_ref_users ORDER BY id"
        );
    ...
    public function testDeleteWhereReferrerIsNull(): void
    {
        $affected = $this->ztdExec(
            "DELETE FROM mi_ref_users WHERE referrer_id IS NULL"
        );

        $this->assertEquals(2, $affected);

        $rows = $this->ztdQuery(
            "SELECT name FROM mi_ref_users ORDER BY id"
        );
