 ...
 */
class RestApiPatternTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $raw->query('DROP TABLE IF EXISTS mi_api_pc');
        $raw->query('DROP TABLE IF EXISTS mi_api_products');
        $raw->query('DROP TABLE IF EXISTS mi_api_categories');
        $raw->query('CREATE TABLE mi_api_products (id INT PRIMARY KEY, name VARCHAR(100), price DECIMAL(10,2), stock INT, active INT DEFAULT 1)');
        $raw->query('CREATE TABLE mi_api_categories (id INT PRIMARY KEY, name VARCHAR(50))');
        $raw->query('CREATE TABLE mi_api_pc (product_id INT, category_id INT, PRIMARY KEY (product_id, category_id))');
        $raw->close();
    }
