...
/**
 * Tests a product review/rating system through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.108
 */
class ReviewRatingTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_rr_products (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                category VARCHAR(255),
                price DECIMAL(10,2)
            )',
            'CREATE TABLE mi_rr_reviews (
                id INT AUTO_INCREMENT PRIMARY KEY,
                product_id INT,
                user_id INT,
                rating INT,
            ...
            'CREATE TABLE mi_rr_review_votes (
                id INT AUTO_INCREMENT PRIMARY KEY,
                review_id INT,
                user_id INT,
                helpful INT
    ...
    {
        return ['mi_rr_review_votes', 'mi_rr_reviews', 'mi_rr_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Products
        $this->mysqli->query("INSERT INTO mi_rr_products VALUES (1, 'Laptop', 'electronics', 999.99)");
        $this->mysqli->query("INSERT INTO mi_rr_products VALUES (2, 'Headphones', 'electronics', 149.99)");
        $this->mysqli->query("INSERT INTO mi_rr_products VALUES (3, 'Mouse', 'accessories', 49.99)");

        // Reviews (8 total)
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (1, 1, 1, 5, 'Amazing laptop', 'Best purchase ever', '2026-01-15')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (2, 1, 2, 4, 'Good value', 'Works well for the price', '2026-01-20')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (3, 1, 3, 2, 'Disappointing', 'Battery life is poor', '2026-02-01')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (4, 2, 1, 5, 'Perfect sound', 'Crystal clear audio', '2026-01-18')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (5, 2, 4, 3, 'Decent', 'Okay for the price', '2026-02-05')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (6, 2, 5, 4, 'Great headphones', 'Comfortable fit', '2026-02-10')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (7, 3, 2, 5, 'Best mouse ever', 'Precise and ergonomic', '2026-01-25')");
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (8, 3, 6, 1, 'Broke quickly', 'Stopped working after a week', '2026-02-15')");

        // Review votes (~10)
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (1, 1, 4, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (2, 1, 5, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (3, 2, 3, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (4, 2, 6, 0)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (5, 3, 1, 0)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (6, 4, 2, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (7, 4, 3, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (8, 4, 6, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (9, 7, 1, 1)");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (10, 8, 3, 0)");
    }
    ...
    public function testAverageRatingPerProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name,
             ...
             FROM mi_rr_products p
             JOIN mi_rr_reviews r ON r.product_id = p.id
    ...
    public function testMostHelpfulReviews(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.id AS review_id,
             ...
             FROM mi_rr_reviews r
             LEFT JOIN mi_rr_review_votes v ON v.review_id = r.id
    ...
    public function testAddReviewAndVerify(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (9, 1, 6, 3, 'Its okay', 'Average laptop', '2026-03-01')");
    ...
    public function testVoteOnReviewAndVerify(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (11, 7, 5, 1)");

        $rows = $this->ztdQuery(
            "SELECT COALESCE(SUM(v.helpful), 0) AS helpful_count
             FROM mi_rr_review_votes v
             WHERE v.review_id = 7"
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_rr_reviews VALUES (9, 1, 6, 3, 'Its okay', 'Average laptop', '2026-03-01')");
        $this->mysqli->query("INSERT INTO mi_rr_review_votes VALUES (11, 9, 1, 1)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_rr_reviews");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_rr_review_votes");
        $this->assertEquals(11, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_rr_reviews');
