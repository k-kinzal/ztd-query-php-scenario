...
/**
 * Tests a sales reporting workflow through ZTD shadow store (MySQLi).
 * Covers pivot aggregation, regional breakdown, product metrics,
 * net sales with returns, and physical isolation.
 * @spec SPEC-10.2.74
 */
class SalesReportTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_sr_regions (
                id INT PRIMARY KEY,
                name VARCHAR(100)
            )',
            'CREATE TABLE mi_sr_products (
                id INT PRIMARY KEY,
                name VARCHAR(255),
                category VARCHAR(100)
            )',
            'CREATE TABLE mi_sr_sales (
                id INT PRIMARY KEY,
                product_id INT,
                region_id INT,
                quantity INT,
    ...
    {
        return ['mi_sr_sales', 'mi_sr_products', 'mi_sr_regions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Regions
        $this->mysqli->query("INSERT INTO mi_sr_regions VALUES (1, 'North')");
        $this->mysqli->query("INSERT INTO mi_sr_regions VALUES (2, 'South')");
        $this->mysqli->query("INSERT INTO mi_sr_regions VALUES (3, 'West')");

        // Products
        $this->mysqli->query("INSERT INTO mi_sr_products VALUES (1, 'Alpha Widget', 'Electronics')");
        $this->mysqli->query("INSERT INTO mi_sr_products VALUES (2, 'Beta Gadget', 'Electronics')");
        $this->mysqli->query("INSERT INTO mi_sr_products VALUES (3, 'Gamma Tool', 'Hardware')");

        // Sales data (mix of sales and returns across regions and quarters)
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (1, 1, 1, 10, 49.99, 'sale', 'Q1', '2026-01-15')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (2, 1, 1, 5, 49.99, 'sale', 'Q2', '2026-04-10')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (3, 1, 2, 8, 49.99, 'sale', 'Q1', '2026-02-20')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (4, 2, 1, 15, 29.99, 'sale', 'Q1', '2026-01-25')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (5, 2, 3, 20, 29.99, 'sale', 'Q2', '2026-05-05')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (6, 3, 2, 12, 19.99, 'sale', 'Q1', '2026-03-01')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (7, 1, 1, 2, 49.99, 'return', 'Q1', '2026-02-01')");
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (8, 3, 2, 3, 19.99, 'return', 'Q1', '2026-03-15')");
    }
    ...
    public function testSalesByRegion(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name AS region,
             ...
             FROM mi_sr_regions r
             LEFT JOIN mi_sr_sales s ON s.region_id = r.id
    ...
    public function testSalesByProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS product, p.category,
             ...
             FROM mi_sr_products p
             LEFT JOIN mi_sr_sales s ON s.product_id = p.id
    ...
    public function testRegionFilterWithHaving(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name AS region,
                    SUM(CASE WHEN s.sale_type = 'sale' THEN s.quantity * s.unit_price ELSE 0 END) AS revenue
             FROM mi_sr_regions r
             JOIN mi_sr_sales s ON s.region_id = r.id
             GROUP BY r.id, r.name
    ...
    public function testRegionSalesPrepared(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT p.name AS product, s.quantity, s.unit_price, s.sale_type
             FROM mi_sr_sales s
             JOIN mi_sr_products p ON p.id = s.product_id
    ...
    {
        $this->mysqli->query("INSERT INTO mi_sr_sales VALUES (9, 1, 3, 100, 49.99, 'sale', 'Q3', '2026-07-01')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_sr_sales");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_sr_sales');
