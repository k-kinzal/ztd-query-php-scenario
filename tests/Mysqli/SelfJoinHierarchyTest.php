 ...
 * @spec SPEC-3.3
 */
class SelfJoinHierarchyTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return "CREATE TABLE sjh_employees (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO sjh_employees (id, name, manager_id, salary) VALUES
            (1, 'Alice', NULL, 120000),
            (2, 'Bob', 1, 90000),
            (3, 'Carol', 1, 95000),
            (4, 'Dave', 2, 70000),
        ...
        try {
            $rows = $this->ztdQuery(
                "SELECT e.name AS employee, m.name AS manager
                 FROM sjh_employees e
                 LEFT JOIN sjh_employees m ON e.manager_id = m.id
                 ORDER BY e.id"
            );
    ...
    {
        try {
            $this->mysqli->query("INSERT INTO sjh_employees (id, name, manager_id, salary) VALUES (6, 'Frank', 3, 65000)");

            $rows = $this->ztdQuery(
                "SELECT e.name AS employee, m.name AS manager
                 FROM sjh_employees e
                 INNER JOIN sjh_employees m ON e.manager_id = m.id
                 WHERE m.name = 'Carol'
                 ORDER BY e.id"
            );
    ...
    public function testSelfJoinAfterUpdate(): void
    {
        try {
            ...
            $this->mysqli->query("UPDATE sjh_employees SET manager_id = 3 WHERE id = 4");
    ...
    public function testSelfJoinAfterDelete(): void
    {
        try {
            ...
            $this->mysqli->query("DELETE FROM sjh_employees WHERE id = 2");
    ...
    {
        $this->disableZtd();
        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM sjh_employees");
