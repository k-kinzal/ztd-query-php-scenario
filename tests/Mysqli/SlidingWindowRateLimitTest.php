...
/**
 * Tests rolling time-window rate limiting with token bucket patterns through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.119
 */
class SlidingWindowRateLimitTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_rl_api_clients (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_name VARCHAR(255),
                tier VARCHAR(255),
                requests_per_hour INT
            )',
            'CREATE TABLE mi_rl_api_requests (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_id INT,
            ...
            'CREATE TABLE mi_rl_rate_limit_overrides (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_id INT,
                endpoint VARCHAR(255),
                custom_limit INT
    ...
    {
        return ['mi_rl_rate_limit_overrides', 'mi_rl_api_requests', 'mi_rl_api_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 api clients
        $this->mysqli->query("INSERT INTO mi_rl_api_clients VALUES (1, 'acme-api', 'basic', 100)");
        $this->mysqli->query("INSERT INTO mi_rl_api_clients VALUES (2, 'bigcorp', 'pro', 500)");
        $this->mysqli->query("INSERT INTO mi_rl_api_clients VALUES (3, 'enterprise-co', 'enterprise', 2000)");

        // 10 api requests spanning a 1-hour window
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (1, 1, '/users', '2026-03-09 10:05:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (2, 1, '/users', '2026-03-09 10:10:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (3, 1, '/upload', '2026-03-09 10:12:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (4, 2, '/export', '2026-03-09 10:01:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (5, 2, '/export', '2026-03-09 10:02:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (6, 2, '/users', '2026-03-09 10:15:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (7, 3, '/users', '2026-03-09 10:20:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (8, 3, '/users', '2026-03-09 10:21:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (9, 3, '/data', '2026-03-09 10:22:00', 200)");
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (10, 1, '/users', '2026-03-09 10:03:00', 429)");

        // 2 rate limit overrides
        $this->mysqli->query("INSERT INTO mi_rl_rate_limit_overrides VALUES (1, 1, '/upload', 50)");
        $this->mysqli->query("INSERT INTO mi_rl_rate_limit_overrides VALUES (2, 2, '/export', 200)");
    }
    ...
    public function testRateLimitCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, COUNT(*) AS req_count, c.requests_per_hour
             FROM mi_rl_api_clients c
             JOIN mi_rl_api_requests r ON r.client_id = c.id
    ...
    public function testOverrideApplied(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, r.endpoint,
             ...
             FROM mi_rl_api_clients c
             JOIN mi_rl_api_requests r ON r.client_id = c.id
             LEFT JOIN mi_rl_rate_limit_overrides o
    ...
    {
        $this->mysqli->query("INSERT INTO mi_rl_api_requests VALUES (11, 1, '/health', '2026-03-09 10:30:00', 200)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_rl_api_requests");
        $this->assertSame(11, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT endpoint FROM mi_rl_api_requests WHERE id = 11");
        $this->assertSame('/health', $rows[0]['endpoint']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_rl_api_requests');
