...
/**
 * Tests a subscription billing workflow through ZTD shadow store (MySQLi).
...
class SubscriptionBillingTest extends AbstractMysqliTestCase
{
    ...
    {
        return [
            'CREATE TABLE mi_sb_subscriptions (
                id INT PRIMARY KEY,
    ...
    {
        return ['mi_sb_credits', 'mi_sb_billing', 'mi_sb_subscriptions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 subscriptions
        $this->mysqli->query("INSERT INTO mi_sb_subscriptions VALUES (1, 'Alice', 'premium', 99.99, 'active', '2025-01-01 00:00:00')");
        $this->mysqli->query("INSERT INTO mi_sb_subscriptions VALUES (2, 'Bob', 'basic', 29.99, 'active', '2025-02-01 00:00:00')");
        $this->mysqli->query("INSERT INTO mi_sb_subscriptions VALUES (3, 'Charlie', 'premium', 99.99, 'suspended', '2025-03-01 00:00:00')");

        // Billing records
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (1, 1, 99.99, '2025-02-01', 1)");
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (2, 1, 99.99, '2025-03-01', 1)");
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (3, 1, 99.99, '2025-04-01', 0)");
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (4, 2, 29.99, '2025-03-01', 1)");
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (5, 2, 29.99, '2025-04-01', 0)");

        // Credits
        $this->mysqli->query("INSERT INTO mi_sb_credits VALUES (1, 1, 25.00, 'Referral bonus', '2025-03-15 10:00:00')");
        $this->mysqli->query("INSERT INTO mi_sb_credits VALUES (2, 1, 10.00, 'Service outage', '2025-04-01 12:00:00')");
    }
    ...
    public function testActiveSubscriptionsWithBilling(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.id, s.customer_name, s.plan,
             ...
             FROM mi_sb_subscriptions s
             LEFT JOIN mi_sb_billing b ON b.subscription_id = s.id
    ...
    {
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (6, 2, 29.99, '2025-05-01', 0)");
    ...
    public function testBalanceWithCredits(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.id, s.customer_name,
                    COALESCE((SELECT SUM(b.amount) FROM mi_sb_billing b WHERE b.subscription_id = s.id AND b.paid = 0), 0) AS total_unpaid,
                    COALESCE((SELECT SUM(c.amount) FROM mi_sb_credits c WHERE c.subscription_id = s.id), 0) AS total_credits
             FROM mi_sb_subscriptions s
             WHERE s.id = 1"
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_sb_credits VALUES (3, 2, 15.00, 'Loyalty discount', '2025-04-10 09:00:00')");
    ...
    public function testMarkBillingPaid(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_sb_billing SET paid = 1 WHERE id = 3 AND paid = 0");
    ...
    {
        $this->mysqli->query("INSERT INTO mi_sb_billing VALUES (6, 3, 99.99, '2025-04-01', 0)");
        $this->mysqli->query("UPDATE mi_sb_subscriptions SET status = 'cancelled' WHERE id = 3");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_sb_billing");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT status FROM mi_sb_subscriptions WHERE id = 3");
        $this->assertSame('cancelled', $rows[0]['status']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_sb_billing');
