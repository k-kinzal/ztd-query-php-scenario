 ...
 * prepared statement for subscriber lookup, and physical isolation check (MySQLi).
 * @spec SPEC-10.2.149
 */
class SubscriptionBillingTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_sb_plans (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(50),
                price_monthly DECIMAL(10,2),
                features TEXT
            )',
            'CREATE TABLE mi_sb_subscribers (
                id INT AUTO_INCREMENT PRIMARY KEY,
                email VARCHAR(100),
                name VARCHAR(100),
                plan_id INT,
    ...
    {
        return ['mi_sb_payments', 'mi_sb_invoices', 'mi_sb_subscribers', 'mi_sb_plans'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 plans
        $this->mysqli->query("INSERT INTO mi_sb_plans VALUES (1, 'Basic', 9.99, 'Email support, 1 user')");
        $this->mysqli->query("INSERT INTO mi_sb_plans VALUES (2, 'Pro', 29.99, 'Priority support, 5 users, API access')");
        $this->mysqli->query("INSERT INTO mi_sb_plans VALUES (3, 'Enterprise', 99.99, 'Dedicated support, unlimited users, SLA')");

        // 5 subscribers
        $this->mysqli->query("INSERT INTO mi_sb_subscribers VALUES (1, 'alice@example.com', 'Alice Johnson', 1, 'active', '2025-01-15')");
        $this->mysqli->query("INSERT INTO mi_sb_subscribers VALUES (2, 'bob@example.com', 'Bob Smith', 2, 'active', '2025-02-01')");
        $this->mysqli->query("INSERT INTO mi_sb_subscribers VALUES (3, 'carol@example.com', 'Carol White', 3, 'active', '2025-03-10')");
        $this->mysqli->query("INSERT INTO mi_sb_subscribers VALUES (4, 'dan@example.com', 'Dan Brown', 1, 'cancelled', '2025-01-20')");
        $this->mysqli->query("INSERT INTO mi_sb_subscribers VALUES (5, 'eve@example.com', 'Eve Davis', 2, 'active', '2025-04-01')");

        // 8 invoices
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (1, 1, 9.99, '2025-02-15', 'paid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (2, 1, 9.99, '2025-03-15', 'paid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (3, 2, 29.99, '2025-03-01', 'paid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (4, 2, 29.99, '2025-04-01', 'unpaid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (5, 3, 99.99, '2025-04-10', 'paid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (6, 3, 99.99, '2025-05-10', 'unpaid')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (7, 4, 9.99, '2025-02-20', 'overdue')");
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (8, 5, 29.99, '2025-05-01', 'unpaid')");

        // 5 payments (invoices 4, 6, 7, 8 have no payment)
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (1, 1, 9.99, '2025-02-14', 'credit_card')");
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (2, 2, 9.99, '2025-03-14', 'credit_card')");
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (3, 3, 29.99, '2025-02-28', 'paypal')");
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (4, 5, 99.99, '2025-04-09', 'bank_transfer')");
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (5, 1, 5.00, '2025-02-15', 'credit_card')");
    }
    ...
    public function testActiveSubscriptionSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.id, s.name AS subscriber_name, s.email, p.name AS plan_name, p.price_monthly, s.start_date
             FROM mi_sb_subscribers s
             JOIN mi_sb_plans p ON p.id = s.plan_id
    ...
    public function testRevenueByPlan(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS plan_name, COUNT(i.id) AS invoice_count, SUM(i.amount) AS total_revenue
             FROM mi_sb_plans p
             JOIN mi_sb_subscribers s ON s.plan_id = p.id
             JOIN mi_sb_invoices i ON i.subscriber_id = s.id
             GROUP BY p.name
    ...
    public function testSubscriberInvoiceHistory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id AS invoice_id, i.amount, i.due_date, i.status AS invoice_status,
             ...
             FROM mi_sb_invoices i
             LEFT JOIN mi_sb_payments p ON p.invoice_id = i.id
    ...
    public function testOverdueInvoices(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id, s.name AS subscriber_name, i.amount, i.due_date, i.status,
             ...
             FROM mi_sb_invoices i
             JOIN mi_sb_subscribers s ON s.id = i.subscriber_id
    ...
    public function testUpgradeSubscriptionAndVerify(): void
    {
        ...
        $this->mysqli->query("UPDATE mi_sb_subscribers SET plan_id = 2 WHERE id = 1");
        $this->assertSame(1, $this->mysqli->lastAffectedRows());

        // Generate a new invoice for the upgraded plan
        $this->mysqli->query("INSERT INTO mi_sb_invoices VALUES (9, 1, 29.99, '2025-04-15', 'unpaid')");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_sb_payments VALUES (6, 4, 29.99, '2025-04-02', 'credit_card')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_sb_payments");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_sb_payments');
