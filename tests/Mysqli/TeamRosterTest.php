...
/**
 * Tests team roster with string aggregation via GROUP_CONCAT in multi-table JOINs (MySQLi).
 ...
 * @spec SPEC-10.2.172
 */
class TeamRosterTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_tr_team (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL,
                department VARCHAR(100) NOT NULL
            ) ENGINE=InnoDB',
            'CREATE TABLE mi_tr_member (
                id INT PRIMARY KEY,
    ...
    {
        return ['mi_tr_member', 'mi_tr_team'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO mi_tr_team VALUES (1, 'Backend', 'Engineering')");
        $this->mysqli->query("INSERT INTO mi_tr_team VALUES (2, 'Frontend', 'Engineering')");
        $this->mysqli->query("INSERT INTO mi_tr_team VALUES (3, 'Design', 'Product')");
        $this->mysqli->query("INSERT INTO mi_tr_team VALUES (4, 'QA', 'Engineering')");

        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (1, 1, 'Alice', 'lead', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (2, 1, 'Bob', 'developer', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (3, 1, 'Carol', 'developer', 0)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (4, 2, 'Dave', 'lead', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (5, 2, 'Eve', 'developer', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (6, 3, 'Frank', 'designer', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (7, 3, 'Grace', 'designer', 1)");
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (8, 3, 'Heidi', 'lead', 1)");
    }
    ...
    public function testGroupConcatMemberNamesPerTeam(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name AS team,
             ...
             FROM mi_tr_team t
             JOIN mi_tr_member m ON m.team_id = t.id
    ...
    public function testLeftJoinGroupConcatIncludesEmptyTeams(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name AS team,
                    ...
                    COUNT(m.id) AS member_count
             FROM mi_tr_team t
             LEFT JOIN mi_tr_member m ON m.team_id = t.id
             GROUP BY t.id, t.name
             ORDER BY t.name"
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_tr_member VALUES (9, 4, 'Ivan', 'tester', 1)");
    ...
    {
        $this->ztdExec("DELETE FROM mi_tr_member WHERE id = 2");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_tr_member");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $this->disableZtd();
        $result = $this->mysqli->query("SELECT COUNT(*) AS cnt FROM mi_tr_member");
