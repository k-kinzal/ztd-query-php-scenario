 ...
 * @spec SPEC-3.3
 */
class ThreeTableJoinChainTest extends AbstractMysqliTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->mysqli->query("INSERT INTO ttj_authors (id, name) VALUES (1, 'Tolkien'), (2, 'Asimov'), (3, 'Bradbury')");
        $this->mysqli->query("INSERT INTO ttj_books (id, author_id, title, genre) VALUES
            (1, 1, 'The Hobbit', 'Fantasy'), (2, 1, 'LOTR', 'Fantasy'),
            (3, 2, 'Foundation', 'SciFi'), (4, 3, 'Fahrenheit 451', 'SciFi')");
        $this->mysqli->query("INSERT INTO ttj_reviews (id, book_id, rating, reviewer) VALUES
    ...
    public function testThreeTableAfterMutations(): void
    {
        ...
        $this->mysqli->query("INSERT INTO ttj_books (id, author_id, title, genre) VALUES (5, 3, 'The Martian Chronicles', 'SciFi')");
        $this->mysqli->query("INSERT INTO ttj_reviews (id, book_id, rating, reviewer) VALUES (7, 5, 4, 'Grace')");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM ttj_authors');
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt']);

        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM ttj_books');
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt']);

        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM ttj_reviews');
        $this->assertSame(0, (int) $result->fetch_assoc()['cnt']);
    }
}
