...
/**
 * Tests a time-tracking workflow through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.80
 */
class TimeTrackingTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_tt_clients (
                id INT PRIMARY KEY,
                name VARCHAR(255),
                rate_per_hour DECIMAL(10,2)
            )',
            'CREATE TABLE mi_tt_projects (
                id INT PRIMARY KEY,
                client_id INT,
                name VARCHAR(255),
                budget_hours DECIMAL(8,2)
            )',
            'CREATE TABLE mi_tt_time_entries (
                id INT PRIMARY KEY,
                project_id INT,
    ...
    {
        return ['mi_tt_time_entries', 'mi_tt_projects', 'mi_tt_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 clients
        $this->mysqli->query("INSERT INTO mi_tt_clients VALUES (1, 'Acme Corp', 150.00)");
        $this->mysqli->query("INSERT INTO mi_tt_clients VALUES (2, 'Globex Inc', 200.00)");

        // 3 projects
        $this->mysqli->query("INSERT INTO mi_tt_projects VALUES (1, 1, 'Website Redesign', 40.00)");
        $this->mysqli->query("INSERT INTO mi_tt_projects VALUES (2, 1, 'Mobile App', 100.00)");
        $this->mysqli->query("INSERT INTO mi_tt_projects VALUES (3, 2, 'Data Migration', 20.00)");

        // Time entries
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (1, 1, 'Alice', 8.00, '2026-03-02 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (2, 1, 'Alice', 6.50, '2026-03-03 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (3, 1, 'Bob', 7.00, '2026-03-02 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (4, 1, 'Bob', 2.00, '2026-03-04 09:00:00', 0)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (5, 2, 'Alice', 5.00, '2026-03-02 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (6, 2, 'Charlie', 8.00, '2026-03-03 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (7, 3, 'Charlie', 10.00, '2026-03-02 09:00:00', 1)");
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (8, 3, 'Charlie', 12.00, '2026-03-03 09:00:00', 1)");
    }
    ...
    public function testBillableHoursByProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS project_name, SUM(te.hours) AS total_billable
             FROM mi_tt_time_entries te
             JOIN mi_tt_projects p ON p.id = te.project_id
             WHERE te.billable = 1
        ...
        // Project 1: 8 + 6.5 + 7 = 21.5 (Bob's 2h non-billable excluded)
    ...
    public function testClientInvoiceSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.name AS client_name,
             ...
             FROM mi_tt_time_entries te
             JOIN mi_tt_projects p ON p.id = te.project_id
             JOIN mi_tt_clients c ON c.id = p.client_id
             WHERE te.billable = 1
        ...
        // Acme Corp: 21.5 + 13 = 34.5 hours * $150 = $5175
    ...
    public function testOverBudgetProjects(): void
    {
        ...
        // Project 3 (Data Migration): 22h > 20h budget
    ...
    {
        // Before: Project 2 billable = 13h
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM mi_tt_time_entries WHERE project_id = 2 AND billable = 1"
        );
        $this->assertEquals(13.00, (float) $before[0]['total']);

        // Add 4h entry
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (9, 2, 'Diana', 4.00, '2026-03-04 09:00:00', 1)");
    ...
    {
        // Before: Project 1 billable = 21.5h
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM mi_tt_time_entries WHERE project_id = 1 AND billable = 1"
        );
        $this->assertEquals(21.50, (float) $before[0]['total']);

        // Mark entry 1 (8h) as non-billable
        $this->mysqli->query("UPDATE mi_tt_time_entries SET billable = 0 WHERE id = 1");
    ...
    {
        $this->mysqli->query("INSERT INTO mi_tt_time_entries VALUES (9, 1, 'Eve', 3.00, '2026-03-05 09:00:00', 1)");
        $this->mysqli->query("UPDATE mi_tt_time_entries SET billable = 0 WHERE id = 2");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_tt_time_entries");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_tt_time_entries');
