 ...
 * and date gap analysis (MySQLi).
 * @spec SPEC-10.2.116
 */
class UserActivityStreakTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_uas_users (
                id INT AUTO_INCREMENT PRIMARY KEY,
                username VARCHAR(255)
            )',
            'CREATE TABLE mi_uas_activity_log (
                id INT AUTO_INCREMENT PRIMARY KEY,
                user_id INT,
    ...
    {
        return ['mi_uas_activity_log', 'mi_uas_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Users
        $this->mysqli->query("INSERT INTO mi_uas_users VALUES (1, 'alice')");
        $this->mysqli->query("INSERT INTO mi_uas_users VALUES (2, 'bob')");

        // Activity log: alice has a 4-day streak, a gap, then a 2-day streak
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (1, 1, '2026-03-01', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (2, 1, '2026-03-02', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (3, 1, '2026-03-03', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (4, 1, '2026-03-04', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (5, 1, '2026-03-07', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (6, 1, '2026-03-08', 'login')");

        // Activity log: bob has a 3-day streak
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (7, 2, '2026-03-02', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (8, 2, '2026-03-03', 'login')");
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (9, 2, '2026-03-04', 'login')");
    }
    ...
    {
        $this->mysqli->query("INSERT INTO mi_uas_activity_log VALUES (10, 1, '2026-03-09', 'login')");
        $this->mysqli->query("UPDATE mi_uas_users SET username = 'alice_updated' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_uas_activity_log");
        $this->assertEquals(10, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT username FROM mi_uas_users WHERE id = 1");
        $this->assertSame('alice_updated', $rows[0]['username']);

        // Physical tables untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_uas_activity_log');
