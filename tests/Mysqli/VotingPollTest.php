...
/**
 * Tests a polling/voting system with result tallying through ZTD shadow store (MySQLi).
 ...
 * @spec SPEC-10.2.137
 */
class VotingPollTest extends AbstractMysqliTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mi_vp_polls (
            ...
            'CREATE TABLE mi_vp_options (
                id INT AUTO_INCREMENT PRIMARY KEY,
                poll_id INT,
                option_text VARCHAR(255),
                display_order INT
            )',
            'CREATE TABLE mi_vp_votes (
                id INT AUTO_INCREMENT PRIMARY KEY,
                poll_id INT,
                option_id INT,
    ...
    {
        return ['mi_vp_votes', 'mi_vp_options', 'mi_vp_polls'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 polls
        $this->mysqli->query("INSERT INTO mi_vp_polls VALUES (1, 'Best Programming Language', '2025-09-01', '2025-09-30', 'closed')");
        $this->mysqli->query("INSERT INTO mi_vp_polls VALUES (2, 'Office Lunch Preference', '2025-10-01', '2025-10-15', 'open')");

        // Options for poll 1
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (1, 1, 'Python', 1)");
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (2, 1, 'JavaScript', 2)");
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (3, 1, 'PHP', 3)");
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (4, 1, 'Go', 4)");

        // Options for poll 2
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (5, 2, 'Pizza', 1)");
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (6, 2, 'Sushi', 2)");
        $this->mysqli->query("INSERT INTO mi_vp_options VALUES (7, 2, 'Salad', 3)");

        // Votes for poll 1: Python=3, JavaScript=2, PHP=1, Go=0
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (1, 1, 1, 'Alice', '2025-09-02')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (2, 1, 1, 'Bob', '2025-09-03')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (3, 1, 2, 'Carol', '2025-09-04')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (4, 1, 3, 'Dave', '2025-09-05')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (5, 1, 1, 'Eve', '2025-09-06')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (6, 1, 2, 'Frank', '2025-09-07')");

        // Votes for poll 2: Pizza=2, Sushi=1, Salad=1
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (7, 2, 5, 'Alice', '2025-10-02')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (8, 2, 6, 'Bob', '2025-10-03')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (9, 2, 5, 'Carol', '2025-10-04')");
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (10, 2, 7, 'Dave', '2025-10-05')");
    }
    ...
    public function testPollResults(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.option_text,
                    ...
                    ROUND(COUNT(v.id) * 100.0 / (SELECT COUNT(*) FROM mi_vp_votes WHERE poll_id = 1), 1) AS percentage
             FROM mi_vp_options o
             LEFT JOIN mi_vp_votes v ON v.option_id = o.id AND v.poll_id = 1
             WHERE o.poll_id = 1
    ...
    public function testPollWinner(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.option_text, COUNT(v.id) AS vote_count
             FROM mi_vp_options o
             JOIN mi_vp_votes v ON v.option_id = o.id AND v.poll_id = 1
             WHERE o.poll_id = 1
             GROUP BY o.id, o.option_text
             ORDER BY vote_count DESC, o.option_text
             LIMIT 1"
            ...
            "SELECT DISTINCT voter_name
             ...
             WHERE poll_id = 1
               AND voter_name NOT IN (
                   SELECT voter_name FROM mi_vp_votes WHERE poll_id = 2
               )
             ORDER BY voter_name"
        );
    ...
    {
        $this->mysqli->query("INSERT INTO mi_vp_votes VALUES (11, 2, 6, 'Eve', '2025-10-06')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mi_vp_votes");
        $this->assertSame(11, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->mysqli->disableZtd();
        $result = $this->mysqli->query('SELECT COUNT(*) AS cnt FROM mi_vp_votes');
