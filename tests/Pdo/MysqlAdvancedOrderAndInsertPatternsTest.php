...
class MysqlAdvancedOrderAndInsertPatternsTest extends AbstractMysqlPdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO aoi_users_m VALUES (3, 'Charlie', 'moderator', 85)");
        $this->pdo->exec("INSERT INTO aoi_users_m VALUES (4, 'Diana', 'admin', 95)");
    }

    public function testCaseWhenInOrderBy(): void
    {
        $stmt = $this->pdo->query(
            "SELECT name, role FROM aoi_users_m ORDER BY
             CASE role
                 WHEN 'admin' THEN 1
    ...
    public function testCaseWhenInOrderByWithPrepared(): void
    {
        $stmt = $this->pdo->prepare(
            "SELECT name FROM aoi_users_m WHERE score > ?
    ...
    {
        $stmtByRole = $this->pdo->prepare('SELECT name FROM aoi_users_m WHERE role = ? ORDER BY name');
        $stmtByScore = $this->pdo->prepare('SELECT name FROM aoi_users_m WHERE score > ? ORDER BY score DESC');
