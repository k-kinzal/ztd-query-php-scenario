...
class MysqlAdvancedSubqueryTest extends AbstractMysqlPdoTestCase
{
    ...
    {
        $stmt = $this->pdo->query("
            SELECT e.name FROM mysql_advsq_employees e
            WHERE e.dept_id IN (
                SELECT p.dept_id FROM mysql_advsq_projects p
                WHERE p.status = 'active'
                AND p.dept_id IN (
                    SELECT d.id FROM mysql_advsq_departments d WHERE d.budget > 100000
                )
            )
            ORDER BY e.name
        ");
    ...
    public function testScalarSubqueryInSelect(): void
    {
        $stmt = $this->pdo->query("
            SELECT d.name,
                   (SELECT COUNT(*) FROM mysql_advsq_employees e WHERE e.dept_id = d.id) AS emp_count,
                   (SELECT SUM(e.salary) FROM mysql_advsq_employees e WHERE e.dept_id = d.id) AS total_salary
            FROM mysql_advsq_departments d
            ORDER BY d.id
        ");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        $this->pdo->exec("UPDATE mysql_advsq_departments SET budget = (SELECT SUM(salary) FROM mysql_advsq_employees WHERE mysql_advsq_employees.dept_id = mysql_advsq_departments.id) WHERE id = 1");

        $stmt = $this->pdo->query("SELECT budget FROM mysql_advsq_departments WHERE id = 1");
        $this->assertSame(360000.0, (float) $stmt->fetch(PDO::FETCH_ASSOC)['budget']);
    }

    public function testExistsAndNotExists(): void
    {
        $stmt = $this->pdo->query("
            SELECT d.name FROM mysql_advsq_departments d
            WHERE EXISTS (SELECT 1 FROM mysql_advsq_projects p WHERE p.dept_id = d.id AND p.status = 'active')
            ORDER BY d.name
        ");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $stmt = $this->pdo->query("
            SELECT d.name FROM mysql_advsq_departments d
            WHERE NOT EXISTS (SELECT 1 FROM mysql_advsq_projects p WHERE p.dept_id = d.id)
        ");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testUpdateWithSelfReferencingSubqueryAvg(): void
    {
        ...
        // CTE rewriter incorrectly updates ALL rows instead of only Engineering dept
        // Expected: only dept_id=1 (avg salary 120k > 100k) should be deactivated
        // Actual: all employees are deactivated — CTE rewriter mishandles self-referencing subquery
        $this->pdo->exec("UPDATE mysql_advsq_employees SET active = 0 WHERE dept_id IN (SELECT dept_id FROM mysql_advsq_employees GROUP BY dept_id HAVING AVG(salary) > 100000)");

        $stmt = $this->pdo->query("SELECT name, active FROM mysql_advsq_employees ORDER BY id");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        // All rows have active=0 due to CTE rewriter bug (should be only Engineering)
        $this->assertSame(0, (int) $rows[0]['active']); // Alice
        $this->assertSame(0, (int) $rows[2]['active']); // Charlie — should be 1
    }
    ...
    public function testMultipleJoinsAcrossThreeTables(): void
    {
        $stmt = $this->pdo->query("
            ...
            FROM mysql_advsq_employees e
            JOIN mysql_advsq_departments d ON e.dept_id = d.id
            JOIN mysql_advsq_projects p ON d.id = p.dept_id
            WHERE p.status = 'active'
