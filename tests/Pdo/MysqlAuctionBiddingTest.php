...
/**
 * Tests an auction bidding system workflow through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.85
 */
class MysqlAuctionBiddingTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_ab_auctions (
                id INT PRIMARY KEY,
                title VARCHAR(255),
    ...
    {
        return ['mp_ab_bids', 'mp_ab_auctions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 auctions
        $this->pdo->exec("INSERT INTO mp_ab_auctions VALUES (1, 'Vintage Watch', 100.00, 250.00, '2026-04-01 18:00:00', 'active')");
        $this->pdo->exec("INSERT INTO mp_ab_auctions VALUES (2, 'Rare Painting', 500.00, 750.00, '2026-03-15 20:00:00', 'active')");
        $this->pdo->exec("INSERT INTO mp_ab_auctions VALUES (3, 'Signed Book', 20.00, 20.00, '2026-05-01 12:00:00', 'active')");

        // 5 bids across auctions
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (1, 1, 'Alice', 150.00, '2026-03-01 10:00:00')");
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (2, 1, 'Bob', 200.00, '2026-03-02 11:00:00')");
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (3, 1, 'Charlie', 250.00, '2026-03-03 12:00:00')");
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (4, 2, 'Diana', 650.00, '2026-03-05 14:00:00')");
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (5, 2, 'Eve', 750.00, '2026-03-06 15:00:00')");
    }
    ...
    {
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (6, 1, 'Frank', 300.00, '2026-03-09 10:00:00')");
        $this->pdo->exec("UPDATE mp_ab_auctions SET current_price = 300.00 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT current_price FROM mp_ab_auctions WHERE id = 1");
        $this->assertCount(1, $rows);
        $this->assertEqualsWithDelta(300.00, (float) $rows[0]['current_price'], 0.01);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_ab_bids WHERE auction_id = 1");
    ...
    public function testGetHighestBid(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT MAX(bid_amount) AS highest_bid FROM mp_ab_bids WHERE auction_id = ?",
            [1]
        );
    ...
    public function testBidHistory(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT b.bidder_name, b.bid_amount, b.bid_time, a.title
             FROM mp_ab_bids b
             JOIN mp_ab_auctions a ON a.id = b.auction_id
             WHERE b.auction_id = ?
    ...
    public function testAuctionSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.title,
             ...
             FROM mp_ab_auctions a
             LEFT JOIN mp_ab_bids b ON b.auction_id = a.id
    ...
    {
        $affected = $this->pdo->exec("UPDATE mp_ab_auctions SET status = 'closed' WHERE id = 1");
        $this->assertSame(1, $affected);

        // Verify status changed
        $rows = $this->ztdQuery("SELECT status FROM mp_ab_auctions WHERE id = 1");
        ...
        $rows = $this->ztdQuery(
            "SELECT b.bidder_name, b.bid_amount
             ...
             WHERE b.auction_id = 1
               AND b.bid_amount = (SELECT MAX(bid_amount) FROM mp_ab_bids WHERE auction_id = 1)"
        );
        $this->assertCount(1, $rows);
        $this->assertSame('Charlie', $rows[0]['bidder_name']);
        $this->assertEqualsWithDelta(250.00, (float) $rows[0]['bid_amount'], 0.01);
    ...
    public function testActiveBidderStats(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.title, COUNT(DISTINCT b.bidder_name) AS unique_bidders
             FROM mp_ab_auctions a
             JOIN mp_ab_bids b ON b.auction_id = a.id
             GROUP BY a.id, a.title
    ...
    {
        $this->pdo->exec("INSERT INTO mp_ab_bids VALUES (6, 3, 'Zara', 50.00, '2026-03-09 09:00:00')");
        $this->pdo->exec("UPDATE mp_ab_auctions SET status = 'closed' WHERE id = 2");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_ab_bids");
        $this->assertSame(6, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT status FROM mp_ab_auctions WHERE id = 2");
        $this->assertSame('closed', $rows[0]['status']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_ab_bids")->fetchAll(PDO::FETCH_ASSOC);
