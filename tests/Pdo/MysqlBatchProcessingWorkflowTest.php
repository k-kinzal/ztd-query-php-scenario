...
class MysqlBatchProcessingWorkflowTest extends AbstractMysqlPdoTestCase
{
    ...
    public function testBatchDebitCreditProcessing(): void
    {
        ...
        // Explicit processed=0 since shadow store doesn't apply defaults
        ...
        $stmt = $this->pdo->query("
            SELECT SUM(CASE WHEN processed = 1 THEN 1 ELSE 0 END) AS ok,
                   SUM(CASE WHEN processed = -1 THEN 1 ELSE 0 END) AS skipped
            FROM bp_transactions
        ");
        $summary = $stmt->fetch(PDO::FETCH_ASSOC);
        $this->assertSame(3, (int) $summary['ok']);
