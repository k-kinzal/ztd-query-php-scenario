 ...
 * for SaaS analytics, marketing analysis, and user engagement tracking (MySQL PDO).
 * @spec SPEC-10.2.124
 */
class MysqlCohortRetentionAnalysisTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_cr_users (
                id INT AUTO_INCREMENT PRIMARY KEY,
                username VARCHAR(255),
                signup_month VARCHAR(255)
            )',
            'CREATE TABLE mp_cr_user_activities (
                id INT AUTO_INCREMENT PRIMARY KEY,
                user_id INT,
    ...
    {
        return ['mp_cr_user_activities', 'mp_cr_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (1, 'alice',   '2025-01')");
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (2, 'bob',     '2025-01')");
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (3, 'charlie', '2025-02')");
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (4, 'diana',   '2025-02')");
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (5, 'eve',     '2025-02')");
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (6, 'frank',   '2025-03')");

        // User activities
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (1,  1, '2025-01', 10)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (2,  1, '2025-02', 8)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (3,  1, '2025-03', 5)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (4,  2, '2025-01', 15)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (5,  2, '2025-02', 3)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (6,  2, '2025-04', 1)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (7,  3, '2025-02', 12)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (8,  3, '2025-03', 7)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (9,  4, '2025-02', 20)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (10, 4, '2025-03', 15)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (11, 4, '2025-04', 10)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (12, 5, '2025-02', 5)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (13, 6, '2025-03', 25)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (14, 6, '2025-04', 20)");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (15, 6, '2025-05', 12)");
    }
    ...
    public function testRetentionByMonth(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.activity_month, COUNT(DISTINCT a.user_id) AS active_users
             FROM mp_cr_users u
             JOIN mp_cr_user_activities a ON a.user_id = u.id
    ...
    public function testChurnDetection(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.id, u.username, u.signup_month
             FROM mp_cr_users u
             WHERE EXISTS (
                 SELECT 1 FROM mp_cr_user_activities a1
                 WHERE a1.user_id = u.id
                   AND a1.activity_month = u.signup_month
             )
             AND NOT EXISTS (
                 SELECT 1 FROM mp_cr_user_activities a2
    ...
    {
        $this->pdo->exec("INSERT INTO mp_cr_users VALUES (7, 'grace', '2025-04')");
        $this->pdo->exec("INSERT INTO mp_cr_user_activities VALUES (16, 7, '2025-04', 30)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_cr_users");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_cr_user_activities");
        $this->assertEquals(16, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_cr_users")->fetchAll(PDO::FETCH_ASSOC);
