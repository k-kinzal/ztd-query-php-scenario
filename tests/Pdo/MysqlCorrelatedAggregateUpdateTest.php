 ...
 * @spec SPEC-3.3
 */
class MysqlCorrelatedAggregateUpdateTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_cau_customers (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                order_total DECIMAL(10,2) NOT NULL DEFAULT 0,
                order_count INT NOT NULL DEFAULT 0
            ) ENGINE=InnoDB',
            'CREATE TABLE mp_cau_orders (
                id INT PRIMARY KEY,
    ...
    {
        return ['mp_cau_orders', 'mp_cau_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_cau_customers VALUES (1, 'Alice', 0, 0)");
        $this->pdo->exec("INSERT INTO mp_cau_customers VALUES (2, 'Bob', 0, 0)");
        $this->pdo->exec("INSERT INTO mp_cau_customers VALUES (3, 'Carol', 0, 0)");

        $this->pdo->exec("INSERT INTO mp_cau_orders VALUES (1, 1, 100.00)");
        $this->pdo->exec("INSERT INTO mp_cau_orders VALUES (2, 1, 200.00)");
        $this->pdo->exec("INSERT INTO mp_cau_orders VALUES (3, 2, 150.00)");
        $this->pdo->exec("INSERT INTO mp_cau_orders VALUES (4, 1, 50.00)");
    }
    ...
    public function testUpdateSetWithCorrelatedSum(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_cau_customers SET order_total = (SELECT COALESCE(SUM(amount), 0) FROM mp_cau_orders WHERE mp_cau_orders.customer_id = mp_cau_customers.id)"
            );

            $rows = $this->ztdQuery("SELECT id, name, order_total FROM mp_cau_customers ORDER BY id");
    ...
    public function testUpdateSetWithCorrelatedCount(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_cau_customers SET order_count = (SELECT COUNT(*) FROM mp_cau_orders WHERE mp_cau_orders.customer_id = mp_cau_customers.id)"
            );

            $rows = $this->ztdQuery("SELECT name, order_count FROM mp_cau_customers ORDER BY id");
    ...
    public function testUpdateSetMultipleCorrelatedSubqueries(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_cau_customers SET
                    order_total = (SELECT COALESCE(SUM(amount), 0) FROM mp_cau_orders WHERE mp_cau_orders.customer_id = mp_cau_customers.id),
                    order_count = (SELECT COUNT(*) FROM mp_cau_orders WHERE mp_cau_orders.customer_id = mp_cau_customers.id)"
            );

            $rows = $this->ztdQuery("SELECT name, order_total, order_count FROM mp_cau_customers ORDER BY id");
    ...
    public function testCorrelatedUpdateWithWhereClause(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_cau_customers SET order_total = (SELECT COALESCE(SUM(amount), 0) FROM mp_cau_orders WHERE mp_cau_orders.customer_id = mp_cau_customers.id) WHERE name IN ('Alice', 'Bob')"
            );

            $rows = $this->ztdQuery("SELECT name, order_total FROM mp_cau_customers ORDER BY id");
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO mp_cau_orders VALUES (5, 3, 500.00)");
