...
/**
 * Tests a coupon/discount system workflow through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.69
 */
class MysqlCouponDiscountTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_cp_coupons (
                id INT PRIMARY KEY,
                code VARCHAR(50),
                discount_pct INT,
                max_uses INT,
    ...
    {
        return ['mp_cp_uses', 'mp_cp_coupons'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 4 coupons
        $this->pdo->exec("INSERT INTO mp_cp_coupons VALUES (1, 'SAVE10', 10, 100, '2026-01-01', '2026-12-31', 1)");
        $this->pdo->exec("INSERT INTO mp_cp_coupons VALUES (2, 'HALF50', 50, 10, '2026-01-01', '2026-06-30', 1)");
        $this->pdo->exec("INSERT INTO mp_cp_coupons VALUES (3, 'EXPIRED20', 20, 50, '2025-01-01', '2025-12-31', 1)");
        $this->pdo->exec("INSERT INTO mp_cp_coupons VALUES (4, 'DISABLED15', 15, 200, '2026-01-01', '2026-12-31', 0)");

        // 5 coupon uses
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (1, 1, 'Alice', '2026-02-01 10:00:00', 200.00, 20.00)");
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (2, 1, 'Bob', '2026-02-15 14:30:00', 150.00, 15.00)");
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (3, 1, 'Charlie', '2026-03-01 09:00:00', 300.00, 30.00)");
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (4, 2, 'Diana', '2026-03-05 16:00:00', 500.00, 250.00)");
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (5, 2, 'Eve', '2026-03-06 11:00:00', 120.00, 60.00)");
    }
    ...
    public function testUsageLimitCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id, c.code, c.max_uses, COUNT(u.id) AS use_count
             FROM mp_cp_coupons c
             LEFT JOIN mp_cp_uses u ON u.coupon_id = c.id
             WHERE c.is_active = 1
    ...
    public function testApplyCoupon(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (6, 1, 'Frank', '2026-03-09 12:00:00', {$orderTotal}, {$discountAmount})");

        $rows = $this->ztdQuery("SELECT order_total, discount_amount FROM mp_cp_uses WHERE id = 6");
        $this->assertCount(1, $rows);
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS use_count FROM mp_cp_uses WHERE coupon_id = 1"
        );
    ...
    public function testDeactivateCoupon(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT is_active FROM mp_cp_coupons WHERE code = 'HALF50'");
        $this->assertEquals(1, (int) $rows[0]['is_active']);

        // Deactivate
        $affected = $this->pdo->exec("UPDATE mp_cp_coupons SET is_active = 0 WHERE code = 'HALF50'");
        $this->assertSame(1, $affected);

        // Verify excluded from active lookup
        $rows = $this->ztdQuery(
            "SELECT code FROM mp_cp_coupons WHERE is_active = 1 ORDER BY code"
        );
        $codes = array_column($rows, 'code');
    ...
    {
        $this->pdo->exec("INSERT INTO mp_cp_uses VALUES (6, 1, 'NewUser', '2026-03-09 18:00:00', 100.00, 10.00)");
        $this->pdo->exec("UPDATE mp_cp_coupons SET is_active = 0 WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_cp_uses");
        $this->assertSame(6, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT is_active FROM mp_cp_coupons WHERE id = 1");
        $this->assertEquals(0, (int) $rows[0]['is_active']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_cp_uses")->fetchAll(PDO::FETCH_ASSOC);
