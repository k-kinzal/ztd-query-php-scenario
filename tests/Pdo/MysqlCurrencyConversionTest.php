 ...
 * conversion, UPDATE with CASE, and SUM CASE cross-currency totals (MySQL PDO).
 ...
 * @spec SPEC-10.2.177
 */
class MysqlCurrencyConversionTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_fx_account (
                id INT PRIMARY KEY,
                name VARCHAR(100),
                currency VARCHAR(3),
                balance DECIMAL(12,2)
            )',
            'CREATE TABLE mp_fx_txn (
                id INT PRIMARY KEY,
                account_id INT,
                amount DECIMAL(12,2),
                currency VARCHAR(3),
    ...
    {
        return ['mp_fx_txn', 'mp_fx_rate', 'mp_fx_account'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_fx_account VALUES (1, 'US Operations', 'USD', 50000.00)");
        $this->pdo->exec("INSERT INTO mp_fx_account VALUES (2, 'EU Operations', 'EUR', 40000.00)");
        $this->pdo->exec("INSERT INTO mp_fx_account VALUES (3, 'UK Operations', 'GBP', 30000.00)");
        $this->pdo->exec("INSERT INTO mp_fx_account VALUES (4, 'JP Operations', 'JPY', 5000000.00)");

        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (1, 1, 5000.00, 'USD', 'Client payment', '2025-01-15')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (2, 1, -2000.00, 'USD', 'Vendor payment', '2025-01-20')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (3, 2, 3000.00, 'EUR', 'Service revenue', '2025-01-18')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (4, 2, -1500.00, 'EUR', 'Office rent', '2025-02-01')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (5, 3, 4000.00, 'GBP', 'Consulting fee', '2025-01-25')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (6, 3, -500.00, 'GBP', 'Travel expense', '2025-02-05')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (7, 4, 500000.00, 'JPY', 'License fee', '2025-01-30')");
        $this->pdo->exec("INSERT INTO mp_fx_txn VALUES (8, 4, -200000.00, 'JPY', 'Equipment', '2025-02-10')");

        $this->pdo->exec("INSERT INTO mp_fx_rate VALUES (1, 'USD', 'USD', 1.000000, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_fx_rate VALUES (2, 'EUR', 'USD', 1.085000, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_fx_rate VALUES (3, 'GBP', 'USD', 1.270000, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_fx_rate VALUES (4, 'JPY', 'USD', 0.006700, '2025-01-01')");
    }
    ...
    public function testRoundWithCurrencyConversion(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.name, a.currency, a.balance,
             ...
             FROM mp_fx_account a
             JOIN mp_fx_rate r ON r.from_currency = a.currency AND r.to_currency = 'USD'
    ...
    {
        $this->ztdExec(
            "UPDATE mp_fx_account SET balance =
                CASE currency
        ...
        $rows = $this->ztdQuery("SELECT name, currency, balance FROM mp_fx_account ORDER BY id");
        $this->assertCount(4, $rows);
    ...
    public function testSumCaseCurrencyBreakdown(): void
    {
        $rows = $this->ztdQuery(
            "SELECT
             ...
             FROM mp_fx_txn t
             JOIN mp_fx_rate r ON r.from_currency = t.currency AND r.to_currency = 'USD'"
        );
    ...
    {
        $this->ztdExec("UPDATE mp_fx_account SET balance = 99999.99 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT balance FROM mp_fx_account WHERE id = 1");
        $this->assertEqualsWithDelta(99999.99, (float) $rows[0]['balance'], 0.01);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_fx_account")->fetchAll(PDO::FETCH_ASSOC);
