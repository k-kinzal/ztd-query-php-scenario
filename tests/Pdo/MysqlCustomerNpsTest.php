...
/**
 * Tests a customer feedback NPS scenario through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.162
 */
class MysqlCustomerNpsTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_nps_customers (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                segment VARCHAR(255),
                signup_date TEXT
            )',
            'CREATE TABLE mp_nps_surveys (
                id INT AUTO_INCREMENT PRIMARY KEY,
                customer_id INT,
                score INT,
                channel VARCHAR(255),
    ...
    {
        return ['mp_nps_surveys', 'mp_nps_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 6 customers
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (1, 'Tech Corp', 'enterprise', '2024-01-15')");
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (2, 'Shop LLC', 'smb', '2024-06-20')");
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (3, 'Dev Studio', 'startup', '2025-01-10')");
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (4, 'Mega Inc', 'enterprise', '2024-03-01')");
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (5, 'Quick Biz', 'smb', '2025-05-15')");
        $this->pdo->exec("INSERT INTO mp_nps_customers VALUES (6, 'Solo App', 'startup', '2025-09-01')");

        // 9 surveys (customer 6 has none)
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (1, 1, 9, 'email', '2025-06-01', 'Great service')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (2, 1, 10, 'web', '2025-12-15', 'Excellent support')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (3, 2, 7, 'email', '2025-07-20', 'Okay experience')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (4, 3, 3, 'web', '2025-08-10', 'Too expensive')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (5, 3, 6, 'email', '2026-01-05', 'Improving but slow')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (6, 4, 8, 'phone', '2025-09-12', 'Decent product')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (7, 4, 9, 'web', '2026-02-01', 'Love the new feature')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (8, 5, 2, 'phone', '2025-10-30', 'Very disappointed')");
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (9, 5, 4, 'email', '2026-02-15', 'Still not great')");
    }
    ...
    public function testCustomersWithoutFeedback(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.name, c.segment
             FROM mp_nps_customers c
             LEFT JOIN mp_nps_surveys s ON s.customer_id = c.id
             WHERE s.id IS NULL
             ORDER BY c.id"
        );
    ...
    public function testPreparedScoreRange(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT c.name, s.score, s.channel
             FROM mp_nps_surveys s
             JOIN mp_nps_customers c ON c.id = s.customer_id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_nps_surveys VALUES (10, 6, 8, 'web', '2026-03-01', 'First feedback')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_nps_surveys");
        $this->assertEquals(10, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM mp_nps_surveys')->fetchAll(PDO::FETCH_ASSOC);
