...
/**
 * Tests CREATE TABLE mid-session then DML on the new table (MySQL PDO).
 *
 * The ZTD session starts with only mp_dmd_base reflected. A new table mp_dmd_dynamic
 ...
 * @spec SPEC-4.1
 */
class MysqlDdlMidSessionDmlTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mp_dmd_base (id INT PRIMARY KEY, name VARCHAR(50))';
    }

    protected function getTableNames(): array
    {
        return ['mp_dmd_base', 'mp_dmd_dynamic'];
    }

    protected function setUp(): void
    {
        // Drop mp_dmd_dynamic via raw connection in case a previous test left it
        $this->dropTable('mp_dmd_dynamic');
        parent::setUp();
    }
    ...
    {
        try {
            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'alpha')");

            $stmt = $this->pdo->query('SELECT id, value FROM mp_dmd_dynamic WHERE id = 1');
    ...
    public function testCreateTableThenInsertAndSelect(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'alpha')");
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (2, 'beta')");
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (3, 'gamma')");

            $stmt = $this->pdo->query('SELECT id, value FROM mp_dmd_dynamic ORDER BY id');
    ...
    public function testCreateTableThenUpdate(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'original')");

            $this->pdo->exec("UPDATE mp_dmd_dynamic SET value = 'updated' WHERE id = 1");

            $stmt = $this->pdo->query('SELECT value FROM mp_dmd_dynamic WHERE id = 1');
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function testCreateTableThenDelete(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'keep')");
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (2, 'remove')");

            $this->pdo->exec('DELETE FROM mp_dmd_dynamic WHERE id = 2');

            $stmt = $this->pdo->query('SELECT COUNT(*) AS cnt FROM mp_dmd_dynamic');
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO mp_dmd_base (id, name) VALUES (1, 'Alice')");
            $this->pdo->exec("INSERT INTO mp_dmd_base (id, name) VALUES (2, 'Bob')");

            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'score_90')");
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (2, 'score_80')");

            $stmt = $this->pdo->query("
                SELECT b.name, d.value
                FROM mp_dmd_base b
                JOIN mp_dmd_dynamic d ON d.id = b.id
                ORDER BY b.id
            ");
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testPhysicalIsolation(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE mp_dmd_dynamic (id INT PRIMARY KEY, value VARCHAR(50))');
            $this->pdo->exec("INSERT INTO mp_dmd_dynamic (id, value) VALUES (1, 'shadow_data')");
