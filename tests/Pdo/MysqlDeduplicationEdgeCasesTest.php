 ...
 * @spec SPEC-10.2.38
 */
class MysqlDeduplicationEdgeCasesTest extends AbstractMysqlPdoTestCase
{
    ...
    public function testDeleteDuplicatesKeepFirst(): void
    {
        ...
        // MySQL does not allow referencing the target table directly in a subquery
        // of a DELETE, so we use a derived table workaround.
        $this->pdo->exec("
    ...
    public function testMultiPassDedup(): void
    {
        ...
        $this->pdo->exec("
            ...
            AND id NOT IN (
                SELECT first_id FROM (
                    SELECT MIN(id) AS first_id FROM contacts WHERE email IS NOT NULL
                    GROUP BY email
                ) tmp
            )
            AND score < 80
