 ...
 * @spec SPEC-3.3
 */
class MysqlDerivedTableAndNestingTest extends AbstractMysqlPdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO dtn_products (id, name, price, category) VALUES
            (1, 'Widget', 10.00, 'basic'),
            (2, 'Gadget', 45.00, 'advanced'),
            (3, 'Gizmo', 200.00, 'premium'),
            (4, 'Doohickey', 30.00, 'basic')");
        $this->pdo->exec("INSERT INTO dtn_regions (id, name) VALUES
            (1, 'East'), (2, 'West'), (3, 'Central')");
        $this->pdo->exec("INSERT INTO dtn_sales (id, product_id, qty, sale_date) VALUES
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO dtn_sales (id, product_id, qty, sale_date) VALUES (7, 3, 20, '2025-04-01')");
    ...
    {
        $this->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM dtn_products")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical products table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM dtn_sales")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical sales table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM dtn_regions")
            ->fetchAll(PDO::FETCH_ASSOC);
