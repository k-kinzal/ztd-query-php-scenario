...
/**
 * Tests donation campaign tracking with fundraising goals and donor management (MySQL PDO).
 ...
 * @spec SPEC-10.2.171
 */
class MysqlDonationCampaignTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_dc_donor (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                email VARCHAR(200),
                joined_date TEXT
            )',
            'CREATE TABLE mp_dc_campaign (
                id INT AUTO_INCREMENT PRIMARY KEY,
                title VARCHAR(200),
                goal DECIMAL(12,2),
                raised DECIMAL(12,2),
    ...
    {
        return ['mp_dc_donation', 'mp_dc_campaign', 'mp_dc_donor'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Donors
        $this->pdo->exec("INSERT INTO mp_dc_donor VALUES (1, 'Alice', 'alice@example.com', '2024-01-15')");
        $this->pdo->exec("INSERT INTO mp_dc_donor VALUES (2, 'Bob', 'bob@example.com', '2024-03-20')");
        $this->pdo->exec("INSERT INTO mp_dc_donor VALUES (3, 'Carol', 'carol@example.com', '2024-06-01')");
        $this->pdo->exec("INSERT INTO mp_dc_donor VALUES (4, 'Dave', 'dave@example.com', '2025-01-10')");

        // Campaigns
        $this->pdo->exec("INSERT INTO mp_dc_campaign VALUES (1, 'School Library Fund', 10000.00, 6500.00, 'active', '2025-01-01', '2025-06-30')");
        $this->pdo->exec("INSERT INTO mp_dc_campaign VALUES (2, 'Community Garden', 5000.00, 5200.00, 'completed', '2025-01-01', '2025-03-31')");
        $this->pdo->exec("INSERT INTO mp_dc_campaign VALUES (3, 'Youth Sports Equipment', 8000.00, 0.00, 'active', '2025-07-01', '2025-12-31')");

        // Donations
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (1, 1, 1, 2000.00, '2025-01-15', 'Monthly pledge')");
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (2, 1, 2, 1500.00, '2025-02-10', NULL)");
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (3, 1, 1, 1000.00, '2025-03-01', 'Second donation')");
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (4, 1, 3, 2000.00, '2025-04-15', 'Matching gift')");
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (5, 2, 2, 3000.00, '2025-01-20', 'Corporate match')");
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (6, 2, 4, 2200.00, '2025-02-28', NULL)");
    }
    ...
    {
        $this->pdo->exec(
            "INSERT INTO mp_dc_donation (amount, donor_id, campaign_id, donated_at, id, note)
    ...
    {
        $this->ztdExec("UPDATE mp_dc_campaign SET raised = raised + 500.00 WHERE id = 3");

        $rows = $this->ztdQuery("SELECT raised FROM mp_dc_campaign WHERE id = 3");
        $this->assertCount(1, $rows);
    ...
    {
        $this->ztdExec("UPDATE mp_dc_campaign SET raised = raised + 750.00 WHERE id = 1");
        $this->ztdExec("UPDATE mp_dc_campaign SET raised = raised + 250.00 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT raised FROM mp_dc_campaign WHERE id = 1");
        $this->assertCount(1, $rows);
    ...
    public function testUniqueDonorsPerCampaign(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.title,
             ...
             FROM mp_dc_campaign c
             LEFT JOIN mp_dc_donation d ON d.campaign_id = c.id
    ...
    public function testDonorGivingSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT dn.name,
             ...
             FROM mp_dc_donor dn
             JOIN mp_dc_donation d ON d.donor_id = dn.id
    ...
    {
        $affected = $this->ztdExec("DELETE FROM mp_dc_donation WHERE id = 6");
    ...
    public function testPreparedDonationsByDonorEmail(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.amount, d.donated_at, c.title
             FROM mp_dc_donation d
             JOIN mp_dc_donor dn ON dn.id = d.donor_id
             JOIN mp_dc_campaign c ON c.id = d.campaign_id
             WHERE dn.email = ?
    ...
    {
        $this->pdo->exec("INSERT INTO mp_dc_donation VALUES (7, 3, 1, 100.00, '2025-07-01', 'Test')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_dc_donation");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_dc_donation")->fetchAll(PDO::FETCH_ASSOC);
