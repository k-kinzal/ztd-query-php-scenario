 ...
 * and GROUP BY with HAVING for price anomaly detection (MySQL PDO).
 * @spec SPEC-10.2.143
 */
class MysqlDynamicPricingTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_dp_products (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                category VARCHAR(100)
            )',
            'CREATE TABLE mp_dp_price_history (
                id INT AUTO_INCREMENT PRIMARY KEY,
                product_id INT,
                price DECIMAL(10,2),
                effective_date TEXT
            )',
            'CREATE TABLE mp_dp_competitor_prices (
                id INT AUTO_INCREMENT PRIMARY KEY,
                product_id INT,
    ...
    {
        return ['mp_dp_competitor_prices', 'mp_dp_price_history', 'mp_dp_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Products
        $this->pdo->exec("INSERT INTO mp_dp_products VALUES (1, 'Widget A', 'Electronics')");
        $this->pdo->exec("INSERT INTO mp_dp_products VALUES (2, 'Gadget B', 'Electronics')");
        $this->pdo->exec("INSERT INTO mp_dp_products VALUES (3, 'Tool C', 'Hardware')");

        // Price history
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (1, 1, 29.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (2, 1, 24.99, '2025-03-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (3, 1, 27.99, '2025-06-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (4, 2, 49.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (5, 2, 44.99, '2025-04-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (6, 2, 39.99, '2025-07-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (7, 3, 19.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (8, 3, 22.99, '2025-05-01')");

        // Competitor prices
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (1, 1, 'CompetitorX', 26.99, '2025-06-15')");
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (2, 1, 'CompetitorY', 28.99, '2025-06-15')");
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (3, 2, 'CompetitorX', 42.99, '2025-07-15')");
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (4, 2, 'CompetitorY', 37.99, '2025-07-15')");
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (5, 3, 'CompetitorX', 21.99, '2025-05-15')");
        $this->pdo->exec("INSERT INTO mp_dp_competitor_prices VALUES (6, 3, 'CompetitorY', 24.99, '2025-05-15')");
    }
    ...
    public function testCurrentPricePerProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name, ph.price, ph.effective_date
             FROM mp_dp_products p
             JOIN mp_dp_price_history ph ON ph.product_id = p.id
                 AND ph.effective_date = (
                     SELECT MAX(ph2.effective_date)
                     FROM mp_dp_price_history ph2
    ...
    public function testCompetitorComparison(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name, comp.competitor_name, ph.price AS own_price,
             ...
             JOIN mp_dp_competitor_prices comp ON comp.product_id = p.id
    ...
    {
        $this->pdo->exec("INSERT INTO mp_dp_price_history VALUES (9, 1, 25.99, '2025-08-01')");

        // ZTD sees the new entry
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_dp_price_history");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_dp_price_history")->fetchAll(PDO::FETCH_ASSOC);
