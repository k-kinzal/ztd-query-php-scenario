 ...
 * @spec SPEC-10.2.172
 */
class MysqlEmployeeOrgChartTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mp_eoc_employees (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_eoc_employees VALUES (1, 'Alice', 'CEO', NULL)");
        $this->pdo->exec("INSERT INTO mp_eoc_employees VALUES (2, 'Bob', 'VP Engineering', 1)");
        $this->pdo->exec("INSERT INTO mp_eoc_employees VALUES (3, 'Carol', 'VP Sales', 1)");
        $this->pdo->exec("INSERT INTO mp_eoc_employees VALUES (4, 'Dave', 'Senior Engineer', 2)");
        $this->pdo->exec("INSERT INTO mp_eoc_employees VALUES (5, 'Eve', 'Sales Rep', 3)");
    }
    ...
    public function testSelfJoinEmployeeManager(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name, m.name AS manager_name
             FROM mp_eoc_employees e
             LEFT JOIN mp_eoc_employees m ON e.manager_id = m.id
             ORDER BY e.id"
        );
    ...
    public function testCountDirectReports(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name, COUNT(e.id) AS direct_reports
             FROM mp_eoc_employees m
             LEFT JOIN mp_eoc_employees e ON e.manager_id = m.id
    ...
    {
        $this->ztdExec(
            "INSERT INTO mp_eoc_employees VALUES (6, 'Frank', 'Engineer', 2)"
        );
    ...
    public function testSelfJoinWithPreparedStatement(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT e.name
             FROM mp_eoc_employees e
             JOIN mp_eoc_employees m ON e.manager_id = m.id
             WHERE m.role = ?
             ORDER BY e.name",
            ['CEO']
        );
    ...
    public function testSubqueryReferencesSameTable(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM mp_eoc_employees
             WHERE id IN (SELECT manager_id FROM mp_eoc_employees WHERE manager_id IS NOT NULL)
             ORDER BY name"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_eoc_employees");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_eoc_employees")->fetchAll(PDO::FETCH_ASSOC);
