 ...
 */
class MysqlEventLogTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_el_events (
                id INT PRIMARY KEY,
    ...
    {
        return ['mp_el_events', 'mp_el_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_el_users VALUES (1, 'Alice', 'admin')");
        $this->pdo->exec("INSERT INTO mp_el_users VALUES (2, 'Bob', 'editor')");
        $this->pdo->exec("INSERT INTO mp_el_users VALUES (3, 'Charlie', 'viewer')");

        // Event log entries
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (1, 'create', 'article', 100, 1, '{\"title\":\"Hello\"}', '2024-03-01 09:00:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (2, 'update', 'article', 100, 2, '{\"field\":\"body\"}', '2024-03-01 10:30:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (3, 'create', 'article', 101, 1, '{\"title\":\"World\"}', '2024-03-02 08:15:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (4, 'delete', 'comment', 200, 2, '{\"reason\":\"spam\"}', '2024-03-02 14:00:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (5, 'update', 'article', 100, 1, '{\"field\":\"title\"}', '2024-03-03 11:00:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (6, 'create', 'comment', 201, 3, '{\"body\":\"Nice!\"}', '2024-03-03 15:30:00')");
    }
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT id, event_type, created_at FROM mp_el_events
    ...
    public function testPreparedTimeRange(): void
    {
        $stmt = $this->pdo->prepare(
            'SELECT COUNT(*) AS cnt FROM mp_el_events
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT id, created_at FROM mp_el_events
    ...
    {
        // Append new events
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (7, 'update', 'article', 101, 2, '{\"field\":\"body\"}', '2024-03-04 09:00:00')");
        $this->pdo->exec("INSERT INTO mp_el_events VALUES (8, 'delete', 'article', 100, 1, '{\"reason\":\"archived\"}', '2024-03-04 10:00:00')");

        $rows = $this->ztdQuery(
            "SELECT id, event_type, entity_id FROM mp_el_events
    ...
    public function testEventLogWithUserJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.event_type, e.entity_type, e.entity_id,
             ...
             FROM mp_el_events e
             JOIN mp_el_users u ON e.user_id = u.id
    ...
    public function testActivityPerUser(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.name, COUNT(e.id) AS event_count
             FROM mp_el_users u
             LEFT JOIN mp_el_events e ON u.id = e.user_id
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT event_type, created_at FROM mp_el_events
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_el_events");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Nothing in physical table
        $this->pdo->disableZtd();
        $stmt = $this->pdo->query('SELECT COUNT(*) FROM mp_el_events');
