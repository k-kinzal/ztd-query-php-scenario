...
class MysqlExpressionClausesTest extends AbstractMysqlPdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pdo_expr_test VALUES (5, 'Eve', 50, NULL, 'C', NULL)");
    }
    ...
    {
        $stmt = $this->pdo->query("
            SELECT name FROM pdo_expr_test
    ...
    {
        $stmt = $this->pdo->query('SELECT name FROM pdo_expr_test ORDER BY COALESCE(bonus, 0)');
        $rows = $stmt->fetchAll(PDO::FETCH_COLUMN);
    ...
    {
        $stmt = $this->pdo->query("SELECT name FROM pdo_expr_test WHERE search_term LIKE '%!%%' ESCAPE '!'");
        $rows = $stmt->fetchAll(PDO::FETCH_COLUMN);
        $this->assertCount(2, $rows); // 'hello%world', '100%_done'
    }

    /**
     * LIKE with ESCAPE clause for literal _.
     */
    public function testLikeWithEscapeForUnderscore(): void
    {
        $stmt = $this->pdo->query("SELECT name FROM pdo_expr_test WHERE search_term LIKE '%!_%' ESCAPE '!'");
        $rows = $stmt->fetchAll(PDO::FETCH_COLUMN);
        $this->assertCount(2, $rows); // '100%_done', 'test_data'
    }
