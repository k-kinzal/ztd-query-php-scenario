 ...
 * with ZTD shadow store on MySQL.
 */
class MysqlFetchModeAdvancedTest extends TestCase
{
    private ZtdPdo $pdo;
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $raw->exec('DROP TABLE IF EXISTS orders_m');
        $raw->exec('CREATE TABLE orders_m (id INT PRIMARY KEY, customer VARCHAR(50), product VARCHAR(50), amount INT)');
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec('DELETE FROM orders_m');
        $this->pdo->exec("INSERT INTO orders_m VALUES (1, 'Alice', 'Widget', 100)");
        $this->pdo->exec("INSERT INTO orders_m VALUES (2, 'Alice', 'Gadget', 200)");
        $this->pdo->exec("INSERT INTO orders_m VALUES (3, 'Bob', 'Widget', 150)");
        $this->pdo->exec("INSERT INTO orders_m VALUES (4, 'Charlie', 'Gizmo', 300)");
        $this->pdo->exec("INSERT INTO orders_m VALUES (5, 'Alice', 'Gizmo', 50)");
    }

    public function testFetchGroupByCustomer(): void
    {
        $stmt = $this->pdo->query('SELECT customer, product, amount FROM orders_m ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT id, customer, product FROM orders_m ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT customer, product FROM orders_m ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT customer, product FROM orders_m WHERE id = 1');
    ...
    {
        $stmt = $this->pdo->query('SELECT product FROM orders_m ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT customer, product, amount FROM orders_m WHERE id = 1');
