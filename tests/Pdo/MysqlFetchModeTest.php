...
class MysqlFetchModeTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mysql_fetch_test (id, name, score) VALUES (3, 'Charlie', 70)");
    }

    public function testQueryWithFetchModeArg(): void
    {
        $stmt = $this->pdo->query('SELECT * FROM mysql_fetch_test ORDER BY id', PDO::FETCH_ASSOC);
        $rows = $stmt->fetchAll();
    ...
    {
        $stmt = $this->pdo->query('SELECT * FROM mysql_fetch_test WHERE id = 1');
    ...
    {
        $stmt = $this->pdo->query('SELECT id, name, score FROM mysql_fetch_test WHERE id = 1');
    ...
    {
        $stmt = $this->pdo->query('SELECT id, name FROM mysql_fetch_test WHERE id = 1');
    ...
    {
        $stmt = $this->pdo->query('SELECT * FROM mysql_fetch_test ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT id, name FROM mysql_fetch_test ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT name FROM mysql_fetch_test ORDER BY id');
    ...
    {
        $stmt = $this->pdo->query('SELECT id, name, score FROM mysql_fetch_test LIMIT 1');
        $this->assertSame(3, $stmt->columnCount());
    }

    public function testRowCount(): void
    {
        $stmt = $this->pdo->prepare('UPDATE mysql_fetch_test SET score = score + 5 WHERE score < 100');
        $stmt->execute();
