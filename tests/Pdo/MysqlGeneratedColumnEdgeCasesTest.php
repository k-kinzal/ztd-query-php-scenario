 ...
 * @spec SPEC-10.2.22
 */
class MysqlGeneratedColumnEdgeCasesTest extends AbstractMysqlPdoTestCase
{
    ...
    {
        $rows = $this->ztdQuery("SELECT name FROM products WHERE total > 100 ORDER BY name");
    ...
    {
        $rows = $this->ztdQuery("SELECT name, total FROM products ORDER BY total DESC");
        $this->assertSame('Gadget', $rows[0]['name']);
    }
    ...
    public function testPreparedSelectWithGeneratedColumn(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name, total FROM products WHERE total > ? ORDER BY total",
            [100]
        );
        $this->assertCount(1, $rows);
        $this->assertSame('Gadget', $rows[0]['name']);
    ...
    {
        $this->pdo->exec("DELETE FROM products WHERE total < 100");
