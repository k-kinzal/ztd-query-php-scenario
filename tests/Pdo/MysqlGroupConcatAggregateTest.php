...
/**
 * GROUP_CONCAT aggregate function on shadow data via MySQL PDO.
 ...
 *
 * Cross-platform parity with GroupConcatAggregateTest (MySQLi).
 * @spec SPEC-3.3
 */
class MysqlGroupConcatAggregateTest extends AbstractMysqlPdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO gca_posts (id, title) VALUES
            (1, 'Getting Started'),
            (2, 'Advanced Tips'),
            (3, 'FAQ')");

        $this->pdo->exec("INSERT INTO gca_tags (id, post_id, tag_name) VALUES
            (1, 1, 'beginner'),
            (2, 1, 'tutorial'),
            (3, 1, 'setup'),
            (4, 2, 'advanced'),
    ...
    public function testGroupConcatDistinct(): void
    {
        try {
            // Add duplicate tag
            $this->pdo->exec("INSERT INTO gca_tags (id, post_id, tag_name) VALUES (7, 2, 'tutorial')");
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO gca_tags (id, post_id, tag_name) VALUES (7, 3, 'help'), (8, 3, 'support')");
    ...
    {
        $this->disableZtd();
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM gca_posts");
