 ...
 * CASE-based SLA evaluation, LEFT JOIN metrics, and date string comparison (MySQL PDO).
 * @spec SPEC-10.2.126
 */
class MysqlHelpDeskSlaTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_hd_tickets (
    ...
    {
        return ['mp_hd_responses', 'mp_hd_tickets'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Tickets
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (1, 'Login broken', 'critical', 'resolved', '2025-10-01 08:00', 'agent_a')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (2, 'Slow dashboard', 'high', 'open', '2025-10-01 09:30', 'agent_a')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (3, 'Feature request', 'low', 'open', '2025-10-01 10:00', 'agent_b')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (4, 'Data export fail', 'critical', 'in_progress', '2025-10-02 07:00', 'agent_b')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (5, 'UI glitch', 'medium', 'resolved', '2025-10-02 11:00', 'agent_c')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (6, 'Password reset', 'high', 'resolved', '2025-10-03 06:00', 'agent_a')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (7, 'API timeout', 'critical', 'open', '2025-10-03 14:00', 'agent_c')");
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (8, 'Billing question', 'low', 'resolved', '2025-10-04 09:00', NULL)");

        // Responses
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (1, 1, 'agent_a', '2025-10-01 08:15', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (2, 1, 'agent_a', '2025-10-01 09:00', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (3, 2, 'agent_a', '2025-10-01 11:00', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (4, 2, 'agent_a', '2025-10-01 11:30', 1)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (5, 4, 'agent_b', '2025-10-02 07:30', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (6, 4, 'agent_b', '2025-10-02 08:00', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (7, 5, 'agent_c', '2025-10-02 11:20', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (8, 5, 'agent_c', '2025-10-02 12:00', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (9, 6, 'agent_a', '2025-10-03 06:10', 0)");
        $this->pdo->exec("INSERT INTO mp_hd_responses VALUES (10, 6, 'agent_a', '2025-10-03 06:30', 0)");
    }
    ...
    public function testFirstResponseTime(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.id, t.subject, t.priority,
             ...
             FROM mp_hd_tickets t
             LEFT JOIN mp_hd_responses r ON r.ticket_id = t.id AND r.is_internal = 0
    ...
    public function testAgentWorkload(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.assigned_agent,
             ...
             FROM mp_hd_tickets t
             LEFT JOIN mp_hd_responses r ON r.ticket_id = t.id
    ...
    public function testUnrespondedTickets(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.id, t.subject, t.priority, t.status
             FROM mp_hd_tickets t
             WHERE NOT EXISTS (
                 SELECT 1 FROM mp_hd_responses r
    ...
    {
        $this->pdo->exec("INSERT INTO mp_hd_tickets VALUES (9, 'New issue', 'low', 'open', '2025-10-05 10:00', 'agent_a')");
        $this->pdo->exec("UPDATE mp_hd_tickets SET status = 'closed' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_hd_tickets");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT status FROM mp_hd_tickets WHERE id = 1");
        $this->assertSame('closed', $rows[0]['status']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_hd_tickets")->fetchAll(PDO::FETCH_ASSOC);
