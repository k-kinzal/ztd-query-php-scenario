...
/**
 * Tests INSERT...SELECT with ORDER BY and LIMIT on MySQL via PDO.
 *
 ...
 * @spec SPEC-4.1
 */
class MysqlInsertSelectLimitTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_isl_scores (
                id INT PRIMARY KEY,
                player VARCHAR(30),
                score INT,
                created_at DATETIME
            ) ENGINE=InnoDB',
            'CREATE TABLE mp_isl_leaderboard (
    ...
    {
        return ['mp_isl_leaderboard', 'mp_isl_scores'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_isl_scores (id, player, score, created_at) VALUES
    ...
    public function testInsertSelectWithOrderByLimit(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO mp_isl_leaderboard (player, score)
                 SELECT player, score FROM mp_isl_scores ORDER BY score DESC LIMIT 3"
            );

            $rows = $this->ztdQuery("SELECT player, score FROM mp_isl_leaderboard ORDER BY score DESC");
    ...
    public function testInsertSelectWithWhereAndLimit(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO mp_isl_leaderboard (player, score)
                 SELECT player, score FROM mp_isl_scores
                 WHERE score >= 50
    ...
    {
        $this->pdo->exec("INSERT INTO mp_isl_scores (id, player, score, created_at) VALUES (7, 'Grace', 99, '2025-01-02 10:00:00')");
    ...
    public function testInsertSelectWithOffset(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO mp_isl_leaderboard (player, score)
                 SELECT player, score FROM mp_isl_scores ORDER BY score DESC LIMIT 3 OFFSET 1"
            );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_isl_leaderboard")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical leaderboard table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_isl_scores")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical scores table should be empty');
    }
}
