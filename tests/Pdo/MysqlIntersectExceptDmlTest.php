 ...
 */
class MysqlIntersectExceptDmlTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE my_setop_list_a (id INT PRIMARY KEY, item VARCHAR(50)) ENGINE=InnoDB',
            'CREATE TABLE my_setop_list_b (id INT PRIMARY KEY, item VARCHAR(50)) ENGINE=InnoDB',
            'CREATE TABLE my_setop_result (id INT AUTO_INCREMENT PRIMARY KEY, item VARCHAR(50)) ENGINE=InnoDB',
        ];
    }

    protected function getTableNames(): array
    {
        return ['my_setop_result', 'my_setop_list_b', 'my_setop_list_a'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO my_setop_list_a (id, item) VALUES (1, 'apple')");
        $this->pdo->exec("INSERT INTO my_setop_list_a (id, item) VALUES (2, 'banana')");
        $this->pdo->exec("INSERT INTO my_setop_list_a (id, item) VALUES (3, 'cherry')");
        $this->pdo->exec("INSERT INTO my_setop_list_a (id, item) VALUES (4, 'date')");

        // list_b: banana, cherry, elderberry, fig
        $this->pdo->exec("INSERT INTO my_setop_list_b (id, item) VALUES (1, 'banana')");
        $this->pdo->exec("INSERT INTO my_setop_list_b (id, item) VALUES (2, 'cherry')");
        $this->pdo->exec("INSERT INTO my_setop_list_b (id, item) VALUES (3, 'elderberry')");
        $this->pdo->exec("INSERT INTO my_setop_list_b (id, item) VALUES (4, 'fig')");
    }
    ...
    public function testInsertSelectIntersect(): void
    {
        try {
            $this->ztdExec(
                "INSERT INTO my_setop_result (item)
                 SELECT item FROM my_setop_list_a
                 INTERSECT
                 SELECT item FROM my_setop_list_b"
            );

            $rows = $this->ztdQuery(
                "SELECT item FROM my_setop_result ORDER BY item"
            );
    ...
    public function testDeleteWhereInIntersect(): void
    {
        try {
            ...
            $rows = $this->ztdQuery(
                "SELECT item FROM my_setop_list_a ORDER BY item"
            );
    ...
    public function testUpdateWhereInExcept(): void
    {
        try {
            ...
            $rows = $this->ztdQuery(
                "SELECT id, item FROM my_setop_list_a ORDER BY id"
            );
    ...
    public function testPreparedInsertSelectIntersect(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "INSERT INTO my_setop_result (item)
                 SELECT item FROM my_setop_list_a WHERE id > ?
                 INTERSECT
                 SELECT item FROM my_setop_list_b"
            );
            $stmt->execute([1]);
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $cnt = $this->pdo->query("SELECT COUNT(*) FROM my_setop_list_a")->fetchColumn();
        $this->assertSame(0, (int) $cnt, 'my_setop_list_a should be empty');

        $cnt = $this->pdo->query("SELECT COUNT(*) FROM my_setop_list_b")->fetchColumn();
        $this->assertSame(0, (int) $cnt, 'my_setop_list_b should be empty');

        $cnt = $this->pdo->query("SELECT COUNT(*) FROM my_setop_result")->fetchColumn();
        $this->assertSame(0, (int) $cnt, 'my_setop_result should be empty');
    }
}
