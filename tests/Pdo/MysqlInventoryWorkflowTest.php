 ...
 * @spec SPEC-4.2
 */
class MysqlInventoryWorkflowTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_inv_products (
                id INT PRIMARY KEY,
    ...
    {
        return ['mp_inv_orders', 'mp_inv_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_inv_products VALUES (1, 'SKU-001', 'Widget A', 100, 20, 9.99)");
        $this->pdo->exec("INSERT INTO mp_inv_products VALUES (2, 'SKU-002', 'Widget B', 50, 15, 19.99)");
        $this->pdo->exec("INSERT INTO mp_inv_products VALUES (3, 'SKU-003', 'Gadget C', 5, 10, 49.99)");
        $this->pdo->exec("INSERT INTO mp_inv_products VALUES (4, 'SKU-004', 'Gadget D', 0, 5, 99.99)");
        $this->pdo->exec("INSERT INTO mp_inv_products VALUES (5, 'SKU-005', 'Gizmo E', 200, 50, 4.99)");
    }
    ...
    {
        $this->pdo->exec("UPDATE mp_inv_products SET stock = stock - 10 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT stock FROM mp_inv_products WHERE id = 1");
    ...
    public function testConditionalStockDecrement(): void
    {
        ...
        $result = $this->pdo->exec(
            "UPDATE mp_inv_products SET stock = stock - 3 WHERE id = 3 AND stock >= 3"
        );
        $this->assertSame(1, $result);

        $rows = $this->ztdQuery("SELECT stock FROM mp_inv_products WHERE id = 3");
        $this->assertEquals(2, (int) $rows[0]['stock']);

        // Try to order more than remaining stock
        $result = $this->pdo->exec(
            "UPDATE mp_inv_products SET stock = stock - 5 WHERE id = 3 AND stock >= 5"
        );
    ...
    {
        // Step 1: Check stock
        $rows = $this->ztdQuery("SELECT stock FROM mp_inv_products WHERE id = 2");
        $initialStock = (int) $rows[0]['stock'];
        $this->assertEquals(50, $initialStock);

        // Step 2: Record order
        $this->pdo->exec("INSERT INTO mp_inv_orders VALUES (1, 2, 5, 'sale', '2024-03-15')");

        // Step 3: Decrement stock
        $this->pdo->exec("UPDATE mp_inv_products SET stock = stock - 5 WHERE id = 2");
        ...
        $rows = $this->ztdQuery("SELECT quantity, order_type FROM mp_inv_orders WHERE product_id = 2");
    ...
    {
        $this->pdo->exec(
            "UPDATE mp_inv_products SET stock = CASE
        ...
        $rows = $this->ztdQuery(
            "SELECT id, stock FROM mp_inv_products ORDER BY id"
        );
    ...
    {
        for ($i = 0; $i < 5; $i++) {
            $this->pdo->exec("UPDATE mp_inv_products SET stock = stock - 1 WHERE id = 1");
        }
    ...
    {
        $this->pdo->exec("UPDATE mp_inv_products SET stock = stock - 50 WHERE id = 1");
        $this->pdo->exec("INSERT INTO mp_inv_orders VALUES (1, 1, 50, 'sale', '2024-03-15')");
        ...
        $this->pdo->disableZtd();
        $stmt = $this->pdo->query('SELECT COUNT(*) FROM mp_inv_products');
