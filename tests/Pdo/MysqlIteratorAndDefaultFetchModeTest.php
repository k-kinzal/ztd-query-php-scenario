 ...
 */
class MysqlIteratorAndDefaultFetchModeTest extends TestCase
{
    private ZtdPdo $pdo;
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $raw->exec('DROP TABLE IF EXISTS iter_users_m');
        $raw->exec('CREATE TABLE iter_users_m (id INT PRIMARY KEY, name VARCHAR(50), score INT)');
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO iter_users_m (id, name, score) VALUES (1, 'Alice', 100)");
        $this->pdo->exec("INSERT INTO iter_users_m (id, name, score) VALUES (2, 'Bob', 85)");
        $this->pdo->exec("INSERT INTO iter_users_m (id, name, score) VALUES (3, 'Charlie', 70)");
    }

    public function testForeachOnStatement(): void
    {
        $stmt = $this->pdo->query('SELECT name FROM iter_users_m ORDER BY id');
        $names = [];
        foreach ($stmt as $row) {
            $names[] = $row['name'];
        }
    ...
    {
        $this->pdo->exec("INSERT INTO iter_users_m (id, name, score) VALUES (4, 'Diana', 95)");
    ...
    {
        $this->pdo->exec("DELETE FROM iter_users_m WHERE id = 2");
    ...
    public function testDefaultFetchModeAssoc(): void
    {
        ...
        $stmt = $this->pdo->query('SELECT name, score FROM iter_users_m WHERE id = 1');
        $row = $stmt->fetch();
    ...
    {
        $stmt = $this->pdo->query('SELECT name FROM iter_users_m ORDER BY id', PDO::FETCH_COLUMN, 0);
        $names = $stmt->fetchAll();
        $this->assertSame(['Alice', 'Bob', 'Charlie'], $names);
    }
    ...
    {
        $stmt = $this->pdo->prepare('SELECT name FROM iter_users_m WHERE score > ?');
        $stmt->execute([75]);
    ...
    {
        $stmt = $this->pdo->query('SELECT id, name FROM iter_users_m ORDER BY id');
