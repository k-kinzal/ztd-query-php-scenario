 ...
 * @spec SPEC-10.2.138
 */
class MysqlKanbanBoardTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_kb_boards (
                id INT AUTO_INCREMENT PRIMARY KEY,
                board_name VARCHAR(255),
                created_at TEXT
            )',
            'CREATE TABLE mp_kb_tasks (
                id INT AUTO_INCREMENT PRIMARY KEY,
                board_id INT,
                title VARCHAR(255),
                status VARCHAR(50),
            ...
            'CREATE TABLE mp_kb_task_dependencies (
                id INT AUTO_INCREMENT PRIMARY KEY,
                task_id INT,
    ...
    {
        return ['mp_kb_task_dependencies', 'mp_kb_tasks', 'mp_kb_boards'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 boards
        $this->pdo->exec("INSERT INTO mp_kb_boards VALUES (1, 'Sprint 42', '2025-10-01')");
        $this->pdo->exec("INSERT INTO mp_kb_boards VALUES (2, 'Backlog', '2025-09-01')");

        // 6 tasks on Sprint 42
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (1, 1, 'Auth module', 'done', 'Alice', 1, '2025-10-01', '2025-10-05')");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (2, 1, 'API endpoints', 'in_progress', 'Bob', 1, '2025-10-01', '2025-10-10')");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (3, 1, 'Unit tests', 'todo', 'Carol', 2, '2025-10-01', '2025-10-12')");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (4, 1, 'Documentation', 'todo', 'Alice', 3, '2025-10-01', '2025-10-15')");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (5, 1, 'Code review', 'review', 'Bob', 2, '2025-10-01', '2025-10-08')");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (6, 1, 'Deploy script', 'todo', 'Dave', 2, '2025-10-01', '2025-10-14')");

        // 2 tasks on Backlog
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (7, 2, 'Refactor DB layer', 'todo', 'Carol', 3, '2025-09-01', NULL)");
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (8, 2, 'Performance tuning', 'todo', NULL, 3, '2025-09-01', NULL)");

        // Dependencies
        $this->pdo->exec("INSERT INTO mp_kb_task_dependencies VALUES (1, 3, 1)"); // Unit tests depends on Auth module
        $this->pdo->exec("INSERT INTO mp_kb_task_dependencies VALUES (2, 3, 2)"); // Unit tests depends on API endpoints
        $this->pdo->exec("INSERT INTO mp_kb_task_dependencies VALUES (3, 4, 2)"); // Documentation depends on API endpoints
        $this->pdo->exec("INSERT INTO mp_kb_task_dependencies VALUES (4, 6, 5)"); // Deploy script depends on Code review
    }
    ...
    public function testBoardTaskCountByStatus(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.board_name, t.status, COUNT(*) AS task_count
             FROM mp_kb_tasks t
             JOIN mp_kb_boards b ON b.id = t.board_id
        ...
        $rows = $this->ztdQuery(
            "SELECT t.id, t.title
             ...
             WHERE t.status = 'todo'
               AND EXISTS (
                   SELECT 1
                   FROM mp_kb_task_dependencies d
                   JOIN mp_kb_tasks dep ON dep.id = d.depends_on_task_id
    ...
    {
        $this->pdo->exec("INSERT INTO mp_kb_tasks VALUES (9, 1, 'Hotfix', 'todo', 'Eve', 1, '2025-10-09', '2025-10-10')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_kb_tasks");
        $this->assertSame(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_kb_tasks")->fetchAll(PDO::FETCH_ASSOC);
