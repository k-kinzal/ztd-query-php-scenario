...
/**
 * Tests an employee leave balance scenario through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.155
 */
class MysqlLeaveBalanceTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_lv_employees (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                department VARCHAR(50),
                hire_date TEXT
            )',
            'CREATE TABLE mp_lv_leave_types (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                annual_days INT
            )',
            'CREATE TABLE mp_lv_leave_requests (
                id INT AUTO_INCREMENT PRIMARY KEY,
                employee_id INT,
                leave_type_id INT,
                start_date TEXT,
    ...
    {
        return ['mp_lv_leave_requests', 'mp_lv_employees', 'mp_lv_leave_types'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 4 employees
        $this->pdo->exec("INSERT INTO mp_lv_employees VALUES (1, 'Alice', 'Engineering', '2024-01-15')");
        $this->pdo->exec("INSERT INTO mp_lv_employees VALUES (2, 'Bob', 'Marketing', '2024-06-01')");
        $this->pdo->exec("INSERT INTO mp_lv_employees VALUES (3, 'Carol', 'Engineering', '2025-01-10')");
        $this->pdo->exec("INSERT INTO mp_lv_employees VALUES (4, 'Dave', 'Marketing', '2023-03-20')");

        // 3 leave types
        $this->pdo->exec("INSERT INTO mp_lv_leave_types VALUES (1, 'Annual Leave', 20)");
        $this->pdo->exec("INSERT INTO mp_lv_leave_types VALUES (2, 'Sick Leave', 10)");
        $this->pdo->exec("INSERT INTO mp_lv_leave_types VALUES (3, 'Personal Leave', 5)");

        // 10 leave requests
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (1, 1, 1, '2026-01-06', '2026-01-10', 5, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (2, 1, 2, '2026-02-03', '2026-02-04', 2, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (3, 2, 1, '2026-01-13', '2026-01-17', 5, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (4, 2, 1, '2026-03-02', '2026-03-06', 5, 'pending')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (5, 3, 1, '2026-02-17', '2026-02-21', 5, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (6, 3, 3, '2026-03-10', '2026-03-11', 2, 'pending')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (7, 4, 1, '2026-01-20', '2026-01-31', 10, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (8, 4, 2, '2026-02-10', '2026-02-12', 3, 'approved')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (9, 1, 1, '2026-03-17', '2026-03-21', 5, 'pending')");
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (10, 4, 1, '2026-03-03', '2026-03-07', 5, 'rejected')");
    }
    ...
    public function testApprovedLeaveByEmployee(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name, SUM(lr.days) AS total_days
             FROM mp_lv_leave_requests lr
             JOIN mp_lv_employees e ON e.id = lr.employee_id
             JOIN mp_lv_leave_types lt ON lt.id = lr.leave_type_id
    ...
    public function testRemainingBalanceByType(): void
    {
        $rows = $this->ztdQuery(
            "SELECT lt.name, lt.annual_days,
             ...
             FROM mp_lv_leave_types lt
             LEFT JOIN mp_lv_leave_requests lr
    ...
    public function testOverlappingRequestDetection(): void
    {
        $rows = $this->ztdQuery(
            "SELECT lr1.id AS request1_id, lr2.id AS request2_id,
                    lr1.employee_id
             FROM mp_lv_leave_requests lr1
             JOIN mp_lv_leave_requests lr2
    ...
    public function testApproveAndVerifyBalance(): void
    {
        ...
        $this->pdo->exec("UPDATE mp_lv_leave_requests SET status = 'approved' WHERE id = 9");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_lv_leave_requests VALUES (11, 2, 2, '2026-04-01', '2026-04-03', 3, 'pending')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_lv_leave_requests");
        $this->assertEquals(11, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_lv_leave_requests")->fetchAll(PDO::FETCH_ASSOC);
