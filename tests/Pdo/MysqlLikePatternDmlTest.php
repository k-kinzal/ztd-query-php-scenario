 ...
 * @spec SPEC-4.3
 */
class MysqlLikePatternDmlTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mpd_lpd_users (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mpd_lpd_users VALUES (1, 'alice@example.com', 'Alice', 'active')");
        $this->pdo->exec("INSERT INTO mpd_lpd_users VALUES (2, 'bob@test.org', 'Bob', 'active')");
        $this->pdo->exec("INSERT INTO mpd_lpd_users VALUES (3, 'carol@example.com', 'Carol', 'inactive')");
        $this->pdo->exec("INSERT INTO mpd_lpd_users VALUES (4, 'dave@test.org', 'Dave', 'active')");
        $this->pdo->exec("INSERT INTO mpd_lpd_users VALUES (5, 'eve@example.com', 'Eve', 'banned')");
    }
    ...
    public function testUpdateWhereLikeSuffix(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mpd_lpd_users SET status = 'verified' WHERE email LIKE '%@example.com'"
            );

            $rows = $this->ztdQuery(
                "SELECT id, status FROM mpd_lpd_users ORDER BY id"
            );
    ...
    public function testDeleteWhereLikePrefix(): void
    {
        try {
            $this->pdo->exec(
                "DELETE FROM mpd_lpd_users WHERE name LIKE 'A%'"
            );

            $rows = $this->ztdQuery("SELECT id, name FROM mpd_lpd_users ORDER BY id");
    ...
    public function testUpdateWhereNotLike(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mpd_lpd_users SET status = 'external' WHERE email NOT LIKE '%@example.com'"
            );

            $rows = $this->ztdQuery(
                "SELECT id, status FROM mpd_lpd_users WHERE status = 'external' ORDER BY id"
            );
    ...
    public function testPreparedUpdateWhereLike(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE mpd_lpd_users SET status = 'matched' WHERE email LIKE ?"
            );
            $stmt->execute(['%@test.org']);

            $rows = $this->ztdQuery(
                "SELECT id, status FROM mpd_lpd_users WHERE status = 'matched' ORDER BY id"
            );
    ...
    public function testDeleteWhereLikeAndCondition(): void
    {
        try {
            $this->pdo->exec(
                "DELETE FROM mpd_lpd_users WHERE email LIKE '%@example.com' AND status = 'active'"
            );
