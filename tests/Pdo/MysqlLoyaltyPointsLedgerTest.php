 ...
 * and HAVING clause behavior (MySQL PDO).
 ...
 * @spec SPEC-10.2.174
 */
class MysqlLoyaltyPointsLedgerTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_lpl_members (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100),
                tier VARCHAR(20),
                balance INT
            )',
            'CREATE TABLE mp_lpl_points_ledger (
                id INT AUTO_INCREMENT PRIMARY KEY,
                member_id INT,
                txn_type VARCHAR(20),
                points INT,
    ...
    {
        return ['mp_lpl_points_ledger', 'mp_lpl_members'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Members
        $this->pdo->exec("INSERT INTO mp_lpl_members VALUES (1, 'Alice', 'gold', 500)");
        $this->pdo->exec("INSERT INTO mp_lpl_members VALUES (2, 'Bob', 'silver', 200)");
        $this->pdo->exec("INSERT INTO mp_lpl_members VALUES (3, 'Carol', 'gold', 1000)");

        // Points ledger
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (1, 1, 'earn', 300, 'Purchase bonus')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (2, 1, 'earn', 200, 'Referral reward')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (3, 1, 'redeem', -100, 'Gift card')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (4, 2, 'earn', 150, 'Purchase bonus')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (5, 2, 'earn', 50, 'Survey reward')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (6, 3, 'earn', 500, 'Annual bonus')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (7, 3, 'redeem', -200, 'Merchandise')");
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (8, 3, 'earn', 700, 'Purchase bonus')");
    }
    ...
    public function testNetPointsByMember(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name, SUM(l.points) AS net_points
             FROM mp_lpl_members m
             JOIN mp_lpl_points_ledger l ON l.member_id = m.id
    ...
    public function testSequentialMutationsConsistency(): void
    {
        ...
        $this->ztdExec("INSERT INTO mp_lpl_points_ledger VALUES (9, 2, 'earn', 500, 'Bonus event')");
        $this->ztdExec("INSERT INTO mp_lpl_points_ledger VALUES (10, 2, 'redeem', -100, 'Coupon used')");
        $this->ztdExec("UPDATE mp_lpl_members SET balance = balance + 400 WHERE id = 2");

        // Verify Bob's ledger net: 150+50+500-100 = 600
        $rows = $this->ztdQuery(
            "SELECT SUM(l.points) AS net_points
             FROM mp_lpl_points_ledger l
             WHERE l.member_id = 2"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT balance FROM mp_lpl_members WHERE id = 2"
        );
    ...
    {
        $affected = $this->ztdExec("DELETE FROM mp_lpl_points_ledger WHERE id = 8");
    ...
    {
        $this->pdo->exec("INSERT INTO mp_lpl_points_ledger VALUES (9, 1, 'earn', 100, 'Test')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_lpl_points_ledger");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_lpl_points_ledger")->fetchAll(PDO::FETCH_ASSOC);
