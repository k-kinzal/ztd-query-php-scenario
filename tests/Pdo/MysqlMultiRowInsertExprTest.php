 ...
 * @spec SPEC-4.1
 */
class MysqlMultiRowInsertExprTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE mpd_mrie_products (
            id INT PRIMARY KEY,
    ...
    public function testMultiRowInsertWithStringFunctions(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO mpd_mrie_products (id, code, price, quantity) VALUES
                    (1, UPPER('widget'), 9.99, 10),
                    (2, LOWER('GADGET'), 19.99, 5),
                    (3, CONCAT('item-', '003'), 29.99, 3)"
            );

            $rows = $this->ztdQuery("SELECT id, code FROM mpd_mrie_products ORDER BY id");
    ...
    public function testMultiRowInsertWithArithmetic(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, price, quantity FROM mpd_mrie_products ORDER BY id");
