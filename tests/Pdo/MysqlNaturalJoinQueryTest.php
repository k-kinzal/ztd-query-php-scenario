...
/**
 * NATURAL JOIN on shadow data (MySQL PDO).
 ...
 * @spec SPEC-3.3
 */
class MysqlNaturalJoinQueryTest extends AbstractMysqlPdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO njq_departments (department_id, dept_name, floor_number) VALUES
            (1, 'Engineering', 3),
            (2, 'Marketing', 2),
            (3, 'Sales', 1)");

        $this->pdo->exec("INSERT INTO njq_staff (id, name, department_id, role) VALUES
            (1, 'Alice', 1, 'Engineer'),
            (2, 'Bob', 1, 'Lead'),
            (3, 'Carol', 2, 'Designer'),
            (4, 'Dave', 3, 'Rep')");
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO njq_staff (id, name, department_id, role) VALUES (5, 'Eve', 2, 'Manager')");
    ...
    public function testNaturalLeftJoin(): void
    {
        try {
            ...
            $this->pdo->exec("INSERT INTO njq_departments (department_id, dept_name, floor_number) VALUES (4, 'HR', 4)");
    ...
    {
        $this->disableZtd();
        $stmt = $this->pdo->query("SELECT COUNT(*) AS cnt FROM njq_staff");
