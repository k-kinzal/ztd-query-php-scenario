 ...
 * @spec SPEC-4.3
 */
class MysqlNullifInDmlTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_nid_products (
                id INT PRIMARY KEY,
    ...
    {
        return ['mp_nid_summary', 'mp_nid_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_nid_products VALUES (1, 'Widget', 'electronics', 100.00, 100.00)");
        $this->pdo->exec("INSERT INTO mp_nid_products VALUES (2, 'Gadget', 'electronics', 200.00, 150.00)");
        $this->pdo->exec("INSERT INTO mp_nid_products VALUES (3, 'Gizmo', 'toys', 50.00, 50.00)");
        $this->pdo->exec("INSERT INTO mp_nid_products VALUES (4, 'Doohickey', 'toys', 75.00, NULL)");
    }
    ...
    public function testUpdateSetNullif(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_nid_products SET discount_price = NULLIF(discount_price, price)"
            );

            $rows = $this->ztdQuery(
                "SELECT id, name, discount_price FROM mp_nid_products ORDER BY id"
            );
    ...
    public function testUpdateSetNullifPrepared(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE mp_nid_products SET discount_price = NULLIF(discount_price, ?) WHERE id = ?"
            );
            $stmt->execute([100.00, 1]);

            $rows = $this->ztdQuery("SELECT discount_price FROM mp_nid_products WHERE id = 1");
    ...
    public function testDeleteWhereNullifIsNull(): void
    {
        try {
            $this->pdo->exec(
                "DELETE FROM mp_nid_products WHERE NULLIF(discount_price, price) IS NULL"
            );

            $rows = $this->ztdQuery("SELECT name FROM mp_nid_products ORDER BY name");
    ...
    public function testDeleteWhereNullifIsNotNull(): void
    {
        try {
            $this->pdo->exec(
                "DELETE FROM mp_nid_products WHERE NULLIF(discount_price, price) IS NOT NULL"
            );
    ...
    public function testDeleteWhereNullifPrepared(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "DELETE FROM mp_nid_products WHERE NULLIF(category, ?) IS NULL"
            );
            $stmt->execute(['toys']);
    ...
    public function testInsertSelectWithNullif(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO mp_nid_summary (id, name, effective_price)
                 SELECT id, name, NULLIF(discount_price, price)
                 FROM mp_nid_products
                 ORDER BY id"
            );

            $rows = $this->ztdQuery("SELECT id, name, effective_price FROM mp_nid_summary ORDER BY id");
            $this->assertCount(4, $rows);
    ...
    public function testUpdateNullifWithWhereClause(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE mp_nid_products
                 SET discount_price = NULLIF(discount_price, price)
                 WHERE category = 'electronics'"
            );

            $rows = $this->ztdQuery("SELECT id, discount_price FROM mp_nid_products ORDER BY id");
            $this->assertCount(4, $rows);
    ...
    public function testMultipleNullifInUpdateSet(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, name, category, discount_price FROM mp_nid_products ORDER BY id");
            $this->assertCount(4, $rows);
