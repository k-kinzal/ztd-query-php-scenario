...
/**
 * Tests an employee onboarding checklist scenario through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.151
 */
class MysqlOnboardingChecklistTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_ob_departments (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(100)
            )',
            'CREATE TABLE mp_ob_employees (
                id INT AUTO_INCREMENT PRIMARY KEY,
                department_id INT,
                name VARCHAR(100),
                start_date TEXT
            )',
            'CREATE TABLE mp_ob_checklist_items (
                id INT AUTO_INCREMENT PRIMARY KEY,
                title VARCHAR(255),
                category VARCHAR(50)
            )',
            'CREATE TABLE mp_ob_completions (
                id INT AUTO_INCREMENT PRIMARY KEY,
                employee_id INT,
                item_id INT,
                completed_at TEXT
    ...
    {
        return ['mp_ob_completions', 'mp_ob_checklist_items', 'mp_ob_employees', 'mp_ob_departments'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Departments
        $this->pdo->exec("INSERT INTO mp_ob_departments VALUES (1, 'Engineering')");
        $this->pdo->exec("INSERT INTO mp_ob_departments VALUES (2, 'Marketing')");

        // Employees
        $this->pdo->exec("INSERT INTO mp_ob_employees VALUES (1, 1, 'Alice', '2026-01-15')");
        $this->pdo->exec("INSERT INTO mp_ob_employees VALUES (2, 1, 'Bob', '2026-02-01')");
        $this->pdo->exec("INSERT INTO mp_ob_employees VALUES (3, 2, 'Carol', '2026-01-20')");
        $this->pdo->exec("INSERT INTO mp_ob_employees VALUES (4, 2, 'Dave', '2026-03-01')");

        // Checklist items (5 items every employee must complete)
        $this->pdo->exec("INSERT INTO mp_ob_checklist_items VALUES (1, 'Sign employment contract', 'admin')");
        $this->pdo->exec("INSERT INTO mp_ob_checklist_items VALUES (2, 'Set up workstation', 'setup')");
        $this->pdo->exec("INSERT INTO mp_ob_checklist_items VALUES (3, 'Complete security training', 'training')");
        $this->pdo->exec("INSERT INTO mp_ob_checklist_items VALUES (4, 'Meet with manager', 'intro')");
        $this->pdo->exec("INSERT INTO mp_ob_checklist_items VALUES (5, 'Review company handbook', 'training')");

        // Completions: Alice 5/5, Bob 3/5, Carol 4/5, Dave 1/5
        // Alice: all done
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (1, 1, 1, '2026-01-15')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (2, 1, 2, '2026-01-15')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (3, 1, 3, '2026-01-16')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (4, 1, 4, '2026-01-16')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (5, 1, 5, '2026-01-17')");

        // Bob: items 1, 2, 4 done (missing 3: security training, 5: handbook)
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (6, 2, 1, '2026-02-01')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (7, 2, 2, '2026-02-01')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (8, 2, 4, '2026-02-02')");

        // Carol: items 1, 2, 3, 5 done (missing 4: meet with manager)
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (9, 3, 1, '2026-01-20')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (10, 3, 2, '2026-01-20')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (11, 3, 3, '2026-01-21')");
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (12, 3, 5, '2026-01-22')");

        // Dave: item 1 only
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (13, 4, 1, '2026-03-01')");
    }
    ...
    public function testCompletionProgressPerEmployee(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
                    ...
                    (SELECT COUNT(*) FROM mp_ob_checklist_items) AS total,
                    ROUND(COUNT(c.id) * 100.0 / (SELECT COUNT(*) FROM mp_ob_checklist_items)) AS pct
             FROM mp_ob_employees e
             LEFT JOIN mp_ob_completions c ON c.employee_id = e.id
    ...
    public function testOutstandingItemsForEmployee(): void
    {
        $rows = $this->ztdQuery(
            "SELECT ci.title, ci.category
             FROM mp_ob_checklist_items ci
             WHERE NOT EXISTS (
                 SELECT 1 FROM mp_ob_completions c
    ...
    public function testDepartmentProgress(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name AS department,
             ...
             FROM mp_ob_departments d
             JOIN mp_ob_employees e ON e.department_id = d.id
             JOIN (
                 SELECT e2.id AS emp_id,
                        COUNT(c.id) * 100.0 / (SELECT COUNT(*) FROM mp_ob_checklist_items) AS pct
                 FROM mp_ob_employees e2
                 LEFT JOIN mp_ob_completions c ON c.employee_id = e2.id
                 GROUP BY e2.id
    ...
    public function testCompletionStatusLabels(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
                    CASE
                        WHEN COUNT(c.id) * 100 / (SELECT COUNT(*) FROM mp_ob_checklist_items) = 100 THEN 'complete'
                        WHEN COUNT(c.id) * 100 / (SELECT COUNT(*) FROM mp_ob_checklist_items) >= 50 THEN 'in_progress'
                        ELSE 'behind'
                    END AS status
             FROM mp_ob_employees e
             LEFT JOIN mp_ob_completions c ON c.employee_id = e.id
    ...
    public function testCompletionByCategory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT ci.category,
             ...
             FROM mp_ob_checklist_items ci
             LEFT JOIN mp_ob_completions c ON c.item_id = ci.id
    ...
    {
        $this->pdo->exec("INSERT INTO mp_ob_completions VALUES (14, 4, 2, '2026-03-09')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_ob_completions");
        $this->assertEquals(14, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $result = $this->pdo->query('SELECT COUNT(*) AS cnt FROM mp_ob_completions');
