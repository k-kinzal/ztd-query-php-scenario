...
/**
 * Tests a parking garage scenario through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.154
 */
class MysqlParkingGarageTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_pkg_levels (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(50),
                capacity INT
            )',
            'CREATE TABLE mp_pkg_passes (
                id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    {
        return ['mp_pkg_entries', 'mp_pkg_passes', 'mp_pkg_levels'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Levels
        $this->pdo->exec("INSERT INTO mp_pkg_levels VALUES (1, 'Level A', 50)");
        $this->pdo->exec("INSERT INTO mp_pkg_levels VALUES (2, 'Level B', 40)");
        $this->pdo->exec("INSERT INTO mp_pkg_levels VALUES (3, 'Level C', 30)");

        // Passes
        $this->pdo->exec("INSERT INTO mp_pkg_passes VALUES (1, 'ABC-1234', 'Alice Smith', 'monthly', '2026-02-01', '2026-02-28')");
        $this->pdo->exec("INSERT INTO mp_pkg_passes VALUES (2, 'DEF-5678', 'Bob Jones', 'annual', '2026-01-01', '2026-12-31')");
        $this->pdo->exec("INSERT INTO mp_pkg_passes VALUES (3, 'GHI-9012', 'Carol White', 'monthly', '2026-03-01', '2026-03-31')");

        // Entries
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (1, 'ABC-1234', 1, '2026-03-01 08:00', '2026-03-01 10:30', 5.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (2, 'DEF-5678', 1, '2026-03-01 09:00', '2026-03-01 14:00', 0.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (3, 'XYZ-9999', 2, '2026-03-01 10:00', '2026-03-01 11:00', 3.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (4, 'LMN-4567', 2, '2026-03-01 07:00', '2026-03-01 17:00', 20.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (5, 'ABC-1234', 1, '2026-03-02 08:00', '2026-03-02 09:30', 3.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (6, 'GHI-9012', 3, '2026-03-02 10:00', '2026-03-02 16:00', 0.00)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (7, 'PQR-1111', 1, '2026-03-02 11:00', NULL, NULL)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (8, 'STU-2222', 2, '2026-03-02 12:00', NULL, NULL)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (9, 'DEF-5678', 3, '2026-03-02 14:00', NULL, NULL)");
        $this->pdo->exec("INSERT INTO mp_pkg_entries VALUES (10, 'VWX-3333', 1, '2026-03-02 15:00', NULL, NULL)");
    }
    ...
    public function testCurrentOccupancy(): void
    {
        $rows = $this->ztdQuery(
            "SELECT lv.name, COUNT(e.id) AS occupied
             FROM mp_pkg_levels lv
             JOIN mp_pkg_entries e ON e.level_id = lv.id
    ...
    public function testCapacityUtilization(): void
    {
        $rows = $this->ztdQuery(
            "SELECT lv.name, lv.capacity,
             ...
             FROM mp_pkg_levels lv
             LEFT JOIN mp_pkg_entries e ON e.level_id = lv.id AND e.exit_time IS NULL
    ...
    public function testPassHolderActivity(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.member_name, p.pass_type, p.plate_number,
             ...
             FROM mp_pkg_passes p
             LEFT JOIN mp_pkg_entries e ON e.plate_number = p.plate_number
    ...
    public function testRevenueByLevel(): void
    {
        $rows = $this->ztdQuery(
            "SELECT lv.name, COALESCE(SUM(e.amount_charged), 0) AS revenue
             FROM mp_pkg_levels lv
             LEFT JOIN mp_pkg_entries e ON e.level_id = lv.id AND e.exit_time IS NOT NULL
             GROUP BY lv.id, lv.name
    ...
    public function testIdentifyNonPassVehiclesParked(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.plate_number, lv.name AS level_name, e.entry_time
             FROM mp_pkg_entries e
             JOIN mp_pkg_levels lv ON lv.id = e.level_id
             LEFT JOIN mp_pkg_passes p ON p.plate_number = e.plate_number
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec(
            "INSERT INTO mp_pkg_entries VALUES (11, 'NEW-0001', 1, '2026-03-03 08:00', NULL, NULL)"
        );

        // ZTD sees the new entry (10 original + 1 new = 11)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_pkg_entries");
        $this->assertEquals(11, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM mp_pkg_entries')->fetchAll(PDO::FETCH_ASSOC);
