...
/**
 * Tests token-based password reset authentication flows through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.113
 */
class MysqlPasswordResetTokenTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_prt_users (
                id INT AUTO_INCREMENT PRIMARY KEY,
                email VARCHAR(255),
                status VARCHAR(20)
            )',
            'CREATE TABLE mp_prt_tokens (
                id INT AUTO_INCREMENT PRIMARY KEY,
                user_id INT,
                token VARCHAR(255),
                expires_at DATE,
    ...
    {
        return ['mp_prt_tokens', 'mp_prt_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 users
        $this->pdo->exec("INSERT INTO mp_prt_users VALUES (1, 'alice@example.com', 'active')");
        $this->pdo->exec("INSERT INTO mp_prt_users VALUES (2, 'bob@example.com', 'active')");
        $this->pdo->exec("INSERT INTO mp_prt_users VALUES (3, 'carol@example.com', 'locked')");

        // 5 tokens
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (1, 1, 'token1', '2026-03-15', 0)");
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (2, 2, 'token2', '2026-02-01', 0)");
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (3, 1, 'token3', '2026-03-20', 0)");
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (4, 3, 'token4', '2026-03-10', 1)");
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (5, 2, 'token5', '2026-01-15', 0)");
    }
    ...
    {
        // Consume token1
        $this->pdo->exec("UPDATE mp_prt_tokens SET consumed = 1 WHERE token = 'token1'");

        // Verify token1 is now consumed
        $rows = $this->ztdQuery("SELECT consumed FROM mp_prt_tokens WHERE token = 'token1'");
        $this->assertCount(1, $rows);
    ...
    {
        $this->pdo->exec("DELETE FROM mp_prt_tokens WHERE expires_at < '2026-03-09'");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_prt_tokens");
        $this->assertEquals(3, (int) $rows[0]['cnt']);

        // Verify the remaining tokens are token1, token3, token4
        $rows = $this->ztdQuery("SELECT token FROM mp_prt_tokens ORDER BY id");
    ...
    public function testLockedUserTokenIgnored(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.token, u.email
             FROM mp_prt_tokens t
             JOIN mp_prt_users u ON u.id = t.user_id
    ...
    {
        $this->pdo->exec("INSERT INTO mp_prt_tokens VALUES (6, 1, 'token6', '2026-04-01', 0)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_prt_tokens");
        $this->assertSame(6, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT token FROM mp_prt_tokens WHERE id = 6");
        $this->assertSame('token6', $rows[0]['token']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_prt_tokens")->fetchAll(PDO::FETCH_ASSOC);
