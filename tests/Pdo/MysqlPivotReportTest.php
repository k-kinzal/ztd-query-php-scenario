 ...
 * @spec SPEC-3.3
 */
class MysqlPivotReportTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_pr_sales (
                id INT PRIMARY KEY,
                product VARCHAR(100),
                category VARCHAR(50),
                amount DOUBLE,
    ...
    protected function setUp(): void
    {
        ...
        // January sales
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (1, 'Laptop', 'Electronics', 999.00, '2024-01', 'North')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (2, 'Phone', 'Electronics', 699.00, '2024-01', 'South')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (3, 'Shirt', 'Clothing', 29.00, '2024-01', 'North')");
        // February sales
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (4, 'Laptop', 'Electronics', 999.00, '2024-02', 'South')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (5, 'Book', 'Books', 15.00, '2024-02', 'North')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (6, 'Pants', 'Clothing', 49.00, '2024-02', 'North')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (7, 'Phone', 'Electronics', 699.00, '2024-02', 'North')");
        // March sales
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (8, 'Tablet', 'Electronics', 399.00, '2024-03', 'South')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (9, 'Jacket', 'Clothing', 89.00, '2024-03', 'North')");
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (10, 'Book', 'Books', 25.00, '2024-03', 'South')");
    }
    ...
    public function testScalarSubqueryInSelectWorks(): void
    {
        $rows = $this->ztdQuery(
            "SELECT category,
                    ...
                    ROUND(SUM(amount) * 100.0 / (SELECT SUM(amount) FROM mp_pr_sales), 1) AS pct
             FROM mp_pr_sales
             GROUP BY category
             ORDER BY category"
        );
    ...
    /**
     * CROSS JOIN workaround does NOT work on MySQL.
     * On SQLite, derived tables in JOINs ARE rewritten (SPEC-3.3a), so the workaround works.
     * On MySQL, derived table references are NOT rewritten (SPEC-3.3a), so the CROSS JOIN
     * with a derived table also reads from the empty physical table, producing 0 rows.
     * @spec SPEC-3.3a
     */
    public function testRevenueMixPercentageWorkaround(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.category,
             ...
             FROM mp_pr_sales s
             CROSS JOIN (SELECT SUM(amount) AS total FROM mp_pr_sales) t
             GROUP BY s.category, t.total
             ORDER BY pct DESC"
        );

        // On MySQL, derived tables always return empty (SPEC-3.3a).
        // The CROSS JOIN workaround that works on SQLite does not work here.
    ...
    public function testPivotAfterInsert(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_pr_sales VALUES (11, 'Server', 'Electronics', 5000.00, '2024-03', 'North')");

        $rows = $this->ztdQuery(
            "SELECT SUM(CASE WHEN sale_month = '2024-03' THEN amount ELSE 0 END) AS mar_total
             FROM mp_pr_sales WHERE category = 'Electronics'"
        );
    ...
    public function testPivotAfterUpdate(): void
    {
        ...
        $this->pdo->exec("UPDATE mp_pr_sales SET amount = 799.00 WHERE id = 1");
    ...
    {
        $this->pdo->exec("DELETE FROM mp_pr_sales WHERE id = 2"); // Remove Jan phone sale (699)
