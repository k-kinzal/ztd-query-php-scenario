 ...
 * genre distribution, and most-played ranking through ZTD shadow store (MySQL PDO).
 * Covers UPDATE arithmetic, SUM CASE, GROUP BY + JOIN, and physical isolation.
 * @spec SPEC-10.2.140
 */
class MysqlPlaylistManagementTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_pm_playlists (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                created_by VARCHAR(255)
            )',
            'CREATE TABLE mp_pm_songs (
                id INT AUTO_INCREMENT PRIMARY KEY,
                title VARCHAR(255),
                artist VARCHAR(255),
                genre VARCHAR(50),
            ...
            'CREATE TABLE mp_pm_playlist_songs (
                id INT AUTO_INCREMENT PRIMARY KEY,
                playlist_id INT,
                song_id INT,
                position INT,
    ...
    {
        return ['mp_pm_playlist_songs', 'mp_pm_songs', 'mp_pm_playlists'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 playlists
        $this->pdo->exec("INSERT INTO mp_pm_playlists VALUES (1, 'Road Trip Mix', 'alice')");
        $this->pdo->exec("INSERT INTO mp_pm_playlists VALUES (2, 'Workout Beats', 'bob')");

        // 6 songs
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (1, 'Highway Star', 'Deep Purple', 'Rock', 367)");
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (2, 'Bohemian Rhapsody', 'Queen', 'Rock', 354)");
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (3, 'Billie Jean', 'Michael Jackson', 'Pop', 294)");
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (4, 'Lose Yourself', 'Eminem', 'HipHop', 326)");
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (5, 'Blinding Lights', 'The Weeknd', 'Pop', 200)");
        $this->pdo->exec("INSERT INTO mp_pm_songs VALUES (6, 'Thunderstruck', 'ACDC', 'Rock', 292)");

        // playlist_songs: playlist 1 has 4 songs, playlist 2 has 3 songs
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (1, 1, 1, 1, 15)");  // Road Trip: Highway Star pos 1
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (2, 1, 2, 2, 22)");  // Road Trip: Bohemian Rhapsody pos 2
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (3, 1, 3, 3, 8)");   // Road Trip: Billie Jean pos 3
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (4, 1, 5, 4, 30)");  // Road Trip: Blinding Lights pos 4
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (5, 2, 4, 1, 45)");  // Workout: Lose Yourself pos 1
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (6, 2, 6, 2, 38)");  // Workout: Thunderstruck pos 2
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (7, 2, 1, 3, 20)");  // Workout: Highway Star pos 3
    }
    ...
    public function testPlaylistContents(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS playlist_name, s.title, s.artist, ps.position
             FROM mp_pm_playlist_songs ps
             JOIN mp_pm_playlists p ON p.id = ps.playlist_id
             JOIN mp_pm_songs s ON s.id = ps.song_id
    ...
    public function testMovePositionUp(): void
    {
        ...
        $this->pdo->exec(
            "UPDATE mp_pm_playlist_songs SET position = position + 1
             WHERE playlist_id = 1 AND position >= 2 AND position < 4"
        );

        // Move Blinding Lights (id=4) to position 2
        $this->pdo->exec(
            "UPDATE mp_pm_playlist_songs SET position = 2 WHERE id = 4"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO mp_pm_playlist_songs VALUES (8, 2, 3, 4, 5)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_pm_playlist_songs");
        $this->assertSame(8, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_pm_playlist_songs")->fetchAll(PDO::FETCH_ASSOC);
