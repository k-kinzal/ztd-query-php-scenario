...
/**
 * Tests re-preparing different SQL on the same PDOStatement variable (MySQL PDO).
 *
 ...
 * @spec SPEC-4.2
 */
class MysqlPreparedRePrepareTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_rp_users (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                status VARCHAR(20) NOT NULL DEFAULT \'active\'
            ) ENGINE=InnoDB',
            'CREATE TABLE mp_rp_logs (
                id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_rp_users VALUES (1, 'Alice', 'active')");
        $this->pdo->exec("INSERT INTO mp_rp_users VALUES (2, 'Bob', 'active')");
        $this->pdo->exec("INSERT INTO mp_rp_users VALUES (3, 'Carol', 'inactive')");
    }
    ...
    public function testRePrepareInsertThenSelect(): void
    {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO mp_rp_users VALUES (?, ?, ?)");
            $stmt->execute([4, 'Dave', 'active']);

            // Re-prepare same variable: SELECT
            $stmt = $this->pdo->prepare("SELECT name FROM mp_rp_users WHERE status = ? ORDER BY name");
            $stmt->execute(['active']);
    ...
    public function testRePrepareSelectUpdateSelect(): void
    {
        try {
            // First: SELECT
            $stmt = $this->pdo->prepare("SELECT COUNT(*) AS cnt FROM mp_rp_users WHERE status = ?");
            $stmt->execute(['active']);
            $before = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $this->assertEquals(2, (int) $before[0]['cnt']);

            // Re-prepare: UPDATE
            $stmt = $this->pdo->prepare("UPDATE mp_rp_users SET status = ? WHERE id = ?");
    ...
    {
        try {
            $stmt = $this->pdo->prepare("DELETE FROM mp_rp_users WHERE id = ?");
            $stmt->execute([3]);

            $stmt = $this->pdo->prepare("SELECT name FROM mp_rp_users ORDER BY name");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testRePrepareDifferentTables(): void
    {
        try {
            // INSERT into logs
            $stmt = $this->pdo->prepare("INSERT INTO mp_rp_logs VALUES (?, ?, ?)");
            ...
            $stmt = $this->pdo->prepare(
                "SELECT u.name, l.action
                 FROM mp_rp_users u
                 INNER JOIN mp_rp_logs l ON u.id = l.user_id
                 WHERE l.action = ?
                 ORDER BY u.name"
            );
            $stmt->execute(['login']);
    ...
    public function testRePrepareDifferentParamCount(): void
    {
        try {
            ...
            // 1 param
            $stmt = $this->pdo->prepare("SELECT name FROM mp_rp_users WHERE id = ?");
            $stmt->execute([4]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            // 2 params
            $stmt = $this->pdo->prepare("UPDATE mp_rp_users SET name = ? WHERE id = ?");
            $stmt->execute(['David', 4]);

            // 0 params
            $stmt = $this->pdo->prepare("SELECT name FROM mp_rp_users WHERE id = 4");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testRapidRePrepare(): void
    {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT COUNT(*) AS cnt FROM mp_rp_users");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $sql = "SELECT name FROM mp_rp_users WHERE id = ?";
            ...
            // Mutate
            $this->pdo->exec("UPDATE mp_rp_users SET name = 'Alicia' WHERE id = 1");
