...
/**
 * Tests a project timesheet scenario through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.168
 */
class MysqlProjectTimesheetTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_ts_employees (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                department VARCHAR(255)
            )',
            'CREATE TABLE mp_ts_projects (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255),
                status VARCHAR(255)
            )',
            'CREATE TABLE mp_ts_entries (
                id INT AUTO_INCREMENT PRIMARY KEY,
                employee_id INT,
                project_id INT,
                hours DECIMAL(8,2),
    ...
    {
        return ['mp_ts_entries', 'mp_ts_projects', 'mp_ts_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 4 employees
        $this->pdo->exec("INSERT INTO mp_ts_employees VALUES (1, 'Alice', 'engineering')");
        $this->pdo->exec("INSERT INTO mp_ts_employees VALUES (2, 'Bob', 'engineering')");
        $this->pdo->exec("INSERT INTO mp_ts_employees VALUES (3, 'Carol', 'design')");
        $this->pdo->exec("INSERT INTO mp_ts_employees VALUES (4, 'Dave', 'design')");

        // 3 projects
        $this->pdo->exec("INSERT INTO mp_ts_projects VALUES (1, 'Alpha', 'active')");
        $this->pdo->exec("INSERT INTO mp_ts_projects VALUES (2, 'Beta', 'active')");
        $this->pdo->exec("INSERT INTO mp_ts_projects VALUES (3, 'Legacy', 'maintenance')");

        // 15 time entries
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (1, 1, 1, 8.0, '2025-11-01', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (2, 1, 1, 6.5, '2025-11-02', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (3, 1, 2, 4.0, '2025-11-03', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (4, 1, 3, 2.0, '2025-11-04', 0)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (5, 2, 1, 7.0, '2025-11-01', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (6, 2, 2, 5.5, '2025-11-02', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (7, 2, 2, 3.0, '2025-11-03', 0)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (8, 3, 1, 4.0, '2025-11-01', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (9, 3, 2, 6.0, '2025-11-02', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (10, 3, 3, 3.5, '2025-11-03', 0)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (11, 4, 2, 8.0, '2025-11-01', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (12, 4, 2, 7.0, '2025-11-02', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (13, 4, 3, 2.5, '2025-11-03', 0)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (14, 1, 1, 5.0, '2025-11-05', 1)");
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (15, 2, 1, 4.5, '2025-11-05', 1)");
    }
    ...
    public function testHoursByDepartmentAndProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT COALESCE(e.department, '(All)') AS department,
             ...
             FROM mp_ts_entries t
             JOIN mp_ts_employees e ON e.id = t.employee_id
             JOIN mp_ts_projects p ON p.id = t.project_id
    ...
    {
        $this->pdo->exec("INSERT INTO mp_ts_entries VALUES (16, 1, 1, 3.0, '2025-11-06', 1)");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_ts_entries");
        $this->assertEquals(16, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM mp_ts_entries')->fetchAll(PDO::FETCH_ASSOC);
