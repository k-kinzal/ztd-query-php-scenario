...
/**
 * Tests REPLACE and ON DUPLICATE KEY UPDATE (upsert) edge cases through ZTD shadow store on MySQL.
 *
 * MySQL supports REPLACE INTO and INSERT ... ON DUPLICATE KEY UPDATE.
 * These patterns interact with the shadow store's PK tracking and row replacement.
 * @spec SPEC-4.8
 */
class MysqlReplaceAndUpsertEdgeCasesTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE ru_items (id INT PRIMARY KEY, name VARCHAR(50), version INT) ENGINE=InnoDB',
        ];
    }
    ...
    /**
     * INSERT ... ON DUPLICATE KEY UPDATE: upsert existing row.
     */
    ...
    /**
     * INSERT ... ON DUPLICATE KEY UPDATE with self-referencing increment expression.
     * Issue #16: self-referencing expression loses original value on MySQL.
     */
    public function testOnDuplicateKeyUpdateIncrement(): void
    {
        $this->pdo->exec(
        ...
        $rows = $this->ztdQuery('SELECT * FROM ru_items WHERE id = 1');
        $this->assertCount(1, $rows);
        // Bug: self-referencing expression version + 1 evaluates to 0 instead of 2
        if ((string) $rows[0]['version'] === '0') {
            $this->markTestIncomplete(
                'Issue #16: ON DUPLICATE KEY UPDATE self-referencing expression loses original value (got 0, expected 2)'
            );
        }
        $this->assertSame('2', (string) $rows[0]['version']);
    }
    ...
    /**
     * INSERT IGNORE INTO: should not modify existing row on duplicate key.
     * Related to Issue #41 on SQLite.
     */
    public function testInsertIgnoreExistingRow(): void
    {
        $this->pdo->exec(
            "INSERT IGNORE INTO ru_items VALUES (1, 'should-not-appear', 99)"
        );
        ...
        if ($rows[0]['name'] !== 'alpha') {
            $this->markTestIncomplete('Issue #41: INSERT IGNORE may not enforce PK uniqueness in shadow store');
        }
