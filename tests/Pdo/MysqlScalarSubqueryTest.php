...
/**
 * Tests scalar subqueries in SELECT lists through CTE shadow store (MySQL PDO).
 *
 ...
 * @spec SPEC-3.3
 */
class MysqlScalarSubqueryTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE my_ssq_departments (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL
            )',
            'CREATE TABLE my_ssq_employees (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL,
                department_id INT NOT NULL,
                salary DECIMAL(12,2) NOT NULL
            )',
            'CREATE TABLE my_ssq_projects (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL,
                department_id INT NOT NULL,
    ...
    {
        return ['my_ssq_projects', 'my_ssq_employees', 'my_ssq_departments'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 departments
        $this->pdo->exec("INSERT INTO my_ssq_departments VALUES (1, 'Engineering')");
        $this->pdo->exec("INSERT INTO my_ssq_departments VALUES (2, 'Marketing')");
        $this->pdo->exec("INSERT INTO my_ssq_departments VALUES (3, 'Sales')");

        // 6 employees, 2 per department
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (1, 'Alice',   1, 90000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (2, 'Bob',     1, 85000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (3, 'Carol',   2, 70000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (4, 'Dave',    2, 65000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (5, 'Eve',     3, 60000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (6, 'Frank',   3, 55000.00)");

        // 4 projects spread across departments (dept 3 has none)
        $this->pdo->exec("INSERT INTO my_ssq_projects VALUES (1, 'Alpha',   1, 500000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_projects VALUES (2, 'Beta',    1, 300000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_projects VALUES (3, 'Gamma',   2, 200000.00)");
        $this->pdo->exec("INSERT INTO my_ssq_projects VALUES (4, 'Delta',   2, 150000.00)");
    }
    ...
    public function testSingleScalarSubquery(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM my_ssq_employees e WHERE e.department_id = d.id) AS emp_count
             FROM my_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testMultipleScalarSubqueries(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM my_ssq_employees e WHERE e.department_id = d.id) AS emp_count,
                    (SELECT COALESCE(SUM(p.budget), 0) FROM my_ssq_projects p WHERE p.department_id = d.id) AS total_budget
             FROM my_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testCorrelatedScalarSubqueryWithAggregate(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT MAX(e.salary) FROM my_ssq_employees e WHERE e.department_id = d.id) AS max_salary
             FROM my_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testScalarSubqueryWithWhere(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM my_ssq_employees e WHERE e.department_id = d.id) AS emp_count
             FROM my_ssq_departments d
             WHERE (SELECT SUM(p.budget) FROM my_ssq_projects p WHERE p.department_id = d.id) > 300000
             ORDER BY d.id"
        );
    ...
    public function testScalarSubqueryReferencingMultipleTables(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT AVG(e.salary)
                     FROM my_ssq_employees e
                     WHERE e.department_id = d.id
                       AND EXISTS (
                           SELECT 1 FROM my_ssq_projects p
    ...
    {
        $this->pdo->exec("INSERT INTO my_ssq_departments VALUES (4, 'Research')");
    ...
    public function testScalarSubqueryAfterInsert(): void
    {
        ...
        $before = $this->ztdQuery(
            "SELECT (SELECT COUNT(*) FROM my_ssq_employees e WHERE e.department_id = 3) AS cnt"
        );
        $this->assertEquals(2, (int) $before[0]['cnt']);

        // Mutate: add a new employee to Sales
        $this->pdo->exec("INSERT INTO my_ssq_employees VALUES (7, 'Grace', 3, 58000.00)");
    ...
    public function testPreparedStatementWithScalarSubquery(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.name,
                    (SELECT SUM(e.salary) FROM my_ssq_employees e WHERE e.department_id = d.id) AS total_salary
             FROM my_ssq_departments d
             WHERE d.id = ?",
            [1]
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM my_ssq_employees")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM my_ssq_departments")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM my_ssq_projects")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);
    }
}
