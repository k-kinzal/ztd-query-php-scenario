...
/**
 * Tests SELECT ... FOR UPDATE / FOR SHARE behavior through the ZTD shadow store (PDO MySQL).
 *
...
class MysqlSelectForUpdateTest extends AbstractMysqlPdoTestCase
{
    ...
    {
        return 'CREATE TABLE my_forlock_accounts (id INT PRIMARY KEY, balance DECIMAL(10,2), owner VARCHAR(50))';
    }

    protected function getTableNames(): array
    {
        return ['my_forlock_accounts'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO my_forlock_accounts VALUES (1, 1000.00, 'Alice')");
        $this->pdo->exec("INSERT INTO my_forlock_accounts VALUES (2, 2000.00, 'Bob')");
        $this->pdo->exec("INSERT INTO my_forlock_accounts VALUES (3, 3000.00, 'Carol')");
    }
    ...
    {
        try {
            $rows = $this->ztdQuery('SELECT * FROM my_forlock_accounts WHERE id = 1 FOR UPDATE');
    ...
    public function testSelectForUpdateInTransaction(): void
    {
        try {
            ...
            $rows = $this->ztdQuery('SELECT * FROM my_forlock_accounts WHERE id <= 2 FOR UPDATE');
            $this->ztdCommit();
    ...
    {
        try {
            $rows = $this->ztdQuery("SELECT * FROM my_forlock_accounts WHERE owner = 'Carol' FOR SHARE");
    ...
    public function testDmlAfterSelectForUpdate(): void
    {
        try {
            ...
            $rows = $this->ztdQuery('SELECT balance FROM my_forlock_accounts WHERE id = 1 FOR UPDATE');
            $this->assertCount(1, $rows);
            $oldBalance = (float) $rows[0]['balance'];

            $this->pdo->exec("UPDATE my_forlock_accounts SET balance = balance - 100 WHERE id = 1");
            $this->pdo->exec("UPDATE my_forlock_accounts SET balance = balance + 100 WHERE id = 2");

            $this->ztdCommit();

            // Verify the transfer
            $rows = $this->ztdQuery('SELECT id, balance FROM my_forlock_accounts WHERE id IN (1, 2) ORDER BY id');
            $this->assertCount(2, $rows);
