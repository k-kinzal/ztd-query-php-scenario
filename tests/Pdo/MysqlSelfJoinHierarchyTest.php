 ...
 * @spec SPEC-3.3
 */
class MysqlSelfJoinHierarchyTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return "CREATE TABLE mp_sjh_employees (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_sjh_employees (id, name, manager_id, salary) VALUES
            (1, 'Alice', NULL, 120000),
            (2, 'Bob', 1, 90000),
            (3, 'Carol', 1, 95000),
            (4, 'Dave', 2, 70000),
        ...
        try {
            $rows = $this->ztdQuery(
                "SELECT e.name AS employee, m.name AS manager
                 FROM mp_sjh_employees e
                 LEFT JOIN mp_sjh_employees m ON e.manager_id = m.id
                 ORDER BY e.id"
            );
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO mp_sjh_employees (id, name, manager_id, salary) VALUES (6, 'Frank', 3, 65000)");

            $rows = $this->ztdQuery(
                "SELECT e.name AS employee, m.name AS manager
                 FROM mp_sjh_employees e
                 INNER JOIN mp_sjh_employees m ON e.manager_id = m.id
                 WHERE m.name = 'Carol'
                 ORDER BY e.id"
            );
    ...
    public function testSelfJoinAfterUpdate(): void
    {
        try {
            ...
            $this->pdo->exec("UPDATE mp_sjh_employees SET manager_id = 3 WHERE id = 4");
    ...
    public function testSelfJoinAfterDelete(): void
    {
        try {
            ...
            $this->pdo->exec("DELETE FROM mp_sjh_employees WHERE id = 2");
    ...
    {
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_sjh_employees")
