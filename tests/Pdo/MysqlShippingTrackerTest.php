 ...
 * correlated subquery for latest event, prepared statement with date range (MySQL PDO).
 * @spec SPEC-10.2.132
 */
class MysqlShippingTrackerTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_st_orders (
                id INT AUTO_INCREMENT PRIMARY KEY,
                customer_name VARCHAR(100),
                order_date TEXT,
                total_amount DECIMAL(10,2)
            )',
            'CREATE TABLE mp_st_shipments (
                id INT AUTO_INCREMENT PRIMARY KEY,
                order_id INT,
                carrier VARCHAR(50),
            ...
            'CREATE TABLE mp_st_tracking_events (
                id INT AUTO_INCREMENT PRIMARY KEY,
                shipment_id INT,
                event_date TEXT,
                event_type TEXT,
    ...
    {
        return ['mp_st_tracking_events', 'mp_st_shipments', 'mp_st_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Orders
        $this->pdo->exec("INSERT INTO mp_st_orders VALUES (1, 'Alice', '2025-09-01', 150.00)");
        $this->pdo->exec("INSERT INTO mp_st_orders VALUES (2, 'Bob', '2025-09-05', 89.99)");
        $this->pdo->exec("INSERT INTO mp_st_orders VALUES (3, 'Carol', '2025-09-10', 320.00)");
        $this->pdo->exec("INSERT INTO mp_st_orders VALUES (4, 'Dave', '2025-09-12', 45.00)");

        // Shipments
        $this->pdo->exec("INSERT INTO mp_st_shipments VALUES (1, 1, 'FedEx', 'FX100001', '2025-09-02', '2025-09-05', '2025-09-04')");
        $this->pdo->exec("INSERT INTO mp_st_shipments VALUES (2, 2, 'UPS', 'UP200002', '2025-09-06', '2025-09-10', '2025-09-09')");
        $this->pdo->exec("INSERT INTO mp_st_shipments VALUES (3, 3, 'DHL', 'DH300003', '2025-09-11', '2025-09-15', NULL)");
        $this->pdo->exec("INSERT INTO mp_st_shipments VALUES (4, 4, 'FedEx', 'FX400004', '2025-09-13', '2025-09-16', NULL)");

        // Tracking events — Shipment 1 (FedEx, delivered)
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (1, 1, '2025-09-02', 'picked_up', 'warehouse')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (2, 1, '2025-09-03', 'in_transit', 'hub')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (3, 1, '2025-09-04', 'out_for_delivery', 'local')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (4, 1, '2025-09-04', 'delivered', 'doorstep')");

        // Tracking events — Shipment 2 (UPS, delivered)
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (5, 2, '2025-09-06', 'picked_up', 'warehouse')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (6, 2, '2025-09-07', 'in_transit', 'hub')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (7, 2, '2025-09-09', 'delivered', 'mailroom')");

        // Tracking events — Shipment 3 (DHL, in transit)
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (8, 3, '2025-09-11', 'picked_up', 'warehouse')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (9, 3, '2025-09-12', 'in_transit', 'hub')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (10, 3, '2025-09-13', 'in_transit', 'regional')");

        // Tracking events — Shipment 4 (FedEx, exception)
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (11, 4, '2025-09-13', 'picked_up', 'warehouse')");
        $this->pdo->exec("INSERT INTO mp_st_tracking_events VALUES (12, 4, '2025-09-14', 'exception', 'customs')");
    }
    ...
    public function testOrderShipmentSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.id AS order_id, o.customer_name, s.carrier, s.shipped_date, s.actual_delivery
             FROM mp_st_orders o
             JOIN mp_st_shipments s ON s.order_id = o.id
             ORDER BY o.id"
        );
    ...
    public function testLatestTrackingEventPerShipment(): void
    {
        $rows = $this->ztdQuery(
            "SELECT te.shipment_id, te.event_type, te.event_date, te.location
             FROM mp_st_tracking_events te
             WHERE te.event_date = (
                 SELECT MAX(te2.event_date)
                 FROM mp_st_tracking_events te2
                 WHERE te2.shipment_id = te.shipment_id
             )
             AND te.id = (
                 SELECT MAX(te3.id)
                 FROM mp_st_tracking_events te3
    ...
    {
        $this->pdo->exec("INSERT INTO mp_st_orders VALUES (5, 'Eve', '2025-09-15', 200.00)");

        // ZTD sees the new order
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_st_orders");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_st_orders")->fetchAll(PDO::FETCH_ASSOC);
