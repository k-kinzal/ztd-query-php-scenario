...
/**
 * Tests rolling time-window rate limiting with token bucket patterns through ZTD shadow store (MySQL PDO).
 ...
 * @spec SPEC-10.2.119
 */
class MysqlSlidingWindowRateLimitTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_rl_api_clients (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_name VARCHAR(255),
                tier VARCHAR(255),
                requests_per_hour INT
            )',
            'CREATE TABLE mp_rl_api_requests (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_id INT,
            ...
            'CREATE TABLE mp_rl_rate_limit_overrides (
                id INT AUTO_INCREMENT PRIMARY KEY,
                client_id INT,
                endpoint VARCHAR(255),
                custom_limit INT
    ...
    {
        return ['mp_rl_rate_limit_overrides', 'mp_rl_api_requests', 'mp_rl_api_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 api clients
        $this->pdo->exec("INSERT INTO mp_rl_api_clients VALUES (1, 'acme-api', 'basic', 100)");
        $this->pdo->exec("INSERT INTO mp_rl_api_clients VALUES (2, 'bigcorp', 'pro', 500)");
        $this->pdo->exec("INSERT INTO mp_rl_api_clients VALUES (3, 'enterprise-co', 'enterprise', 2000)");

        // 10 api requests spanning a 1-hour window
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (1, 1, '/users', '2026-03-09 10:05:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (2, 1, '/users', '2026-03-09 10:10:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (3, 1, '/upload', '2026-03-09 10:12:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (4, 2, '/export', '2026-03-09 10:01:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (5, 2, '/export', '2026-03-09 10:02:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (6, 2, '/users', '2026-03-09 10:15:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (7, 3, '/users', '2026-03-09 10:20:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (8, 3, '/users', '2026-03-09 10:21:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (9, 3, '/data', '2026-03-09 10:22:00', 200)");
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (10, 1, '/users', '2026-03-09 10:03:00', 429)");

        // 2 rate limit overrides
        $this->pdo->exec("INSERT INTO mp_rl_rate_limit_overrides VALUES (1, 1, '/upload', 50)");
        $this->pdo->exec("INSERT INTO mp_rl_rate_limit_overrides VALUES (2, 2, '/export', 200)");
    }
    ...
    public function testRateLimitCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, COUNT(*) AS req_count, c.requests_per_hour
             FROM mp_rl_api_clients c
             JOIN mp_rl_api_requests r ON r.client_id = c.id
    ...
    public function testOverrideApplied(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, r.endpoint,
             ...
             FROM mp_rl_api_clients c
             JOIN mp_rl_api_requests r ON r.client_id = c.id
             LEFT JOIN mp_rl_rate_limit_overrides o
    ...
    {
        $this->pdo->exec("INSERT INTO mp_rl_api_requests VALUES (11, 1, '/health', '2026-03-09 10:30:00', 200)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_rl_api_requests");
        $this->assertSame(11, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT endpoint FROM mp_rl_api_requests WHERE id = 11");
        $this->assertSame('/health', $rows[0]['endpoint']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_rl_api_requests")->fetchAll(PDO::FETCH_ASSOC);
