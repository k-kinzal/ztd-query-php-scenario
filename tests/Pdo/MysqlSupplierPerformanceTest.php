 ...
 * multiple scalar subqueries in SELECT, and prepared LIKE (MySQL PDO).
 ...
 * @spec SPEC-10.2.176
 */
class MysqlSupplierPerformanceTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_spf_supplier (
                id INT PRIMARY KEY,
                name VARCHAR(100),
                region VARCHAR(50),
                rating DECIMAL(3,1)
            )',
            'CREATE TABLE mp_spf_purchase_order (
                id INT PRIMARY KEY,
                supplier_id INT,
                product VARCHAR(100),
                quantity INT,
    ...
    {
        return ['mp_spf_delivery', 'mp_spf_purchase_order', 'mp_spf_supplier'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_spf_supplier VALUES (1, 'Acme Parts', 'North', 4.2)");
        $this->pdo->exec("INSERT INTO mp_spf_supplier VALUES (2, 'Beta Supply', 'South', 3.8)");
        $this->pdo->exec("INSERT INTO mp_spf_supplier VALUES (3, 'Gamma Materials', 'North', 4.5)");
        $this->pdo->exec("INSERT INTO mp_spf_supplier VALUES (4, 'Delta Components', 'East', 3.5)");

        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (1, 1, 'Bolts', 1000, 0.50, '2025-01-10')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (2, 1, 'Nuts', 2000, 0.30, '2025-01-20')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (3, 2, 'Screws', 500, 0.80, '2025-02-05')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (4, 2, 'Washers', 3000, 0.10, '2025-02-15')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (5, 3, 'Bolts', 1500, 0.45, '2025-01-15')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (6, 3, 'Gaskets', 800, 1.20, '2025-03-01')");
        $this->pdo->exec("INSERT INTO mp_spf_purchase_order VALUES (7, 4, 'Rivets', 2000, 0.25, '2025-03-10')");

        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (1, 1, '2025-01-15', 95, 1000)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (2, 2, '2025-01-25', 90, 1950)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (3, 3, '2025-02-12', 80, 480)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (4, 4, '2025-02-20', 85, 2900)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (5, 5, '2025-01-18', 98, 1500)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (6, 6, '2025-03-05', 92, 790)");
        $this->pdo->exec("INSERT INTO mp_spf_delivery VALUES (7, 7, '2025-03-20', 70, 1800)");
    }
    ...
    public function testMultipleScalarSubqueriesInSelect(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name,
                    (SELECT COUNT(*) FROM mp_spf_purchase_order po WHERE po.supplier_id = s.id) AS order_count,
                    (SELECT ROUND(AVG(dl.quality_score), 1)
                     FROM mp_spf_delivery dl
                     JOIN mp_spf_purchase_order po ON po.id = dl.order_id
    ...
    public function testAvgWithCaseConditional(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name,
             ...
             FROM mp_spf_supplier s
             JOIN mp_spf_purchase_order po ON po.supplier_id = s.id
             JOIN mp_spf_delivery dl ON dl.order_id = po.id
    ...
    /**
     * UPDATE SET with CASE expression: set rating tier.
     */
    public function testUpdateWithCaseExpression(): void
    {
        $this->ztdExec(
            "UPDATE mp_spf_supplier SET rating =
        ...
        $rows = $this->ztdQuery("SELECT name, rating FROM mp_spf_supplier ORDER BY name");
        $this->assertCount(4, $rows);
    ...
    /**
     * Prepared region filter with LEFT JOIN count.
     */
    public function testPreparedRegionFilter(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT s.name, COUNT(po.id) AS order_count
             FROM mp_spf_supplier s
             LEFT JOIN mp_spf_purchase_order po ON po.supplier_id = s.id
             WHERE s.region = ?
             GROUP BY s.id, s.name
             ORDER BY s.name",
            ['North']
    ...
    {
        $this->ztdExec("UPDATE mp_spf_supplier SET rating = 5.0 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT rating FROM mp_spf_supplier WHERE id = 1");
        $this->assertEqualsWithDelta(5.0, (float) $rows[0]['rating'], 0.01);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_spf_supplier")->fetchAll(PDO::FETCH_ASSOC);
