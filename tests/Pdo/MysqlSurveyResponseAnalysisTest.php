 ...
 * @spec SPEC-10.2.182
 */
class MysqlSurveyResponseAnalysisTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_surv_questions (
                id INT PRIMARY KEY,
                survey_id INT,
                question_text VARCHAR(200),
                category VARCHAR(50)
            )',
            'CREATE TABLE mp_surv_responses (
                id INT PRIMARY KEY,
                question_id INT,
    ...
    {
        return ['mp_surv_responses', 'mp_surv_summary', 'mp_surv_questions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO mp_surv_questions VALUES (1, 1, 'How satisfied are you?', 'satisfaction')");
        $this->pdo->exec("INSERT INTO mp_surv_questions VALUES (2, 1, 'Would you recommend?', 'nps')");
        $this->pdo->exec("INSERT INTO mp_surv_questions VALUES (3, 1, 'Rate ease of use', 'usability')");

        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (1, 1, 'alice', 5, 'Great!')");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (2, 1, 'bob', 4, NULL)");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (3, 1, 'carol', 3, 'OK')");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (4, 2, 'alice', 5, 'Definitely')");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (5, 2, 'bob', 5, NULL)");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (6, 2, 'carol', 2, 'Not sure')");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (7, 2, 'dave', 4, NULL)");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (8, 3, 'alice', 4, NULL)");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (9, 3, 'bob', 3, 'Could improve')");
        $this->pdo->exec("INSERT INTO mp_surv_responses VALUES (10, 3, 'dave', 5, 'Love it')");
    }
    ...
    public function testConditionalAggregatesSumCaseWhen(): void
    {
        $rows = $this->ztdQuery(
            "SELECT q.category,
             ...
             FROM mp_surv_responses r
             JOIN mp_surv_questions q ON r.question_id = q.id
             GROUP BY q.category
             ORDER BY q.category"
        );
    ...
    {
        $this->ztdExec(
            "INSERT INTO mp_surv_summary (id, question_id, response_count, avg_rating, promoter_count)
    ...
    {
        $this->ztdExec(
            "INSERT INTO mp_surv_responses VALUES (11, 1, 'eve', 1, 'Terrible')"
        );
    ...
    {
        $this->ztdExec(
            "DELETE FROM mp_surv_responses WHERE rating <= 2"
        );

        // Only response 6 (carol, rating=2) removed; 10 - 1 = 9
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_surv_responses");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // carol still has responses with rating=3 (id=3), so still 4 unique respondents
        $rows = $this->ztdQuery(
            "SELECT COUNT(DISTINCT respondent) AS unique_resp FROM mp_surv_responses"
        );
