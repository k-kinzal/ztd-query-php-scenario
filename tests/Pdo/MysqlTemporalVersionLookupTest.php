 ...
 * a common pattern for pricing, configuration, and employee records (MySQL PDO).
 * @spec SPEC-10.2.122
 */
class MysqlTemporalVersionLookupTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE mp_tv_products (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255)
            )',
            'CREATE TABLE mp_tv_product_prices (
                id INT AUTO_INCREMENT PRIMARY KEY,
                product_id INT,
                price DECIMAL(10,2),
    ...
    {
        return ['mp_tv_product_prices', 'mp_tv_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Products
        $this->pdo->exec("INSERT INTO mp_tv_products VALUES (1, 'Widget')");
        $this->pdo->exec("INSERT INTO mp_tv_products VALUES (2, 'Gadget')");
        $this->pdo->exec("INSERT INTO mp_tv_products VALUES (3, 'Gizmo')");

        // Price history
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (1, 1, 9.99,  '2025-01-01', '2025-06-30')");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (2, 1, 12.99, '2025-07-01', '2026-01-31')");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (3, 1, 14.99, '2026-02-01', NULL)");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (4, 2, 24.99, '2025-01-01', '2025-12-31')");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (5, 2, 29.99, '2026-01-01', NULL)");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (6, 3, 4.99,  '2025-01-01', '2025-03-31')");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (7, 3, 5.99,  '2025-04-01', '2025-12-31')");
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (8, 3, 6.99,  '2026-01-01', NULL)");
    }
    ...
    public function testCurrentPrices(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name, pp.price
             FROM mp_tv_products p
             JOIN mp_tv_product_prices pp ON pp.product_id = p.id
    ...
    public function testPriceIncreasePct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name,
             ...
             FROM mp_tv_products p
             JOIN mp_tv_product_prices curr ON curr.product_id = p.id AND curr.effective_to IS NULL
             JOIN mp_tv_product_prices first ON first.product_id = p.id
               AND first.effective_from = (
                   SELECT MIN(pp2.effective_from)
                   FROM mp_tv_product_prices pp2
    ...
    public function testUpdateCurrentPrice(): void
    {
        ...
        $this->pdo->exec(
            "UPDATE mp_tv_product_prices SET effective_to = '2026-03-08' WHERE id = 3"
        );

        // Insert new current price for Widget
        $this->pdo->exec(
            "INSERT INTO mp_tv_product_prices VALUES (9, 1, 16.99, '2026-03-09', NULL)"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM mp_tv_product_prices WHERE product_id = 1"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO mp_tv_product_prices VALUES (9, 1, 16.99, '2026-03-09', NULL)");
        $this->pdo->exec("UPDATE mp_tv_products SET name = 'Widget Pro' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM mp_tv_product_prices");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT name FROM mp_tv_products WHERE id = 1");
        $this->assertSame('Widget Pro', $rows[0]['name']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM mp_tv_product_prices")->fetchAll(PDO::FETCH_ASSOC);
