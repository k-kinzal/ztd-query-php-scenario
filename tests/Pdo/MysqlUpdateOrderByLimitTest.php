...
/**
 * Tests UPDATE/DELETE with ORDER BY and LIMIT through ZTD shadow store on MySQL PDO.
 *
 ...
 */
class MysqlUpdateOrderByLimitTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE my_updlimit_queue (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_low', 10, 'pending')");
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_med1', 50, 'pending')");
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_med2', 50, 'pending')");
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_high1', 90, 'pending')");
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_high2', 80, 'pending')");
        $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('task_top', 100, 'pending')");
    }
    ...
    public function testUpdateOrderByLimit(): void
    {
        try {
            $affected = $this->ztdExec(
                "UPDATE my_updlimit_queue SET status = 'processing' ORDER BY priority DESC LIMIT 3"
            );

            $processing = $this->ztdQuery(
                "SELECT task, priority FROM my_updlimit_queue WHERE status = 'processing' ORDER BY priority DESC"
            );
            $pending = $this->ztdQuery(
                "SELECT task FROM my_updlimit_queue WHERE status = 'pending' ORDER BY priority ASC"
            );
            ...
            if (count($processing) !== 3) {
                $all = $this->ztdQuery("SELECT task, priority, status FROM my_updlimit_queue ORDER BY priority DESC");
                $this->markTestIncomplete(
    ...
    public function testDeleteOrderByLimit(): void
    {
        try {
            $affected = $this->ztdExec(
                "DELETE FROM my_updlimit_queue ORDER BY priority ASC LIMIT 2"
            );

            $remaining = $this->ztdQuery(
                "SELECT task, priority FROM my_updlimit_queue ORDER BY priority ASC"
            );
    ...
    public function testPreparedUpdateOrderByLimit(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE my_updlimit_queue SET status = 'processing' ORDER BY priority DESC LIMIT ?"
            );
            $stmt->execute([2]);
    ...
    public function testUpdateLimitOnShadowData(): void
    {
        try {
            ...
            $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('shadow_a', 200, 'pending')");
            $this->pdo->exec("INSERT INTO my_updlimit_queue (task, priority, status) VALUES ('shadow_b', 150, 'pending')");

            // UPDATE top 2 by priority — should pick shadow_a (200) and shadow_b (150)
            $affected = $this->ztdExec(
                "UPDATE my_updlimit_queue SET status = 'claimed' ORDER BY priority DESC LIMIT 2"
            );

            $claimed = $this->ztdQuery(
                "SELECT task, priority FROM my_updlimit_queue WHERE status = 'claimed' ORDER BY priority DESC"
            );
            ...
            $pending = $this->ztdQuery(
                "SELECT task FROM my_updlimit_queue WHERE status = 'pending' ORDER BY priority DESC"
            );
