...
/**
 * Tests UPDATE with correlated subqueries in SET and WHERE clauses through ZTD CTE shadow store (MySQL PDO).
 * Mirror of SqliteUpdateSubqueryTest to verify whether Issue #51 (UPDATE SET correlated subquery bug)
 * also affects MySQL.
 *
 ...
 */
class MysqlUpdateSubqueryTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE my_usq_categories (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                discount_pct DECIMAL(5,2) NOT NULL
            )',
            'CREATE TABLE my_usq_products (
                id INT PRIMARY KEY,
    ...
    {
        return ['my_usq_products', 'my_usq_categories'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO my_usq_categories VALUES (1, 'Electronics', 5.00)");
        $this->pdo->exec("INSERT INTO my_usq_categories VALUES (2, 'Office', 10.00)");
        $this->pdo->exec("INSERT INTO my_usq_categories VALUES (3, 'Accessories', 15.00)");

        // 6 products across categories (2 per category)
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (1, 'Laptop', 1000.00, 1)");
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (2, 'Phone', 800.00, 1)");
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (3, 'Desk', 500.00, 2)");
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (4, 'Chair', 300.00, 2)");
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (5, 'Cable', 20.00, 3)");
        $this->pdo->exec("INSERT INTO my_usq_products VALUES (6, 'Case', 50.00, 3)");
    }
     ...
     *
     * Expected to fail based on Issue #51 (UPDATE SET correlated subquery bug).
     */
    public function testUpdateSetCorrelatedSubqueryDiscount(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE my_usq_products SET price = price * (1 - (SELECT discount_pct / 100.0 FROM my_usq_categories WHERE id = my_usq_products.category_id))"
            );
        ...
        $rows = $this->ztdQuery("SELECT id, name, price FROM my_usq_products ORDER BY id");
    ...
    public function testUpdateWhereInSubquery(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE my_usq_products SET price = price * 0.9 WHERE category_id IN (SELECT id FROM my_usq_categories WHERE name = 'Electronics')"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
     ...
     *
     * Expected to fail based on Issue #51.
     */
    public function testUpdateSetScalarSubquerySameTable(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE my_usq_products SET price = (SELECT MAX(price) FROM my_usq_products p2 WHERE p2.category_id = my_usq_products.category_id) WHERE name = 'Phone'"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
    ...
    public function testUpdateZeroRowsSubqueryReturnsEmpty(): void
    {
        ...
        try {
            $affectedRows = $this->pdo->exec(
                "UPDATE my_usq_products SET price = 0.00 WHERE category_id IN (SELECT id FROM my_usq_categories WHERE name = 'Furniture')"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
        ...
        $rows = $this->ztdQuery("SELECT id, price FROM my_usq_products ORDER BY id");
        $this->assertCount(6, $rows);
    ...
    public function testUpdateCorrelatedSetWithWhereFilter(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE my_usq_products SET price = price * (1 - (SELECT discount_pct / 100.0 FROM my_usq_categories WHERE id = my_usq_products.category_id)) WHERE price > 100"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $shadowRows = $this->ztdQuery("SELECT id, price FROM my_usq_products WHERE id = 1");
        ...
        $this->pdo->disableZtd();
        $physicalRows = $this->pdo->query('SELECT id, price FROM my_usq_products')->fetchAll(PDO::FETCH_ASSOC);
