...
/**
 * Tests SELECT from views after shadow DML on MySQL.
 *
 ...
 */
class MysqlViewDmlTest extends AbstractMysqlPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE my_vw_employees (
                id INT PRIMARY KEY,
            ...
            'CREATE VIEW my_vw_active_employees AS
                SELECT id, name, department, salary
                FROM my_vw_employees
                WHERE active = 1',
            'CREATE VIEW my_vw_dept_summary AS
    ...
    {
        return ['my_vw_dept_summary', 'my_vw_active_employees', 'my_vw_employees'];
    }

    protected function setUp(): void
    {
        // Drop views before the parent tries DROP TABLE on them
        $raw = new PDO(
        ...
        $raw->exec('DROP VIEW IF EXISTS my_vw_dept_summary');
        $raw->exec('DROP VIEW IF EXISTS my_vw_active_employees');

        parent::setUp();

        $this->pdo->exec("INSERT INTO my_vw_employees VALUES (1, 'Alice', 'Engineering', 120000, 1)");
        $this->pdo->exec("INSERT INTO my_vw_employees VALUES (2, 'Bob', 'Engineering', 110000, 1)");
        $this->pdo->exec("INSERT INTO my_vw_employees VALUES (3, 'Charlie', 'Sales', 95000, 1)");
        $this->pdo->exec("INSERT INTO my_vw_employees VALUES (4, 'Diana', 'Sales', 90000, 0)");
    }
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO my_vw_employees VALUES (5, 'Eve', 'Engineering', 130000, 1)");

            $rows = $this->ztdQuery("SELECT name FROM my_vw_active_employees WHERE department = 'Engineering' ORDER BY name");
    ...
    public function testViewAfterUpdateChangesVisibility(): void
    {
        try {
            ...
            $this->pdo->exec("UPDATE my_vw_employees SET active = 0 WHERE id = 2");

            $rows = $this->ztdQuery("SELECT name FROM my_vw_active_employees ORDER BY name");
    ...
    public function testAggregateViewAfterDml(): void
    {
        try {
            ...
            $this->pdo->exec("DELETE FROM my_vw_employees WHERE id = 3");

            $rows = $this->ztdQuery("SELECT department, emp_count FROM my_vw_dept_summary ORDER BY department");
    ...
    public function testViewAvgAfterSalaryUpdate(): void
    {
        try {
            // Give Alice a raise
            $this->pdo->exec("UPDATE my_vw_employees SET salary = 150000 WHERE id = 1");

            $rows = $this->ztdQuery(
                "SELECT avg_salary FROM my_vw_dept_summary WHERE department = 'Engineering'"
            );
    ...
    public function testPreparedSelectFromView(): void
    {
        try {
            ...
            $rows = $this->ztdPrepareAndExecute(
                "SELECT name FROM my_vw_active_employees WHERE salary > ? ORDER BY salary DESC",
                [115000]
            );
        ...
        try {
            $rows = $this->ztdQuery(
                "SELECT v.name, v.salary, e.active
                 FROM my_vw_active_employees v
                 JOIN my_vw_employees e ON v.id = e.id
                 ORDER BY v.name"
            );
