...
class MysqlZtdLifecycleTest extends AbstractMysqlPdoTestCase
{
    ...
    {
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (1, 'shadow')");

        $stmt = $this->pdo->query('SELECT * FROM mysql_lifecycle_test');
    ...
    {
        $this->pdo->disableZtd();
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (1, 'physical')");

        $stmt = $this->pdo->query('SELECT * FROM mysql_lifecycle_test WHERE id = 1');
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $this->assertCount(1, $rows);
        ...
        $this->pdo->disableZtd();
        $this->pdo->exec('DELETE FROM mysql_lifecycle_test WHERE id = 1');
    }

    public function testMultipleToggleCycles(): void
    {
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (1, 'cycle1')");

        $this->pdo->disableZtd();
        $this->pdo->enableZtd();
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (2, 'cycle2')");

        $this->pdo->disableZtd();
        $this->pdo->enableZtd();
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (3, 'cycle3')");

        $stmt = $this->pdo->query('SELECT * FROM mysql_lifecycle_test ORDER BY id');
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (1, 'before')");

        $this->pdo->disableZtd();
        $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES (2, 'physical')");
        $this->pdo->enableZtd();
        ...
        $stmt = $this->pdo->query('SELECT * FROM mysql_lifecycle_test WHERE id = 2');
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $this->assertCount(0, $rows);

        // Clean up physical data
        $this->pdo->disableZtd();
        $this->pdo->exec('DELETE FROM mysql_lifecycle_test WHERE id = 2');
    }

    public function testManySequentialOperations(): void
    {
        for ($i = 1; $i <= 50; $i++) {
            $this->pdo->exec("INSERT INTO mysql_lifecycle_test (id, val) VALUES ($i, 'item_$i')");
        }
        ...
        $this->pdo->exec("UPDATE mysql_lifecycle_test SET val = 'updated' WHERE id <= 25");

        $stmt = $this->pdo->query("SELECT COUNT(*) as cnt FROM mysql_lifecycle_test WHERE val = 'updated'");
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $this->assertSame(25, (int) $row['cnt']);

        $this->pdo->exec('DELETE FROM mysql_lifecycle_test WHERE id <= 12');
