...
/**
 * Tests access log sessionization through ZTD shadow store (PostgreSQL PDO).
 * Covers window functions (LAG, ROW_NUMBER) for session detection,
 * page visit counting, funnel analysis, and physical isolation.
 * @spec SPEC-10.2.106
 */
class PostgresAccessLogSessionTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_al_logs (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (1,  1, '/home',     'view',  '2026-03-01 09:00:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (2,  1, '/products', 'view',  '2026-03-01 09:05:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (3,  1, '/products/1','view', '2026-03-01 09:08:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (4,  1, '/cart',     'add',   '2026-03-01 09:10:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (5,  1, '/checkout', 'view',  '2026-03-01 09:15:00')");
        // Session gap (next day)
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (6,  1, '/home',     'view',  '2026-03-02 14:00:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (7,  1, '/products', 'view',  '2026-03-02 14:03:00')");

        // User 2 (Bob): one session
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (8,  2, '/home',     'view',  '2026-03-01 10:00:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (9,  2, '/products', 'view',  '2026-03-01 10:02:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (10, 2, '/cart',     'add',   '2026-03-01 10:05:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (11, 2, '/checkout', 'view',  '2026-03-01 10:08:00')");
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (12, 2, '/checkout', 'purchase','2026-03-01 10:10:00')");
    }
    ...
    public function testTimeBetweenVisits(): void
    {
        ...
        $this->assertStringContainsString('2026-03-01', $rows[1]['prev_visit']);
        $this->assertSame('/products', $rows[1]['page']);
    }
    ...
    {
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (13, 1, '/checkout', 'purchase', '2026-03-02 14:10:00')");

        // Alice now also has a purchase
        $rows = $this->ztdQuery(
            "SELECT COUNT(DISTINCT user_id) AS purchasers FROM pg_al_logs WHERE action = 'purchase'"
        );
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT SUBSTR(CAST(created_at AS VARCHAR), 1, 10) AS visit_date,
                    COUNT(DISTINCT user_id) AS active_users,
                    COUNT(*) AS total_events
             FROM pg_al_logs
             GROUP BY SUBSTR(CAST(created_at AS VARCHAR), 1, 10)
             ORDER BY visit_date"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_al_logs VALUES (14, 3, '/home', 'view', '2026-03-03 10:00:00')");
        $this->pdo->exec("DELETE FROM pg_al_logs WHERE id = 1");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_al_logs");
        $this->assertEquals(12, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_al_logs")->fetchAll(PDO::FETCH_ASSOC);
