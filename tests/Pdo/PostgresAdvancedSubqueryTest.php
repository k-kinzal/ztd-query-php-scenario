 ...
 */
class PostgresAdvancedSubqueryTest extends TestCase
{
    private ZtdPdo $pdo;
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $raw->exec('DROP TABLE IF EXISTS pg_advsq_projects');
        $raw->exec('DROP TABLE IF EXISTS pg_advsq_employees');
        $raw->exec('DROP TABLE IF EXISTS pg_advsq_departments');
        $raw->exec('CREATE TABLE pg_advsq_departments (id INT PRIMARY KEY, name VARCHAR(255), budget DECIMAL(12,2))');
        $raw->exec('CREATE TABLE pg_advsq_employees (id INT PRIMARY KEY, name VARCHAR(255), dept_id INT, salary DECIMAL(10,2), active INT DEFAULT 1)');
        $raw->exec('CREATE TABLE pg_advsq_projects (id INT PRIMARY KEY, name VARCHAR(255), dept_id INT, status VARCHAR(50))');
    }
