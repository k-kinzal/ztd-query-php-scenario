 ...
 * anti-join patterns (PostgreSQL PDO).
...
class PostgresAppointmentSchedulingTest extends AbstractPostgresPdoTestCase
{
    ...
    {
        return [
            'CREATE TABLE pg_appt_room (
                id INT PRIMARY KEY,
                name VARCHAR(50),
                capacity INT,
                building VARCHAR(50)
            )',
            'CREATE TABLE pg_appt_booking (
                id INT PRIMARY KEY,
                room_id INT,
                title VARCHAR(100),
    ...
    {
        return ['pg_appt_booking', 'pg_appt_room'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_appt_room VALUES (1, 'Alpha', 10, 'HQ')");
        $this->pdo->exec("INSERT INTO pg_appt_room VALUES (2, 'Beta', 6, 'HQ')");
        $this->pdo->exec("INSERT INTO pg_appt_room VALUES (3, 'Gamma', 20, 'Annex')");

        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (1, 1, 'Sprint Planning', 'alice', '2025-03-10 09:00', '2025-03-10 10:00', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (2, 1, 'Design Review', 'bob', '2025-03-10 10:30', '2025-03-10 11:30', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (3, 2, 'Standup', 'carol', '2025-03-10 09:00', '2025-03-10 09:15', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (4, 1, 'Lunch Talk', 'dave', '2025-03-10 12:00', '2025-03-10 13:00', 'tentative', NULL)");
        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (5, 3, 'All Hands', 'alice', '2025-03-10 14:00', '2025-03-10 15:00', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO pg_appt_booking VALUES (6, 2, 'Retrospective', 'bob', '2025-03-10 15:00', '2025-03-10 16:00', 'cancelled', NULL)");
    }
    ...
    public function testBetweenTimeRange(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.title, b.start_time, r.name AS room
             FROM pg_appt_booking b
             JOIN pg_appt_room r ON r.id = b.room_id
    ...
    public function testExistsSubquery(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name, r.building
             FROM pg_appt_room r
             WHERE EXISTS (
                 SELECT 1 FROM pg_appt_booking b
    ...
    {
        $this->ztdExec(
            "UPDATE pg_appt_booking SET status = 'cancelled'
             WHERE start_time BETWEEN '2025-03-10 09:00' AND '2025-03-10 11:59'
               AND status != 'cancelled'"
        );

        $rows = $this->ztdQuery(
            "SELECT title, status FROM pg_appt_booking ORDER BY id"
        );
    ...
    public function testCountCaseStatusSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name,
             ...
             FROM pg_appt_room r
             LEFT JOIN pg_appt_booking b ON b.room_id = r.id
    ...
    {
        $this->ztdExec("DELETE FROM pg_appt_booking WHERE status = 'cancelled'");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_appt_booking");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_appt_booking")->fetchAll(PDO::FETCH_ASSOC);
