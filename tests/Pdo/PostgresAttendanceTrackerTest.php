 ...
 * prepared BETWEEN for date ranges, HAVING for filtering aggregates (PostgreSQL PDO).
 * @spec SPEC-10.2.142
 */
class PostgresAttendanceTrackerTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_at_employees (
                id SERIAL PRIMARY KEY,
                name TEXT,
                department TEXT
            )',
            'CREATE TABLE pg_at_attendance (
                id SERIAL PRIMARY KEY,
                employee_id INTEGER,
                attend_date TEXT,
                status TEXT,
    ...
    {
        return ['pg_at_attendance', 'pg_at_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Employees
        $this->pdo->exec("INSERT INTO pg_at_employees VALUES (1, 'Alice Chen', 'Engineering')");
        $this->pdo->exec("INSERT INTO pg_at_employees VALUES (2, 'Bob Park', 'Marketing')");
        $this->pdo->exec("INSERT INTO pg_at_employees VALUES (3, 'Carol Kim', 'Engineering')");

        // Attendance — August 2025
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (1, 1, '2025-08-01', 'present', '08:55')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (2, 1, '2025-08-02', 'present', '08:50')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (3, 1, '2025-08-03', 'late', '09:15')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (4, 1, '2025-08-04', 'present', '08:45')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (5, 1, '2025-08-05', 'absent', NULL)");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (6, 2, '2025-08-01', 'present', '08:58')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (7, 2, '2025-08-02', 'late', '09:20')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (8, 2, '2025-08-03', 'late', '09:10')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (9, 2, '2025-08-04', 'absent', NULL)");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (10, 2, '2025-08-05', 'present', '08:40')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (11, 3, '2025-08-01', 'present', '08:30')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (12, 3, '2025-08-02', 'present', '08:45')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (13, 3, '2025-08-03', 'present', '08:50')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (14, 3, '2025-08-04', 'present', '08:35')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (15, 3, '2025-08-05', 'present', '08:40')");

        // Attendance — September 2025
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (16, 1, '2025-09-01', 'present', '08:50')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (17, 1, '2025-09-02', 'late', '09:05')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (18, 2, '2025-09-01', 'absent', NULL)");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (19, 2, '2025-09-02', 'present', '08:55')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (20, 3, '2025-09-01', 'present', '08:30')");
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (21, 3, '2025-09-02', 'present', '08:35')");
    }
    ...
    public function testAttendanceSummaryPerEmployee(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
             ...
             FROM pg_at_employees e
             JOIN pg_at_attendance a ON e.id = a.employee_id
             GROUP BY e.name
             ORDER BY e.name"
        );
    ...
    public function testAbsentEmployeesOnDate(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name
             FROM pg_at_employees e
             LEFT JOIN pg_at_attendance a ON e.id = a.employee_id AND a.attend_date = '2025-08-06'
             WHERE a.id IS NULL
             ORDER BY e.name"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_at_attendance VALUES (22, 1, '2025-09-03', 'present', '08:50')");

        // ZTD sees the new record
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_at_attendance");
        $this->assertEquals(22, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_at_attendance")->fetchAll(PDO::FETCH_ASSOC);
