...
/**
 * Tests DML with BETWEEN and prepared parameters through ZTD shadow store on PostgreSQL.
 *
 * @spec SPEC-4.2, SPEC-4.3, SPEC-3.2
 */
class PostgresBetweenParamDmlTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_btw_products (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_btw_products (name, price, stock) VALUES ('Widget', 9.99, 100)");
        $this->pdo->exec("INSERT INTO pg_btw_products (name, price, stock) VALUES ('Gadget', 24.99, 50)");
        $this->pdo->exec("INSERT INTO pg_btw_products (name, price, stock) VALUES ('Doohickey', 49.99, 25)");
        $this->pdo->exec("INSERT INTO pg_btw_products (name, price, stock) VALUES ('Thingamajig', 74.99, 10)");
        $this->pdo->exec("INSERT INTO pg_btw_products (name, price, stock) VALUES ('Whatchamacallit', 99.99, 5)");
    }
    ...
    public function testPreparedUpdateBetween(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE pg_btw_products SET stock = stock + 10 WHERE price BETWEEN ? AND ?"
            );
            $stmt->execute([20.00, 75.00]);

            $rows = $this->ztdQuery("SELECT name, stock FROM pg_btw_products ORDER BY id");
            $byName = [];
            foreach ($rows as $r) {
    ...
    public function testPreparedDeleteBetween(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "DELETE FROM pg_btw_products WHERE price BETWEEN ? AND ?"
            );
            $stmt->execute([25.00, 100.00]);

            $rows = $this->ztdQuery("SELECT name FROM pg_btw_products ORDER BY id");
    ...
    public function testPreparedUpdateBetweenWithLogicalAnd(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE pg_btw_products SET stock = 0 WHERE price BETWEEN ? AND ? AND stock < ?"
            );
