 ...
 * calculations, and CASE conditional aggregation (PostgreSQL PDO).
 * @spec SPEC-10.2.128
 */
class PostgresBudgetRolloverTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_br_departments (
                id SERIAL PRIMARY KEY,
                name TEXT,
                annual_budget NUMERIC(12,2)
            )',
            'CREATE TABLE pg_br_expenses (
                id SERIAL PRIMARY KEY,
                department_id INT,
                expense_month TEXT,
                category TEXT,
    ...
    {
        return ['pg_br_expenses', 'pg_br_departments'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Departments
        $this->pdo->exec("INSERT INTO pg_br_departments VALUES (1, 'Engineering', 120000.00)");
        $this->pdo->exec("INSERT INTO pg_br_departments VALUES (2, 'Marketing', 60000.00)");
        $this->pdo->exec("INSERT INTO pg_br_departments VALUES (3, 'Sales', 90000.00)");

        // Expenses — Engineering (monthly budget = 10000)
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (1, 1, '2025-01', 'cloud', 3000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (2, 1, '2025-01', 'tools', 2000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (3, 1, '2025-02', 'cloud', 3500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (4, 1, '2025-02', 'tools', 1500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (5, 1, '2025-02', 'training', 4000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (6, 1, '2025-03', 'cloud', 3000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (7, 1, '2025-03', 'tools', 1000.00)");

        // Expenses — Marketing (monthly budget = 5000)
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (8, 2, '2025-01', 'ads', 4500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (9, 2, '2025-01', 'events', 1000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (10, 2, '2025-02', 'ads', 3000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (11, 2, '2025-03', 'ads', 5000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (12, 2, '2025-03', 'events', 2000.00)");

        // Expenses — Sales (monthly budget = 7500)
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (13, 3, '2025-01', 'travel', 2000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (14, 3, '2025-01', 'entertainment', 1500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (15, 3, '2025-02', 'travel', 3000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (16, 3, '2025-02', 'entertainment', 2500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (17, 3, '2025-02', 'supplies', 500.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (18, 3, '2025-03', 'travel', 4000.00)");
    }
    ...
    public function testMonthlySpendingPerDepartment(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name, e.expense_month, SUM(e.amount) AS monthly_total
             FROM pg_br_departments d
             JOIN pg_br_expenses e ON e.department_id = d.id
    ...
    public function testCategoryBreakdown(): void
    {
        $rows = $this->ztdQuery(
            "SELECT category, SUM(amount) AS total,
                    ...
                    ROUND(SUM(amount) * 100.0 / (SELECT SUM(amount) FROM pg_br_expenses WHERE 1=1), 1) AS pct_of_total
             FROM pg_br_expenses
             GROUP BY category
             ORDER BY total DESC"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_br_departments VALUES (4, 'HR', 50000.00)");
        $this->pdo->exec("INSERT INTO pg_br_expenses VALUES (19, 4, '2025-01', 'recruiting', 2000.00)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_br_departments");
        $this->assertEquals(4, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_br_expenses");
        $this->assertEquals(19, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_br_departments")->fetchAll(PDO::FETCH_ASSOC);
