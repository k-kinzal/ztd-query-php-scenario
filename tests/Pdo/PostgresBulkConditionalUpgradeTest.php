 ...
 *
 * Known Issue on SQLite: UPDATE WHERE IN (SELECT ... GROUP BY ... HAVING ...) causes "incomplete input"
 * because the CTE rewriter truncates the SQL. On PostgreSQL, this may behave differently.
 * The workaround (query eligible IDs first, then UPDATE by explicit ID list) is kept as the
 * recommended portable pattern. The direct approach is also tested to document PostgreSQL behavior.
 *
 * @spec SPEC-4.2
 * @spec SPEC-3.3
 */
class PostgresBulkConditionalUpgradeTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_bu_customers (
                id INTEGER PRIMARY KEY,
                name TEXT,
                email TEXT,
                tier TEXT DEFAULT \'bronze\'
            )',
            'CREATE TABLE pg_bu_orders (
                id INTEGER PRIMARY KEY,
                customer_id INTEGER,
                amount NUMERIC(10,2),
                status TEXT,
    ...
    {
        return ['pg_bu_orders', 'pg_bu_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_bu_customers VALUES (1, 'Alice', 'alice@example.com', 'bronze')");
        $this->pdo->exec("INSERT INTO pg_bu_customers VALUES (2, 'Bob', 'bob@example.com', 'bronze')");
        $this->pdo->exec("INSERT INTO pg_bu_customers VALUES (3, 'Carol', 'carol@example.com', 'bronze')");
        $this->pdo->exec("INSERT INTO pg_bu_customers VALUES (4, 'Dave', 'dave@example.com', 'bronze')");
        $this->pdo->exec("INSERT INTO pg_bu_customers VALUES (5, 'Eve', 'eve@example.com', 'bronze')");

        // Alice: high spender (total 2500)
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (1, 1, 1000.00, 'completed', '2024-01-01 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (2, 1, 800.00, 'completed', '2024-02-01 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (3, 1, 700.00, 'completed', '2024-03-01 00:00:00')");

        // Bob: medium spender (total 600)
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (4, 2, 300.00, 'completed', '2024-01-15 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (5, 2, 300.00, 'completed', '2024-02-15 00:00:00')");

        // Carol: low spender (total 150)
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (6, 3, 150.00, 'completed', '2024-01-20 00:00:00')");

        // Dave: has only pending orders
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (7, 4, 5000.00, 'pending', '2024-03-01 00:00:00')");

        // Eve: no orders at all
    }

    /**
     * Known Issue: UPDATE WHERE IN (SELECT ... GROUP BY ... HAVING ...) may fail on PostgreSQL
     * due to CTE rewriter truncating the SQL (same root cause as SQLite).
     * If this test passes, it means PostgreSQL handles it; if it throws, the workaround is needed.
    ...
    public function testBulkUpgradeWithAggregateSubqueryFails(): void
    {
        ...
        $this->pdo->exec(
            "UPDATE pg_bu_customers SET tier = 'gold'
             WHERE id IN (
                SELECT customer_id FROM pg_bu_orders
    ...
    public function testBulkUpgradeWorkaround(): void
    {
        ...
        if (!empty($ids)) {
            ...
            $this->pdo->exec("UPDATE pg_bu_customers SET tier = 'gold' WHERE id IN ({$placeholders})");
        }

        $rows = $this->ztdQuery("SELECT name, tier FROM pg_bu_customers ORDER BY name");
    ...
    public function testSequentialTierUpgrades(): void
    {
        ...
        $silverIds = array_column($this->ztdQuery(
            "SELECT customer_id FROM pg_bu_orders WHERE status = 'completed'
             GROUP BY customer_id HAVING SUM(amount) >= 500"
        ), 'customer_id');

        if (!empty($silverIds)) {
            $this->pdo->exec("UPDATE pg_bu_customers SET tier = 'silver' WHERE id IN (" . implode(',', $silverIds) . ")");
        }
        ...
        if (!empty($goldIds)) {
            $this->pdo->exec("UPDATE pg_bu_customers SET tier = 'gold' WHERE id IN (" . implode(',', $goldIds) . ")");
        }
    ...
    {
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (8, 2, 500.00, 'completed', '2024-03-15 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_bu_orders VALUES (9, 2, 300.00, 'completed', '2024-03-20 00:00:00')");
    ...
    public function testVerifyTierWithOrderAggregates(): void
    {
        ...
        $this->pdo->exec("UPDATE pg_bu_customers SET tier = 'gold' WHERE id = 1");

        $rows = $this->ztdQuery(
            "SELECT c.name, c.tier, COALESCE(SUM(o.amount), 0) AS total_spent
             FROM pg_bu_customers c
             LEFT JOIN pg_bu_orders o ON c.id = o.customer_id AND o.status = 'completed'
    ...
    public function testTierCountSummary(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT tier, COUNT(*) AS cnt FROM pg_bu_customers GROUP BY tier ORDER BY tier"
        );
    ...
    {
        $this->pdo->exec("UPDATE pg_bu_customers SET tier = 'gold' WHERE id = 4");

        $rows = $this->ztdQuery(
            "SELECT c.name FROM pg_bu_customers c
             WHERE c.tier = 'gold'
             AND c.id NOT IN (
                SELECT customer_id FROM pg_bu_orders
                WHERE status = 'completed'
    ...
    public function testUpdateWithSimpleSubqueryWorks(): void
    {
        ...
        $this->pdo->exec(
            "UPDATE pg_bu_customers SET tier = 'active'
             WHERE id IN (SELECT DISTINCT customer_id FROM pg_bu_orders WHERE status = 'completed')"
        );
    ...
    {
        $this->pdo->exec("UPDATE pg_bu_orders SET status = 'completed' WHERE id = 7");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT tier FROM pg_bu_customers WHERE id = 1");
        $this->assertEquals('gold', $rows[0]['tier']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT tier FROM pg_bu_customers")->fetchAll(PDO::FETCH_ASSOC);
