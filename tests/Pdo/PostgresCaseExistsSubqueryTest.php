...
/**
 * Tests CASE WHEN EXISTS (subquery) patterns via PostgreSQL PDO.
 *
 ...
 */
class PostgresCaseExistsSubqueryTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ceq_users (
                id SERIAL PRIMARY KEY,
                name VARCHAR(100) NOT NULL
            )',
            'CREATE TABLE pg_ceq_orders (
                id SERIAL PRIMARY KEY,
    ...
    {
        return ['pg_ceq_orders', 'pg_ceq_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ceq_users (id, name) VALUES (1, 'Alice')");
        $this->pdo->exec("INSERT INTO pg_ceq_users (id, name) VALUES (2, 'Bob')");
        $this->pdo->exec("INSERT INTO pg_ceq_users (id, name) VALUES (3, 'Charlie')");

        $this->pdo->exec("INSERT INTO pg_ceq_orders (id, user_id, amount) VALUES (1, 1, 100)");
        $this->pdo->exec("INSERT INTO pg_ceq_orders (id, user_id, amount) VALUES (2, 1, 200)");
        $this->pdo->exec("INSERT INTO pg_ceq_orders (id, user_id, amount) VALUES (3, 3, 50)");
    }
    ...
    public function testCaseExistsInSelect(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT u.id, u.name,
                        CASE WHEN EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id)
    ...
    public function testCaseExistsWithAggregate(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT u.id, u.name,
                        CASE
                            WHEN EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id HAVING SUM(o.amount) >= 200)
                                THEN 'high'
                            WHEN EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id)
                                THEN 'low'
                            ELSE 'none'
                        END AS tier
                 FROM pg_ceq_users u
    ...
    public function testCaseExistsInWhere(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT u.id, u.name
                 FROM pg_ceq_users u
                 WHERE CASE WHEN EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id)
                            THEN 1 ELSE 0 END = 1
                 ORDER BY u.id"
            )->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_ceq_orders (id, user_id, amount) VALUES (4, 2, 150)");
    ...
    public function testCaseNotExistsInSelect(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT u.id, u.name,
                        CASE WHEN NOT EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id)
    ...
    public function testPreparedCaseExists(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "SELECT u.id, u.name,
                        CASE WHEN EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id AND o.amount >= $1)
    ...
    {
        $this->pdo->query(
            "SELECT u.id FROM pg_ceq_users u
             WHERE EXISTS (SELECT 1 FROM pg_ceq_orders o WHERE o.user_id = u.id)"
        )->fetchAll(PDO::FETCH_ASSOC);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_ceq_users")
            ->fetchAll(PDO::FETCH_ASSOC);
