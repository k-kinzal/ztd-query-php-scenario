 ...
 * @spec SPEC-10.2
 */
class PostgresChainedCteShadowTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ccs_customers (
                id INT PRIMARY KEY,
                name VARCHAR(100) NOT NULL
            )',
            'CREATE TABLE pg_ccs_orders (
                id INT PRIMARY KEY,
    ...
    {
        return ['pg_ccs_orders', 'pg_ccs_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Seed customers
        $this->ztdExec("INSERT INTO pg_ccs_customers (id, name) VALUES (1, 'Alice')");
        $this->ztdExec("INSERT INTO pg_ccs_customers (id, name) VALUES (2, 'Bob')");
        $this->ztdExec("INSERT INTO pg_ccs_customers (id, name) VALUES (3, 'Charlie')");

        // Seed orders
        $this->ztdExec("INSERT INTO pg_ccs_orders (id, customer_id, amount, status) VALUES (1, 1, 100.00, 'active')");
        $this->ztdExec("INSERT INTO pg_ccs_orders (id, customer_id, amount, status) VALUES (2, 1, 200.00, 'active')");
        $this->ztdExec("INSERT INTO pg_ccs_orders (id, customer_id, amount, status) VALUES (3, 2, 150.00, 'active')");
        $this->ztdExec("INSERT INTO pg_ccs_orders (id, customer_id, amount, status) VALUES (4, 2, 50.00, 'cancelled')");
        $this->ztdExec("INSERT INTO pg_ccs_orders (id, customer_id, amount, status) VALUES (5, 3, 300.00, 'active')");
    }
     ...
     *
     * WITH
     *   active AS (SELECT * FROM orders WHERE status = 'active'),
     *   summary AS (SELECT customer_id, SUM(amount) as total FROM active GROUP BY customer_id)
     * SELECT c.name, s.total FROM summary s JOIN customers c ON s.customer_id = c.id
     */
    public function testChainedCteTwoLevelAfterInsert(): void
    {
        try {
            $rows = $this->ztdQuery("
                WITH
                    active AS (SELECT * FROM pg_ccs_orders WHERE status = 'active'),
                    summary AS (SELECT customer_id, SUM(amount) AS total FROM active GROUP BY customer_id)
                SELECT c.name, s.total
                FROM summary s
                JOIN pg_ccs_customers c ON s.customer_id = c.id
                ORDER BY c.name
            ");
    ...
    public function testChainedCteThreeLevel(): void
    {
        try {
            $rows = $this->ztdQuery("
                ...
                FROM high_value hv
                JOIN pg_ccs_customers c ON hv.customer_id = c.id
            ...
            // Both Alice and Charlie have total 300.00
     ...
     *
     * First CTE computes per-customer stats, second CTE filters by average.
     */
    public function testCteAggregateToFilter(): void
    {
        try {
            $rows = $this->ztdQuery("
                ...
                JOIN pg_ccs_customers c ON aa.customer_id = c.id
                ORDER BY c.name
            ");

            // Alice: avg (100+200)/2=150 > 100 -> included, total=300
            // Bob: avg (150+50)/2=100 -> NOT > 100 -> excluded
            // Charlie: avg 300/1=300 > 100 -> included, total=300
            if (count($rows) !== 2) {
                $this->markTestIncomplete(
    ...
    public function testChainedCteAfterUpdate(): void
    {
        try {
            ...
            $this->ztdExec("UPDATE pg_ccs_orders SET status = 'cancelled' WHERE id = 1");
            ...
            $this->assertCount(3, $rows);
            $this->assertSame('Alice', $rows[0]['name']);
            $this->assertSame('200.00', $rows[0]['total']); // was 300, now 200 after cancelling order 1
            $this->assertSame('Bob', $rows[1]['name']);
            $this->assertSame('150.00', $rows[1]['total']);
            $this->assertSame('Charlie', $rows[2]['name']);
            $this->assertSame('300.00', $rows[2]['total']);
    ...
    public function testChainedCteAfterDelete(): void
    {
        try {
            ...
            $this->ztdExec("DELETE FROM pg_ccs_orders WHERE id = 5");
            ...
            // Charlie had only one order (id=5), now deleted
            if (count($rows) !== 2) {
                $this->markTestIncomplete(
     ...
     *
     * First CTE selects from orders, second CTE joins the first CTE with customers.
     */
    public function testChainedCteWithJoinBetweenCteAndTable(): void
    {
        try {
            $rows = $this->ztdQuery("
                WITH
                    ...
                    enriched AS (
                        ...
                        FROM active a
                        JOIN pg_ccs_customers c ON a.customer_id = c.id
                        WHERE a.total >= 150
                    )
                SELECT name, total
                FROM enriched
            ...
            $this->assertCount(3, $rows);
            // 300.00 rows first (Alice, Charlie), then 150.00 (Bob)
            $this->assertSame('300.00', $rows[0]['total']);
