 ...
 * for latest message, multi-table INSERT chain, COUNT DISTINCT participants (PostgreSQL PDO).
 * @spec SPEC-10.2.134
 */
class PostgresChatMessagingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_cm_users (
                id SERIAL PRIMARY KEY,
                username TEXT,
                display_name TEXT
            )',
            'CREATE TABLE pg_cm_conversations (
                id SERIAL PRIMARY KEY,
                title TEXT,
                created_at TEXT
            )',
            'CREATE TABLE pg_cm_messages (
                id SERIAL PRIMARY KEY,
            ...
            'CREATE TABLE pg_cm_read_receipts (
                id SERIAL PRIMARY KEY,
                message_id INTEGER,
                user_id INTEGER,
                read_at TEXT
    ...
    {
        return ['pg_cm_read_receipts', 'pg_cm_messages', 'pg_cm_conversations', 'pg_cm_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Users
        $this->pdo->exec("INSERT INTO pg_cm_users VALUES (1, 'alice', 'Alice Smith')");
        $this->pdo->exec("INSERT INTO pg_cm_users VALUES (2, 'bob', 'Bob Jones')");
        $this->pdo->exec("INSERT INTO pg_cm_users VALUES (3, 'carol', 'Carol Lee')");

        // Conversations
        $this->pdo->exec("INSERT INTO pg_cm_conversations VALUES (1, 'Project Alpha', '2025-09-01')");
        $this->pdo->exec("INSERT INTO pg_cm_conversations VALUES (2, 'Lunch Plans', '2025-09-05')");

        // Messages in conversation 1
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (1, 1, 1, 'Hey team, project kickoff tomorrow!', '2025-09-01 10:00')");
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (2, 1, 2, 'Sounds good, I will prepare the slides.', '2025-09-01 10:05')");
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (3, 1, 3, 'Count me in!', '2025-09-01 10:10')");
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (4, 1, 1, 'Great, see you all at 9am.', '2025-09-01 10:15')");

        // Messages in conversation 2
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (5, 2, 2, 'Anyone up for sushi?', '2025-09-05 11:30')");
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (6, 2, 3, 'I am in!', '2025-09-05 11:35')");

        // Read receipts
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (1, 1, 2, '2025-09-01 10:02')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (2, 1, 3, '2025-09-01 10:03')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (3, 2, 1, '2025-09-01 10:06')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (4, 2, 3, '2025-09-01 10:07')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (5, 3, 1, '2025-09-01 10:11')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (6, 3, 2, '2025-09-01 10:12')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (7, 4, 2, '2025-09-01 10:16')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (8, 5, 3, '2025-09-05 11:32')");
        $this->pdo->exec("INSERT INTO pg_cm_read_receipts VALUES (9, 6, 2, '2025-09-05 11:36')");
    }
    ...
    public function testLatestMessagePerConversation(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id AS conversation_id, c.title, m.content, u.display_name AS sender
             FROM pg_cm_conversations c
             JOIN pg_cm_messages m ON m.conversation_id = c.id
                 AND m.sent_at = (
                     SELECT MAX(m2.sent_at)
                     FROM pg_cm_messages m2
                     WHERE m2.conversation_id = c.id
                 )
             JOIN pg_cm_users u ON u.id = m.sender_id
             ORDER BY c.id"
        );
    ...
    public function testUnreadCountPerUserConversation(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.conversation_id,
             ...
             FROM pg_cm_messages m
             WHERE m.sender_id != 1
               AND NOT EXISTS (
                   SELECT 1 FROM pg_cm_read_receipts rr
    ...
    public function testUserMessageStats(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.username,
             ...
             FROM pg_cm_users u
             JOIN pg_cm_messages m ON m.sender_id = u.id
             GROUP BY u.username
             ORDER BY u.username"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_cm_messages VALUES (7, 1, 2, 'Running a bit late!', '2025-09-01 10:20')");

        // ZTD sees the new message
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_cm_messages");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_cm_messages")->fetchAll(PDO::FETCH_ASSOC);
