...
/**
 * Tests INSERT with non-DDL column ordering and multi-row VALUES on PostgreSQL PDO.
 *
 ...
 * @spec SPEC-4.1
 */
class PostgresColumnOrderAndMultiRowInsertTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_coi_items (
            id INT PRIMARY KEY,
            name VARCHAR(50),
            price NUMERIC(10,2),
            category VARCHAR(30)
    ...
    {
        $this->pdo->exec("INSERT INTO pg_coi_items (category, price, name, id) VALUES ('electronics', 9.99, 'Widget', 1)");

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items WHERE id = 1');
    ...
    {
        $this->pdo->exec("INSERT INTO pg_coi_items (id, name) VALUES (2, 'Gadget')");

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items WHERE id = 2');
    ...
    {
        $this->pdo->exec("INSERT INTO pg_coi_items (category, id) VALUES ('books', 3)");

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items WHERE id = 3');
    ...
    {
        $this->pdo->exec("INSERT INTO pg_coi_items (id, name, price, category) VALUES (1, 'A', 1.00, 'cat1')");
        $this->pdo->exec("INSERT INTO pg_coi_items (category, id, name, price) VALUES ('cat2', 2, 'B', 2.00)");
        $this->pdo->exec("INSERT INTO pg_coi_items (price, category, id, name) VALUES (3.00, 'cat3', 3, 'C')");

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items ORDER BY id');
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO pg_coi_items (price, id, name) VALUES (?, ?, ?)');
        $stmt->execute([19.99, 10, 'PrepItem']);

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items WHERE id = 10');
    ...
    {
        $this->pdo->exec("INSERT INTO pg_coi_items (name, id, category) VALUES ('OldName', 5, 'cat')");
        $this->pdo->exec("UPDATE pg_coi_items SET name = 'NewName' WHERE id = 5");

        $rows = $this->ztdQuery('SELECT * FROM pg_coi_items WHERE id = 5');
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
    ...
    public function testMultiRowInsertThenUpdateOne(): void
    {
        try {
            $this->pdo->exec("INSERT INTO pg_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
            $this->pdo->exec("UPDATE pg_coi_items SET price = 99.99 WHERE id = 2");
    ...
    public function testMultiRowInsertThenDeleteOne(): void
    {
        try {
            $this->pdo->exec("INSERT INTO pg_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B'), (3, 'Gamma', 30.00, 'C')");
            $this->pdo->exec("DELETE FROM pg_coi_items WHERE id = 2");
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_coi_items (category, name, id, price) VALUES ('X', 'First', 1, 5.00), ('Y', 'Second', 2, 15.00)");
    ...
    {
        try {
            $stmt = $this->pdo->prepare('INSERT INTO pg_coi_items VALUES (?, ?, ?, ?), (?, ?, ?, ?)');
    ...
    public function testMultiRowInsertWithStringConcat(): void
    {
        try {
            $this->pdo->exec("INSERT INTO pg_coi_items VALUES (1, 'Alpha', 10.00, 'A'), (2, 'Beta', 20.00, 'B')");

            $rows = $this->ztdQuery(
                "SELECT id, name || ' (' || category || ')' AS label FROM pg_coi_items ORDER BY id"
            );
            $this->assertCount(2, $rows);
            $this->assertSame('Alpha (A)', $rows[0]['label']);
            $this->assertSame('Beta (B)', $rows[1]['label']);
        } catch (\Throwable $e) {
            $this->markTestIncomplete('Multi-row INSERT with string concat query failed: ' . $e->getMessage());
        }
    }

    public function testPhysicalIsolation(): void
    {
        $this->pdo->exec("INSERT INTO pg_coi_items VALUES (1, 'Test', 5.00, 'cat')");
        $this->pdo->disableZtd();
        $stmt = $this->pdo->query('SELECT COUNT(*) FROM pg_coi_items');
