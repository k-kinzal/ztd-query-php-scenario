 ...
 * the ZTD shadow store on PostgreSQL.
 *
 * PostgreSQL supports a WHERE clause on the DO UPDATE action, making the update
 * conditional. This is useful for optimistic-concurrency patterns (e.g. only
 * update when the incoming version is newer).
 *
 * @spec SPEC-4.2
 */
class PostgresConditionalUpsertTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_condup_products (
            id INT PRIMARY KEY,
            name VARCHAR(100),
            price DECIMAL(10,2),
     ...
     *
     * Scenario: existing version=1, incoming version=2 => version < EXCLUDED.version is true => update happens.
     *
     * @spec SPEC-4.2
     */
    public function testConditionalUpsertUpdatesWhenWhereMatches(): void
    {
        try {
            // Seed with version 1
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Widget', 10.00, 1)"
            );

            // Upsert with version 2 — WHERE condition should match (1 < 2)
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Widget', 15.00, 2)
                 ON CONFLICT (id) DO UPDATE
                 SET price = EXCLUDED.price, version = EXCLUDED.version
                 WHERE pg_condup_products.version < EXCLUDED.version"
            );

            $rows = $this->ztdQuery('SELECT id, name, price, version FROM pg_condup_products WHERE id = 1');
     ...
     *
     * Scenario: existing version=5, incoming version=2 => version < EXCLUDED.version is false => no update.
     *
     * @spec SPEC-4.2
     */
    public function testConditionalUpsertSkipsWhenWhereNotMatches(): void
    {
        try {
            // Seed with version 5
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Widget', 50.00, 5)"
            );
    ...
    public function testConditionalUpsertSelectVerify(): void
    {
        try {
            // Seed
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Alpha', 20.00, 1)"
            );

            // Conflicting upsert — version goes from 1 to 3
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Alpha', 25.00, 3)
                 ON CONFLICT (id) DO UPDATE
                 SET price = EXCLUDED.price, version = EXCLUDED.version
                 WHERE pg_condup_products.version < EXCLUDED.version"
            );
    ...
    public function testPreparedConditionalUpsert(): void
    {
        try {
            // Seed
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Beta', 30.00, 1)"
            );

            $sql = "INSERT INTO pg_condup_products (id, name, price, version) VALUES (?, ?, ?, ?)
                    ON CONFLICT (id) DO UPDATE
                    SET price = EXCLUDED.price, version = EXCLUDED.version
                    WHERE pg_condup_products.version < EXCLUDED.version";
    ...
    public function testConditionalUpsertWithExcludedInWhere(): void
    {
        try {
            ...
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Gamma', 100.00, 1)"
            );

            // Attempt upsert with lower price (50.00) — should NOT update
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Gamma', 50.00, 2)
                 ON CONFLICT (id) DO UPDATE
                 SET price = EXCLUDED.price, version = EXCLUDED.version
                 WHERE EXCLUDED.price > pg_condup_products.price"
            );

            $rows = $this->ztdQuery('SELECT id, price, version FROM pg_condup_products WHERE id = 1');
            ...
            $this->pdo->exec(
                "INSERT INTO pg_condup_products (id, name, price, version) VALUES (1, 'Gamma', 150.00, 3)
                 ON CONFLICT (id) DO UPDATE
                 SET price = EXCLUDED.price, version = EXCLUDED.version
                 WHERE EXCLUDED.price > pg_condup_products.price"
            );

            $rows2 = $this->ztdQuery('SELECT id, price, version FROM pg_condup_products WHERE id = 1');
