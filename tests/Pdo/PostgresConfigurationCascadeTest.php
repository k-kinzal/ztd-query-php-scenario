 ...
 * overrides, and environment-specific settings through ZTD shadow store (PostgreSQL PDO).
 * Covers cascade resolution, priority-based overrides, and effective config reports.
 * @spec SPEC-10.2.66
 */
class PostgresConfigurationCascadeTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_cc_defaults (
                id INTEGER PRIMARY KEY,
                config_key VARCHAR(255),
                config_value VARCHAR(255),
                description VARCHAR(255)
            )',
            'CREATE TABLE pg_cc_overrides (
                id INTEGER PRIMARY KEY,
                config_key VARCHAR(255),
    ...
    {
        return ['pg_cc_overrides', 'pg_cc_defaults'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 5 default configs
        $this->pdo->exec("INSERT INTO pg_cc_defaults VALUES (1, 'app.name', 'MyApp', 'Application name')");
        $this->pdo->exec("INSERT INTO pg_cc_defaults VALUES (2, 'app.debug', 'false', 'Debug mode flag')");
        $this->pdo->exec("INSERT INTO pg_cc_defaults VALUES (3, 'db.host', 'localhost', 'Database host')");
        $this->pdo->exec("INSERT INTO pg_cc_defaults VALUES (4, 'db.port', '3306', 'Database port')");
        $this->pdo->exec("INSERT INTO pg_cc_defaults VALUES (5, 'cache.ttl', '3600', 'Cache time-to-live in seconds')");

        // 4 overrides
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (1, 'app.debug', 'staging', 'true', 10)");
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (2, 'app.debug', 'production', 'false', 10)");
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (3, 'db.host', 'production', 'db.prod.internal', 10)");
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (4, 'cache.ttl', 'staging', '60', 5)");
    }
    ...
    public function testOverrideResolution(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
             ...
             FROM pg_cc_defaults d
             LEFT JOIN pg_cc_overrides o ON o.config_key = d.config_key AND o.environment = ?
    ...
    public function testEnvironmentSpecificConfig(): void
    {
        $staging = $this->ztdPrepareAndExecute(
            "SELECT config_value FROM pg_cc_overrides
    ...
    public function testPriorityBasedOverride(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (5, 'cache.ttl', 'staging', '30', 20)");

        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
                    COALESCE(
                        (SELECT o.config_value FROM pg_cc_overrides o
    ...
    public function testAddOverride(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_cc_overrides VALUES (5, 'db.port', 'production', '5432', 10)");
    ...
    public function testRemoveOverrideRestoresDefault(): void
    {
        ...
        $affected = $this->pdo->exec("DELETE FROM pg_cc_overrides WHERE config_key = 'cache.ttl' AND environment = 'staging'");
