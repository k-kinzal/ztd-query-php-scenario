...
/**
 * Tests a content moderation queue scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.149
 */
class PostgresContentModerationQueueTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_cmq_users (
                id SERIAL PRIMARY KEY,
                username VARCHAR(50),
                role VARCHAR(20)
            )',
            'CREATE TABLE pg_cmq_content (
                id SERIAL PRIMARY KEY,
                author_id INTEGER,
                title VARCHAR(255),
                body TEXT,
    ...
    {
        return ['pg_cmq_actions', 'pg_cmq_flags', 'pg_cmq_content', 'pg_cmq_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (1, 'alice', 'user')");
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (2, 'bob', 'user')");
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (3, 'carol', 'user')");
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (4, 'dave', 'user')");
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (5, 'emma', 'moderator')");
        $this->pdo->exec("INSERT INTO pg_cmq_users VALUES (6, 'frank', 'moderator')");

        // Content: 8 items with mixed statuses
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (1, 1, 'Travel Tips', 'Best places to visit', 'published', '2026-01-10')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (2, 2, 'Best Recipes', 'Top 10 recipes', 'flagged', '2026-01-11')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (3, 1, 'Tech Review', 'Latest gadgets', 'flagged', '2026-01-12')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (4, 3, 'Movie Night', 'Weekend picks', 'published', '2026-01-13')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (5, 4, 'Spam Post', 'Buy now!!!', 'removed', '2026-01-14')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (6, 2, 'Garden Ideas', 'Spring planting', 'approved', '2026-01-15')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (7, 3, 'Book Club', 'March reading list', 'published', '2026-01-16')");
        $this->pdo->exec("INSERT INTO pg_cmq_content VALUES (8, 4, 'DIY Crafts', 'Easy projects', 'flagged', '2026-01-17')");

        // Flags: 6 flags on various content
        // Content 2: 2 flags
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (1, 2, 3, 'spam', '2026-01-20')");
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (2, 2, 4, 'offensive', '2026-01-21')");
        // Content 3: 1 flag
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (3, 3, 2, 'misleading', '2026-01-22')");
        // Content 5: 2 flags
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (4, 5, 1, 'spam', '2026-01-23')");
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (5, 5, 3, 'harassment', '2026-01-24')");
        // Content 8: 1 flag
        $this->pdo->exec("INSERT INTO pg_cmq_flags VALUES (6, 8, 1, 'spam', '2026-01-25')");

        // Moderator actions: 4 actions
        $this->pdo->exec("INSERT INTO pg_cmq_actions VALUES (1, 2, 5, 'escalate', 'Multiple flags reported', '2026-02-01')");
        $this->pdo->exec("INSERT INTO pg_cmq_actions VALUES (2, 5, 5, 'remove', 'Confirmed spam content', '2026-02-02')");
        $this->pdo->exec("INSERT INTO pg_cmq_actions VALUES (3, 6, 6, 'approve', 'Content is appropriate', '2026-02-03')");
        $this->pdo->exec("INSERT INTO pg_cmq_actions VALUES (4, 3, 6, 'remove', 'Misleading information', '2026-02-04')");
    }
    ...
    public function testFlaggedContentWithFlagCount(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id AS content_id, c.title, COUNT(f.id) AS flag_count
             FROM pg_cmq_content c
             JOIN pg_cmq_flags f ON f.content_id = c.id
    ...
    public function testModeratorActionSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.username AS moderator, COUNT(a.id) AS action_count
             FROM pg_cmq_actions a
             JOIN pg_cmq_users u ON a.moderator_id = u.id
             GROUP BY u.username
             ORDER BY u.username"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT c.id AS content_id, c.title
             ...
             WHERE c.status = 'published'
               AND NOT EXISTS (
                   SELECT 1 FROM pg_cmq_flags f WHERE f.content_id = c.id
               )
             ORDER BY c.id"
        );
    ...
    public function testPreparedContentByModerator(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT c.id AS content_id, c.title, a.action, a.notes
             FROM pg_cmq_actions a
             JOIN pg_cmq_content c ON a.content_id = c.id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec(
            "INSERT INTO pg_cmq_flags VALUES (7, 7, 4, 'offensive', '2026-02-10')"
        );

        // ZTD sees the new flag (6 original + 1 new = 7)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_cmq_flags");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_cmq_flags')->fetchAll(PDO::FETCH_ASSOC);
