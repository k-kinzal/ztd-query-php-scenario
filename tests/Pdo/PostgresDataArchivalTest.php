...
/**
 * Tests a data archival workflow through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.104
 */
class PostgresDataArchivalTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_da_orders (
            ...
            'CREATE TABLE pg_da_archived_orders (
                id INTEGER PRIMARY KEY,
                customer VARCHAR(255),
                product VARCHAR(255),
                amount NUMERIC(10,2),
    ...
    {
        return ['pg_da_archived_orders', 'pg_da_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_da_orders VALUES (1, 'Alice', 'Widget', 29.99, 'completed', '2025-11-01', '2025-11-05')");
        $this->pdo->exec("INSERT INTO pg_da_orders VALUES (2, 'Bob', 'Gadget', 49.99, 'completed', '2025-12-01', '2025-12-10')");
        $this->pdo->exec("INSERT INTO pg_da_orders VALUES (3, 'Charlie', 'Widget', 29.99, 'completed', '2026-01-15', '2026-01-20')");
        $this->pdo->exec("INSERT INTO pg_da_orders VALUES (4, 'Alice', 'Gizmo', 99.99, 'pending', '2026-03-01', NULL)");
        $this->pdo->exec("INSERT INTO pg_da_orders VALUES (5, 'Diana', 'Widget', 29.99, 'shipped', '2026-03-05', NULL)");
    }

    /**
     * Archive old completed orders using INSERT SELECT.
     * On PostgreSQL, the INSERT SELECT computed column issue does NOT apply,
     * so archived_at should contain the literal value.
     */
    public function testArchiveCompletedOrders(): void
    {
        // Archive orders completed before 2026-01-01
        $affected = $this->pdo->exec(
            "INSERT INTO pg_da_archived_orders (id, customer, product, amount, status, created_at, completed_at, archived_at)
        ...
        $rows = $this->ztdQuery("SELECT id, customer, archived_at FROM pg_da_archived_orders ORDER BY id");
        $this->assertCount(2, $rows);
        $this->assertSame('Alice', $rows[0]['customer']);
        // On PostgreSQL, literal value in INSERT SELECT is preserved
        $this->assertSame('2026-03-09', substr($rows[0]['archived_at'], 0, 10));
    ...
    public function testDeleteAfterArchival(): void
    {
        ...
        $affected = $this->pdo->exec(
            "DELETE FROM pg_da_orders WHERE status = 'completed' AND completed_at < '2026-01-01'"
        );
        $this->assertSame(2, $affected);

        // Active table should have 3 remaining orders
        $rows = $this->ztdQuery("SELECT id FROM pg_da_orders ORDER BY id");
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_da_archived_orders");
        $this->assertEquals(2, (int) $rows[0]['cnt']);
    }

    /**
     * Total revenue across active and archived tables using UNION ALL in derived table.
     * On PostgreSQL, UNION ALL in a derived table works (unlike SQLite).
     */
    public function testTotalRevenueAcrossTables(): void
    {
        ...
        // UNION ALL in derived table works on PostgreSQL
        $rows = $this->ztdQuery(
            "SELECT COALESCE(SUM(amount), 0) AS total_revenue
             FROM (
                SELECT amount FROM pg_da_orders
                UNION ALL
                SELECT amount FROM pg_da_archived_orders
             ) AS combined"
        );
    ...
    public function testArchiveSummaryByProduct(): void
    {
        ...
        $this->pdo->exec("DELETE FROM pg_da_orders WHERE status = 'completed'");

        // Verify active table only has non-completed orders
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_da_orders");
        $this->assertEquals(2, (int) $rows[0]['cnt']);
    ...
    public function testCustomerHistoryAcrossTables(): void
    {
        ...
        $this->pdo->exec("DELETE FROM pg_da_orders WHERE status = 'completed' AND completed_at < '2026-01-01'");

        // Alice's history from both tables
        $rows = $this->ztdPrepareAndExecute(
            "SELECT id, product, amount, status, 'active' AS source
             FROM pg_da_orders WHERE customer = ?
             UNION ALL
             SELECT id, product, amount, status, 'archived' AS source
             FROM pg_da_archived_orders WHERE customer = ?
             ORDER BY id",
            ['Alice', 'Alice']
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_da_archived_orders")->fetchAll(PDO::FETCH_ASSOC);
