 ...
 * @spec SPEC-3.3
 */
class PostgresDerivedTableAndNestingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE pg_dtn_products (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                price NUMERIC(10,2) NOT NULL,
                category VARCHAR(30) NOT NULL
            )",
            "CREATE TABLE pg_dtn_sales (
                id INT PRIMARY KEY,
                product_id INT NOT NULL,
                qty INT NOT NULL,
                sale_date DATE NOT NULL
            )",
            "CREATE TABLE pg_dtn_regions (
                id INT PRIMARY KEY,
    ...
    {
        return ['pg_dtn_sales', 'pg_dtn_regions', 'pg_dtn_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_dtn_products (id, name, price, category) VALUES
            (1, 'Widget', 10.00, 'basic'),
            (2, 'Gadget', 45.00, 'advanced'),
            (3, 'Gizmo', 200.00, 'premium'),
            (4, 'Doohickey', 30.00, 'basic')");
        $this->pdo->exec("INSERT INTO pg_dtn_regions (id, name) VALUES
            (1, 'East'), (2, 'West'), (3, 'Central')");
        $this->pdo->exec("INSERT INTO pg_dtn_sales (id, product_id, qty, sale_date) VALUES
            ...
            $rows = $this->ztdQuery(
                "SELECT sub.name, sub.total
                 FROM (
                     ...
                     FROM pg_dtn_products p
                     JOIN pg_dtn_sales s ON s.product_id = p.id
    ...
    public function testDerivedTableWithWhere(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT * FROM (
                     SELECT name, price FROM pg_dtn_products WHERE price > 20
                 ) sub
                 WHERE sub.price < 100
                 ORDER BY sub.name"
            );
    ...
    public function testNestedSubqueryTwoLevels(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT name FROM pg_dtn_products
                 WHERE id IN (
                     SELECT product_id FROM pg_dtn_sales
                     WHERE qty > (SELECT AVG(qty) FROM pg_dtn_sales)
                 )
                 ORDER BY name"
            );
    ...
    public function testNestedSubqueryThreeLevels(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT name FROM pg_dtn_products
                 WHERE category IN (
                     SELECT category FROM pg_dtn_products
                     WHERE id IN (
                         SELECT product_id FROM pg_dtn_sales WHERE qty >= 5
                     )
                 )
                 ORDER BY name"
            );
    ...
    public function testMultipleSubqueriesInWhere(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT name, price FROM pg_dtn_products
                 WHERE price > (SELECT MIN(price) FROM pg_dtn_products)
                   AND price < (SELECT MAX(price) FROM pg_dtn_products)
                 ORDER BY price"
            );
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_dtn_sales (id, product_id, qty, sale_date) VALUES (7, 3, 20, '2025-04-01')");
    ...
    {
        $this->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_dtn_products")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical products table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_dtn_sales")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical sales table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_dtn_regions")
            ->fetchAll(PDO::FETCH_ASSOC);
