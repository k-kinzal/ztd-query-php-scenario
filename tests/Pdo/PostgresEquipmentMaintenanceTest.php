...
/**
 * Tests an equipment maintenance scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * platform-specific date arithmetic (date subtraction), LEFT JOIN with SUM,
 * correlated MAX subquery, ROUND(AVG), prepared statement with JOIN,
 * and physical isolation check.
 * @spec SPEC-10.2.158
 */
class PostgresEquipmentMaintenanceTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_em_equipment (
            ...
            'CREATE TABLE pg_em_technicians (
                id SERIAL PRIMARY KEY,
                name VARCHAR(100),
                specialty VARCHAR(50)
            )',
            'CREATE TABLE pg_em_service_records (
                id SERIAL PRIMARY KEY,
    ...
    {
        return ['pg_em_service_records', 'pg_em_technicians', 'pg_em_equipment'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (1, 'Rooftop HVAC Unit', 'HVAC', 'Building A', '2025-11-15', 90)");
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (2, 'Main Electrical Panel', 'electrical', 'Building A', '2026-01-10', 180)");
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (3, 'Lobby HVAC Unit', 'HVAC', 'Building B', '2025-08-20', 90)");
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (4, 'Water Heater', 'plumbing', 'Building C', '2026-02-28', 365)");
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (5, 'Emergency Generator', 'electrical', 'Building A', '2025-06-01', 180)");

        // 3 technicians
        $this->pdo->exec("INSERT INTO pg_em_technicians VALUES (1, 'Marco', 'HVAC')");
        $this->pdo->exec("INSERT INTO pg_em_technicians VALUES (2, 'Priya', 'electrical')");
        $this->pdo->exec("INSERT INTO pg_em_technicians VALUES (3, 'Tom', 'plumbing')");

        // 7 service records
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (1, 1, 1, 'preventive', '2025-11-15', 350.00, 'Filter replacement')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (2, 2, 2, 'inspection', '2026-01-10', 150.00, 'Annual safety check')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (3, 3, 1, 'repair', '2025-08-20', 820.00, 'Compressor replacement')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (4, 5, 2, 'preventive', '2025-06-01', 500.00, 'Load test and oil change')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (5, 1, 1, 'preventive', '2025-08-10', 280.00, 'Coil cleaning')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (6, 4, 3, 'inspection', '2026-02-28', 120.00, 'Pressure valve check')");
        $this->pdo->exec("INSERT INTO pg_em_service_records VALUES (7, 5, 2, 'repair', '2025-09-15', 1200.00, 'Starter motor replacement')");
    }
    ...
    /**
     * Overdue equipment detection using PostgreSQL date subtraction.
    ...
    public function testOverdueEquipment(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name, e.category, e.last_service_date, e.service_interval_days
             FROM pg_em_equipment e
             WHERE ('2026-03-09'::date - e.last_service_date::date) > e.service_interval_days
             ORDER BY e.name"
        );
    ...
    public function testTechnicianWorkload(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name, COUNT(sr.id) AS total_jobs, SUM(sr.cost) AS cost_total
             FROM pg_em_technicians t
             LEFT JOIN pg_em_service_records sr ON sr.technician_id = t.id
    ...
    public function testMostRecentServicePerEquipment(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name, sr.service_date, sr.service_type
             FROM pg_em_equipment e
             JOIN pg_em_service_records sr ON sr.equipment_id = e.id
             WHERE sr.service_date = (
                 SELECT MAX(sr2.service_date)
                 FROM pg_em_service_records sr2
    ...
    public function testCostBreakdownByCategory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.category, SUM(sr.cost) AS total_cost, ROUND(AVG(sr.cost), 2) AS avg_cost
             FROM pg_em_service_records sr
             JOIN pg_em_equipment e ON e.id = sr.equipment_id
             GROUP BY e.category
             ORDER BY e.category"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_em_equipment VALUES (6, 'Backup AC Unit', 'HVAC', 'Building D', '2026-03-01', 90)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_em_equipment");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_em_equipment")->fetchAll(PDO::FETCH_ASSOC);
