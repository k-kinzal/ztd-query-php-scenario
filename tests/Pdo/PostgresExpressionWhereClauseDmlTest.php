...
/**
 * Tests DELETE/UPDATE with complex expression WHERE clauses via PostgreSQL PDO.
 *
 ...
 */
class PostgresExpressionWhereClauseDmlTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_ewd_test (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ewd_test VALUES (1, 'alice', 85, 'A')");
        $this->pdo->exec("INSERT INTO pg_ewd_test VALUES (2, 'BOB', 60, 'B')");
        $this->pdo->exec("INSERT INTO pg_ewd_test VALUES (3, 'Charlie', 95, 'A')");
        $this->pdo->exec("INSERT INTO pg_ewd_test VALUES (4, 'dave', 45, 'C')");
        $this->pdo->exec("INSERT INTO pg_ewd_test VALUES (5, 'Eve', 70, 'B')");
    }
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_ewd_test WHERE LENGTH(name) > 4");

            $rows = $this->pdo->query("SELECT id, name FROM pg_ewd_test ORDER BY id")
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_ewd_test WHERE CASE WHEN score > 80 THEN 1 ELSE 0 END = 1");

            $rows = $this->pdo->query("SELECT id FROM pg_ewd_test ORDER BY id")
                ->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_ewd_test SET score = 0 WHERE CASE WHEN score > 80 THEN 1 ELSE 0 END = 1");

            $rows = $this->pdo->query("SELECT id, score FROM pg_ewd_test ORDER BY id")
                ->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_ewd_test WHERE LOWER(name) = 'bob'");
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_ewd_test SET category = 'X' WHERE LENGTH(name) <= 3");

            $rows = $this->pdo->query("SELECT id, category FROM pg_ewd_test ORDER BY id")
                ->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_ewd_test SET score = score + 10 WHERE score * 2 > 150");
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_ewd_test WHERE ABS(score - 70) <= 15");
    ...
    /**
     * UPDATE with || string concatenation in WHERE clause (PostgreSQL syntax).
     *
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_ewd_test SET score = 100 WHERE name || category LIKE '%A'");
     ...
     *
     * DELETE FROM pg_ewd_test WHERE score > ? AND LENGTH(name) > ?
    ...
    {
        try {
            $stmt = $this->pdo->prepare('DELETE FROM pg_ewd_test WHERE score > ? AND LENGTH(name) > ?');
            $stmt->execute([60, 3]);
    ...
    {
        $this->pdo->exec("UPDATE pg_ewd_test SET score = 999 WHERE LENGTH(name) > 3");

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_ewd_test")->fetchAll(PDO::FETCH_ASSOC);
