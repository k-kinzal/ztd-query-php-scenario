...
/**
 * Tests a double-entry financial ledger through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.71
 */
class PostgresFinancialLedgerTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_fl_accounts (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255),
                account_type VARCHAR(50)
            )',
            'CREATE TABLE pg_fl_entries (
                id INTEGER PRIMARY KEY,
                account_id INTEGER,
    ...
    {
        return ['pg_fl_entries', 'pg_fl_accounts'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 accounts
        $this->pdo->exec("INSERT INTO pg_fl_accounts VALUES (1, 'Operating', 'asset')");
        $this->pdo->exec("INSERT INTO pg_fl_accounts VALUES (2, 'Revenue', 'income')");
        $this->pdo->exec("INSERT INTO pg_fl_accounts VALUES (3, 'Expenses', 'expense')");

        // Transaction TXN-001: Revenue received ($500 debit to Operating, credit to Revenue)
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (1, 1, 'TXN-001', 'debit', 500.00, 'Client payment', '2026-01-15 09:00:00')");
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (2, 2, 'TXN-001', 'credit', 500.00, 'Client payment', '2026-01-15 09:00:00')");

        // Transaction TXN-002: Expense paid ($150 credit from Operating, debit to Expenses)
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (3, 1, 'TXN-002', 'credit', 150.00, 'Office supplies', '2026-01-20 14:00:00')");
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (4, 3, 'TXN-002', 'debit', 150.00, 'Office supplies', '2026-01-20 14:00:00')");

        // Transaction TXN-003: Another revenue ($300)
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (5, 1, 'TXN-003', 'debit', 300.00, 'Consulting fee', '2026-02-01 10:00:00')");
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (6, 2, 'TXN-003', 'credit', 300.00, 'Consulting fee', '2026-02-01 10:00:00')");
    }
    ...
    public function testAccountBalances(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.id, a.name, a.account_type,
             ...
             FROM pg_fl_accounts a
             LEFT JOIN pg_fl_entries e ON e.account_id = a.id
    ...
    public function testRecordTransaction(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (7, 1, 'TXN-004', 'credit', 75.50, 'Internet bill', '2026-02-10 09:00:00')");
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (8, 3, 'TXN-004', 'debit', 75.50, 'Internet bill', '2026-02-10 09:00:00')");
    ...
    public function testReversalEntry(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (7, 1, 'TXN-002-REV', 'debit', 150.00, 'Reverse: Office supplies', '2026-02-15 11:00:00')");
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (8, 3, 'TXN-002-REV', 'credit', 150.00, 'Reverse: Office supplies', '2026-02-15 11:00:00')");
    ...
    {
        $this->pdo->exec("INSERT INTO pg_fl_entries VALUES (7, 1, 'TXN-005', 'debit', 999.99, 'Test entry', '2026-03-01 09:00:00')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_fl_entries");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_fl_entries")->fetchAll(PDO::FETCH_ASSOC);
