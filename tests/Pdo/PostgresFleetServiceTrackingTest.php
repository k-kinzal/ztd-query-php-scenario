...
/**
 * Tests a fleet service tracking scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.130
 */
class PostgresFleetServiceTrackingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_fs_vehicles (
            ...
            'CREATE TABLE pg_fs_service_records (
                id SERIAL PRIMARY KEY,
                vehicle_id INTEGER,
                service_date TEXT,
                service_type TEXT,
    ...
    {
        return ['pg_fs_service_records', 'pg_fs_vehicles'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Vehicles
        $this->pdo->exec("INSERT INTO pg_fs_vehicles VALUES (1, 'ABC-001', 'Toyota', 'Camry', 2022, 'active', 6)");
        $this->pdo->exec("INSERT INTO pg_fs_vehicles VALUES (2, 'DEF-002', 'Honda', 'Civic', 2021, 'active', 6)");
        $this->pdo->exec("INSERT INTO pg_fs_vehicles VALUES (3, 'GHI-003', 'Ford', 'F-150', 2020, 'active', 3)");
        $this->pdo->exec("INSERT INTO pg_fs_vehicles VALUES (4, 'JKL-004', 'Tesla', 'Model 3', 2023, 'inactive', 12)");
        $this->pdo->exec("INSERT INTO pg_fs_vehicles VALUES (5, 'MNO-005', 'Chevy', 'Malibu', 2019, 'active', 6)");

        // Vehicle 1 service records (regular maintenance)
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (1, 1, '2025-01-15', 'oil_change', 75.00, 15000, 'Regular service')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (2, 1, '2025-04-20', 'oil_change', 80.00, 18000, 'Regular service')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (3, 1, '2025-07-10', 'oil_change', 75.00, 21000, 'Regular service')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (4, 1, '2025-07-10', 'tire_rotation', 50.00, 21000, 'With oil change')");

        // Vehicle 2 service records (less frequent)
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (5, 2, '2025-02-01', 'oil_change', 70.00, 22000, 'Regular service')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (6, 2, '2025-06-15', 'brake_service', 350.00, 25000, 'Front brakes replaced')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (7, 2, '2025-08-20', 'oil_change', 75.00, 27000, 'Regular service')");

        // Vehicle 3 service records (expensive, frequent)
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (8, 3, '2025-01-05', 'oil_change', 95.00, 45000, 'Synthetic oil')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (9, 3, '2025-03-10', 'oil_change', 95.00, 48000, 'Synthetic oil')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (10, 3, '2025-05-15', 'transmission', 1200.00, 51000, 'Transmission flush')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (11, 3, '2025-06-20', 'oil_change', 95.00, 53000, 'Synthetic oil')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (12, 3, '2025-09-01', 'oil_change', 95.00, 56000, 'Synthetic oil')");

        // Vehicle 4 service records (inactive, only 1 record)
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (13, 4, '2025-03-01', 'inspection', 150.00, 5000, 'Annual inspection')");

        // Vehicle 5 service records (overdue)
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (14, 5, '2024-06-10', 'oil_change', 65.00, 60000, 'Regular service')");
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (15, 5, '2024-12-15', 'oil_change', 70.00, 63000, 'Regular service')");
    }
    ...
    public function testLastServicePerVehicle(): void
    {
        $rows = $this->ztdQuery(
            "SELECT v.plate, v.make, v.model,
             ...
             FROM pg_fs_vehicles v
             JOIN pg_fs_service_records sr ON sr.vehicle_id = v.id
    ...
    {
        $this->pdo->exec("INSERT INTO pg_fs_service_records VALUES (16, 1, '2025-10-01', 'oil_change', 80.00, 24000, 'New service')");
        $this->pdo->exec("UPDATE pg_fs_vehicles SET status = 'maintenance' WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_fs_service_records");
        $this->assertEquals(16, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_fs_service_records')->fetchAll(PDO::FETCH_ASSOC);
