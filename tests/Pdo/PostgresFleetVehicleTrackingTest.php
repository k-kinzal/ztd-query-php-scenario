...
/**
 * Tests fleet vehicle tracking with trip logging and mileage accrual (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.170
 */
class PostgresFleetVehicleTrackingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_fvt_vehicle_type (
                id SERIAL PRIMARY KEY,
                name VARCHAR(50),
                fuel_type VARCHAR(20),
                capacity INTEGER
            )',
            'CREATE TABLE pg_fvt_vehicle (
                id SERIAL PRIMARY KEY,
                type_id INTEGER,
                plate VARCHAR(20),
            ...
            'CREATE TABLE pg_fvt_vehicle_trip (
                id SERIAL PRIMARY KEY,
                vehicle_id INTEGER,
                trip_date TEXT,
    ...
    {
        return ['pg_fvt_vehicle_trip', 'pg_fvt_vehicle', 'pg_fvt_vehicle_type'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Vehicle types
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_type VALUES (1, 'Sedan', 'gasoline', 4)");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_type VALUES (2, 'Van', 'diesel', 8)");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_type VALUES (3, 'Truck', 'diesel', 2)");

        // Vehicles
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle VALUES (1, 1, 'ABC-1234', 15000.0, 'active')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle VALUES (2, 1, 'DEF-5678', 42000.0, 'active')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle VALUES (3, 2, 'GHI-9012', 8500.0, 'active')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle VALUES (4, 3, 'JKL-3456', 62000.0, 'maintenance')");

        // Trips
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (1, 1, '2025-09-01', 120.5, 'Alice', 'Warehouse A')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (2, 1, '2025-09-03', 85.0, 'Alice', 'Client B')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (3, 2, '2025-09-01', 200.0, 'Bob', 'Depot C')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (4, 3, '2025-09-02', 310.0, 'Carol', 'Distribution Hub')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (5, 3, '2025-09-04', 150.0, 'Carol', 'Client D')");
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (6, 1, '2025-09-05', 95.0, 'Dave', 'Airport')");
    }
    ...
    public function testThreeTableJoinWithOverlappingNames(): void
    {
        $rows = $this->ztdQuery(
            "SELECT v.plate, vt.name AS type_name, t.trip_date, t.distance, t.driver
             FROM pg_fvt_vehicle v
             JOIN pg_fvt_vehicle_type vt ON vt.id = v.type_id
             JOIN pg_fvt_vehicle_trip t ON t.vehicle_id = v.id
             ORDER BY t.id"
        );
    ...
    public function testTotalDistancePerVehicle(): void
    {
        $rows = $this->ztdQuery(
            "SELECT v.plate,
             ...
             FROM pg_fvt_vehicle v
             LEFT JOIN pg_fvt_vehicle_trip t ON t.vehicle_id = v.id
    ...
    public function testFleetSummaryByType(): void
    {
        $rows = $this->ztdQuery(
            "SELECT vt.name AS type_name,
                    ...
                    COALESCE(SUM(t.distance), 0) AS total_distance
             FROM pg_fvt_vehicle_type vt
             LEFT JOIN pg_fvt_vehicle v ON v.type_id = vt.id
             LEFT JOIN pg_fvt_vehicle_trip t ON t.vehicle_id = v.id
    ...
    {
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle_trip VALUES (7, 1, '2025-09-06', 50.0, 'Alice', 'Office')");

        $this->ztdExec("UPDATE pg_fvt_vehicle SET mileage = mileage + 50.0 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT mileage FROM pg_fvt_vehicle WHERE id = 1");
    ...
    {
        $this->ztdExec("UPDATE pg_fvt_vehicle SET mileage = mileage + 200.0 WHERE id = 2");
        $this->ztdExec("UPDATE pg_fvt_vehicle SET mileage = mileage + 85.0 WHERE id = 2");

        $rows = $this->ztdQuery("SELECT mileage FROM pg_fvt_vehicle WHERE id = 2");
        $this->assertCount(1, $rows);
    ...
    public function testPreparedTripsByDateRange(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT t.trip_date, t.driver, t.destination, v.plate
             FROM pg_fvt_vehicle_trip t
             JOIN pg_fvt_vehicle v ON v.id = t.vehicle_id
    ...
    {
        $this->pdo->exec("INSERT INTO pg_fvt_vehicle VALUES (5, 2, 'MNO-7890', 0.0, 'new')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_fvt_vehicle");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_fvt_vehicle")->fetchAll(PDO::FETCH_ASSOC);
