...
/**
 * Tests a hotel room management scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.159
 */
class PostgresHotelRoomManagementTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_hr_rooms (
            ...
            'CREATE TABLE pg_hr_guests (
                id SERIAL PRIMARY KEY,
                name VARCHAR(255),
                email VARCHAR(255),
                vip_level VARCHAR(255)
            )',
            'CREATE TABLE pg_hr_stays (
                id SERIAL PRIMARY KEY,
                room_id INTEGER,
                guest_id INTEGER,
                arrival_date TEXT,
    ...
    {
        return ['pg_hr_stays', 'pg_hr_guests', 'pg_hr_rooms'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 6 rooms
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (1, '101', 'single', 1, 89.00, 'available')");
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (2, '102', 'double', 1, 149.00, 'occupied')");
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (3, '201', 'double', 2, 149.00, 'available')");
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (4, '202', 'suite', 2, 299.00, 'occupied')");
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (5, '203', 'single', 2, 89.00, 'maintenance')");
        $this->pdo->exec("INSERT INTO pg_hr_rooms VALUES (6, '301', 'suite', 3, 349.00, 'available')");

        // 4 guests
        $this->pdo->exec("INSERT INTO pg_hr_guests VALUES (1, 'Alice Chen', 'alice@example.com', 'gold')");
        $this->pdo->exec("INSERT INTO pg_hr_guests VALUES (2, 'Bob Kim', 'bob@example.com', 'silver')");
        $this->pdo->exec("INSERT INTO pg_hr_guests VALUES (3, 'Carol Davis', 'carol@example.com', 'none')");
        $this->pdo->exec("INSERT INTO pg_hr_guests VALUES (4, 'Dan Wilson', 'dan@example.com', 'gold')");

        // 6 stays
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (1, 2, 1, '2026-02-20', '2026-02-23', 447.00, 5)");
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (2, 4, 4, '2026-02-25', '2026-03-01', 1196.00, 4)");
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (3, 1, 3, '2026-01-10', '2026-01-12', 178.00, 3)");
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (4, 3, 2, '2026-01-15', '2026-01-18', 447.00, 4)");
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (5, 6, 1, '2025-12-24', '2025-12-31', 2443.00, 5)");
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (6, 2, 2, '2026-03-05', '2026-03-10', 745.00, NULL)");
    }
    ...
    public function testRevenueByRoomType(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.room_type,
             ...
             FROM pg_hr_rooms r
             JOIN pg_hr_stays s ON s.room_id = r.id
             GROUP BY r.room_type
             ORDER BY r.room_type"
        );
    ...
    public function testGuestStayHistory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT g.name, COUNT(s.id) AS stay_count, SUM(s.total_charge) AS total_spent
             FROM pg_hr_guests g
             LEFT JOIN pg_hr_stays s ON s.guest_id = g.id
    ...
    public function testHighRatedStays(): void
    {
        $rows = $this->ztdQuery(
            "SELECT g.name, r.room_number, r.room_type, s.rating, s.arrival_date
             FROM pg_hr_stays s
             JOIN pg_hr_rooms r ON r.id = s.room_id
             JOIN pg_hr_guests g ON g.id = s.guest_id
             WHERE s.rating >= 4
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_hr_stays VALUES (7, 1, 3, '2026-04-01', '2026-04-03', 178.00, NULL)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_hr_stays");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_hr_stays")->fetchAll(PDO::FETCH_ASSOC);
