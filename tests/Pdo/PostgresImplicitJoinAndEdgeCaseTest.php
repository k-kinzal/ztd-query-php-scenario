...
/**
 * Implicit comma JOIN and edge-case SQL patterns via PostgreSQL PDO.
 *
 ...
 * @spec SPEC-3.3
 */
class PostgresImplicitJoinAndEdgeCaseTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE pg_ije_customers (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                city VARCHAR(30) NOT NULL
            )",
            "CREATE TABLE pg_ije_orders (
                id INT PRIMARY KEY,
    ...
    {
        return ['pg_ije_orders', 'pg_ije_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ije_customers (id, name, city) VALUES (1, 'Alice', 'NYC'), (2, 'Bob', 'LA'), (3, 'Carol', 'Chicago')");
        $this->pdo->exec("INSERT INTO pg_ije_orders (id, customer_id, amount, status) VALUES
    ...
    public function testImplicitCommaJoin(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name, o.amount FROM pg_ije_customers c, pg_ije_orders o WHERE c.id = o.customer_id ORDER BY o.id"
            );
    ...
    public function testImplicitJoinWithAggregate(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name, SUM(o.amount) AS total FROM pg_ije_customers c, pg_ije_orders o WHERE c.id = o.customer_id GROUP BY c.id, c.name ORDER BY total DESC"
            );
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_ije_orders (id, customer_id, amount, status) VALUES (6, 3, 125.00, 'completed')");

            $rows = $this->ztdQuery(
                "SELECT c.name, o.amount FROM pg_ije_customers c, pg_ije_orders o WHERE c.id = o.customer_id AND c.name = 'Carol' ORDER BY o.id"
            );
        ...
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name,
                 ...
                 FROM pg_ije_customers c
                 JOIN pg_ije_orders o ON c.id = o.customer_id
    ...
    public function testBetweenWithShadowData(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT status, amount FROM pg_ije_orders WHERE amount BETWEEN 50 AND 200 ORDER BY amount"
            );
    ...
    public function testExistsAndNotExists(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT name FROM pg_ije_customers c WHERE EXISTS (SELECT 1 FROM pg_ije_orders o WHERE o.customer_id = c.id AND o.status = 'completed') ORDER BY name"
            );
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_ije_orders WHERE customer_id = 1");

            $rows = $this->ztdQuery(
                "SELECT name FROM pg_ije_customers c WHERE NOT EXISTS (SELECT 1 FROM pg_ije_orders o WHERE o.customer_id = c.id)"
            );
    ...
    {
        $this->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_ije_customers")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical customers table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_ije_orders")
            ->fetchAll(PDO::FETCH_ASSOC);
