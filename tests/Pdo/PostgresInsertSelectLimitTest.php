...
/**
 * Tests INSERT...SELECT with ORDER BY and LIMIT on PostgreSQL via PDO.
 *
 ...
 * @spec SPEC-4.1
 */
class PostgresInsertSelectLimitTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_isl_scores (
                id INT PRIMARY KEY,
                player VARCHAR(30),
                score INT,
                created_at TIMESTAMP
            )',
            'CREATE TABLE pg_isl_leaderboard (
    ...
    {
        return ['pg_isl_leaderboard', 'pg_isl_scores'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_isl_scores (id, player, score, created_at) VALUES
    ...
    public function testInsertSelectWithOrderByLimit(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_isl_leaderboard (player, score)
                 SELECT player, score FROM pg_isl_scores ORDER BY score DESC LIMIT 3"
            );

            $rows = $this->ztdQuery("SELECT player, score FROM pg_isl_leaderboard ORDER BY score DESC");
    ...
    public function testInsertSelectWithWhereAndLimit(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_isl_leaderboard (player, score)
                 SELECT player, score FROM pg_isl_scores
                 WHERE score >= 50
    ...
    {
        $this->pdo->exec("INSERT INTO pg_isl_scores (id, player, score, created_at) VALUES (7, 'Grace', 99, '2025-01-02 10:00:00')");
    ...
    public function testInsertSelectWithOffset(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_isl_leaderboard (player, score)
                 SELECT player, score FROM pg_isl_scores ORDER BY score DESC LIMIT 3 OFFSET 1"
            );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_isl_leaderboard")
