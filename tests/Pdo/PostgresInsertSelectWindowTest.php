...
/**
 * Tests INSERT...SELECT with window functions on PostgreSQL PDO.
 *
 ...
 * @spec SPEC-10.2.369
 */
class PostgresInsertSelectWindowTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE pg_isw_employees (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                salary NUMERIC(10,2) NOT NULL,
                dept VARCHAR(30) NOT NULL
            )",
            "CREATE TABLE pg_isw_ranked (
                id INT PRIMARY KEY,
                name VARCHAR(50),
                salary NUMERIC(10,2),
                dept VARCHAR(30),
    ...
    {
        return ['pg_isw_ranked', 'pg_isw_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (1, 'Alice', 90000, 'Engineering')");
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (2, 'Bob', 80000, 'Engineering')");
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (3, 'Carol', 95000, 'Engineering')");
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (4, 'Dave', 70000, 'Sales')");
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (5, 'Eve', 75000, 'Sales')");
        $this->pdo->exec("INSERT INTO pg_isw_employees (id, name, salary, dept) VALUES (6, 'Frank', 85000, 'Marketing')");
    }
    ...
    public function testInsertSelectRowNumberPartitioned(): void
    {
        try {
            $affected = $this->pdo->exec("
                INSERT INTO pg_isw_ranked (id, name, salary, dept, rank_in_dept)
                SELECT id, name, salary, dept,
                       ROW_NUMBER() OVER (PARTITION BY dept ORDER BY salary DESC)
                FROM pg_isw_employees
            ");
            $this->assertSame(6, $affected);

            $rows = $this->ztdQuery("SELECT * FROM pg_isw_ranked ORDER BY dept, rank_in_dept");
    ...
    public function testInsertSelectRankOverSalary(): void
    {
        try {
            // Add a tie in salary
            $this->pdo->exec("UPDATE pg_isw_employees SET salary = 80000 WHERE id = 4");

            $this->pdo->exec("DELETE FROM pg_isw_ranked");
            ...
            $rows = $this->ztdQuery("SELECT * FROM pg_isw_ranked ORDER BY rank_in_dept, name");
            $this->assertCount(6, $rows);
    ...
    public function testInsertSelectRunningTotalSumOver(): void
    {
        try {
            ...
            $affected = $this->pdo->exec("
                INSERT INTO pg_isw_ranked (id, name, salary, dept, rank_in_dept)
                SELECT id, name, salary, dept,
                       CAST(SUM(salary) OVER (PARTITION BY dept ORDER BY id) AS INT)
                FROM pg_isw_employees
            ");
            $this->assertSame(6, $affected);

            $rows = $this->ztdQuery("SELECT * FROM pg_isw_ranked WHERE dept = 'Engineering' ORDER BY id");
    ...
    public function testTargetRowCountMatchesSource(): void
    {
        try {
            ...
            $sourceCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_isw_employees");
            $targetCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_isw_ranked");
    ...
    public function testInsertSelectWindowAfterSourceUpdate(): void
    {
        try {
            ...
            $this->pdo->exec("UPDATE pg_isw_employees SET salary = 120000 WHERE name = 'Carol'");
            // Demote Bob
            $this->pdo->exec("UPDATE pg_isw_employees SET salary = 60000 WHERE name = 'Bob'");
            ...
            $rows = $this->ztdQuery("SELECT * FROM pg_isw_ranked WHERE dept = 'Engineering' ORDER BY rank_in_dept");
    ...
    /**
     * INSERT...SELECT with window using prepared statement ($1 placeholder).
     *
    ...
    public function testInsertSelectWindowPreparedWithParam(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT * FROM pg_isw_ranked ORDER BY rank_in_dept");
            $this->assertCount(3, $rows);
            $this->assertSame('Carol', $rows[0]['name']);
            $this->assertSame(1, (int) $rows[0]['rank_in_dept']);
        } catch (\Throwable $e) {
