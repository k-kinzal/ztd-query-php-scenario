...
/**
 * Tests INSERT with scalar subquery in VALUES position on PostgreSQL PDO.
 *
 ...
 * @spec SPEC-4.1
 */
class PostgresInsertValuesSubqueryTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ivs_source (id INT PRIMARY KEY, category VARCHAR(50), amount NUMERIC(10,2))',
            'CREATE TABLE pg_ivs_target (id INT PRIMARY KEY, total_count INT, total_amount NUMERIC(10,2))',
        ];
    }

    protected function getTableNames(): array
    {
        return ['pg_ivs_target', 'pg_ivs_source'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ivs_source VALUES (1, 'A', 100.00)");
        $this->pdo->exec("INSERT INTO pg_ivs_source VALUES (2, 'A', 200.00)");
        $this->pdo->exec("INSERT INTO pg_ivs_source VALUES (3, 'B', 150.00)");
    }
    ...
    public function testInsertWithCountSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_ivs_target (id, total_count, total_amount) VALUES (1, (SELECT COUNT(*) FROM pg_ivs_source), 0)"
            );

            $rows = $this->pdo->query('SELECT total_count FROM pg_ivs_target WHERE id = 1')->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testInsertWithSumSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_ivs_target (id, total_count, total_amount) VALUES (1, 0, (SELECT SUM(amount) FROM pg_ivs_source))"
            );

            $rows = $this->pdo->query('SELECT total_amount FROM pg_ivs_target WHERE id = 1')->fetchAll(PDO::FETCH_ASSOC);
            $this->assertCount(1, $rows);
    ...
    public function testInsertWithFilteredSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_ivs_target (id, total_count, total_amount) VALUES (1, (SELECT COUNT(*) FROM pg_ivs_source WHERE category = 'A'), (SELECT SUM(amount) FROM pg_ivs_source WHERE category = 'A'))"
            );

            $rows = $this->pdo->query('SELECT total_count, total_amount FROM pg_ivs_target WHERE id = 1')->fetchAll(PDO::FETCH_ASSOC);
            $this->assertCount(1, $rows);
    ...
    public function testInsertWithMaxSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_ivs_target (id, total_count, total_amount) VALUES (1, 0, (SELECT MAX(amount) FROM pg_ivs_source))"
            );
    ...
    public function testSubqueryReferencingShadowData(): void
    {
        try {
            ...
            $this->pdo->exec("INSERT INTO pg_ivs_source VALUES (4, 'C', 250.00)");

            // The subquery should see the new shadow row (4 rows total)
            $this->pdo->exec(
                "INSERT INTO pg_ivs_target (id, total_count, total_amount) VALUES (1, (SELECT COUNT(*) FROM pg_ivs_source), (SELECT SUM(amount) FROM pg_ivs_source))"
            );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_ivs_target')->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt'], 'Physical target table should be empty');

        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_ivs_source')->fetchAll(PDO::FETCH_ASSOC);
