...
/**
 * Tests an inventory allocation workflow through ZTD shadow store (PostgreSQL PDO).
 * Covers stock reservation, sale conversion, capacity guards, warehouse reporting,
 * and physical isolation.
 * @spec SPEC-10.2.72
 */
class PostgresInventoryAllocationTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ia_warehouses (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255),
                location VARCHAR(255)
            )',
            'CREATE TABLE pg_ia_products (
                id INTEGER PRIMARY KEY,
            ...
            'CREATE TABLE pg_ia_reservations (
                id INTEGER PRIMARY KEY,
                product_id INTEGER,
    ...
    {
        return ['pg_ia_reservations', 'pg_ia_products', 'pg_ia_warehouses'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 warehouses
        $this->pdo->exec("INSERT INTO pg_ia_warehouses VALUES (1, 'Main Warehouse', 'New York')");
        $this->pdo->exec("INSERT INTO pg_ia_warehouses VALUES (2, 'West Coast', 'Los Angeles')");

        // Products: total_stock, reserved, sold
        $this->pdo->exec("INSERT INTO pg_ia_products VALUES (1, 1, 'SKU-001', 'Widget A', 100, 10, 20)");
        $this->pdo->exec("INSERT INTO pg_ia_products VALUES (2, 1, 'SKU-002', 'Widget B', 50, 5, 30)");
        $this->pdo->exec("INSERT INTO pg_ia_products VALUES (3, 2, 'SKU-001', 'Widget A', 80, 0, 10)");
        $this->pdo->exec("INSERT INTO pg_ia_products VALUES (4, 2, 'SKU-003', 'Gadget C', 200, 25, 50)");

        // Existing reservations
        $this->pdo->exec("INSERT INTO pg_ia_reservations VALUES (1, 1, 'Alice', 5, 'active', '2026-03-01 09:00:00')");
        $this->pdo->exec("INSERT INTO pg_ia_reservations VALUES (2, 1, 'Bob', 5, 'active', '2026-03-02 10:00:00')");
        $this->pdo->exec("INSERT INTO pg_ia_reservations VALUES (3, 4, 'Charlie', 25, 'active', '2026-03-03 11:00:00')");
    }
    ...
    public function testReserveStock(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ia_reservations VALUES (4, 2, 'Diana', 3, 'active', '2026-03-04 09:00:00')");
        $this->pdo->exec("UPDATE pg_ia_products SET reserved = reserved + 3 WHERE id = 2");

        // Verify reservation exists
        $rows = $this->ztdQuery("SELECT quantity, status FROM pg_ia_reservations WHERE id = 4");
        $this->assertEquals(3, (int) $rows[0]['quantity']);
        $this->assertSame('active', $rows[0]['status']);

        // Verify reserved count updated
        $rows = $this->ztdQuery("SELECT reserved, (total_stock - reserved - sold) AS available FROM pg_ia_products WHERE id = 2");
    ...
    public function testConvertReservationToSale(): void
    {
        ...
        $affected = $this->pdo->exec("UPDATE pg_ia_reservations SET status = 'converted' WHERE id = 1 AND status = 'active'");
        $this->assertSame(1, $affected);

        $this->pdo->exec("UPDATE pg_ia_products SET reserved = reserved - 5, sold = sold + 5 WHERE id = 1");

        // Verify reservation converted
        $rows = $this->ztdQuery("SELECT status FROM pg_ia_reservations WHERE id = 1");
        ...
        $rows = $this->ztdQuery(
            "SELECT total_stock, reserved, sold, (total_stock - reserved - sold) AS available
             FROM pg_ia_products WHERE id = 1"
        );
    ...
    {
        $affected = $this->pdo->exec("UPDATE pg_ia_reservations SET status = 'cancelled' WHERE id = 2 AND status = 'active'");
        $this->assertSame(1, $affected);

        $this->pdo->exec("UPDATE pg_ia_products SET reserved = reserved - 5 WHERE id = 1");
    ...
    public function testWarehouseStockReport(): void
    {
        $rows = $this->ztdQuery(
            "SELECT w.name AS warehouse,
             ...
             FROM pg_ia_warehouses w
             LEFT JOIN pg_ia_products p ON p.warehouse_id = w.id
    ...
    public function testStockLookupBySku(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT p.id, w.name AS warehouse, p.total_stock, p.reserved, p.sold,
             ...
             FROM pg_ia_products p
             JOIN pg_ia_warehouses w ON w.id = p.warehouse_id
             WHERE p.sku = ?
             ORDER BY w.name",
            ['SKU-001']
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_ia_reservations VALUES (4, 3, 'Eve', 10, 'active', '2026-03-05 09:00:00')");
        $this->pdo->exec("UPDATE pg_ia_products SET reserved = reserved + 10 WHERE id = 3");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_ia_reservations");
        $this->assertEquals(4, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_ia_reservations')->fetchAll(PDO::FETCH_ASSOC);
