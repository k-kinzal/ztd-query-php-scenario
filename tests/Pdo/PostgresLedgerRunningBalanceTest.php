 ...
 * @spec SPEC-3.3
 */
class PostgresLedgerRunningBalanceTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_led_accounts (
                id INTEGER PRIMARY KEY,
                name TEXT,
                balance NUMERIC(10,2)
            )',
            'CREATE TABLE pg_led_transactions (
                id INTEGER PRIMARY KEY,
                account_id INTEGER,
                amount NUMERIC(10,2),
                type TEXT,
    ...
    {
        return ['pg_led_transactions', 'pg_led_accounts'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_led_accounts VALUES (1, 'Checking', 1000.00)");
        $this->pdo->exec("INSERT INTO pg_led_accounts VALUES (2, 'Savings', 5000.00)");

        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (1, 1, 500.00, 'credit', 'Deposit', '2024-01-01 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (2, 1, -200.00, 'debit', 'Rent', '2024-01-05 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (3, 1, -50.00, 'debit', 'Groceries', '2024-01-10 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (4, 1, 1500.00, 'credit', 'Salary', '2024-01-15 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (5, 1, -100.00, 'debit', 'Utilities', '2024-01-20 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (6, 2, 200.00, 'credit', 'Transfer in', '2024-01-02 00:00:00')");
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (7, 2, -50.00, 'debit', 'Fee', '2024-01-15 00:00:00')");
    }
    ...
    {
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (8, 1, -300.00, 'debit', 'Car payment', '2024-01-25 00:00:00')");
    ...
    {
        $this->pdo->exec("UPDATE pg_led_accounts SET balance = balance + 500 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT balance FROM pg_led_accounts WHERE id = 1");
    ...
    {
        $this->pdo->exec("UPDATE pg_led_accounts SET balance = balance - 200 WHERE id = 1");
    ...
    public function testBalanceConsistencyCheck(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT SUM(amount) AS net_change FROM pg_led_transactions WHERE account_id = 1"
        );
    ...
    public function testFullTransactionWorkflow(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (8, 1, 750.00, 'credit', 'Bonus', '2024-01-25 00:00:00')");

        // Step 2: Update account balance
        $this->pdo->exec("UPDATE pg_led_accounts SET balance = balance + 750 WHERE id = 1");
    ...
    {
        $this->pdo->exec("INSERT INTO pg_led_transactions VALUES (8, 1, 9999.00, 'credit', 'Big deposit', '2024-02-01 00:00:00')");
        $this->pdo->exec("UPDATE pg_led_accounts SET balance = balance + 9999 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT balance FROM pg_led_accounts WHERE id = 1");
        $this->assertEqualsWithDelta(10999.00, (float) $rows[0]['balance'], 0.01);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT balance FROM pg_led_accounts WHERE id = 1")->fetchAll(PDO::FETCH_ASSOC);
