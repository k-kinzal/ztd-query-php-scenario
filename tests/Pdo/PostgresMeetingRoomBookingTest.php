...
/**
 * Tests meeting room booking scenarios through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.127
 */
class PostgresMeetingRoomBookingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_mrb_rooms (
                id SERIAL PRIMARY KEY,
                name TEXT,
                capacity INTEGER,
                floor_num INTEGER
            )',
            'CREATE TABLE pg_mrb_bookings (
                id SERIAL PRIMARY KEY,
                room_id INTEGER,
                booked_by TEXT,
                booking_date TEXT,
    ...
    {
        return ['pg_mrb_bookings', 'pg_mrb_rooms'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_mrb_rooms VALUES (1, 'Alpha', 10, 1)");
        $this->pdo->exec("INSERT INTO pg_mrb_rooms VALUES (2, 'Beta', 6, 1)");
        $this->pdo->exec("INSERT INTO pg_mrb_rooms VALUES (3, 'Gamma', 20, 2)");
        $this->pdo->exec("INSERT INTO pg_mrb_rooms VALUES (4, 'Delta', 4, 2)");

        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (1, 1, 'alice', '2025-10-15', '09:00', '10:00', 'Standup')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (2, 1, 'bob', '2025-10-15', '10:30', '12:00', 'Sprint Planning')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (3, 1, 'charlie', '2025-10-15', '14:00', '15:00', 'Retrospective')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (4, 2, 'alice', '2025-10-15', '09:00', '09:30', 'Quick sync')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (5, 2, 'diana', '2025-10-15', '11:00', '12:30', 'Design review')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (6, 3, 'bob', '2025-10-15', '13:00', '15:00', 'All-hands')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (7, 4, 'eve', '2025-10-15', '08:00', '09:00', '1:1 meeting')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (8, 4, 'eve', '2025-10-15', '15:00', '16:00', 'Interview')");
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (9, 1, 'alice', '2025-10-16', '09:00', '10:00', 'Follow-up')");
    }
    ...
    public function testRoomAvailabilityCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.id, r.name, r.capacity
             FROM pg_mrb_rooms r
             WHERE NOT EXISTS (
                 SELECT 1 FROM pg_mrb_bookings b
    ...
    public function testBookingsPerRoom(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name, COUNT(b.id) AS booking_count
             FROM pg_mrb_rooms r
             LEFT JOIN pg_mrb_bookings b ON b.room_id = r.id AND b.booking_date = '2025-10-15'
    ...
    {
        $this->pdo->exec("INSERT INTO pg_mrb_bookings VALUES (10, 1, 'frank', '2025-10-15', '16:00', '17:00', 'Wrap-up')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_mrb_bookings");
        $this->assertSame(10, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_mrb_bookings")->fetchAll(PDO::FETCH_ASSOC);
