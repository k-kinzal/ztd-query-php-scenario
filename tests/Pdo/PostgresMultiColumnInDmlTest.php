...
/**
 * Tests multi-column IN clause with tuple syntax in DML operations on PostgreSQL PDO.
 *
 ...
 * @spec SPEC-10.2
 */
class PostgresMultiColumnInDmlTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return "CREATE TABLE pg_mci_enrollments (
    ...
    protected function setUp(): void
    {
        ...
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (1, 101, 'B+', '2025-01-15')");
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (1, 102, 'A-', '2025-01-16')");
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (2, 101, 'C', '2025-02-01')");
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (2, 102, 'B', '2025-02-02')");
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (3, 101, 'A', '2025-03-01')");
        $this->ztdExec("INSERT INTO pg_mci_enrollments (student_id, course_id, grade, enrolled_date) VALUES (3, 103, 'B-', '2025-03-10')");
    }
    ...
    {
        try {
            $this->ztdExec("DELETE FROM pg_mci_enrollments WHERE (student_id, course_id) IN ((1, 101), (2, 102))");

            $rows = $this->ztdQuery("SELECT student_id, course_id FROM pg_mci_enrollments ORDER BY student_id, course_id");
            ...
            // Verify the deleted rows are gone
    ...
    {
        try {
            $this->ztdExec("UPDATE pg_mci_enrollments SET grade = 'A' WHERE (student_id, course_id) IN ((1, 101))");

            $rows = $this->ztdQuery("SELECT grade FROM pg_mci_enrollments WHERE student_id = 1 AND course_id = 101");
            ...
            $other = $this->ztdQuery("SELECT grade FROM pg_mci_enrollments WHERE student_id = 1 AND course_id = 102");
    ...
    public function testSelectWithMultiColumnInSubquery(): void
    {
        try {
            ...
            $this->ztdExec("UPDATE pg_mci_enrollments SET grade = 'A' WHERE student_id = 3");

            $rows = $this->ztdQuery(
                "SELECT student_id, course_id, grade FROM pg_mci_enrollments
                 WHERE (student_id, course_id) IN (
                     SELECT student_id, course_id FROM pg_mci_enrollments WHERE grade = 'A'
                 )
    ...
    /**
     * Prepared DELETE with (col1, col2) IN (($1, $2)) syntax.
     */
    public function testPreparedDeleteWithMultiColumnIn(): void
    {
        try {
            $stmt = $this->ztdPrepare("DELETE FROM pg_mci_enrollments WHERE (student_id, course_id) IN (($1, $2))");
            $stmt->execute([1, 101]);
    ...
    public function testVerifyRemainingRowsAfterMultiColumnInDml(): void
    {
        try {
            ...
            $this->ztdExec("DELETE FROM pg_mci_enrollments WHERE (student_id, course_id) IN ((1, 101), (3, 103))");

            // Update one enrollment
            $this->ztdExec("UPDATE pg_mci_enrollments SET grade = 'A+' WHERE (student_id, course_id) IN ((2, 101))");

            $rows = $this->ztdQuery("SELECT student_id, course_id, grade FROM pg_mci_enrollments ORDER BY student_id, course_id");
