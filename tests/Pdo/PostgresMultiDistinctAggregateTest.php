 ...
 * @spec SPEC-3.3
 */
class PostgresMultiDistinctAggregateTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return "CREATE TABLE pg_mda_orders (
            id INT PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_mda_orders (id, customer_id, product, region, amount, status) VALUES
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_mda_orders (id, customer_id, product, region, amount, status) VALUES (8, 40, 'Monitor', 'North', 800.00, 'completed')");
    ...
    {
        $this->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_mda_orders")
