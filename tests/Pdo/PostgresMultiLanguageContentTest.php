...
/**
 * Tests internationalization content storage with language fallback chains (PostgreSQL PDO).
 *
 ...
 * @spec SPEC-10.2.114
 */
class PostgresMultiLanguageContentTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_mlc_languages (
                code VARCHAR(10) PRIMARY KEY,
                name VARCHAR(50),
                is_default INTEGER
            )',
            'CREATE TABLE pg_mlc_content_items (
                id SERIAL PRIMARY KEY,
                slug VARCHAR(100),
                content_type VARCHAR(50)
            )',
            'CREATE TABLE pg_mlc_translations (
                id SERIAL PRIMARY KEY,
                content_id INTEGER,
    ...
    {
        return ['pg_mlc_translations', 'pg_mlc_content_items', 'pg_mlc_languages'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Languages
        $this->pdo->exec("INSERT INTO pg_mlc_languages VALUES ('en', 'English', 1)");
        $this->pdo->exec("INSERT INTO pg_mlc_languages VALUES ('ja', 'Japanese', 0)");
        $this->pdo->exec("INSERT INTO pg_mlc_languages VALUES ('fr', 'French', 0)");
        $this->pdo->exec("INSERT INTO pg_mlc_languages VALUES ('de', 'German', 0)");

        // Content items
        $this->pdo->exec("INSERT INTO pg_mlc_content_items VALUES (1, 'welcome-page', 'page')");
        $this->pdo->exec("INSERT INTO pg_mlc_content_items VALUES (2, 'faq-returns', 'faq')");
        $this->pdo->exec("INSERT INTO pg_mlc_content_items VALUES (3, 'blog-launch', 'article')");

        // Translations for welcome-page: en, ja, fr
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (1, 1, 'en', 'Welcome', 'Welcome to our site')");
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (2, 1, 'ja', 'ようこそ', 'サイトへようこそ')");
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (3, 1, 'fr', 'Bienvenue', 'Bienvenue sur notre site')");

        // Translations for faq-returns: en, ja
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (4, 2, 'en', 'Return Policy', 'You can return within 30 days')");
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (5, 2, 'ja', '返品ポリシー', '30日以内に返品可能です')");

        // Translations for blog-launch: en only
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (6, 3, 'en', 'Launch Day', 'We are live!')");
    }
    ...
    public function testDirectTranslation(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.title, t.body
             FROM pg_mlc_translations t
             JOIN pg_mlc_content_items c ON c.id = t.content_id
    ...
    public function testFallbackToDefault(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.slug,
             ...
             FROM pg_mlc_content_items c
             LEFT JOIN pg_mlc_translations req ON req.content_id = c.id AND req.lang_code = 'de'
             LEFT JOIN pg_mlc_translations def ON def.content_id = c.id AND def.lang_code = 'en'
    ...
    public function testFallbackForMissingLanguage(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.slug,
                    ...
                    COALESCE(req.body, def.body) AS body
             FROM pg_mlc_content_items c
             LEFT JOIN pg_mlc_translations req ON req.content_id = c.id AND req.lang_code = 'ja'
             LEFT JOIN pg_mlc_translations def ON def.content_id = c.id AND def.lang_code = 'en'
    ...
    public function testTranslationCoverageStats(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.slug, COUNT(t.id) AS translation_count
             FROM pg_mlc_content_items c
             LEFT JOIN pg_mlc_translations t ON t.content_id = c.id
             GROUP BY c.slug
    ...
    public function testAllContentWithLanguage(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT c.slug,
                    ...
                    COALESCE(req.body, def.body) AS body
             FROM pg_mlc_content_items c
             LEFT JOIN pg_mlc_translations req ON req.content_id = c.id AND req.lang_code = ?
             LEFT JOIN pg_mlc_translations def ON def.content_id = c.id AND def.lang_code = 'en'
             ORDER BY c.slug",
            ['fr']
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_mlc_translations VALUES (7, 1, 'de', 'Willkommen', 'Willkommen auf unserer Seite')");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_mlc_translations");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table is empty
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_mlc_translations")->fetchAll(PDO::FETCH_ASSOC);
