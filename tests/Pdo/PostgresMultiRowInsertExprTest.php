 ...
 * @spec SPEC-4.1
 */
class PostgresMultiRowInsertExprTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_mrie_products (
            id INT PRIMARY KEY,
    ...
    public function testMultiRowInsertWithStringFunctions(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_mrie_products (id, code, price, quantity) VALUES
                    (1, UPPER('widget'), 9.99, 10),
                    (2, LOWER('GADGET'), 19.99, 5),
                    (3, 'item-' || '003', 29.99, 3)"
            );

            $rows = $this->ztdQuery("SELECT id, code FROM pg_mrie_products ORDER BY id");
    ...
    public function testMultiRowInsertWithArithmetic(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT id, price, quantity FROM pg_mrie_products ORDER BY id");
    ...
    /**
     * PostgreSQL: multi-row INSERT with CASE expression per row.
     */
