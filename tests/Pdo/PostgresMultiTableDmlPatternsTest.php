...
/**
 * Multi-Table DML Patterns: UPDATE with FROM clause and DELETE with subqueries
 * referencing other tables through the CTE rewriter (PostgreSQL PDO).
 *
 * PostgreSQL uses UPDATE ... SET ... FROM ... WHERE syntax instead of
 * MySQL's UPDATE ... JOIN ... SET syntax.
 *
 * @spec SPEC-4.2
 * @spec SPEC-4.3
 */
class PostgresMultiTableDmlPatternsTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_mtd_warehouses (
                id INT PRIMARY KEY,
                name VARCHAR(30) NOT NULL,
                active INT NOT NULL DEFAULT 1
            )',
            'CREATE TABLE pg_mtd_inventory (
                id INT PRIMARY KEY,
            ...
            'CREATE TABLE pg_mtd_restock_log (
    ...
    {
        return ['pg_mtd_restock_log', 'pg_mtd_inventory', 'pg_mtd_warehouses'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_mtd_warehouses (id, name, active) VALUES
            (1, 'Main', 1), (2, 'Backup', 1), (3, 'Decommissioned', 0)");

        $this->pdo->exec("INSERT INTO pg_mtd_inventory (id, product, stock, min_stock, warehouse_id) VALUES
    ...
    /**
     * PostgreSQL UPDATE ... FROM: increase stock for items in 'Main' warehouse.
     * PostgreSQL syntax: UPDATE t SET col = expr FROM other_table WHERE join_condition
     */
    public function testUpdateWithJoin(): void
    {
        try {
            $this->ztdExec(
                "UPDATE pg_mtd_inventory
                 SET stock = pg_mtd_inventory.stock + 10
                 FROM pg_mtd_warehouses
                 WHERE pg_mtd_inventory.warehouse_id = pg_mtd_warehouses.id
                   AND pg_mtd_warehouses.name = 'Main'"
            );

            $rows = $this->ztdQuery(
                "SELECT id, product, stock FROM pg_mtd_inventory ORDER BY id"
            );

            if (count($rows) !== 5) {
                $this->markTestIncomplete('UPDATE FROM: expected 5 rows, got ' . count($rows));
            }
            ...
            $this->markTestIncomplete('UPDATE with FROM failed: ' . $e->getMessage());
        }
    }

    /**
     * PostgreSQL UPDATE ... FROM: zero out stock for items in inactive warehouses.
     */
        ...
        try {
            $this->ztdExec(
                "UPDATE pg_mtd_inventory
                 ...
                 FROM pg_mtd_warehouses
                 WHERE pg_mtd_inventory.warehouse_id = pg_mtd_warehouses.id
                   AND pg_mtd_warehouses.active = 0"
            );
            ...
            if (count($rows) !== 5) {
                $this->markTestIncomplete('UPDATE FROM condition: expected 5 rows, got ' . count($rows));
            }
            ...
            $this->markTestIncomplete('UPDATE with FROM condition failed: ' . $e->getMessage());
        }
    }
    ...
    public function testDeleteWithSubqueryFromOtherTable(): void
    {
        try {
            $this->ztdExec(
                "DELETE FROM pg_mtd_inventory
                 WHERE warehouse_id IN (SELECT id FROM pg_mtd_warehouses WHERE active = 0)"
            );

            $rows = $this->ztdQuery(
                "SELECT id, product FROM pg_mtd_inventory ORDER BY id"
            );
    ...
    public function testDeleteWithExistsSubquery(): void
    {
        try {
            $this->ztdExec(
                "DELETE FROM pg_mtd_inventory
                 WHERE EXISTS (
                     SELECT 1 FROM pg_mtd_warehouses w
                     WHERE w.id = pg_mtd_inventory.warehouse_id AND w.active = 0
                 )"
            );
    ...
    public function testInsertSelectFromJoin(): void
    {
        try {
            $this->ztdExec(
                "INSERT INTO pg_mtd_restock_log (product_id, qty)
                 SELECT i.id, i.min_stock - i.stock
                 FROM pg_mtd_inventory i
                 JOIN pg_mtd_warehouses w ON i.warehouse_id = w.id
                 WHERE i.stock < i.min_stock AND w.active = 1"
            );

            $rows = $this->ztdQuery(
                "SELECT product_id, qty FROM pg_mtd_restock_log ORDER BY product_id"
            );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $d = $this->pdo->query("SELECT COUNT(*) FROM pg_mtd_warehouses")->fetchColumn();
        $this->assertSame(0, (int) $d, 'pg_mtd_warehouses should be empty');

        $d = $this->pdo->query("SELECT COUNT(*) FROM pg_mtd_inventory")->fetchColumn();
        $this->assertSame(0, (int) $d, 'pg_mtd_inventory should be empty');

        $d = $this->pdo->query("SELECT COUNT(*) FROM pg_mtd_restock_log")->fetchColumn();
        $this->assertSame(0, (int) $d, 'pg_mtd_restock_log should be empty');
    }
}
