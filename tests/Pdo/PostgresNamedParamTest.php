...
/**
 * Tests named parameters (:name style) on PostgreSQL PDO through ZTD.
 *
 * PostgreSQL normally uses $N positional params. PDO allows :name syntax
 * which it converts internally. This tests whether ZTD handles that conversion.
 *
 * @spec SPEC-3.1, SPEC-4.2
 */
class PostgresNamedParamTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_np_items (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_np_items VALUES (1, 'Widget', 'tools', 10.00, 100)");
        $this->pdo->exec("INSERT INTO pg_np_items VALUES (2, 'Gadget', 'tools', 20.00, 50)");
        $this->pdo->exec("INSERT INTO pg_np_items VALUES (3, 'Doohickey', 'parts', 30.00, 75)");
        $this->pdo->exec("INSERT INTO pg_np_items VALUES (4, 'Thingamajig', 'parts', 15.00, 200)");
    }
    ...
    {
        $sql = "SELECT name, price FROM pg_np_items WHERE category = :cat ORDER BY name";
    ...
    {
        $sql = "UPDATE pg_np_items SET price = :new_price WHERE id = :id";

        try {
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([':new_price' => 99.99, ':id' => 1]);

            $rows = $this->ztdQuery("SELECT price FROM pg_np_items WHERE id = 1");
    ...
    {
        $sql = "DELETE FROM pg_np_items WHERE category = :cat";

        try {
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([':cat' => 'parts']);

            $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_np_items");
    ...
    {
        $sql = "INSERT INTO pg_np_items (id, name, category, price, stock)
        ...
        try {
            ...
            $rows = $this->ztdQuery("SELECT name, category FROM pg_np_items WHERE id = 5");
    ...
    {
        $sql = "SELECT name FROM pg_np_items
                WHERE price > (SELECT AVG(price) FROM pg_np_items WHERE category = :cat)
                ORDER BY name";
