...
/**
 * Tests nested function expressions, subquery in BETWEEN, and JOIN rate conversion (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.174
 */
class PostgresNestedFunctionExprTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_nfe_vendor (
                id INTEGER PRIMARY KEY,
                name VARCHAR(200) NOT NULL,
                country VARCHAR(10) NOT NULL,
                notes TEXT
            )',
            'CREATE TABLE pg_nfe_invoice (
                id INTEGER PRIMARY KEY,
            ...
            'CREATE TABLE pg_nfe_payment (
                id INTEGER PRIMARY KEY,
                invoice_id INTEGER NOT NULL,
                paid_amount NUMERIC(12,2) NOT NULL,
                paid_date DATE NOT NULL
            )',
            'CREATE TABLE pg_nfe_rate (
    ...
    {
        return ['pg_nfe_payment', 'pg_nfe_invoice', 'pg_nfe_rate', 'pg_nfe_vendor'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_nfe_vendor VALUES (1, 'Acme Corp', 'US', 'Primary supplier')");
        $this->pdo->exec("INSERT INTO pg_nfe_vendor VALUES (2, 'EuroTech', 'DE', '')");
        $this->pdo->exec("INSERT INTO pg_nfe_vendor VALUES (3, 'AsiaLogistics', 'JP', NULL)");
        $this->pdo->exec("INSERT INTO pg_nfe_vendor VALUES (4, 'NullNotes Inc', 'US', '   ')");

        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (1, 1, 1000.00, 'USD', '2025-01-15', 'paid')");
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (2, 1, 2500.00, 'USD', '2025-02-20', 'partial')");
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (3, 2, 1800.00, 'EUR', '2025-03-10', 'pending')");
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (4, 2, 750.00, 'EUR', '2025-04-05', 'paid')");
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (5, 3, 500000.00, 'JPY', '2025-05-01', 'pending')");

        $this->pdo->exec("INSERT INTO pg_nfe_payment VALUES (1, 1, 1000.00, '2025-02-01')");
        $this->pdo->exec("INSERT INTO pg_nfe_payment VALUES (2, 2, 1500.00, '2025-03-01')");
        $this->pdo->exec("INSERT INTO pg_nfe_payment VALUES (3, 4, 750.00, '2025-04-20')");

        $this->pdo->exec("INSERT INTO pg_nfe_rate VALUES ('EUR', 'USD', 1.08)");
        $this->pdo->exec("INSERT INTO pg_nfe_rate VALUES ('JPY', 'USD', 0.0067)");
        $this->pdo->exec("INSERT INTO pg_nfe_rate VALUES ('USD', 'USD', 1.00)");
    }
    ...
    public function testSubqueryInBetween(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id, i.amount, i.issued_date
             FROM pg_nfe_invoice i
             WHERE i.issued_date BETWEEN
                 (SELECT MIN(paid_date) FROM pg_nfe_payment) AND
                 (SELECT MAX(paid_date) FROM pg_nfe_payment)
    ...
    public function testScalarSubqueryBalance(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id,
                    i.amount,
                    COALESCE((SELECT SUM(p.paid_amount) FROM pg_nfe_payment p WHERE p.invoice_id = i.id), 0) AS total_paid,
                    i.amount - COALESCE((SELECT SUM(p.paid_amount) FROM pg_nfe_payment p WHERE p.invoice_id = i.id), 0) AS balance
             FROM pg_nfe_invoice i
             ORDER BY i.id"
        );
    ...
    public function testJoinCurrencyConversion(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id,
             ...
             FROM pg_nfe_invoice i
             JOIN pg_nfe_vendor v ON v.id = i.vendor_id
             JOIN pg_nfe_rate r ON r.from_currency = i.currency AND r.to_currency = 'USD'
             ORDER BY i.id"
        );
     ...
     *
     * NEW FINDING: On PostgreSQL, the CTE rewriter makes the outer `WHERE id IN (...)`
     * ambiguous when the inner subquery uses JOINs with GROUP BY, causing
     * "column reference 'id' is ambiguous". Works correctly on MySQL (MySQLi + PDO).
     */
    public function testUpdateStatusBasedOnPaymentSum(): void
    {
        try {
            $this->ztdExec(
                "UPDATE pg_nfe_invoice SET status = 'paid'
                 WHERE id IN (
                    SELECT i.id
                    FROM pg_nfe_invoice i
                    JOIN (SELECT invoice_id, SUM(paid_amount) AS total_paid FROM pg_nfe_payment GROUP BY invoice_id) p
                    ON p.invoice_id = i.id
                    WHERE p.total_paid >= i.amount
                 )"
            );

            $rows = $this->ztdQuery("SELECT id, status FROM pg_nfe_invoice ORDER BY id");
            ...
            $this->markTestIncomplete(
                'NEW FINDING: UPDATE WHERE IN (SELECT with JOIN+GROUP BY) fails on PostgreSQL — '
                . 'CTE rewriter loses table qualification making column reference ambiguous. Works on MySQL.'
            );
        }
    }
    ...
    {
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (6, 3, 200000.00, 'JPY', '2025-06-01', 'pending')");

        $rows = $this->ztdPrepareAndExecute(
            "SELECT COUNT(*) AS cnt FROM pg_nfe_invoice WHERE vendor_id = ?",
            [3]
        );
        $this->assertEquals(2, (int) $rows[0]['cnt']);

        $this->ztdExec("UPDATE pg_nfe_invoice SET status = 'approved' WHERE id = 6");

        $rows = $this->ztdPrepareAndExecute(
            "SELECT status FROM pg_nfe_invoice WHERE id = ?",
            [6]
        );
    ...
    public function testNestedCaseWithCoalesce(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.id,
                    CASE
                        WHEN COALESCE((SELECT SUM(p.paid_amount) FROM pg_nfe_payment p WHERE p.invoice_id = i.id), 0) = 0
                            THEN 'unpaid'
                        WHEN COALESCE((SELECT SUM(p.paid_amount) FROM pg_nfe_payment p WHERE p.invoice_id = i.id), 0) >= i.amount
                            THEN 'fully paid'
                        ELSE 'partially paid'
    ...
    {
        $this->pdo->exec("INSERT INTO pg_nfe_invoice VALUES (6, 1, 999.00, 'USD', '2025-12-01', 'pending')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_nfe_invoice");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_nfe_invoice")->fetchAll(PDO::FETCH_ASSOC);
