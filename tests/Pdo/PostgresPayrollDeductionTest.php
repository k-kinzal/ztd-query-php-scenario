 ...
 * with CASE expressions, SUM CASE cross-tab, and HAVING with arithmetic (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.175
 */
class PostgresPayrollDeductionTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_pd_employee (
                id INT PRIMARY KEY,
                name VARCHAR(100),
                department VARCHAR(50),
                base_salary NUMERIC(10,2)
            )',
            'CREATE TABLE pg_pd_payroll (
                id INT PRIMARY KEY,
                employee_id INT,
                pay_period TEXT,
    ...
    {
        return ['pg_pd_deduction', 'pg_pd_payroll', 'pg_pd_employee'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_pd_employee VALUES (1, 'Alice', 'Engineering', 8000.00)");
        $this->pdo->exec("INSERT INTO pg_pd_employee VALUES (2, 'Bob', 'Engineering', 7000.00)");
        $this->pdo->exec("INSERT INTO pg_pd_employee VALUES (3, 'Carol', 'Sales', 6000.00)");
        $this->pdo->exec("INSERT INTO pg_pd_employee VALUES (4, 'Dave', 'Sales', 9000.00)");

        $this->pdo->exec("INSERT INTO pg_pd_payroll VALUES (1, 1, '2025-01', 8000.00, NULL, 'pending')");
        $this->pdo->exec("INSERT INTO pg_pd_payroll VALUES (2, 2, '2025-01', 7000.00, NULL, 'pending')");
        $this->pdo->exec("INSERT INTO pg_pd_payroll VALUES (3, 3, '2025-01', 6000.00, NULL, 'pending')");
        $this->pdo->exec("INSERT INTO pg_pd_payroll VALUES (4, 4, '2025-01', 9000.00, NULL, 'pending')");

        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (1, 1, 'tax', 1600.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (2, 1, 'health', 500.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (3, 1, 'retirement', 400.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (4, 2, 'tax', 1400.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (5, 2, 'health', 500.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (6, 2, 'retirement', 350.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (7, 3, 'tax', 1200.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (8, 3, 'health', 500.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (9, 3, 'retirement', 300.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (10, 4, 'tax', 1800.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (11, 4, 'health', 500.00)");
        $this->pdo->exec("INSERT INTO pg_pd_deduction VALUES (12, 4, 'retirement', 450.00)");
    }
    ...
    {
        $this->ztdExec(
            "UPDATE pg_pd_payroll SET net_pay = gross_pay * 0.75, status = 'processed'
             WHERE pay_period = '2025-01'"
        );

        $rows = $this->ztdQuery(
            "SELECT employee_id, gross_pay, net_pay, status
             FROM pg_pd_payroll ORDER BY employee_id"
        );
    ...
    public function testDeductionBreakdownCrossTab(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
             ...
             FROM pg_pd_employee e
             JOIN pg_pd_payroll p ON p.employee_id = e.id
             JOIN pg_pd_deduction d ON d.payroll_id = p.id
    ...
    /**
     * INSERT...SELECT with CASE expression for computed values.
     * Probes near SPEC-11.INSERT-SELECT-COMPUTED on PostgreSQL.
     */
    public function testInsertSelectWithCaseExpression(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_pd_payroll (id, employee_id, pay_period, gross_pay, net_pay, status)
            ...
            $rows = $this->ztdQuery(
                "SELECT p.employee_id, p.gross_pay, e.department
                 FROM pg_pd_payroll p
                 JOIN pg_pd_employee e ON e.id = p.employee_id
                 WHERE p.pay_period = '2025-02'
                 ORDER BY p.employee_id"
            );

            if (count($rows) === 4) {
                // Check if CASE expression was computed or became NULL
                if ($rows[0]['gross_pay'] === null) {
                    $this->markTestIncomplete(
                        'INSERT...SELECT with CASE expression: gross_pay is NULL on PostgreSQL. '
    ...
    public function testDerivedTableWithGroupByHaving(): void
    {
        try {
            ...
            if (count($rows) === 0) {
                $this->markTestIncomplete(
                    'Derived table with GROUP BY + HAVING returned empty on PostgreSQL. '
    ...
    /**
     * Prepared statement with BETWEEN for date range filter ($N params).
     */
    public function testPreparedBetweenDateFilter(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT e.name, p.gross_pay, p.pay_period
             FROM pg_pd_payroll p
             JOIN pg_pd_employee e ON e.id = p.employee_id
             WHERE p.pay_period BETWEEN $1 AND $2
             ORDER BY e.name",
            ['2025-01', '2025-06']
        );
    ...
    {
        $this->ztdExec("UPDATE pg_pd_payroll SET status = 'processed' WHERE id = 1");

        $rows = $this->ztdQuery("SELECT status FROM pg_pd_payroll WHERE id = 1");
        $this->assertSame('processed', $rows[0]['status']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_pd_payroll")->fetchAll(PDO::FETCH_ASSOC);
