 ...
 * @spec SPEC-10.2.34
 */
class PostgresPivotMultiLevelTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_pml_sales (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (1, 'North', 'Electronics', '2024-01', 1000)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (2, 'North', 'Electronics', '2024-02', 1200)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (3, 'North', 'Clothing', '2024-01', 500)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (4, 'South', 'Electronics', '2024-01', 800)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (5, 'South', 'Clothing', '2024-01', 600)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (6, 'South', 'Clothing', '2024-02', 700)");
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (7, 'North', NULL, '2024-01', 200)");
    }
    ...
    public function testPivotAfterInterleavedMutations(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_pml_sales (id, region, category, month, amount) VALUES (8, 'East', 'Electronics', '2024-01', 900)");

        $rows = $this->ztdQuery("
            SELECT region, SUM(amount) AS total
            FROM pg_pml_sales WHERE category = 'Electronics'
            GROUP BY region ORDER BY region
        ");
        $this->assertCount(3, $rows); // East, North, South

        // Update East's amount
        $this->pdo->exec("UPDATE pg_pml_sales SET amount = 950 WHERE id = 8");
    ...
    public function testPreparedPivotWithCategoryParam(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT region, SUM(amount) AS total FROM pg_pml_sales WHERE category = ? GROUP BY region ORDER BY region",
            ['Electronics']
        );
        $this->assertCount(2, $rows);
        $this->assertSame('North', $rows[0]['region']);
