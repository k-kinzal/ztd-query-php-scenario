...
/**
 * Tests a polymorphic comments system through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.103
 */
class PostgresPolymorphicCommentTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_pc_posts (
                id INTEGER PRIMARY KEY,
                title VARCHAR(255),
                body VARCHAR(1000),
                author VARCHAR(255)
            )',
            'CREATE TABLE pg_pc_photos (
                id INTEGER PRIMARY KEY,
                url VARCHAR(255),
                caption VARCHAR(255),
                photographer VARCHAR(255)
            )',
            'CREATE TABLE pg_pc_comments (
                id INTEGER PRIMARY KEY,
    ...
    {
        return ['pg_pc_comments', 'pg_pc_photos', 'pg_pc_posts'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Posts
        $this->pdo->exec("INSERT INTO pg_pc_posts VALUES (1, 'First Post', 'Hello world', 'alice')");
        $this->pdo->exec("INSERT INTO pg_pc_posts VALUES (2, 'Second Post', 'Another post', 'bob')");

        // Photos
        $this->pdo->exec("INSERT INTO pg_pc_photos VALUES (1, '/img/sunset.jpg', 'Beautiful sunset', 'charlie')");
        $this->pdo->exec("INSERT INTO pg_pc_photos VALUES (2, '/img/cat.jpg', 'Cute cat', 'diana')");

        // Comments on posts
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (1, 'post', 1, 'bob', 'Great post!', '2026-03-01 10:00:00')");
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (2, 'post', 1, 'charlie', 'Thanks for sharing', '2026-03-01 11:00:00')");
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (3, 'post', 2, 'alice', 'Nice work', '2026-03-02 09:00:00')");

        // Comments on photos
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (4, 'photo', 1, 'alice', 'Gorgeous!', '2026-03-01 15:00:00')");
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (5, 'photo', 2, 'bob', 'So cute!', '2026-03-03 08:00:00')");
    }
    ...
    public function testJoinCommentsWithParentEntity(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id AS comment_id, c.user_name, c.body AS comment_body,
                    ...
                    END AS parent_title
             FROM pg_pc_comments c
             LEFT JOIN pg_pc_posts p ON c.commentable_type = 'post' AND c.commentable_id = p.id
             LEFT JOIN pg_pc_photos ph ON c.commentable_type = 'photo' AND c.commentable_id = ph.id
             ORDER BY c.id"
        );
    ...
    public function testCommentCountPerPost(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.id, p.title, COUNT(c.id) AS comment_count
             FROM pg_pc_posts p
             LEFT JOIN pg_pc_comments c ON c.commentable_type = 'post' AND c.commentable_id = p.id
    ...
    {
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (6, 'photo', 1, 'eve', 'Amazing colors!', '2026-03-04 12:00:00')");
        ...
        $rows = $this->ztdQuery(
            "SELECT c.user_name, ph.caption
             FROM pg_pc_comments c
             JOIN pg_pc_photos ph ON c.commentable_type = 'photo' AND c.commentable_id = ph.id
             WHERE c.id = 6"
        );
    ...
    {
        $this->pdo->exec("DELETE FROM pg_pc_comments WHERE id = 3");

        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM pg_pc_comments WHERE commentable_type = 'post'"
        );
        $this->assertEquals(2, (int) $rows[0]['cnt']);
    }
    ...
    public function testEntitiesWithNoComments(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_pc_photos VALUES (3, '/img/tree.jpg', 'Old tree', 'eve')");

        $rows = $this->ztdQuery(
            "SELECT ph.id, ph.caption
             FROM pg_pc_photos ph
             LEFT JOIN pg_pc_comments c ON c.commentable_type = 'photo' AND c.commentable_id = ph.id
             WHERE c.id IS NULL"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_pc_comments VALUES (7, 'post', 2, 'frank', 'Hello', '2026-03-05 10:00:00')");
        $this->pdo->exec("DELETE FROM pg_pc_comments WHERE id = 1");

        // ZTD shows 5 comments (original 5 + 1 added - 1 deleted)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_pc_comments");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_pc_comments")->fetchAll(PDO::FETCH_ASSOC);
