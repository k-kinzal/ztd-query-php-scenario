...
/**
 * Tests a prescription tracking system through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.139
 */
class PostgresPrescriptionTrackingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_pt_patients (
                id SERIAL PRIMARY KEY,
                name TEXT,
                date_of_birth TEXT
            )',
            'CREATE TABLE pg_pt_doctors (
                id SERIAL PRIMARY KEY,
                name TEXT,
                specialty TEXT
            )',
            'CREATE TABLE pg_pt_visits (
                id SERIAL PRIMARY KEY,
                patient_id INTEGER,
                doctor_id INTEGER,
                visit_date TEXT,
            ...
            'CREATE TABLE pg_pt_prescriptions (
                id SERIAL PRIMARY KEY,
                visit_id INTEGER,
                drug_name TEXT,
                dosage TEXT,
    ...
    {
        return ['pg_pt_prescriptions', 'pg_pt_visits', 'pg_pt_doctors', 'pg_pt_patients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 patients
        $this->pdo->exec("INSERT INTO pg_pt_patients (id, name, date_of_birth) VALUES (1, 'John Doe', '1985-03-15')");
        $this->pdo->exec("INSERT INTO pg_pt_patients (id, name, date_of_birth) VALUES (2, 'Jane Smith', '1990-07-22')");
        $this->pdo->exec("INSERT INTO pg_pt_patients (id, name, date_of_birth) VALUES (3, 'Bob Wilson', '1978-11-30')");

        // 3 doctors
        $this->pdo->exec("INSERT INTO pg_pt_doctors (id, name, specialty) VALUES (1, 'Dr. Adams', 'General')");
        $this->pdo->exec("INSERT INTO pg_pt_doctors (id, name, specialty) VALUES (2, 'Dr. Baker', 'Cardiology')");
        $this->pdo->exec("INSERT INTO pg_pt_doctors (id, name, specialty) VALUES (3, 'Dr. Clark', 'Neurology')");

        // 5 visits
        $this->pdo->exec("INSERT INTO pg_pt_visits (id, patient_id, doctor_id, visit_date, diagnosis) VALUES (1, 1, 1, '2025-06-01', 'Hypertension')");
        $this->pdo->exec("INSERT INTO pg_pt_visits (id, patient_id, doctor_id, visit_date, diagnosis) VALUES (2, 1, 2, '2025-06-15', 'Heart checkup')");
        $this->pdo->exec("INSERT INTO pg_pt_visits (id, patient_id, doctor_id, visit_date, diagnosis) VALUES (3, 2, 1, '2025-07-01', 'Annual physical')");
        $this->pdo->exec("INSERT INTO pg_pt_visits (id, patient_id, doctor_id, visit_date, diagnosis) VALUES (4, 2, 3, '2025-07-10', 'Migraine')");
        $this->pdo->exec("INSERT INTO pg_pt_visits (id, patient_id, doctor_id, visit_date, diagnosis) VALUES (5, 3, 1, '2025-08-01', 'Flu')");

        // 6 prescriptions
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (1, 1, 'Lisinopril', '10mg', 3, '2025-06-01', '2025-12-01')");
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (2, 1, 'Aspirin', '81mg', 6, '2025-06-01', '2025-12-01')");
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (3, 2, 'Metoprolol', '25mg', 2, '2025-06-15', '2025-09-15')");
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (4, 3, 'Vitamin D', '1000IU', 12, '2025-07-01', '2026-07-01')");
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (5, 4, 'Sumatriptan', '50mg', 4, '2025-07-10', '2026-01-10')");
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (6, 5, 'Oseltamivir', '75mg', 0, '2025-08-01', '2025-08-06')");
    }
    ...
    public function testPatientPrescriptionSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS patient_name, rx.drug_name, rx.dosage,
             ...
             FROM pg_pt_patients p
             JOIN pg_pt_visits v ON v.patient_id = p.id
             JOIN pg_pt_prescriptions rx ON rx.visit_id = v.id
             JOIN pg_pt_doctors d ON d.id = v.doctor_id
    ...
    public function testRefillTracking(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT refills_remaining FROM pg_pt_prescriptions WHERE id = 1"
        );
        $this->assertEquals(3, (int) $rows[0]['refills_remaining']);

        // Decrement by 1
        $this->pdo->exec("UPDATE pg_pt_prescriptions SET refills_remaining = refills_remaining - 1 WHERE id = 1");
    ...
    public function testPrescriptionsPerDoctor(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name AS doctor_name,
             ...
             FROM pg_pt_doctors d
             JOIN pg_pt_visits v ON v.doctor_id = d.id
             JOIN pg_pt_prescriptions rx ON rx.visit_id = v.id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_pt_prescriptions (id, visit_id, drug_name, dosage, refills_remaining, start_date, end_date) VALUES (7, 5, 'Amoxicillin', '500mg', 1, '2025-08-01', '2025-08-11')");

        // ZTD sees 7 prescriptions
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_pt_prescriptions");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table is empty
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_pt_prescriptions")->fetchAll(PDO::FETCH_ASSOC);
