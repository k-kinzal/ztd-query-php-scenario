 ...
 * through the CTE shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.179
 */
class PostgresProductCatalogSearchTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_cat_product (
                id INT PRIMARY KEY,
                sku VARCHAR(20),
                name VARCHAR(100),
                category VARCHAR(50),
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (1, 'WDG-001', 'Blue Widget', 'Widgets', 'Acme', 29.99, NULL, 'A standard blue widget', 1)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (2, 'WDG-002', 'Red Widget', 'Widgets', 'Acme', 34.99, 29.99, 'A premium red widget', 1)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (3, 'GDG-001', 'Gadget Pro', 'Gadgets', 'TechCo', 199.99, 179.99, 'Professional grade gadget', 1)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (4, 'GDG-002', 'Gadget Lite', 'Gadgets', 'TechCo', 99.99, NULL, 'Entry-level gadget', 1)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (5, 'WDG-003', 'Green Widget', 'Widgets', 'BestBrand', 24.99, 19.99, NULL, 1)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (6, 'ACC-001', 'Widget Case', 'Accessories', 'Acme', 9.99, NULL, 'Protective case for widgets', 0)");
        $this->pdo->exec("INSERT INTO pg_cat_product VALUES (7, 'ACC-002', 'Gadget Stand', 'Accessories', NULL, 14.99, NULL, 'Universal gadget stand', 1)");
    }

    public function testLikePrefix(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name FROM pg_cat_product WHERE sku LIKE 'WDG%' ORDER BY id"
        );
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name FROM pg_cat_product WHERE name LIKE '%Widget%' ORDER BY id"
        );
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name FROM pg_cat_product
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, category FROM pg_cat_product
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, category, price FROM pg_cat_product
             WHERE is_active = 1
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, price FROM pg_cat_product
             WHERE is_active = 1
             ORDER BY price ASC
             LIMIT 3 OFFSET 3"
        );
    ...
    public function testPreparedLike(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name, category FROM pg_cat_product
             WHERE name LIKE $1 AND is_active = $2
             ORDER BY name",
            ['%Gadget%', 1]
        );
    ...
    {
        $this->ztdExec(
            "UPDATE pg_cat_product SET price = price * 0.90
             WHERE category = 'Widgets' AND discount_price IS NULL"
        );

        $rows = $this->ztdQuery(
            "SELECT name, price FROM pg_cat_product WHERE category = 'Widgets' ORDER BY id"
        );
    ...
    {
        $this->ztdExec("UPDATE pg_cat_product SET price = 0 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT price FROM pg_cat_product WHERE id = 1");
        $this->assertEqualsWithDelta(0.0, (float) $rows[0]['price'], 0.01);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_cat_product")->fetchAll(PDO::FETCH_ASSOC);
