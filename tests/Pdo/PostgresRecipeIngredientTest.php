...
/**
 * Tests a recipe ingredient management workflow through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.86
 */
class PostgresRecipeIngredientTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ri_recipes (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255),
                servings INTEGER
            )',
            'CREATE TABLE pg_ri_ingredients (
                id INTEGER PRIMARY KEY,
                recipe_id INTEGER,
            ...
            'CREATE TABLE pg_ri_substitutions (
    ...
    {
        return ['pg_ri_substitutions', 'pg_ri_ingredients', 'pg_ri_recipes'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 recipes
        $this->pdo->exec("INSERT INTO pg_ri_recipes VALUES (1, 'Pancakes', 4)");
        $this->pdo->exec("INSERT INTO pg_ri_recipes VALUES (2, 'Omelette', 2)");

        // Ingredients for Pancakes (recipe 1)
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (1, 1, 'flour', 2.00, 'cups')");
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (2, 1, 'eggs', 2.00, 'pieces')");
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (3, 1, 'milk', 1.50, 'cups')");
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (4, 1, 'sugar', 0.25, 'cups')");

        // Ingredients for Omelette (recipe 2)
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (5, 2, 'eggs', 3.00, 'pieces')");
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (6, 2, 'milk', 0.25, 'cups')");
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (7, 2, 'cheese', 0.50, 'cups')");

        // Substitutions: flour -> almond flour (1.0x), milk -> oat milk (1.0x)
        $this->pdo->exec("INSERT INTO pg_ri_substitutions VALUES (1, 'almond flour', 1.00)");
        $this->pdo->exec("INSERT INTO pg_ri_substitutions VALUES (3, 'oat milk', 1.00)");
    }
    ...
    public function testRecipeIngredients(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT r.name AS recipe_name, i.item_name, i.quantity, i.unit
             FROM pg_ri_recipes r
             JOIN pg_ri_ingredients i ON i.recipe_id = r.id
             WHERE r.id = ?
             ORDER BY i.id",
            [1]
        );
    ...
    public function testAvailableSubstitutions(): void
    {
        $rows = $this->ztdQuery(
            "SELECT i.item_name, s.substitute_name, s.ratio
             FROM pg_ri_ingredients i
             LEFT JOIN pg_ri_substitutions s ON s.ingredient_id = i.id
             WHERE i.recipe_id = 1
             ORDER BY i.id"
        );
    ...
    public function testAddIngredient(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM pg_ri_ingredients WHERE recipe_id = 2"
        );
        $this->assertEquals(3, (int) $rows[0]['cnt']);

        // Add salt to omelette
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (8, 2, 'salt', 0.50, 'tsp')");
    ...
    public function testIngredientCountPerRecipe(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name, COUNT(i.id) AS ingredient_count
             FROM pg_ri_recipes r
             LEFT JOIN pg_ri_ingredients i ON i.recipe_id = r.id
    ...
    {
        $this->pdo->exec("INSERT INTO pg_ri_ingredients VALUES (8, 2, 'pepper', 0.25, 'tsp')");
        $this->pdo->exec("UPDATE pg_ri_recipes SET servings = 8 WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_ri_ingredients");
        $this->assertSame(8, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT servings FROM pg_ri_recipes WHERE id = 1");
        $this->assertEquals(8, (int) $rows[0]['servings']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_ri_ingredients")->fetchAll(PDO::FETCH_ASSOC);
