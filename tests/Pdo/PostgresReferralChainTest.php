 ...
 * @spec SPEC-10.2.181
 */
class PostgresReferralChainTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_ref_users (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ref_users VALUES (1, 'Alice', NULL, 'organic', 100)");
        $this->pdo->exec("INSERT INTO pg_ref_users VALUES (2, 'Bob', 1, 'referral', 50)");
        $this->pdo->exec("INSERT INTO pg_ref_users VALUES (3, 'Carol', 1, 'referral', 75)");
        $this->pdo->exec("INSERT INTO pg_ref_users VALUES (4, 'Dave', 2, 'referral', 30)");
        $this->pdo->exec("INSERT INTO pg_ref_users VALUES (5, 'Eve', NULL, 'organic', 200)");
    }
    ...
    public function testSelfJoinReferrerLookup(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.name, r.name AS referrer
             FROM pg_ref_users u
             LEFT JOIN pg_ref_users r ON u.referrer_id = r.id
             ORDER BY u.id"
        );
    ...
    public function testThreeLevelSelfJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.name, r.name AS referrer, gr.name AS grand_referrer
             FROM pg_ref_users u
             LEFT JOIN pg_ref_users r ON u.referrer_id = r.id
             LEFT JOIN pg_ref_users gr ON r.referrer_id = gr.id
             ORDER BY u.id"
        );
    ...
    public function testNotInWithNullableColumn(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM pg_ref_users
             WHERE id NOT IN (
                 SELECT referrer_id FROM pg_ref_users WHERE referrer_id IS NOT NULL
             )
             ORDER BY id"
        );
    ...
    public function testNotInWithoutNullFilter(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name
             FROM pg_ref_users
             WHERE id NOT IN (
                 SELECT referrer_id FROM pg_ref_users
             )
             ORDER BY id"
        );
    ...
    {
        $this->ztdExec(
            "INSERT INTO pg_ref_users VALUES (6, 'Frank', 1, 'referral', 0)"
        );
     ...
     *
     * Note: UPDATE WHERE IN (SELECT from same table) may fail on PostgreSQL
     * per SPEC-11.PG-SELF-REF-UPDATE if the CTE rewriter cannot handle the
     * self-referencing subquery in the UPDATE context.
     */
    public function testUpdateArithmeticOnReferrer(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT name, reward_points FROM pg_ref_users ORDER BY id"
        );
    ...
    public function testDeleteWhereReferrerIsNull(): void
    {
        $affected = $this->ztdExec(
            "DELETE FROM pg_ref_users WHERE referrer_id IS NULL"
        );

        $this->assertEquals(2, $affected);

        $rows = $this->ztdQuery(
            "SELECT name FROM pg_ref_users ORDER BY id"
        );
