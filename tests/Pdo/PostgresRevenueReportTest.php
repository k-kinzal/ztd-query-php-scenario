...
/**
 * Tests multi-source revenue report combining online orders and store sales (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.173
 */
class PostgresRevenueReportTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_rev_online_orders (
            ...
            'CREATE TABLE pg_rev_store_sales (
                id SERIAL PRIMARY KEY,
                cashier VARCHAR(100),
                amount NUMERIC(10,2),
    ...
    {
        return ['pg_rev_store_sales', 'pg_rev_online_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Online orders
        $this->pdo->exec("INSERT INTO pg_rev_online_orders VALUES (1, 'Alice', 150.00, '2025-06-01', 'electronics')");
        $this->pdo->exec("INSERT INTO pg_rev_online_orders VALUES (2, 'Bob', 75.50, '2025-06-01', 'clothing')");
        $this->pdo->exec("INSERT INTO pg_rev_online_orders VALUES (3, 'Alice', 200.00, '2025-06-02', 'electronics')");
        $this->pdo->exec("INSERT INTO pg_rev_online_orders VALUES (4, 'Carol', 50.00, '2025-06-03', 'books')");

        // Store sales
        $this->pdo->exec("INSERT INTO pg_rev_store_sales VALUES (1, 'Dave', 300.00, '2025-06-01', 'electronics')");
        $this->pdo->exec("INSERT INTO pg_rev_store_sales VALUES (2, 'Eve', 120.00, '2025-06-02', 'clothing')");
        $this->pdo->exec("INSERT INTO pg_rev_store_sales VALUES (3, 'Dave', 80.00, '2025-06-03', 'books')");
    }
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT 'online' AS source, customer AS person, amount, category FROM pg_rev_online_orders
             UNION ALL
             SELECT 'store' AS source, cashier AS person, amount, category FROM pg_rev_store_sales
    ...
    public function testUnionAllWithGroupBy(): void
    {
        $rows = $this->ztdQuery(
            "SELECT category, SUM(amount) AS total FROM (
                SELECT category, amount FROM pg_rev_online_orders
                UNION ALL
                SELECT category, amount FROM pg_rev_store_sales
            ) combined
            GROUP BY category
            ORDER BY category"
        );
    ...
    public function testConditionalAggregation(): void
    {
        $rows = $this->ztdQuery(
            ...
            FROM (
                SELECT 'online' AS source, category, amount FROM pg_rev_online_orders
                UNION ALL
                SELECT 'store' AS source, category, amount FROM pg_rev_store_sales
            ) all_revenue
            GROUP BY category
            ORDER BY category"
        );
    ...
    {
        $this->ztdExec(
            "INSERT INTO pg_rev_online_orders VALUES (5, 'Frank', 99.99, '2025-06-04', 'clothing')"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_rev_online_orders");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_rev_online_orders")->fetchAll(PDO::FETCH_ASSOC);
