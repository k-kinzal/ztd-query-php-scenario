...
/**
 * Tests a sales commission scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.167
 */
class PostgresSalesCommissionTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_sc_reps (
                id SERIAL PRIMARY KEY,
                name VARCHAR(255),
                team VARCHAR(255)
            )',
            'CREATE TABLE pg_sc_deals (
                id SERIAL PRIMARY KEY,
                rep_id INT,
                client VARCHAR(255),
                amount NUMERIC(12,2),
    ...
    protected function setUp(): void
    {
        ...
        // 3 reps
        $this->pdo->exec("INSERT INTO pg_sc_reps VALUES (1, 'Alice', 'north')");
        $this->pdo->exec("INSERT INTO pg_sc_reps VALUES (2, 'Bob', 'south')");
        $this->pdo->exec("INSERT INTO pg_sc_reps VALUES (3, 'Carol', 'north')");

        // 8 deals
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (1, 1, 'Acme Corp', 50000.00, '2025-01-15', 0.10)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (2, 1, 'Beta Inc', 30000.00, '2025-03-20', 0.10)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (3, 1, 'Gamma LLC', 75000.00, '2025-06-10', 0.12)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (4, 2, 'Delta Co', 20000.00, '2025-02-05', 0.10)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (5, 2, 'Echo Ltd', 45000.00, '2025-04-18', 0.10)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (6, 2, 'Foxtrot SA', 60000.00, '2025-07-22', 0.12)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (7, 3, 'Golf Corp', 35000.00, '2025-05-01', 0.10)");
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (8, 3, 'Hotel Inc', 90000.00, '2025-08-15', 0.15)");
    }
    ...
    public function testDealSequenceByRep(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.rep_id, r.name, d.client, d.close_date,
             ...
             FROM pg_sc_deals d
             JOIN pg_sc_reps r ON r.id = d.rep_id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_sc_deals VALUES (9, 1, 'India Ltd', 40000.00, '2025-09-01', 0.10)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_sc_deals");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_sc_deals')->fetchAll(PDO::FETCH_ASSOC);
