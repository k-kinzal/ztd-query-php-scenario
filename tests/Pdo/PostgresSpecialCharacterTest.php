 ...
 * Unicode, and edge-case string values in CTE-rewritten queries on PostgreSQL.
 *
...
class PostgresSpecialCharacterTest extends AbstractPostgresPdoTestCase
{
    ...
    {
        $this->pdo->exec("INSERT INTO pg_char_test (id, val) VALUES (1, 'it''s a test')");
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO pg_char_test (id, val) VALUES (?, ?)');
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO pg_char_test (id, val) VALUES (?, ?)');
        $stmt->execute([1, "\xF0\x9F\x8E\x89\xF0\x9F\x9A\x80"]);

        $stmt = $this->pdo->query('SELECT val FROM pg_char_test WHERE id = 1');
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame("\xF0\x9F\x8E\x89\xF0\x9F\x9A\x80", $rows[0]['val']);
    }
    ...
    public function testLikeWithSpecialCharacters(): void
    {
        ...
        $stmt = $this->pdo->query("SELECT * FROM pg_char_test WHERE val LIKE '%done'");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        $this->pdo->exec("INSERT INTO pg_char_test (id, val) VALUES (1, 'original')");

        $stmt = $this->pdo->prepare('UPDATE pg_char_test SET val = ? WHERE id = ?');
