 ...
 * gift card, store credit) with partial refunds and SUM integrity validation (PostgreSQL PDO).
 * @spec SPEC-10.2.115
 */
class PostgresSplitPaymentTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_sp_orders (
                id SERIAL PRIMARY KEY,
                customer_name TEXT,
                total_amount NUMERIC(10,2),
                status TEXT
            )',
            'CREATE TABLE pg_sp_payment_splits (
                id SERIAL PRIMARY KEY,
                order_id INT,
                method TEXT,
                amount NUMERIC(10,2),
    ...
    {
        return ['pg_sp_payment_splits', 'pg_sp_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 orders
        $this->pdo->exec("INSERT INTO pg_sp_orders VALUES (1, 'Alice', 200.00, 'paid')");
        $this->pdo->exec("INSERT INTO pg_sp_orders VALUES (2, 'Bob', 150.00, 'paid')");
        $this->pdo->exec("INSERT INTO pg_sp_orders VALUES (3, 'Carol', 80.00, 'pending')");

        // Payment splits for order 1 (sum=200)
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (1, 1, 'credit_card', 120.00, 0.00)");
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (2, 1, 'gift_card', 50.00, 0.00)");
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (3, 1, 'store_credit', 30.00, 0.00)");

        // Payment splits for order 2 (sum=150)
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (4, 2, 'credit_card', 150.00, 0.00)");

        // Payment splits for order 3 (sum=80)
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (5, 3, 'credit_card', 50.00, 0.00)");
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (6, 3, 'gift_card', 30.00, 0.00)");
    }
    ...
    public function testSplitSumMatchesOrderTotal(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.id, o.total_amount, SUM(ps.amount) AS split_total
             FROM pg_sp_orders o
             JOIN pg_sp_payment_splits ps ON ps.order_id = o.id
    ...
    {
        $this->pdo->exec("UPDATE pg_sp_payment_splits SET refunded_amount = 20.00 WHERE order_id = 1 AND method = 'gift_card'");
        $this->pdo->exec("UPDATE pg_sp_orders SET status = 'partially_refunded' WHERE id = 1");
        ...
        $rows = $this->ztdQuery("SELECT status FROM pg_sp_orders WHERE id = 1");
    ...
    public function testMethodPercentagePerOrder(): void
    {
        $rows = $this->ztdQuery(
            "SELECT ps.method, ROUND(ps.amount * 100.0 / o.total_amount, 1) AS pct
             FROM pg_sp_payment_splits ps
             JOIN pg_sp_orders o ON o.id = ps.order_id
             WHERE o.id = 1
             ORDER BY pct DESC"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (7, 3, 'store_credit', 0.00, 0.00)");

        // Count of splits for order 3 should be 3
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM pg_sp_payment_splits WHERE order_id = 3"
        );
        $this->assertEquals(3, (int) $rows[0]['cnt']);

        // SUM should still be 80
        $rows = $this->ztdQuery(
            "SELECT SUM(amount) AS total FROM pg_sp_payment_splits WHERE order_id = 3"
        );
    ...
    {
        $this->pdo->exec("INSERT INTO pg_sp_payment_splits VALUES (7, 3, 'store_credit', 10.00, 0.00)");

        // ZTD sees the new split
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_sp_payment_splits");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_sp_payment_splits")->fetchAll(PDO::FETCH_ASSOC);
