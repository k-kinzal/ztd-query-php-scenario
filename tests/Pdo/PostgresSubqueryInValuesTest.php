 ...
 * @spec SPEC-3.3
 */
class PostgresSubqueryInValuesTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_siv_items (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                score INT NOT NULL DEFAULT 0
            )',
            'CREATE TABLE pg_siv_counters (
                id INT PRIMARY KEY,
    ...
    {
        return ['pg_siv_counters', 'pg_siv_items'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_siv_items VALUES (1, 'Alpha', 10)");
        $this->pdo->exec("INSERT INTO pg_siv_items VALUES (2, 'Bravo', 20)");
        $this->pdo->exec("INSERT INTO pg_siv_items VALUES (3, 'Charlie', 30)");
    }
    ...
    public function testInsertWithMaxIdSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_siv_items (id, name, score) VALUES ((SELECT MAX(id) FROM pg_siv_items) + 1, 'Delta', 40)"
            );

            $rows = $this->ztdQuery("SELECT id, name, score FROM pg_siv_items ORDER BY id");
    ...
    public function testInsertWithCountSubquery(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_siv_counters (id, label, total) VALUES (1, 'item_count', (SELECT COUNT(*) FROM pg_siv_items))"
            );

            $rows = $this->ztdQuery("SELECT total FROM pg_siv_counters WHERE id = 1");
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_siv_items VALUES (10, 'Zulu', 99)");

            $this->pdo->exec(
                "INSERT INTO pg_siv_items (id, name, score) VALUES ((SELECT MAX(id) FROM pg_siv_items) + 1, 'Beyond', 100)"
            );

            $rows = $this->ztdQuery("SELECT id, name FROM pg_siv_items WHERE name = 'Beyond'");
    ...
    public function testInsertWithSumSubqueryInValues(): void
    {
        try {
            $this->pdo->exec(
                "INSERT INTO pg_siv_counters (id, label, total) VALUES (2, 'high_score_sum', (SELECT SUM(score) FROM pg_siv_items WHERE score >= 20))"
            );

            $rows = $this->ztdQuery("SELECT total FROM pg_siv_counters WHERE id = 2");
    ...
    public function testPreparedInsertWithSubqueryInValues(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "INSERT INTO pg_siv_counters (id, label, total) VALUES (?, ?, (SELECT COUNT(*) FROM pg_siv_items WHERE score > ?))"
            );
            $stmt->execute([3, 'above_threshold', 15]);

            $rows = $this->ztdQuery("SELECT total FROM pg_siv_counters WHERE id = 3");
