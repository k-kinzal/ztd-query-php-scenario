 ...
 * @spec SPEC-3.3
 */
class PostgresThreeTableJoinChainTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_ttj_authors (id INT PRIMARY KEY, name VARCHAR(50))',
            'CREATE TABLE pg_ttj_books (id INT PRIMARY KEY, author_id INT, title VARCHAR(100), genre VARCHAR(30))',
            'CREATE TABLE pg_ttj_reviews (id INT PRIMARY KEY, book_id INT, rating INT, reviewer VARCHAR(50))',
        ];
    }

    protected function getTableNames(): array
    {
        return ['pg_ttj_reviews', 'pg_ttj_books', 'pg_ttj_authors'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ttj_authors (id, name) VALUES (1, 'Tolkien'), (2, 'Asimov'), (3, 'Bradbury')");
        $this->pdo->exec("INSERT INTO pg_ttj_books (id, author_id, title, genre) VALUES
            (1, 1, 'The Hobbit', 'Fantasy'), (2, 1, 'LOTR', 'Fantasy'),
            (3, 2, 'Foundation', 'SciFi'), (4, 3, 'Fahrenheit 451', 'SciFi')");
        $this->pdo->exec("INSERT INTO pg_ttj_reviews (id, book_id, rating, reviewer) VALUES
    ...
    public function testThreeTableInnerJoin(): void
    {
        $rows = $this->ztdQuery(
            'SELECT a.name AS author, b.title, r.rating
             FROM pg_ttj_authors a
             JOIN pg_ttj_books b ON b.author_id = a.id
             JOIN pg_ttj_reviews r ON r.book_id = b.id
    ...
    public function testThreeTableLeftJoin(): void
    {
        $rows = $this->ztdQuery(
            'SELECT a.name, b.title, COUNT(r.id) AS reviews
             FROM pg_ttj_authors a
             LEFT JOIN pg_ttj_books b ON b.author_id = a.id
             LEFT JOIN pg_ttj_reviews r ON r.book_id = b.id
    ...
    public function testThreeTableAfterMutations(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_ttj_books (id, author_id, title, genre) VALUES (5, 3, 'The Martian Chronicles', 'SciFi')");
        $this->pdo->exec("INSERT INTO pg_ttj_reviews (id, book_id, rating, reviewer) VALUES (7, 5, 4, 'Grace')");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM pg_ttj_authors');
        $this->assertSame(0, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM pg_ttj_books');
        $this->assertSame(0, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM pg_ttj_reviews');
        $this->assertSame(0, (int) $rows[0]['cnt']);
    }
}
