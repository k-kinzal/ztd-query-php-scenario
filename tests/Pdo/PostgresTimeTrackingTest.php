...
/**
 * Tests a time-tracking workflow through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.80
 */
class PostgresTimeTrackingTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_tt_clients (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255),
                rate_per_hour NUMERIC(10,2)
            )',
            'CREATE TABLE pg_tt_projects (
                id INTEGER PRIMARY KEY,
                client_id INTEGER,
                name VARCHAR(255),
                budget_hours NUMERIC(8,2)
            )',
            'CREATE TABLE pg_tt_time_entries (
                id INTEGER PRIMARY KEY,
                project_id INTEGER,
    ...
    {
        return ['pg_tt_time_entries', 'pg_tt_projects', 'pg_tt_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_tt_clients VALUES (1, 'Acme Corp', 150.00)");
        $this->pdo->exec("INSERT INTO pg_tt_clients VALUES (2, 'Globex Inc', 200.00)");

        $this->pdo->exec("INSERT INTO pg_tt_projects VALUES (1, 1, 'Website Redesign', 40.00)");
        $this->pdo->exec("INSERT INTO pg_tt_projects VALUES (2, 1, 'Mobile App', 100.00)");
        $this->pdo->exec("INSERT INTO pg_tt_projects VALUES (3, 2, 'Data Migration', 20.00)");

        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (1, 1, 'Alice', 8.00, '2026-03-02 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (2, 1, 'Alice', 6.50, '2026-03-03 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (3, 1, 'Bob', 7.00, '2026-03-02 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (4, 1, 'Bob', 2.00, '2026-03-04 09:00:00', FALSE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (5, 2, 'Alice', 5.00, '2026-03-02 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (6, 2, 'Charlie', 8.00, '2026-03-03 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (7, 3, 'Charlie', 10.00, '2026-03-02 09:00:00', TRUE)");
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (8, 3, 'Charlie', 12.00, '2026-03-03 09:00:00', TRUE)");
    }
    ...
    public function testBillableHoursByProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS project_name, SUM(te.hours) AS total_billable
             FROM pg_tt_time_entries te
             JOIN pg_tt_projects p ON p.id = te.project_id
    ...
    public function testClientInvoiceSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.name AS client_name,
             ...
             FROM pg_tt_time_entries te
             JOIN pg_tt_projects p ON p.id = te.project_id
             JOIN pg_tt_clients c ON c.id = p.client_id
             WHERE te.billable = TRUE
    ...
    public function testAddTimeEntry(): void
    {
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM pg_tt_time_entries WHERE project_id = 2 AND billable = TRUE"
        );
        $this->assertEquals(13.00, (float) $before[0]['total']);

        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (9, 2, 'Diana', 4.00, '2026-03-04 09:00:00', TRUE)");
    ...
    /**
     * UPDATE billable=FALSE, verify billable SUM decreases.
     */
    public function testMarkNonBillable(): void
    {
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM pg_tt_time_entries WHERE project_id = 1 AND billable = TRUE"
        );
        $this->assertEquals(21.50, (float) $before[0]['total']);

        $affected = $this->pdo->exec("UPDATE pg_tt_time_entries SET billable = FALSE WHERE id = 1");
    ...
    {
        $this->pdo->exec("INSERT INTO pg_tt_time_entries VALUES (9, 1, 'Eve', 3.00, '2026-03-05 09:00:00', TRUE)");
        $this->pdo->exec("UPDATE pg_tt_time_entries SET billable = FALSE WHERE id = 2");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_tt_time_entries");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_tt_time_entries')->fetchAll(PDO::FETCH_ASSOC);
