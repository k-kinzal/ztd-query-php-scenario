...
/**
 * Tests SELECT ... UNION ALL / UNION from shadow-modified tables on PostgreSQL.
 *
 * @spec SPEC-4.2
 */
class PostgresUnionSelectAfterDmlTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_usd_users (
                id INT PRIMARY KEY,
                name VARCHAR(50) NOT NULL,
                role VARCHAR(20) NOT NULL
            )',
            'CREATE TABLE pg_usd_logs (
                id INT PRIMARY KEY,
    ...
    {
        return ['pg_usd_logs', 'pg_usd_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_usd_users VALUES (1, 'Alice', 'admin')");
        $this->pdo->exec("INSERT INTO pg_usd_users VALUES (2, 'Bob', 'user')");
        $this->pdo->exec("INSERT INTO pg_usd_users VALUES (3, 'Carol', 'user')");

        $this->pdo->exec("INSERT INTO pg_usd_logs VALUES (1, 'Alice', 'login')");
        $this->pdo->exec("INSERT INTO pg_usd_logs VALUES (2, 'Bob', 'login')");
    }

    public function testUnionAllSameTableAfterInsert(): void
    {
        try {
            $this->pdo->exec("INSERT INTO pg_usd_users VALUES (4, 'Dave', 'admin')");

            $rows = $this->ztdQuery(
                "SELECT name FROM pg_usd_users WHERE role = 'admin'
                 UNION ALL
                 SELECT name FROM pg_usd_users WHERE role = 'user'
                 ORDER BY name"
            );
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO pg_usd_users VALUES (4, 'Dave', 'user')");
            $this->pdo->exec("INSERT INTO pg_usd_logs VALUES (3, 'Carol', 'signup')");

            $rows = $this->ztdQuery(
                "SELECT name AS label FROM pg_usd_users
                 UNION ALL
                 SELECT user_name AS label FROM pg_usd_logs
                 ORDER BY label"
            );
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_usd_users SET role = 'admin' WHERE id = 2");

            $rows = $this->ztdQuery(
                "SELECT name FROM pg_usd_users WHERE role = 'admin'
                 UNION
                 SELECT name FROM pg_usd_users WHERE id <= 3
                 ORDER BY name"
            );
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM pg_usd_users WHERE id = 2");
