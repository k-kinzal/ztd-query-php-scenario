...
class PostgresUpdateSelfReferencingArithmeticTest extends AbstractPostgresPdoTestCase
{
    ...
    {
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 5 WHERE id = 1');

        $stmt = $this->pdo->query('SELECT counter FROM pg_selfref_test WHERE id = 1');
    ...
    {
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 1 WHERE id = 1');
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 1 WHERE id = 1');
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 1 WHERE id = 1');
    ...
    {
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 100');

        $stmt = $this->pdo->query('SELECT counter FROM pg_selfref_test ORDER BY id');
    ...
    {
        $this->pdo->exec("INSERT INTO pg_selfref_test VALUES (4, 'Diana', 0, 0.00)");
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 50 WHERE id = 4');

        $stmt = $this->pdo->query('SELECT counter FROM pg_selfref_test WHERE id = 4');
    ...
    {
        $this->pdo->exec('UPDATE pg_selfref_test SET balance = balance + counter WHERE id = 1');

        $stmt = $this->pdo->query('SELECT balance FROM pg_selfref_test WHERE id = 1');
    ...
    {
        $this->pdo->exec('UPDATE pg_selfref_test SET counter = counter + 999');
