...
/**
 * Tests UPDATE with correlated subqueries in SET and WHERE clauses through ZTD CTE shadow store (PostgreSQL PDO).
 ...
 *
 * NOTE: Deliberately avoids UPDATE ... SET ... FROM ... syntax (SPEC-11.PG-UPDATE-SET-FROM-KEYWORD).
 */
class PostgresUpdateSubqueryTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_usq_categories (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                discount_pct NUMERIC(5,2) NOT NULL
            )',
            'CREATE TABLE pg_usq_products (
                id INTEGER PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
    ...
    {
        return ['pg_usq_products', 'pg_usq_categories'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_usq_categories VALUES (1, 'Electronics', 5.0)");
        $this->pdo->exec("INSERT INTO pg_usq_categories VALUES (2, 'Office', 10.0)");
        $this->pdo->exec("INSERT INTO pg_usq_categories VALUES (3, 'Accessories', 15.0)");

        // 6 products across categories (2 per category)
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (1, 'Laptop', 1000.00, 1)");
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (2, 'Phone', 800.00, 1)");
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (3, 'Desk', 500.00, 2)");
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (4, 'Chair', 300.00, 2)");
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (5, 'Cable', 20.00, 3)");
        $this->pdo->exec("INSERT INTO pg_usq_products VALUES (6, 'Case', 50.00, 3)");
    }
    ...
    public function testUpdateSetCorrelatedSubqueryDiscount(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE pg_usq_products SET price = price * (1 - (SELECT discount_pct / 100.0 FROM pg_usq_categories WHERE id = pg_usq_products.category_id))"
            );
        ...
        $rows = $this->ztdQuery("SELECT id, name, price FROM pg_usq_products ORDER BY id");
    ...
    public function testUpdateWhereInSubquery(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE pg_usq_products SET price = price * 0.9 WHERE category_id IN (SELECT id FROM pg_usq_categories WHERE name = 'Electronics')"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
    ...
    public function testUpdateSetScalarSubquerySameTable(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE pg_usq_products SET price = (SELECT MAX(price) FROM pg_usq_products p2 WHERE p2.category_id = pg_usq_products.category_id) WHERE name = 'Phone'"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
    ...
    public function testUpdateZeroRowsSubqueryReturnsEmpty(): void
    {
        ...
        try {
            $affectedRows = $this->pdo->exec(
                "UPDATE pg_usq_products SET price = 0.00 WHERE category_id IN (SELECT id FROM pg_usq_categories WHERE name = 'Furniture')"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
        ...
        $rows = $this->ztdQuery("SELECT id, price FROM pg_usq_products ORDER BY id");
        $this->assertCount(6, $rows);
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $shadowRows = $this->ztdQuery("SELECT id, price FROM pg_usq_products WHERE id = 1");
        ...
        $this->pdo->disableZtd();
        $physicalRows = $this->pdo->query('SELECT id, price FROM pg_usq_products')->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testUpdateCorrelatedSetWithWhereFilter(): void
    {
        ...
        try {
            $this->pdo->exec(
                "UPDATE pg_usq_products SET price = price * (1 - (SELECT discount_pct / 100.0 FROM pg_usq_categories WHERE id = pg_usq_products.category_id)) WHERE price > 100"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
