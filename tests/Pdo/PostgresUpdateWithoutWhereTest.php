...
/**
 * Tests UPDATE without WHERE clause behavior on PostgreSQL PDO.
 *
 ...
 * @spec SPEC-4.2
 */
class PostgresUpdateWithoutWhereTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE pg_uww_test (id INT PRIMARY KEY, name VARCHAR(50), status VARCHAR(20))';
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_uww_test VALUES (1, 'Alice', 'active')");
        $this->pdo->exec("INSERT INTO pg_uww_test VALUES (2, 'Bob', 'active')");
        $this->pdo->exec("INSERT INTO pg_uww_test VALUES (3, 'Charlie', 'active')");
    }
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_uww_test SET status = 'inactive'");

            $rows = $this->pdo->query("SELECT id, status FROM pg_uww_test ORDER BY id")
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_uww_test SET status = 'inactive' WHERE 1=1");
    ...
    {
        try {
            $this->pdo->exec("UPDATE pg_uww_test SET status = 'inactive' WHERE id = 1");
    ...
    {
        try {
            $affected = $this->pdo->exec("UPDATE pg_uww_test SET status = 'inactive'");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_uww_test")->fetchAll(PDO::FETCH_ASSOC);
