...
/**
 * Tests a tenant usage metering scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.156
 */
class PostgresUsageMeteringTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_um_tenants (
                id SERIAL PRIMARY KEY,
                name VARCHAR(100),
                plan_name VARCHAR(100),
                monthly_quota INTEGER
            )',
            'CREATE TABLE pg_um_usage_records (
                id SERIAL PRIMARY KEY,
                tenant_id INTEGER,
            ...
            'CREATE TABLE pg_um_overage_charges (
                id SERIAL PRIMARY KEY,
                tenant_id INTEGER,
                month VARCHAR(10),
    ...
    {
        return ['pg_um_overage_charges', 'pg_um_usage_records', 'pg_um_tenants'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Tenants
        $this->pdo->exec("INSERT INTO pg_um_tenants VALUES (1, 'Acme Corp', 'pro', 10000)");
        $this->pdo->exec("INSERT INTO pg_um_tenants VALUES (2, 'Beta Inc', 'starter', 1000)");
        $this->pdo->exec("INSERT INTO pg_um_tenants VALUES (3, 'Gamma LLC', 'pro', 10000)");
        $this->pdo->exec("INSERT INTO pg_um_tenants VALUES (4, 'Delta Co', 'enterprise', 50000)");

        // Usage records
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (1, 1, '/api/users', '2026-01-15', 1500)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (2, 1, '/api/users', '2026-01-20', 1200)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (3, 1, '/api/orders', '2026-01-18', 800)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (4, 1, '/api/orders', '2026-01-25', 900)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (5, 2, '/api/users', '2026-01-10', 600)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (6, 2, '/api/users', '2026-01-22', 500)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (7, 3, '/api/users', '2026-01-12', 2000)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (8, 3, '/api/users', '2026-01-28', 1800)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (9, 3, '/api/orders', '2026-01-16', 1500)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (10, 3, '/api/orders', '2026-01-30', 1200)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (11, 4, '/api/users', '2026-01-05', 5000)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (12, 4, '/api/users', '2026-01-19', 4500)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (13, 4, '/api/orders', '2026-01-10', 3000)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (14, 4, '/api/orders', '2026-01-26', 2500)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (15, 1, '/api/users', '2026-02-05', 1600)");
        $this->pdo->exec("INSERT INTO pg_um_usage_records VALUES (16, 2, '/api/users', '2026-02-12', 700)");

        // Overage charges
        $this->pdo->exec("INSERT INTO pg_um_overage_charges VALUES (1, 2, '2026-01', 100, 10.00)");
    }
    ...
    public function testMonthlyUsageSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name, SUBSTR(u.recorded_at, 1, 7) AS month,
             ...
             FROM pg_um_tenants t
             JOIN pg_um_usage_records u ON u.tenant_id = t.id
             GROUP BY t.name, SUBSTR(u.recorded_at, 1, 7)
     ...
     * total_used, quota, utilization_pct (ROUND with CAST to NUMERIC for division),
    ...
    public function testQuotaUtilization(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name,
                    ...
                    ROUND(CAST(SUM(u.request_count) AS NUMERIC) * 100.0 / t.monthly_quota, 1) AS utilization_pct,
    ...
    public function testOverageChargeJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name,
             ...
             FROM pg_um_tenants t
             LEFT JOIN pg_um_overage_charges oc ON oc.tenant_id = t.id AND oc.month = '2026-01'
             ORDER BY t.name"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec(
            "INSERT INTO pg_um_usage_records VALUES (17, 1, '/api/users', '2026-02-10', 500)"
        );

        // ZTD sees the new record (16 original + 1 new = 17)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_um_usage_records");
        $this->assertEquals(17, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM pg_um_usage_records')->fetchAll(PDO::FETCH_ASSOC);
