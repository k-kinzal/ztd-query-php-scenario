...
/**
 * Tests a waitlist reservation scenario through ZTD shadow store (PostgreSQL PDO).
 ...
 * @spec SPEC-10.2.169
 */
class PostgresWaitlistReservationTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE pg_wr_tables (
                id SERIAL PRIMARY KEY,
                name VARCHAR(255),
                capacity INT,
                location VARCHAR(255)
            )',
            'CREATE TABLE pg_wr_reservations (
                id SERIAL PRIMARY KEY,
                table_id INT,
    ...
    {
        return ['pg_wr_waitlist', 'pg_wr_reservations', 'pg_wr_tables'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 5 tables
        $this->pdo->exec("INSERT INTO pg_wr_tables VALUES (1, 'T1', 2, 'patio')");
        $this->pdo->exec("INSERT INTO pg_wr_tables VALUES (2, 'T2', 4, 'indoor')");
        $this->pdo->exec("INSERT INTO pg_wr_tables VALUES (3, 'T3', 4, 'indoor')");
        $this->pdo->exec("INSERT INTO pg_wr_tables VALUES (4, 'T4', 6, 'patio')");
        $this->pdo->exec("INSERT INTO pg_wr_tables VALUES (5, 'T5', 8, 'private')");

        // 7 reservations
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (1, 2, 'Alice', 3, '18:00', 'confirmed')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (2, 3, 'Bob', 4, '18:00', 'confirmed')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (3, 4, 'Carol', 5, '18:00', 'confirmed')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (4, 5, 'Dave', 8, '18:00', 'confirmed')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (5, 2, 'Eve', 2, '20:00', 'confirmed')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (6, 1, 'Frank', 2, '18:00', 'cancelled')");
        $this->pdo->exec("INSERT INTO pg_wr_reservations VALUES (7, 3, 'Grace', 3, '20:00', 'confirmed')");

        // 5 waitlist entries
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (1, 'Hank', 2, '18:00', 1, 'waiting')");
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (2, 'Ivy', 4, '18:00', 2, 'waiting')");
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (3, 'Jack', 6, '20:00', 1, 'waiting')");
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (4, 'Kate', 2, '20:00', 3, 'waiting')");
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (5, 'Leo', 3, '18:00', 2, 'seated')");
    }
    ...
    public function testAvailableTablesNotExists(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name, t.capacity, t.location
             FROM pg_wr_tables t
             WHERE NOT EXISTS (
                 SELECT 1 FROM pg_wr_reservations r
        ...
        $rows = $this->ztdQuery(
            "SELECT r.guest_name, r.party_size, t.capacity,
                CASE
                    ...
                    END
                END AS fit_status
             FROM pg_wr_reservations r
             JOIN pg_wr_tables t ON t.id = r.table_id
             ORDER BY r.id"
        );
    ...
    public function testScalarSubqueriesInSelect(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name,
                (SELECT COUNT(*) FROM pg_wr_reservations r WHERE r.table_id = t.id AND r.status = 'confirmed') AS active_bookings,
                (SELECT COUNT(*) FROM pg_wr_reservations r WHERE r.table_id = t.id AND r.status = 'cancelled') AS cancelled_bookings
             FROM pg_wr_tables t
             ORDER BY t.id"
        );
    ...
    {
        $this->pdo->exec(
            "UPDATE pg_wr_waitlist
             SET status = 'notified'
             WHERE status = 'waiting'
               AND party_size <= (
                   SELECT MIN(t.capacity) FROM pg_wr_tables t
                   WHERE NOT EXISTS (
                       SELECT 1 FROM pg_wr_reservations r
                       WHERE r.table_id = t.id
                         AND r.time_slot = pg_wr_waitlist.requested_time
        ...
        $rows = $this->ztdQuery(
            "SELECT w.guest_name, w.status FROM pg_wr_waitlist w ORDER BY w.id"
        );
    ...
    {
        $this->pdo->exec("DELETE FROM pg_wr_reservations WHERE status = 'cancelled'");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_wr_reservations");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM pg_wr_reservations WHERE table_id = 1 AND status = 'confirmed'"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_wr_waitlist VALUES (6, 'Mia', 2, '20:00', 1, 'waiting')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM pg_wr_waitlist");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM pg_wr_waitlist")->fetchAll(PDO::FETCH_ASSOC);
