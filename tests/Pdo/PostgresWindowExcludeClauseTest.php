...
/**
 * Tests window frame EXCLUDE clause variants through ZTD shadow on PostgreSQL.
 *
 ...
 *
 * PostgreSQL supports all four since version 11. These are tested on data
 * inserted via the ZTD shadow store to verify the CTE rewriter handles
 * window functions with EXCLUDE clauses correctly.
 *
 * @spec SPEC-10.2
 */
class PostgresWindowExcludeClauseTest extends AbstractPostgresPdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE pg_wec_scores (
                id INTEGER PRIMARY KEY,
                player TEXT NOT NULL,
                score INTEGER NOT NULL
            )",
            "CREATE TABLE pg_wec_target (
                id INTEGER PRIMARY KEY,
                player TEXT NOT NULL,
    ...
    {
        return ['pg_wec_target', 'pg_wec_scores'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (1, 'Alice', 10)");
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (2, 'Bob', 20)");
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (3, 'Charlie', 20)");
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (4, 'Diana', 30)");
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (5, 'Eve', 40)");
        $this->pdo->exec("INSERT INTO pg_wec_scores (id, player, score) VALUES (6, 'Frank', 50)");
    }
    ...
    public function testExcludeCurrentRowAfterUpdate(): void
    {
        try {
            ...
            $this->ztdExec("UPDATE pg_wec_scores SET score = 15 WHERE player = 'Alice'");
            $this->ztdExec("UPDATE pg_wec_scores SET score = 35 WHERE player = 'Diana'");
    ...
    public function testExcludeInSubqueryFeedingInsert(): void
    {
        try {
            $this->ztdExec("
                INSERT INTO pg_wec_target (id, player, adjusted_score)
                SELECT id, player,
                    score + COALESCE(
                        SUM(score) OVER (
                            ORDER BY score, id
            ...
            $rows = $this->ztdQuery("SELECT id, player, adjusted_score FROM pg_wec_target ORDER BY adjusted_score");
