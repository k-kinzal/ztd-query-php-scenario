...
class SqliteAppointmentSchedulingTest extends AbstractSqlitePdoTestCase
{
    ...
    {
        return [
            'CREATE TABLE sl_appt_room (
                id INTEGER PRIMARY KEY,
                name TEXT,
                capacity INTEGER,
                building TEXT
            )',
            'CREATE TABLE sl_appt_booking (
                id INTEGER PRIMARY KEY,
                room_id INTEGER,
                title TEXT,
                organizer TEXT,
    ...
    {
        return ['sl_appt_booking', 'sl_appt_room'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_appt_room VALUES (1, 'Alpha', 10, 'HQ')");
        $this->pdo->exec("INSERT INTO sl_appt_room VALUES (2, 'Beta', 6, 'HQ')");
        $this->pdo->exec("INSERT INTO sl_appt_room VALUES (3, 'Gamma', 20, 'Annex')");

        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (1, 1, 'Sprint Planning', 'alice', '2025-03-10 09:00', '2025-03-10 10:00', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (2, 1, 'Design Review', 'bob', '2025-03-10 10:30', '2025-03-10 11:30', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (3, 2, 'Standup', 'carol', '2025-03-10 09:00', '2025-03-10 09:15', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (4, 1, 'Lunch Talk', 'dave', '2025-03-10 12:00', '2025-03-10 13:00', 'tentative', NULL)");
        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (5, 3, 'All Hands', 'alice', '2025-03-10 14:00', '2025-03-10 15:00', 'confirmed', NULL)");
        $this->pdo->exec("INSERT INTO sl_appt_booking VALUES (6, 2, 'Retrospective', 'bob', '2025-03-10 15:00', '2025-03-10 16:00', 'cancelled', NULL)");
    }
    ...
    public function testBetweenTimeRange(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.title, b.start_time, r.name AS room
             FROM sl_appt_booking b
             JOIN sl_appt_room r ON r.id = b.room_id
    ...
    public function testExistsSubquery(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name, r.building
             FROM sl_appt_room r
             WHERE EXISTS (
                 SELECT 1 FROM sl_appt_booking b
    ...
    {
        $this->ztdExec(
            "UPDATE sl_appt_booking SET status = 'cancelled'
             WHERE start_time BETWEEN '2025-03-10 09:00' AND '2025-03-10 11:59'
               AND status != 'cancelled'"
        );

        $rows = $this->ztdQuery(
            "SELECT title, status FROM sl_appt_booking ORDER BY id"
        );
    ...
    public function testCountCaseStatusSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name,
             ...
             FROM sl_appt_room r
             LEFT JOIN sl_appt_booking b ON b.room_id = r.id
    ...
    {
        $this->ztdExec("DELETE FROM sl_appt_booking WHERE status = 'cancelled'");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_appt_booking");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_appt_booking")->fetchAll(PDO::FETCH_ASSOC);
