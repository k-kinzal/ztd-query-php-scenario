...
/**
 * Tests appointment scheduling scenarios through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.135
 */
class SqliteAppointmentSchedulingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_as_providers (
                id INTEGER PRIMARY KEY,
                provider_name TEXT,
                specialty TEXT
            )',
            'CREATE TABLE sl_as_slots (
                id INTEGER PRIMARY KEY,
                provider_id INTEGER,
                slot_date TEXT,
                start_time TEXT,
            ...
            'CREATE TABLE sl_as_appointments (
                id INTEGER PRIMARY KEY,
                slot_id INTEGER,
                patient_name TEXT,
    ...
    {
        return ['sl_as_appointments', 'sl_as_slots', 'sl_as_providers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_as_providers VALUES (1, 'Dr. Smith', 'General')");
        $this->pdo->exec("INSERT INTO sl_as_providers VALUES (2, 'Dr. Patel', 'Cardiology')");

        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (1, 1, '2025-10-06', '09:00', '09:30', 0)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (2, 1, '2025-10-06', '09:30', '10:00', 0)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (3, 1, '2025-10-06', '10:00', '10:30', 1)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (4, 1, '2025-10-06', '10:30', '11:00', 1)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (5, 1, '2025-10-06', '11:00', '11:30', 0)");

        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (6, 2, '2025-10-06', '14:00', '14:30', 0)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (7, 2, '2025-10-06', '14:30', '15:00', 1)");
        $this->pdo->exec("INSERT INTO sl_as_slots VALUES (8, 2, '2025-10-06', '15:00', '15:30', 1)");

        $this->pdo->exec("INSERT INTO sl_as_appointments VALUES (1, 1, 'Alice', 'checkup', '2025-10-01', 'confirmed')");
        $this->pdo->exec("INSERT INTO sl_as_appointments VALUES (2, 2, 'Bob', 'follow-up', '2025-10-02', 'confirmed')");
        $this->pdo->exec("INSERT INTO sl_as_appointments VALUES (3, 5, 'Carol', 'checkup', '2025-10-03', 'cancelled')");
        $this->pdo->exec("INSERT INTO sl_as_appointments VALUES (4, 6, 'Dave', 'consultation', '2025-10-04', 'confirmed')");
    }
    ...
    public function testAvailableSlotsForProvider(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.id, s.start_time, s.end_time, p.provider_name
             FROM sl_as_slots s
             JOIN sl_as_providers p ON p.id = s.provider_id
    ...
    public function testProviderScheduleSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.provider_name,
             ...
             FROM sl_as_providers p
             JOIN sl_as_slots s ON s.provider_id = p.id
    ...
    public function testActiveAppointments(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.id, a.patient_name, a.appointment_type, p.provider_name
             FROM sl_as_appointments a
             JOIN sl_as_slots s ON s.id = a.slot_id
             JOIN sl_as_providers p ON p.id = s.provider_id
    ...
    {
        $this->pdo->exec("INSERT INTO sl_as_appointments VALUES (5, 3, 'Eve', 'checkup', '2025-10-05', 'confirmed')");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_as_appointments");
        $this->assertSame(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_as_appointments")->fetchAll(PDO::FETCH_ASSOC);
