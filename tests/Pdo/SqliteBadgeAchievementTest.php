...
/**
 * Tests a badge achievement system with progress tracking through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.141
 */
class SqliteBadgeAchievementTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ba_users (
                id INTEGER PRIMARY KEY,
                username TEXT
            )',
            'CREATE TABLE sl_ba_badges (
                id INTEGER PRIMARY KEY,
                name TEXT,
                description TEXT,
                threshold INTEGER
            )',
            'CREATE TABLE sl_ba_user_badges (
                id INTEGER PRIMARY KEY,
                user_id INTEGER,
                badge_id INTEGER,
                progress INTEGER,
    ...
    {
        return ['sl_ba_user_badges', 'sl_ba_badges', 'sl_ba_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Users
        $this->pdo->exec("INSERT INTO sl_ba_users VALUES (1, 'alice')");
        $this->pdo->exec("INSERT INTO sl_ba_users VALUES (2, 'bob')");
        $this->pdo->exec("INSERT INTO sl_ba_users VALUES (3, 'carol')");
        $this->pdo->exec("INSERT INTO sl_ba_users VALUES (4, 'dave')");

        // Badges
        $this->pdo->exec("INSERT INTO sl_ba_badges VALUES (1, 'First Post', 'Write your first post', 1)");
        $this->pdo->exec("INSERT INTO sl_ba_badges VALUES (2, 'Prolific Writer', 'Write 10 posts', 10)");
        $this->pdo->exec("INSERT INTO sl_ba_badges VALUES (3, 'Helpful Hand', 'Answer 5 questions', 5)");
        $this->pdo->exec("INSERT INTO sl_ba_badges VALUES (4, 'Veteran', 'Log in 100 days', 100)");

        // User badges
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (1, 1, 1, 1, '2025-01-05')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (2, 1, 2, 7, NULL)");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (3, 1, 3, 5, '2025-03-20')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (4, 1, 4, 42, NULL)");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (5, 2, 1, 1, '2025-02-10')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (6, 2, 2, 10, '2025-06-01')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (7, 2, 3, 3, NULL)");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (8, 3, 1, 1, '2025-01-20')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (9, 3, 4, 100, '2025-08-15')");
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (10, 4, 1, 1, '2025-04-01')");
    }
    ...
    public function testUserBadgeProgress(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.username,
             ...
             FROM sl_ba_users u
             JOIN sl_ba_user_badges ub ON ub.user_id = u.id
             JOIN sl_ba_badges b ON b.id = ub.badge_id
    ...
    public function testProgressUpdate(): void
    {
        ...
        $this->pdo->exec("UPDATE sl_ba_user_badges SET progress = 10 WHERE id = 2");
        // Mark as unlocked
        $this->pdo->exec("UPDATE sl_ba_user_badges SET unlocked_at = '2025-09-01' WHERE id = 2");
    ...
    {
        $this->pdo->exec("INSERT INTO sl_ba_user_badges VALUES (11, 4, 2, 1, NULL)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_ba_user_badges");
        $this->assertSame(11, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ba_user_badges")->fetchAll(PDO::FETCH_ASSOC);
