...
/**
 * Tests a classroom quiz scoring scenario through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.150
 */
class SqliteClassroomQuizScoringTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_cqs_students (
                id INTEGER PRIMARY KEY,
                name TEXT,
                grade_level INTEGER
            )',
            'CREATE TABLE sl_cqs_quizzes (
                id INTEGER PRIMARY KEY,
                title TEXT,
                subject TEXT,
                passing_score INTEGER
            )',
            'CREATE TABLE sl_cqs_questions (
                id INTEGER PRIMARY KEY,
                quiz_id INTEGER,
                question_text TEXT,
                correct_answer TEXT
            )',
            'CREATE TABLE sl_cqs_answers (
                id INTEGER PRIMARY KEY,
                student_id INTEGER,
                question_id INTEGER,
                quiz_id INTEGER,
    ...
    {
        return ['sl_cqs_answers', 'sl_cqs_questions', 'sl_cqs_quizzes', 'sl_cqs_students'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Students
        $this->pdo->exec("INSERT INTO sl_cqs_students VALUES (1, 'Alice', 10)");
        $this->pdo->exec("INSERT INTO sl_cqs_students VALUES (2, 'Bob', 10)");
        $this->pdo->exec("INSERT INTO sl_cqs_students VALUES (3, 'Carol', 10)");
        $this->pdo->exec("INSERT INTO sl_cqs_students VALUES (4, 'Dave', 11)");

        // Quiz 1: Math (pass = 60%)
        $this->pdo->exec("INSERT INTO sl_cqs_quizzes VALUES (1, 'Math Basics', 'math', 60)");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (1, 1, '2 + 2 = ?', '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (2, 1, '3 * 5 = ?', '15')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (3, 1, '10 / 2 = ?', '5')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (4, 1, '7 - 3 = ?', '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (5, 1, '9 + 1 = ?', '10')");

        // Quiz 2: Science (pass = 50%)
        $this->pdo->exec("INSERT INTO sl_cqs_quizzes VALUES (2, 'Science Intro', 'science', 50)");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (6, 2, 'Water formula?', 'H2O')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (7, 2, 'Speed of light unit?', 'c')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (8, 2, 'Gravity force?', '9.8')");
        $this->pdo->exec("INSERT INTO sl_cqs_questions VALUES (9, 2, 'Atomic number of H?', '1')");

        // Alice: Math 4/5 correct (80%), Science 3/4 correct (75%)
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (1, 1, 1, 1, '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (2, 1, 2, 1, '15')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (3, 1, 3, 1, '5')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (4, 1, 4, 1, '3')");   // wrong
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (5, 1, 5, 1, '10')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (6, 1, 6, 2, 'H2O')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (7, 1, 7, 2, 'c')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (8, 1, 8, 2, '10')");  // wrong
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (9, 1, 9, 2, '1')");

        // Bob: Math 2/5 correct (40%), Science 4/4 correct (100%)
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (10, 2, 1, 1, '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (11, 2, 2, 1, '12')");  // wrong
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (12, 2, 3, 1, '6')");   // wrong
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (13, 2, 4, 1, '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (14, 2, 5, 1, '11')");  // wrong
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (15, 2, 6, 2, 'H2O')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (16, 2, 7, 2, 'c')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (17, 2, 8, 2, '9.8')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (18, 2, 9, 2, '1')");

        // Carol: Math 5/5 correct (100%), has NOT taken Science
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (19, 3, 1, 1, '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (20, 3, 2, 1, '15')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (21, 3, 3, 1, '5')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (22, 3, 4, 1, '4')");
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (23, 3, 5, 1, '10')");
    ...
    public function testScorePerStudentPerQuiz(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name, q.title,
             ...
             FROM sl_cqs_answers a
             JOIN sl_cqs_students s ON a.student_id = s.id
             JOIN sl_cqs_questions qu ON a.question_id = qu.id
             JOIN sl_cqs_quizzes q ON a.quiz_id = q.id
    ...
    public function testStudentsWhoMissedQuiz(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT s.name
             FROM sl_cqs_students s
             WHERE NOT EXISTS (
                 SELECT 1 FROM sl_cqs_answers a
    ...
    public function testClassAveragePerQuiz(): void
    {
        $rows = $this->ztdQuery(
            "SELECT q.title,
             ...
             FROM sl_cqs_quizzes q
             JOIN sl_cqs_answers a ON a.quiz_id = q.id
             JOIN (
                 ...
                 FROM sl_cqs_answers a2
                 JOIN sl_cqs_questions qu2 ON a2.question_id = qu2.id
    ...
    public function testQuestionDifficulty(): void
    {
        $rows = $this->ztdQuery(
            "SELECT qu.id AS question_id, qu.question_text,
             ...
             FROM sl_cqs_questions qu
             JOIN sl_cqs_answers a ON a.question_id = qu.id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_cqs_answers VALUES (24, 4, 1, 1, '4')");

        // ZTD sees all answers
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_cqs_answers");
        $this->assertEquals(24, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $result = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_cqs_answers')->fetchAll(PDO::FETCH_ASSOC)[0]['cnt'];
