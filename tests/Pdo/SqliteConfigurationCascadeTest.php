 ...
 * overrides, and environment-specific settings through ZTD shadow store (SQLite PDO).
 * Covers cascade resolution, priority-based overrides, and effective config reports.
 * @spec SPEC-10.2.66
 */
class SqliteConfigurationCascadeTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_cc_defaults (
                id INTEGER PRIMARY KEY,
                config_key TEXT,
                config_value TEXT,
                description TEXT
            )',
            'CREATE TABLE sl_cc_overrides (
                id INTEGER PRIMARY KEY,
                config_key TEXT,
                environment TEXT,
                config_value TEXT,
    ...
    {
        return ['sl_cc_overrides', 'sl_cc_defaults'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 5 default configs
        $this->pdo->exec("INSERT INTO sl_cc_defaults VALUES (1, 'app.name', 'MyApp', 'Application name')");
        $this->pdo->exec("INSERT INTO sl_cc_defaults VALUES (2, 'app.debug', 'false', 'Debug mode flag')");
        $this->pdo->exec("INSERT INTO sl_cc_defaults VALUES (3, 'db.host', 'localhost', 'Database host')");
        $this->pdo->exec("INSERT INTO sl_cc_defaults VALUES (4, 'db.port', '3306', 'Database port')");
        $this->pdo->exec("INSERT INTO sl_cc_defaults VALUES (5, 'cache.ttl', '3600', 'Cache time-to-live in seconds')");

        // 4 overrides
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (1, 'app.debug', 'staging', 'true', 10)");
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (2, 'app.debug', 'production', 'false', 10)");
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (3, 'db.host', 'production', 'db.prod.internal', 10)");
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (4, 'cache.ttl', 'staging', '60', 5)");
    }
    ...
    public function testOverrideResolution(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
             ...
             FROM sl_cc_defaults d
             LEFT JOIN sl_cc_overrides o ON o.config_key = d.config_key AND o.environment = ?
    ...
    public function testEnvironmentSpecificConfig(): void
    {
        $staging = $this->ztdPrepareAndExecute(
            "SELECT config_value FROM sl_cc_overrides
    ...
    public function testPriorityBasedOverride(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (5, 'cache.ttl', 'staging', '30', 20)");

        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.config_key,
                    COALESCE(
                        (SELECT o.config_value FROM sl_cc_overrides o
    ...
    public function testAddOverride(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_cc_overrides VALUES (5, 'db.port', 'production', '5432', 10)");
    ...
    public function testRemoveOverrideRestoresDefault(): void
    {
        ...
        $affected = $this->pdo->exec("DELETE FROM sl_cc_overrides WHERE config_key = 'cache.ttl' AND environment = 'staging'");
