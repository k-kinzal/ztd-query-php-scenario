 ...
 * CASE-based composite scoring (SQLite PDO).
 * @spec SPEC-10.2.125
 */
class SqliteCustomerRfmSegmentationTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_rfm_customers (
                id INTEGER PRIMARY KEY,
                name TEXT,
                signup_date TEXT
            )',
            'CREATE TABLE sl_rfm_orders (
                id INTEGER PRIMARY KEY,
                customer_id INTEGER,
                order_date TEXT,
                amount REAL
    ...
    {
        return ['sl_rfm_orders', 'sl_rfm_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Customers
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (1, 'alice',   '2025-01-15')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (2, 'bob',     '2025-03-20')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (3, 'charlie', '2025-06-01')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (4, 'diana',   '2025-02-10')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (5, 'eve',     '2025-08-05')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (6, 'frank',   '2025-04-12')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (7, 'grace',   '2025-05-25')");
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (8, 'hank',    '2025-07-01')");

        // Orders
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (1,  1, '2025-10-01', 150.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (2,  1, '2025-09-15', 200.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (3,  1, '2025-08-01', 75.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (4,  1, '2025-07-10', 300.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (5,  1, '2025-06-20', 50.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (6,  2, '2025-10-05', 500.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (7,  2, '2025-08-12', 450.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (8,  2, '2025-05-30', 350.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (9,  3, '2025-09-20', 80.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (10, 3, '2025-07-15', 60.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (11, 4, '2025-04-01', 1200.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (12, 5, '2025-03-10', 30.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (13, 6, '2025-10-08', 120.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (14, 6, '2025-09-25', 180.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (15, 6, '2025-09-01', 90.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (16, 7, '2025-08-20', 250.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (17, 7, '2025-06-10', 200.00)");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (18, 8, '2025-05-15', 100.00)");
    }
    ...
    public function testRecencyRanking(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id, c.name, MAX(o.order_date) AS last_order
             FROM sl_rfm_customers c
             JOIN sl_rfm_orders o ON o.customer_id = c.id
    ...
    {
        // Derived table with window functions returns empty on SQLite
        // (extends SPEC-11.DERIVED-TABLE-PREPARED — affects query() too, not just prepare())
        $this->markTestIncomplete('Derived table with window functions returns empty on SQLite via query()');
    ...
    {
        $this->pdo->exec("INSERT INTO sl_rfm_customers VALUES (9, 'ivan', '2025-09-01')");
        $this->pdo->exec("INSERT INTO sl_rfm_orders VALUES (19, 9, '2025-10-10', 999.99)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rfm_customers");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rfm_orders");
        $this->assertEquals(19, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_rfm_customers")->fetchAll(PDO::FETCH_ASSOC);
