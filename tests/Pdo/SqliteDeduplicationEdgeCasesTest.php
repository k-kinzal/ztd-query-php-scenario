 ...
 * @spec SPEC-10.2.38
 */
class SqliteDeduplicationEdgeCasesTest extends AbstractSqlitePdoTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO contacts VALUES (1, 'alice@test.com', 'Alice A', 'web', 80, '2024-01-01')");
        $this->pdo->exec("INSERT INTO contacts VALUES (2, 'alice@test.com', 'Alice B', 'import', 90, '2024-02-01')");
        $this->pdo->exec("INSERT INTO contacts VALUES (3, 'bob@test.com', 'Bob', 'web', 70, '2024-01-15')");
        $this->pdo->exec("INSERT INTO contacts VALUES (4, 'bob@test.com', 'Bobby', 'api', 60, '2024-03-01')");
        $this->pdo->exec("INSERT INTO contacts VALUES (5, NULL, 'Unknown1', 'import', 50, '2024-01-10')");
        $this->pdo->exec("INSERT INTO contacts VALUES (6, NULL, 'Unknown2', 'import', 40, '2024-02-10')");
        $this->pdo->exec("INSERT INTO contacts VALUES (7, 'charlie@test.com', 'Charlie', 'web', 95, '2024-01-20')");
    }
    ...
    public function testDeleteDuplicatesKeepFirstWorkaround(): void
    {
        // Direct DELETE with NOT IN + GROUP BY subquery causes "incomplete input"
        // on SQLite (CTE rewriter truncates SQL with GROUP BY in subquery).
        // Workaround: query IDs to keep first, then delete by explicit list.
        $keepIds = $this->ztdQuery("
            SELECT MIN(id) AS keep_id
            FROM contacts
            WHERE email IS NOT NULL
            GROUP BY email
        ");
        $keepList = implode(',', array_column($keepIds, 'keep_id'));
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM contacts WHERE email IS NOT NULL");
        $this->assertSame(3, (int) $rows[0]['cnt']); // alice(1), bob(3), charlie(7)

        $rows = $this->ztdQuery("SELECT id FROM contacts WHERE email = 'alice@test.com'");
        $this->assertSame(1, (int) $rows[0]['id']);
    }

    public function testDeleteWithGroupBySubqueryCausesIncompleteInput(): void
    {
        // Document known issue: DELETE with NOT IN + GROUP BY subquery fails
        // on SQLite with "incomplete input" — similar to SPEC-11.UPDATE-AGGREGATE-SUBQUERY
    ...
    public function testMultiPassDedupWorkaround(): void
    {
        // Workaround for SQLite: query IDs first, then delete
        $keepIds = $this->ztdQuery("
            SELECT MIN(id) AS keep_id
            FROM contacts
            WHERE email IS NOT NULL
        ...
        // First pass: delete low-score duplicates only
        $this->pdo->exec("
            DELETE FROM contacts
            WHERE email IS NOT NULL
            AND id NOT IN ({$keepList})
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM contacts WHERE email = 'bob@test.com'");
        $this->assertSame(1, (int) $rows[0]['cnt']);

        // Alice's duplicate (id=2, score=90) should NOT be deleted (score >= 80)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM contacts WHERE email = 'alice@test.com'");
        $this->assertSame(2, (int) $rows[0]['cnt']);
    }
    ...
    public function testPreparedDuplicateLookupByEmail(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT id, name, score FROM contacts WHERE email = ? ORDER BY score DESC",
            ['alice@test.com']
        );
    ...
    public function testDedupAfterInsert(): void
    {
        ...
        $this->pdo->exec("INSERT INTO contacts VALUES (8, 'charlie@test.com', 'Charles', 'api', 85, '2024-04-01')");
