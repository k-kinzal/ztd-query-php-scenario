...
/**
 * Tests DELETE with correlated subquery patterns through ZTD CTE shadow store (SQLite PDO).
 *
 ...
 */
class SqliteDeleteCorrelatedTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_dc_departments (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                active INT NOT NULL
            )',
            'CREATE TABLE sl_dc_employees (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
    ...
    {
        return ['sl_dc_employees', 'sl_dc_departments'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_dc_departments VALUES (1, 'Engineering', 1)");
        $this->pdo->exec("INSERT INTO sl_dc_departments VALUES (2, 'Marketing', 0)");
        $this->pdo->exec("INSERT INTO sl_dc_departments VALUES (3, 'Sales', 1)");

        // 6 employees across departments
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (1, 'Alice', 1, 90)");
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (2, 'Bob', 1, 70)");
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (3, 'Charlie', 2, 60)");
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (4, 'Diana', 2, 80)");
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (5, 'Eve', 3, 50)");
        $this->pdo->exec("INSERT INTO sl_dc_employees VALUES (6, 'Frank', 3, 85)");
    }
    ...
    public function testDeleteWhereExistsCorrelated(): void
    {
        ...
        try {
            $this->pdo->exec(
                "DELETE FROM sl_dc_employees WHERE EXISTS (SELECT 1 FROM sl_dc_departments WHERE id = sl_dc_employees.dept_id AND active = 0)"
            );
        ...
        $rows = $this->ztdQuery("SELECT id, name FROM sl_dc_employees ORDER BY id");
    ...
    public function testDeleteWhereBelowAveragePerformance(): void
    {
        ...
        try {
            $this->pdo->exec(
                "DELETE FROM sl_dc_employees WHERE performance < (SELECT AVG(performance) FROM sl_dc_employees)"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
        ...
        $rows = $this->ztdQuery("SELECT id, name, performance FROM sl_dc_employees ORDER BY id");
    ...
    public function testDeleteWhereNotExistsCorrelated(): void
    {
        ...
        $this->pdo->exec("DELETE FROM sl_dc_employees WHERE dept_id = 3");

        $thrown = null;
        try {
            $this->pdo->exec(
                "DELETE FROM sl_dc_departments WHERE NOT EXISTS (SELECT 1 FROM sl_dc_employees WHERE dept_id = sl_dc_departments.id)"
            );
        } catch (ZtdPdoException $e) {
            $thrown = $e;
        }
        ...
        $rows = $this->ztdQuery("SELECT id, name FROM sl_dc_departments ORDER BY id");
    ...
    public function testPreparedDeleteWithCorrelatedSubquery(): void
    {
        ...
        try {
            $stmt = $this->ztdPrepare(
                "DELETE FROM sl_dc_employees WHERE EXISTS (SELECT 1 FROM sl_dc_departments WHERE id = sl_dc_employees.dept_id AND name = ?)"
            );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $shadowRows = $this->ztdQuery("SELECT id FROM sl_dc_employees ORDER BY id");
        ...
        $this->pdo->disableZtd();
        $physicalRows = $this->pdo->query('SELECT id FROM sl_dc_employees')->fetchAll(PDO::FETCH_ASSOC);
