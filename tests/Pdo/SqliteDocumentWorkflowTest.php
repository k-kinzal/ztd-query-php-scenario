...
/**
 * Tests a document workflow pipeline scenario through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.157
 */
class SqliteDocumentWorkflowTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_dw_documents (
            ...
            'CREATE TABLE sl_dw_reviewers (
                id INTEGER PRIMARY KEY,
                name TEXT,
                expertise TEXT
            )',
            'CREATE TABLE sl_dw_reviews (
                id INTEGER PRIMARY KEY,
                document_id INTEGER,
                reviewer_id INTEGER,
                decision TEXT,
    ...
    {
        return ['sl_dw_reviews', 'sl_dw_reviewers', 'sl_dw_documents'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 5 documents
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (1, 'API Design Guide', 'Alice', 'approved', '2026-01-05')");
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (2, 'Security Policy', 'Bob', 'review', '2026-01-20')");
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (3, 'Onboarding Manual', 'Carol', 'draft', '2026-02-01')");
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (4, 'Release Notes v3', 'Alice', 'published', '2025-12-10')");
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (5, 'Database Migration Plan', 'Dave', 'review', '2026-02-15')");

        // 3 reviewers
        $this->pdo->exec("INSERT INTO sl_dw_reviewers VALUES (1, 'Eve', 'architecture')");
        $this->pdo->exec("INSERT INTO sl_dw_reviewers VALUES (2, 'Frank', 'security')");
        $this->pdo->exec("INSERT INTO sl_dw_reviewers VALUES (3, 'Grace', 'documentation')");

        // 7 reviews
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (1, 1, 1, 'approve', '2026-01-10', 'Looks good')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (2, 1, 2, 'approve', '2026-01-12', 'Approved with minor notes')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (3, 2, 1, 'approve', '2026-02-01', 'Architecture is sound')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (4, 2, 2, 'reject', '2026-02-03', 'Needs encryption details')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (5, 4, 1, 'approve', '2025-12-08', 'Ready to ship')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (6, 4, 3, 'approve', '2025-12-09', 'Well written')");
        $this->pdo->exec("INSERT INTO sl_dw_reviews VALUES (7, 5, 3, 'comment', '2026-02-20', 'Consider adding rollback steps')");
    }
    ...
    public function testReviewQuorumCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.title, COUNT(r.id) AS approval_count,
             ...
             FROM sl_dw_documents d
             LEFT JOIN sl_dw_reviews r ON r.document_id = d.id AND r.decision = 'approve'
    ...
    public function testReviewerWorkload(): void
    {
        $rows = $this->ztdQuery(
            "SELECT rv.name,
             ...
             FROM sl_dw_reviewers rv
             LEFT JOIN sl_dw_reviews r ON r.reviewer_id = rv.id
    ...
    {
        $this->pdo->exec("UPDATE sl_dw_documents SET status = 'published' WHERE id = 1");

        // Verify status changed
        $rows = $this->ztdQuery("SELECT status FROM sl_dw_documents WHERE id = 1");
        $this->assertSame('published', $rows[0]['status']);

        // Verify total published count is now 2
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_dw_documents WHERE status = 'published'");
    ...
    public function testLatestReviewPerDocument(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.title, r.reviewed_at AS latest_review, r.decision
             FROM sl_dw_documents d
             JOIN sl_dw_reviews r ON r.document_id = d.id
             WHERE r.reviewed_at = (SELECT MAX(r2.reviewed_at) FROM sl_dw_reviews r2 WHERE r2.document_id = d.id)
             ORDER BY d.id"
        );
    ...
    public function testPreparedReviewerDocuments(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.title, r.decision, r.reviewed_at
             FROM sl_dw_reviews r
             JOIN sl_dw_documents d ON d.id = r.document_id
             WHERE r.reviewer_id = ?
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_dw_documents VALUES (6, 'Style Guide', 'Eve', 'draft', '2026-03-09')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_dw_documents");
        $this->assertEquals(6, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_dw_documents")->fetchAll(PDO::FETCH_ASSOC);
