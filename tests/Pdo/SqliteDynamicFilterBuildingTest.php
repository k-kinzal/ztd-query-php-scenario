 ...
 * @spec SPEC-3.2
 */
class SqliteDynamicFilterBuildingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE sl_df_products (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (1, 'Laptop Pro', 'electronics', 1299.99, 25, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (2, 'Wireless Mouse', 'electronics', 29.99, 150, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (3, 'USB Cable', 'accessories', 9.99, 500, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (4, 'Standing Desk', 'furniture', 599.99, 12, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (5, 'Monitor 27\"', 'electronics', 449.99, 40, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (6, 'Keyboard', 'electronics', 79.99, 200, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (7, 'Desk Lamp', 'furniture', 45.99, 80, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (8, 'Phone Case', 'accessories', 19.99, 300, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (9, 'Webcam HD', 'electronics', 89.99, 60, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (10, 'Headphones', 'electronics', 199.99, 35, 1)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (11, 'Chair Ergonomic', 'furniture', 899.99, 8, 0)");
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (12, 'Screen Protector', 'accessories', 14.99, 1000, 0)");
    }
    ...
    {
        $sql = "SELECT name, price FROM sl_df_products WHERE 1=1 AND category = 'electronics' AND price < 100 ORDER BY price";
    ...
    public function testOptionalFilterCategoryOnly(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name FROM sl_df_products WHERE 1=1 AND category = ? ORDER BY name",
            ['furniture']
        );
    ...
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name, price FROM sl_df_products WHERE 1=1 AND price <= ? ORDER BY price",
            [20.00]
        );
    ...
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT name, price FROM sl_df_products WHERE 1=1 AND category = ? AND price BETWEEN ? AND ? ORDER BY price",
    ...
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT COUNT(*) AS cnt FROM sl_df_products WHERE 1=1",
            []
        );
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name FROM sl_df_products WHERE category = 'accessories' ORDER BY name ASC"
        );
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, price FROM sl_df_products WHERE category = 'accessories' ORDER BY price DESC"
        );
    ...
    public function testDynamicLimitOffsetWithFilter(): void
    {
        $stmt = $this->ztdPrepare(
            "SELECT name FROM sl_df_products WHERE category = ? ORDER BY id LIMIT ? OFFSET ?"
        );
    ...
    public function testSeparateQueriesForVaryingParams(): void
    {
        ...
        $rows1 = $this->ztdPrepareAndExecute(
            "SELECT COUNT(*) AS cnt FROM sl_df_products WHERE category = ?",
            ['electronics']
        );
        ...
        $rows2 = $this->ztdPrepareAndExecute(
            "SELECT COUNT(*) AS cnt FROM sl_df_products WHERE category = ? AND price > ?",
            ['electronics', 100.00]
        );
    ...
    public function testFilterWithIsNull(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_df_products VALUES (13, 'Mystery Item', NULL, 5.00, 1, 0)");

        $rows = $this->ztdQuery(
            "SELECT name FROM sl_df_products WHERE category IS NULL"
        );
    ...
    public function testFilterWithIsNotNull(): void
    {
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM sl_df_products WHERE category IS NOT NULL"
        );
    ...
    public function testFilterWithBooleanLikeColumn(): void
    {
        $activeRows = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM sl_df_products WHERE discontinued = 0"
        );
        $this->assertSame(11, (int) $activeRows[0]['cnt']);

        $discontinuedRows = $this->ztdQuery(
            "SELECT name FROM sl_df_products WHERE discontinued = 1"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_df_products");
        $this->assertSame(12, (int) $rows[0]['cnt']);

        // Disable ZTD and verify physical table is empty
        $this->pdo->disableZtd();
        $stmt = $this->pdo->query("SELECT COUNT(*) FROM sl_df_products");
