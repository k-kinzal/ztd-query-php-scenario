 ...
 * and GROUP BY with HAVING for price anomaly detection (SQLite PDO).
 * @spec SPEC-10.2.143
 */
class SqliteDynamicPricingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_dp_products (
                id INTEGER PRIMARY KEY,
                name TEXT,
                category TEXT
            )',
            'CREATE TABLE sl_dp_price_history (
                id INTEGER PRIMARY KEY,
                product_id INTEGER,
                price REAL,
                effective_date TEXT
            )',
            'CREATE TABLE sl_dp_competitor_prices (
                id INTEGER PRIMARY KEY,
                product_id INTEGER,
    ...
    {
        return ['sl_dp_competitor_prices', 'sl_dp_price_history', 'sl_dp_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Products
        $this->pdo->exec("INSERT INTO sl_dp_products VALUES (1, 'Widget A', 'Electronics')");
        $this->pdo->exec("INSERT INTO sl_dp_products VALUES (2, 'Gadget B', 'Electronics')");
        $this->pdo->exec("INSERT INTO sl_dp_products VALUES (3, 'Tool C', 'Hardware')");

        // Price history
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (1, 1, 29.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (2, 1, 24.99, '2025-03-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (3, 1, 27.99, '2025-06-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (4, 2, 49.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (5, 2, 44.99, '2025-04-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (6, 2, 39.99, '2025-07-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (7, 3, 19.99, '2025-01-01')");
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (8, 3, 22.99, '2025-05-01')");

        // Competitor prices
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (1, 1, 'CompetitorX', 26.99, '2025-06-15')");
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (2, 1, 'CompetitorY', 28.99, '2025-06-15')");
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (3, 2, 'CompetitorX', 42.99, '2025-07-15')");
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (4, 2, 'CompetitorY', 37.99, '2025-07-15')");
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (5, 3, 'CompetitorX', 21.99, '2025-05-15')");
        $this->pdo->exec("INSERT INTO sl_dp_competitor_prices VALUES (6, 3, 'CompetitorY', 24.99, '2025-05-15')");
    }
    ...
    public function testCurrentPricePerProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name, ph.price, ph.effective_date
             FROM sl_dp_products p
             JOIN sl_dp_price_history ph ON ph.product_id = p.id
                 AND ph.effective_date = (
                     SELECT MAX(ph2.effective_date)
                     FROM sl_dp_price_history ph2
    ...
    public function testCompetitorComparison(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name, comp.competitor_name, ph.price AS own_price,
             ...
             JOIN sl_dp_competitor_prices comp ON comp.product_id = p.id
     ...
     *
     * Known issue: derived table with correlated subquery returns empty on SQLite.
     * The CTE rewriter does not rewrite table references inside derived tables.
     * @see SPEC-11.BARE-SUBQUERY-REWRITE
     */
    public function testCategoryPriceRange(): void
    {
        ...
        if (count($rows) === 0) {
            $this->markTestIncomplete(
                'Derived table with correlated subquery returns empty on SQLite (SPEC-11.BARE-SUBQUERY-REWRITE).'
            );
        }
    ...
    {
        $this->pdo->exec("INSERT INTO sl_dp_price_history VALUES (9, 1, 25.99, '2025-08-01')");

        // ZTD sees the new entry
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_dp_price_history");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_dp_price_history")->fetchAll(PDO::FETCH_ASSOC);
