 ...
 * derived projections — a common pattern in CQRS applications (SQLite PDO).
 * @spec SPEC-10.2.120
 */
class SqliteEventSourcingProjectionTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_es_accounts (
                id INTEGER PRIMARY KEY,
                owner_name TEXT
            )',
            'CREATE TABLE sl_es_events (
                id INTEGER PRIMARY KEY,
    ...
    {
        return ['sl_es_snapshots', 'sl_es_events', 'sl_es_accounts'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Accounts (2)
        $this->pdo->exec("INSERT INTO sl_es_accounts VALUES (1, 'Alice')");
        $this->pdo->exec("INSERT INTO sl_es_accounts VALUES (2, 'Bob')");

        // Events (8: credits and debits across both accounts)
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (1, 1, 'credit', 1000.00, '2026-03-09 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (2, 1, 'debit',   200.00, '2026-03-09 09:15:00', 2)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (3, 1, 'credit',  500.00, '2026-03-09 09:30:00', 3)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (4, 1, 'debit',   100.00, '2026-03-09 10:00:00', 4)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (5, 2, 'credit', 2000.00, '2026-03-09 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (6, 2, 'debit',   750.00, '2026-03-09 09:30:00', 2)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (7, 2, 'credit',  300.00, '2026-03-09 10:00:00', 3)");
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (8, 2, 'debit',   100.00, '2026-03-09 10:30:00', 4)");

        // Snapshots (2: balance after version 2 for each account)
        $this->pdo->exec("INSERT INTO sl_es_snapshots VALUES (1, 1, 800.00, 2)");
        $this->pdo->exec("INSERT INTO sl_es_snapshots VALUES (2, 2, 1250.00, 2)");
    }
    ...
    public function testCurrentBalanceFromEvents(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.id, a.owner_name,
             ...
             FROM sl_es_accounts a
             JOIN sl_es_events e ON e.aggregate_id = a.id
    ...
    public function testSnapshotPlusDelta(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.account_id,
             ...
             FROM sl_es_snapshots s
             LEFT JOIN sl_es_events e ON e.aggregate_id = s.account_id AND e.version > s.snapshot_version
    ...
    {
        $this->pdo->exec("INSERT INTO sl_es_events VALUES (9, 1, 'credit', 999.00, '2026-03-09 11:00:00', 5)");
        $this->pdo->exec("UPDATE sl_es_accounts SET owner_name = 'Alice Updated' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_es_events");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT owner_name FROM sl_es_accounts WHERE id = 1");
        $this->assertSame('Alice Updated', $rows[0]['owner_name']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_es_events")->fetchAll(PDO::FETCH_ASSOC);
