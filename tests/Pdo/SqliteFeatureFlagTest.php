...
/**
 * Tests a feature flag / A-B testing system through ZTD shadow store (SQLite PDO).
 * Covers conditional rollout evaluation, percentage-based segmentation,
 * experiment result aggregation, and physical isolation.
 * @spec SPEC-10.2.107
 */
class SqliteFeatureFlagTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ff_features (
            ...
            'CREATE TABLE sl_ff_user_segments (
                id INTEGER PRIMARY KEY,
                user_id INTEGER,
                segment TEXT
            )',
            'CREATE TABLE sl_ff_experiment_results (
                id INTEGER PRIMARY KEY,
                feature_id INTEGER,
                user_id INTEGER,
                variant TEXT,
    ...
    {
        return ['sl_ff_experiment_results', 'sl_ff_user_segments', 'sl_ff_features'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Features
        $this->pdo->exec("INSERT INTO sl_ff_features VALUES (1, 'new_checkout', 1, 50, 'Redesigned checkout flow')");
        $this->pdo->exec("INSERT INTO sl_ff_features VALUES (2, 'dark_mode', 1, 100, 'Dark mode theme')");
        $this->pdo->exec("INSERT INTO sl_ff_features VALUES (3, 'ai_search', 0, 0, 'AI-powered search')");

        // User segments
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (1, 1, 'beta')");
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (2, 2, 'beta')");
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (3, 3, 'general')");
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (4, 4, 'general')");
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (5, 5, 'general')");
        $this->pdo->exec("INSERT INTO sl_ff_user_segments VALUES (6, 6, 'general')");

        // Experiment results for new_checkout A/B test
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (1,  1, 1, 'control', 0, 0)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (2,  1, 2, 'variant', 1, 49.99)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (3,  1, 3, 'control', 1, 29.99)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (4,  1, 4, 'variant', 1, 79.99)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (5,  1, 5, 'control', 0, 0)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (6,  1, 6, 'variant', 1, 59.99)");
    }
    ...
    public function testExperimentResults(): void
    {
        $rows = $this->ztdQuery(
            "SELECT er.variant,
             ...
             FROM sl_ff_experiment_results er
    ...
    public function testResultsBySegment(): void
    {
        $rows = $this->ztdQuery(
            "SELECT us.segment, er.variant,
                    ...
                    SUM(er.converted) AS conversions
             FROM sl_ff_experiment_results er
             JOIN sl_ff_user_segments us ON us.user_id = er.user_id
             WHERE er.feature_id = 1
    ...
    public function testToggleFeature(): void
    {
        ...
        $this->pdo->exec("UPDATE sl_ff_features SET enabled = 0, rollout_pct = 0 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT enabled, rollout_pct FROM sl_ff_features WHERE id = 1");
        $this->assertEquals(0, (int) $rows[0]['enabled']);

        // Enable ai_search
        $this->pdo->exec("UPDATE sl_ff_features SET enabled = 1, rollout_pct = 10 WHERE id = 3");

        $rows = $this->ztdQuery("SELECT name, enabled, rollout_pct FROM sl_ff_features WHERE enabled = 1 ORDER BY name");
        $this->assertCount(2, $rows);
    ...
    public function testAddExperimentResults(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (7,  1, 7, 'control', 1, 39.99)");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (8,  1, 8, 'variant', 0, 0)");

        $rows = $this->ztdQuery(
            "SELECT variant,
                    COUNT(*) AS participants,
                    SUM(converted) AS conversions
             FROM sl_ff_experiment_results
             WHERE feature_id = 1
             GROUP BY variant
             ORDER BY variant"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT f.name AS feature_name,
                    ...
                    CASE
                        ...
                        ELSE 'disabled'
                    END AS status
             FROM sl_ff_features f
             CROSS JOIN sl_ff_user_segments us
             ORDER BY f.name, us.user_id"
        );

        // 3 features × 6 users = 18 rows
    ...
    {
        $this->pdo->exec("UPDATE sl_ff_features SET enabled = 0 WHERE id = 1");
        $this->pdo->exec("INSERT INTO sl_ff_experiment_results VALUES (9, 2, 1, 'control', 1, 10.00)");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT enabled FROM sl_ff_features WHERE id = 1");
        $this->assertEquals(0, (int) $rows[0]['enabled']);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_ff_experiment_results");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ff_experiment_results")->fetchAll(PDO::FETCH_ASSOC);
