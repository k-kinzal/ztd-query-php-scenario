...
/**
 * Tests a gift card redemption workflow through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.77
 */
class SqliteGiftCardRedemptionTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_gc_cards (
            ...
            'CREATE TABLE sl_gc_transactions (
                id INTEGER PRIMARY KEY,
                card_id INTEGER,
                amount REAL,
    ...
    {
        return ['sl_gc_transactions', 'sl_gc_cards'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_gc_cards VALUES (1, 'GC-100', 100.00, 75.50, 'active')");
        $this->pdo->exec("INSERT INTO sl_gc_cards VALUES (2, 'GC-200', 200.00, 200.00, 'active')");
        $this->pdo->exec("INSERT INTO sl_gc_cards VALUES (3, 'GC-050', 50.00, 0.00, 'depleted')");

        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (1, 1, 10.00, 'redemption', '2026-01-15 10:00:00')");
        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (2, 1, 14.50, 'redemption', '2026-02-01 14:30:00')");
        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (3, 3, 50.00, 'redemption', '2026-01-20 09:00:00')");
    }
    ...
    public function testCheckBalance(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT code, current_balance FROM sl_gc_cards WHERE code = ?",
            ['GC-100']
        );
    ...
    {
        $affected = $this->pdo->exec("UPDATE sl_gc_cards SET current_balance = current_balance - 25.50 WHERE id = 1 AND current_balance >= 25.50");
        $this->assertSame(1, $affected);

        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (4, 1, 25.50, 'redemption', '2026-03-09 12:00:00')");

        $rows = $this->ztdQuery("SELECT current_balance FROM sl_gc_cards WHERE id = 1");
        $this->assertEqualsWithDelta(50.00, (float) $rows[0]['current_balance'], 0.01);

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_gc_transactions WHERE card_id = 1");
    ...
    {
        $affected = $this->pdo->exec("UPDATE sl_gc_cards SET current_balance = current_balance + 50.00 WHERE id = 1");
        $this->assertSame(1, $affected);

        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (4, 1, 50.00, 'reload', '2026-03-09 13:00:00')");

        $rows = $this->ztdQuery("SELECT current_balance FROM sl_gc_cards WHERE id = 1");
        $this->assertEqualsWithDelta(125.50, (float) $rows[0]['current_balance'], 0.01);

        $rows = $this->ztdQuery("SELECT transaction_type, amount FROM sl_gc_transactions WHERE id = 4");
    ...
    public function testTransactionHistory(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT c.code, t.amount, t.transaction_type, t.created_at
             FROM sl_gc_transactions t
             JOIN sl_gc_cards c ON c.id = t.card_id
             WHERE c.code = ?
    ...
    {
        $affected = $this->pdo->exec("UPDATE sl_gc_cards SET status = 'expired' WHERE current_balance = initial_balance AND status = 'active'");
        $this->assertSame(1, $affected);

        $rows = $this->ztdQuery("SELECT code, status FROM sl_gc_cards WHERE id = 2");
        $this->assertSame('expired', $rows[0]['status']);

        $rows = $this->ztdQuery("SELECT status FROM sl_gc_cards WHERE id = 1");
    ...
    {
        $this->pdo->exec("INSERT INTO sl_gc_transactions VALUES (4, 1, 5.00, 'redemption', '2026-03-09 15:00:00')");
        $this->pdo->exec("UPDATE sl_gc_cards SET current_balance = current_balance - 5.00 WHERE id = 1");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_gc_transactions");
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_gc_transactions")->fetchAll(PDO::FETCH_ASSOC);
