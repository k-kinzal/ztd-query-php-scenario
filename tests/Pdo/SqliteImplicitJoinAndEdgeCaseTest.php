...
/**
 * Implicit comma JOIN and edge-case SQL patterns via SQLite PDO.
 *
 ...
 * @spec SPEC-3.3
 */
class SqliteImplicitJoinAndEdgeCaseTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE sl_ije_customers (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                city TEXT NOT NULL
            )",
            "CREATE TABLE sl_ije_orders (
                id INTEGER PRIMARY KEY,
    ...
    {
        return ['sl_ije_orders', 'sl_ije_customers'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_ije_customers (id, name, city) VALUES (1, 'Alice', 'NYC'), (2, 'Bob', 'LA'), (3, 'Carol', 'Chicago')");
        $this->pdo->exec("INSERT INTO sl_ije_orders (id, customer_id, amount, status) VALUES
    ...
    public function testImplicitCommaJoin(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name, o.amount FROM sl_ije_customers c, sl_ije_orders o WHERE c.id = o.customer_id ORDER BY o.id"
            );

            $this->assertCount(5, $rows);
            $this->assertSame('Alice', $rows[0]['name']);
            $this->assertEquals(100.00, (float) $rows[0]['amount'], '', 0.01);
            $this->assertSame('Alice', $rows[1]['name']);
            $this->assertEquals(200.00, (float) $rows[1]['amount'], '', 0.01);
            $this->assertSame('Bob', $rows[2]['name']);
            $this->assertEquals(50.00, (float) $rows[2]['amount'], '', 0.01);
            $this->assertSame('Bob', $rows[3]['name']);
            $this->assertEquals(300.00, (float) $rows[3]['amount'], '', 0.01);
            $this->assertSame('Carol', $rows[4]['name']);
            $this->assertEquals(75.00, (float) $rows[4]['amount'], '', 0.01);
        } catch (\Throwable $e) {
    ...
    public function testImplicitJoinWithAggregate(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name, SUM(o.amount) AS total FROM sl_ije_customers c, sl_ije_orders o WHERE c.id = o.customer_id GROUP BY c.id, c.name ORDER BY total DESC"
            );
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO sl_ije_orders (id, customer_id, amount, status) VALUES (6, 3, 125.00, 'completed')");

            $rows = $this->ztdQuery(
                "SELECT c.name, o.amount FROM sl_ije_customers c, sl_ije_orders o WHERE c.id = o.customer_id AND c.name = 'Carol' ORDER BY o.id"
            );

            $this->assertCount(2, $rows);
            $this->assertEquals(75.00, (float) $rows[0]['amount'], '', 0.01);
            $this->assertEquals(125.00, (float) $rows[1]['amount'], '', 0.01);
        } catch (\Throwable $e) {
        ...
        try {
            $rows = $this->ztdQuery(
                "SELECT c.name,
                 ...
                 FROM sl_ije_customers c
                 JOIN sl_ije_orders o ON c.id = o.customer_id
    ...
    public function testBetweenWithShadowData(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT status, amount FROM sl_ije_orders WHERE amount BETWEEN 50 AND 200 ORDER BY amount"
            );
    ...
    public function testExistsAndNotExists(): void
    {
        try {
            $rows = $this->ztdQuery(
                "SELECT name FROM sl_ije_customers c WHERE EXISTS (SELECT 1 FROM sl_ije_orders o WHERE o.customer_id = c.id AND o.status = 'completed') ORDER BY name"
            );
    ...
    {
        try {
            $this->pdo->exec("DELETE FROM sl_ije_orders WHERE customer_id = 1");

            $rows = $this->ztdQuery(
                "SELECT name FROM sl_ije_customers c WHERE NOT EXISTS (SELECT 1 FROM sl_ije_orders o WHERE o.customer_id = c.id)"
            );
    ...
    {
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ije_customers")
            ->fetchAll(PDO::FETCH_ASSOC);
        $this->assertSame(0, (int) $rows[0]['cnt'], 'Physical customers table should be empty');

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ije_orders")
            ->fetchAll(PDO::FETCH_ASSOC);
