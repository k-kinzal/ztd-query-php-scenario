...
/**
 * Tests INSERT...SELECT with window functions on SQLite PDO.
 *
 ...
 *
 * Since window functions produce computed columns, the CTE rewriter is
 * expected to produce NULL for the window-computed column (similar to
 * other computed column behavior on SQLite).
 *
 * @spec SPEC-10.2.371
 */
class SqliteInsertSelectWindowTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            "CREATE TABLE sl_isw_employees (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                salary REAL NOT NULL,
                dept TEXT NOT NULL
            )",
            "CREATE TABLE sl_isw_ranked (
                id INTEGER PRIMARY KEY,
                name TEXT,
                salary REAL,
                dept TEXT,
    ...
    {
        return ['sl_isw_ranked', 'sl_isw_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (1, 'Alice', 90000, 'Engineering')");
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (2, 'Bob', 80000, 'Engineering')");
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (3, 'Carol', 95000, 'Engineering')");
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (4, 'Dave', 70000, 'Sales')");
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (5, 'Eve', 75000, 'Sales')");
        $this->pdo->exec("INSERT INTO sl_isw_employees (id, name, salary, dept) VALUES (6, 'Frank', 85000, 'Marketing')");
    }
    ...
    public function testInsertSelectRowNumberPartitioned(): void
    {
        try {
            $affected = $this->pdo->exec("
                INSERT INTO sl_isw_ranked (id, name, salary, dept, rank_in_dept)
                SELECT id, name, salary, dept,
                       ROW_NUMBER() OVER (PARTITION BY dept ORDER BY salary DESC)
                FROM sl_isw_employees
            ");
            $this->assertSame(6, $affected);

            $rows = $this->ztdQuery("SELECT * FROM sl_isw_ranked ORDER BY dept, rank_in_dept");
    ...
    public function testInsertSelectRankOverSalary(): void
    {
        try {
            // Add a tie in salary
            $this->pdo->exec("UPDATE sl_isw_employees SET salary = 80000 WHERE id = 4");

            $this->pdo->exec("DELETE FROM sl_isw_ranked");
            ...
            $rows = $this->ztdQuery("SELECT * FROM sl_isw_ranked ORDER BY rank_in_dept, name");
            $this->assertCount(6, $rows);
    ...
    public function testInsertSelectRunningTotalSumOver(): void
    {
        try {
            $affected = $this->pdo->exec("
                INSERT INTO sl_isw_ranked (id, name, salary, dept, rank_in_dept)
                SELECT id, name, salary, dept,
                       CAST(SUM(salary) OVER (PARTITION BY dept ORDER BY id) AS INTEGER)
                FROM sl_isw_employees
            ");
            $this->assertSame(6, $affected);

            $rows = $this->ztdQuery("SELECT * FROM sl_isw_ranked WHERE dept = 'Engineering' ORDER BY id");
    ...
    public function testTargetRowCountMatchesSource(): void
    {
        try {
            ...
            $sourceCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_isw_employees");
            $targetCount = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_isw_ranked");
    ...
    public function testInsertSelectWindowAfterSourceUpdate(): void
    {
        try {
            ...
            $this->pdo->exec("UPDATE sl_isw_employees SET salary = 120000 WHERE name = 'Carol'");
            // Demote Bob
            $this->pdo->exec("UPDATE sl_isw_employees SET salary = 60000 WHERE name = 'Bob'");
            ...
            $rows = $this->ztdQuery("SELECT * FROM sl_isw_ranked WHERE dept = 'Engineering' ORDER BY rank_in_dept");
    ...
    public function testInsertSelectWindowPreparedWithParam(): void
    {
        try {
            ...
            $rows = $this->ztdQuery("SELECT * FROM sl_isw_ranked ORDER BY rank_in_dept");
            $this->assertCount(3, $rows);
            $this->assertSame('Carol', $rows[0]['name']);
            $this->assertSame(1, (int) $rows[0]['rank_in_dept']);
        } catch (\Throwable $e) {
