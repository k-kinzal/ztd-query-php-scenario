...
/**
 * Tests insurance claim filing, review, adjudication, and payout tracking (SQLite PDO).
 ...
 * @spec SPEC-10.2.147
 */
class SqliteInsuranceClaimTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ic_policies (
            ...
            'CREATE TABLE sl_ic_claim_notes (
                id INTEGER PRIMARY KEY,
                claim_id INTEGER,
                note_date TEXT,
                author TEXT,
    ...
    {
        return ['sl_ic_claim_notes', 'sl_ic_claims', 'sl_ic_policies'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Policies
        $this->pdo->exec("INSERT INTO sl_ic_policies VALUES (1, 'Alice', 'auto', 50000.00, '2025-01-01', '2025-12-31')");
        $this->pdo->exec("INSERT INTO sl_ic_policies VALUES (2, 'Bob', 'home', 250000.00, '2025-01-01', '2025-12-31')");
        $this->pdo->exec("INSERT INTO sl_ic_policies VALUES (3, 'Carol', 'health', 100000.00, '2025-03-01', '2026-02-28')");
        $this->pdo->exec("INSERT INTO sl_ic_policies VALUES (4, 'Dave', 'auto', 30000.00, '2025-06-01', '2026-05-31')");

        // Claims
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (1, 1, '2025-03-15', 'Fender bender', 3500.00, 'paid', '2025-03-25', 3200.00)");
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (2, 1, '2025-07-20', 'Windshield crack', 800.00, 'approved', '2025-08-01', 800.00)");
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (3, 2, '2025-05-10', 'Water damage', 45000.00, 'under_review', NULL, NULL)");
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (4, 3, '2025-06-01', 'Surgery', 28000.00, 'paid', '2025-06-20', 27500.00)");
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (5, 3, '2025-08-15', 'Lab tests', 1200.00, 'denied', '2025-08-25', NULL)");
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (6, 4, '2025-09-01', 'Collision', 15000.00, 'filed', NULL, NULL)");

        // Claim notes
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (1, 1, '2025-03-16', 'adjuster', 'Photos received')");
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (2, 1, '2025-03-20', 'adjuster', 'Estimate confirmed at \$3200')");
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (3, 3, '2025-05-12', 'adjuster', 'Inspector scheduled')");
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (4, 3, '2025-05-18', 'inspector', 'Damage assessment: \$45000')");
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (5, 5, '2025-08-20', 'reviewer', 'Pre-existing condition noted')");
        $this->pdo->exec("INSERT INTO sl_ic_claim_notes VALUES (6, 6, '2025-09-02', 'system', 'Claim received')");
    }
    ...
    public function testClaimSummaryByPolicy(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.holder_name,
             ...
             FROM sl_ic_policies p
             JOIN sl_ic_claims c ON c.policy_id = p.id
    ...
    public function testCoverageUtilization(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.holder_name,
             ...
             FROM sl_ic_policies p
             LEFT JOIN sl_ic_claims c ON c.policy_id = p.id
    ...
    public function testClaimsWithNoteCount(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.id AS claim_id,
             ...
             FROM sl_ic_claims c
             LEFT JOIN sl_ic_claim_notes cn ON cn.claim_id = c.id
    ...
    public function testPreparedClaimsByPolicyType(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT c.id, c.description, p.holder_name
             FROM sl_ic_claims c
             JOIN sl_ic_policies p ON p.id = c.policy_id
             WHERE p.policy_type = ?
             ORDER BY c.id",
            ['auto']
        );
    ...
    {
        $this->pdo->exec("INSERT INTO sl_ic_claims VALUES (7, 1, '2025-10-01', 'Theft', 10000.00, 'filed', NULL, NULL)");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_ic_claims");
        $this->assertEquals(7, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ic_claims")->fetchAll(PDO::FETCH_ASSOC);
