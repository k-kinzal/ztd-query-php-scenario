...
/**
 * Tests an inventory hold/reservation with expiry workflow through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.111
 */
class SqliteInventoryHoldTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ih_products (
                id INTEGER PRIMARY KEY,
                name TEXT,
                stock INTEGER
            )',
            'CREATE TABLE sl_ih_holds (
                id INTEGER PRIMARY KEY,
                product_id INTEGER,
                user_id INTEGER,
                quantity INTEGER,
    ...
    {
        return ['sl_ih_purchases', 'sl_ih_holds', 'sl_ih_products'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 products
        $this->pdo->exec("INSERT INTO sl_ih_products VALUES (1, 'Widget', 10)");
        $this->pdo->exec("INSERT INTO sl_ih_products VALUES (2, 'Gadget', 5)");
        $this->pdo->exec("INSERT INTO sl_ih_products VALUES (3, 'Gizmo', 20)");

        // 5 holds
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (1, 1, 1, 2, 'held', '2026-03-08', '2026-03-10')");
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (2, 1, 2, 3, 'held', '2026-03-07', '2026-03-08')");
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (3, 2, 3, 2, 'held', '2026-03-08', '2026-03-10')");
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (4, 1, 4, 1, 'confirmed', '2026-03-07', '2026-03-10')");
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (5, 3, 5, 5, 'held', '2026-03-08', '2026-03-10')");

        // 1 purchase from confirmed hold 4
        $this->pdo->exec("INSERT INTO sl_ih_purchases VALUES (1, 4, 1, 4, 1, '2026-03-09')");
    }
    ...
    public function testAvailableStock(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name,
             ...
             FROM sl_ih_products p
             LEFT JOIN sl_ih_holds h ON h.product_id = p.id
    ...
    public function testActiveHoldsPerProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name,
             ...
             FROM sl_ih_products p
             JOIN sl_ih_holds h ON h.product_id = p.id
    ...
    /**
     * Confirm a hold: UPDATE status to 'confirmed', INSERT a purchase, verify both.
     */
    public function testConfirmHold(): void
    {
        // Confirm hold 1 (Widget, user 1, qty 2)
        $this->pdo->exec("UPDATE sl_ih_holds SET status = 'confirmed' WHERE id = 1");
        $this->pdo->exec("INSERT INTO sl_ih_purchases VALUES (2, 1, 1, 1, 2, '2026-03-09')");

        // Verify hold status changed
        $rows = $this->ztdQuery("SELECT status FROM sl_ih_holds WHERE id = 1");
        $this->assertCount(1, $rows);
        $this->assertSame('confirmed', $rows[0]['status']);

        // Verify purchase recorded
        $rows = $this->ztdQuery("SELECT hold_id, product_id, user_id, quantity FROM sl_ih_purchases WHERE id = 2");
        $this->assertCount(1, $rows);
        ...
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_ih_purchases");
    ...
    {
        // Mark expired holds
        $this->pdo->exec("UPDATE sl_ih_holds SET status = 'expired' WHERE status = 'held' AND expires_at < '2026-03-09'");

        // Verify hold 2 is now 'expired'
        $rows = $this->ztdQuery("SELECT status FROM sl_ih_holds WHERE id = 2");
    ...
    public function testHoldToPurchaseConversion(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (6, 3, 6, 3, 'held', '2026-03-09', '2026-03-10')");

        // Verify hold exists
        $rows = $this->ztdQuery("SELECT status, quantity FROM sl_ih_holds WHERE id = 6");
        $this->assertCount(1, $rows);
        $this->assertSame('held', $rows[0]['status']);
        $this->assertEquals(3, (int) $rows[0]['quantity']);

        // Step 2: Confirm the hold
        $this->pdo->exec("UPDATE sl_ih_holds SET status = 'confirmed' WHERE id = 6");

        // Step 3: Insert purchase
        $this->pdo->exec("INSERT INTO sl_ih_purchases VALUES (2, 6, 3, 6, 3, '2026-03-09')");

        // Verify purchase
        $rows = $this->ztdQuery("SELECT hold_id, quantity FROM sl_ih_purchases WHERE hold_id = 6");
        $this->assertCount(1, $rows);
    ...
    {
        $this->pdo->exec("INSERT INTO sl_ih_holds VALUES (6, 1, 6, 1, 'held', '2026-03-09', '2026-03-10')");
        $this->pdo->exec("UPDATE sl_ih_holds SET status = 'confirmed' WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_ih_holds");
        ...
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ih_holds")->fetchAll(PDO::FETCH_ASSOC);
