...
/**
 * Tests a library book lending scenario through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.152
 */
class SqliteLibraryLendingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_lib_books (
            ...
            'CREATE TABLE sl_lib_members (
                id INTEGER PRIMARY KEY,
                name TEXT,
                email TEXT,
                membership_type TEXT
            )',
            'CREATE TABLE sl_lib_loans (
                id INTEGER PRIMARY KEY,
                book_id INTEGER,
                member_id INTEGER,
                borrow_date TEXT,
    ...
    {
        return ['sl_lib_loans', 'sl_lib_members', 'sl_lib_books'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 6 books
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (1, 'The Great Gatsby', 'Fitzgerald', '978-0-7432-7356-5', 'fiction', 0.50)");
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (2, 'Clean Code', 'Robert Martin', '978-0-13-235088-4', 'technical', 0.75)");
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (3, '1984', 'Orwell', '978-0-452-28423-4', 'fiction', 0.50)");
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (4, 'Design Patterns', 'GoF', '978-0-201-63361-0', 'technical', 0.75)");
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (5, 'Sapiens', 'Harari', '978-0-06-231609-7', 'nonfiction', 0.60)");
        $this->pdo->exec("INSERT INTO sl_lib_books VALUES (6, 'Dune', 'Herbert', '978-0-441-17271-9', 'fiction', 0.50)");

        // 4 members
        $this->pdo->exec("INSERT INTO sl_lib_members VALUES (1, 'Alice', 'alice@example.com', 'premium')");
        $this->pdo->exec("INSERT INTO sl_lib_members VALUES (2, 'Bob', 'bob@example.com', 'standard')");
        $this->pdo->exec("INSERT INTO sl_lib_members VALUES (3, 'Carol', 'carol@example.com', 'standard')");
        $this->pdo->exec("INSERT INTO sl_lib_members VALUES (4, 'Dave', 'dave@example.com', 'premium')");

        // 8 loans
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (1, 1, 1, '2026-01-05', '2026-01-19', '2026-01-18')");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (2, 2, 1, '2026-02-01', '2026-02-15', NULL)");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (3, 3, 2, '2026-01-10', '2026-01-24', '2026-01-30')");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (4, 4, 2, '2026-02-10', '2026-02-24', NULL)");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (5, 5, 3, '2026-01-15', '2026-01-29', '2026-01-28')");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (6, 1, 3, '2026-02-05', '2026-02-19', '2026-02-19')");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (7, 6, 4, '2026-02-20', '2026-03-06', NULL)");
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (8, 3, 1, '2026-02-25', '2026-03-11', NULL)");
    }
    ...
    public function testCurrentlyBorrowedBooks(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.title, m.name AS borrower, l.borrow_date, l.due_date
             FROM sl_lib_loans l
             JOIN sl_lib_books b ON b.id = l.book_id
             JOIN sl_lib_members m ON m.id = l.member_id
    ...
    /**
     * Late fee calculation for returned-late books using JULIANDAY.
    ...
    public function testLateFeeCalculation(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.title, m.name AS borrower, l.due_date, l.return_date,
                    CAST(JULIANDAY(l.return_date) - JULIANDAY(l.due_date) AS INTEGER) AS days_late,
                    CAST(JULIANDAY(l.return_date) - JULIANDAY(l.due_date) AS INTEGER) * b.daily_fee AS late_fee
             FROM sl_lib_loans l
             JOIN sl_lib_books b ON b.id = l.book_id
             JOIN sl_lib_members m ON m.id = l.member_id
    ...
    public function testBookAvailability(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.title, b.author,
             ...
             FROM sl_lib_books b
             LEFT JOIN sl_lib_loans l ON l.book_id = b.id AND l.return_date IS NULL
             ORDER BY b.id"
        );
    ...
    public function testMemberBorrowingStats(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name,
             ...
             FROM sl_lib_members m
             LEFT JOIN sl_lib_loans l ON l.member_id = m.id
    ...
    public function testCategoryPopularity(): void
    {
        $rows = $this->ztdQuery(
            "SELECT b.category, COUNT(l.id) AS loan_count
             FROM sl_lib_books b
             JOIN sl_lib_loans l ON l.book_id = b.id
             GROUP BY b.category
             HAVING COUNT(l.id) >= 2
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_lib_loans VALUES (9, 5, 4, '2026-03-09', '2026-03-23', NULL)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_lib_loans");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_lib_loans")->fetchAll(PDO::FETCH_ASSOC);
