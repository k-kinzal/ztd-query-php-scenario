 ...
 * @spec SPEC-3.1
 */
class SqliteMultiColumnSortingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE sl_mcs_products (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (1, 'Laptop', 'electronics', 'high', 999.99, 4.5, 25)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (2, 'Mouse', 'electronics', 'low', 29.99, 4.2, 150)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (3, 'Desk', 'furniture', 'medium', 599.99, 4.8, 10)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (4, 'Chair', 'furniture', 'high', 899.99, NULL, 8)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (5, 'Monitor', 'electronics', 'high', 449.99, 4.6, 30)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (6, 'Keyboard', 'electronics', 'medium', 79.99, 4.0, 200)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (7, 'Lamp', 'furniture', 'low', 45.99, NULL, 80)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (8, 'Cable', 'accessories', 'low', 9.99, 3.5, 500)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (9, 'Webcam', 'electronics', 'medium', 89.99, 4.1, 60)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (10, 'Headphones', 'electronics', 'high', 199.99, 4.7, 35)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (11, 'Shelf', 'furniture', NULL, 149.99, 3.9, 20)");
        $this->pdo->exec("INSERT INTO sl_mcs_products VALUES (12, 'Case', 'accessories', NULL, 19.99, NULL, 300)");
    }
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, category, price FROM sl_mcs_products ORDER BY category ASC, price DESC"
        );
    ...
    /**
     * Three-column sort: category ASC, priority (CASE-mapped), name ASC.
     */
    public function testThreeColumnSort(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name, category, priority, price FROM sl_mcs_products
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, priority FROM sl_mcs_products
             WHERE priority IS NOT NULL
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name, LENGTH(name) AS name_len FROM sl_mcs_products ORDER BY LENGTH(name) ASC, name ASC LIMIT 5"
        );
     ...
     * SQLite sorts NULLs first by default in ASC order.
     */
    public function testOrderByWithNulls(): void
    {
        $rows = $this->ztdQuery(
            "SELECT name, rating FROM sl_mcs_products ORDER BY rating ASC"
        );

        $this->assertCount(12, $rows);

        // In SQLite, NULL sorts first in ASC order
    ...
    public function testSortDirectionChangeAfterUpdate(): void
    {
        ...
        $before = $this->ztdQuery(
            "SELECT name, price FROM sl_mcs_products WHERE category = 'electronics' ORDER BY price ASC LIMIT 1"
        );
        $this->assertSame('Mouse', $before[0]['name']); // 29.99 is cheapest

        // Update Mouse price to be the most expensive
        $this->pdo->exec("UPDATE sl_mcs_products SET price = 2000.00 WHERE id = 2");
        ...
        $highest = $this->ztdQuery(
            "SELECT name, price FROM sl_mcs_products WHERE category = 'electronics' ORDER BY price DESC LIMIT 1"
        );
    ...
    public function testPreparedOrderByWithLimit(): void
    {
        $stmt = $this->ztdPrepare(
            "SELECT name, price FROM sl_mcs_products WHERE category = ? ORDER BY price DESC LIMIT ?"
        );
    ...
    {
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_mcs_products");
        $this->assertSame(12, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $stmt = $this->pdo->query("SELECT COUNT(*) FROM sl_mcs_products");
