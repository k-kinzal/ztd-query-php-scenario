...
/**
 * Tests multiple user-defined CTEs in a single query through ZTD shadow store.
 *
 ...
 * @spec SPEC-3.3
 */
class SqliteMultiCteUserQueryTest extends AbstractSqlitePdoTestCase
{
     ...
     * SQLite returns 0 rows when two user-defined CTEs are JOINed.
     */
    public function testTwoCtesJoinedInFinalSelect(): void
    {
        ...
        if (count($rows) === 0) {
            $this->markTestIncomplete('Issue #52: two user CTEs JOINed in final SELECT returns empty on SQLite');
        }
        $this->assertCount(3, $rows);
        $this->assertEquals(300.00, (float) $rows[0]['total']);
     ...
     * Issue #52: two user CTEs JOINed returns empty on SQLite.
     */
    public function testTwoCtesJoinedWithBaseTable(): void
    {
        ...
        if (count($rows) === 0) {
            $this->markTestIncomplete('Issue #52: two user CTEs JOINed with base table returns empty on SQLite');
        }
        $this->assertCount(3, $rows);
        $this->assertSame('Alice', $rows[0]['name']);
    }
