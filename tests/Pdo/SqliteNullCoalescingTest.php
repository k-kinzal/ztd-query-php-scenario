...
/**
 * Tests NULL handling edge cases through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.89
 */
class SqliteNullCoalescingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_nc_users (
    ...
    {
        return ['sl_nc_orders', 'sl_nc_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_nc_users VALUES (1, 'Alice', 'alice@example.com', '555-0101', 'email')");
        $this->pdo->exec("INSERT INTO sl_nc_users VALUES (2, 'Bob', NULL, '555-0202', 'phone')");
        $this->pdo->exec("INSERT INTO sl_nc_users VALUES (3, 'Charlie', 'charlie@example.com', NULL, NULL)");
        $this->pdo->exec("INSERT INTO sl_nc_users VALUES (4, 'Diana', NULL, NULL, NULL)");

        $this->pdo->exec("INSERT INTO sl_nc_orders VALUES (1, 1, '123 Main St', '123 Main St', 'Rush delivery')");
        $this->pdo->exec("INSERT INTO sl_nc_orders VALUES (2, 1, '456 Oak Ave', NULL, NULL)");
        $this->pdo->exec("INSERT INTO sl_nc_orders VALUES (3, 2, NULL, '789 Pine Rd', 'Gift wrap')");
        $this->pdo->exec("INSERT INTO sl_nc_orders VALUES (4, 3, NULL, NULL, NULL)");
        $this->pdo->exec("INSERT INTO sl_nc_orders VALUES (5, 5, '999 Elm St', '999 Elm St', NULL)");
    }
    ...
    {
        $rows = $this->ztdQuery(
            "SELECT name FROM sl_nc_users WHERE email IS NULL ORDER BY id"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT COUNT(*) AS total, COUNT(email) AS with_email FROM sl_nc_users"
        );
    ...
    public function testLeftJoinWithNullKeys(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.id AS order_id, o.user_id, u.name
             FROM sl_nc_orders o
             LEFT JOIN sl_nc_users u ON u.id = o.user_id
             ORDER BY o.id"
        );
    ...
    {
        $this->pdo->exec("UPDATE sl_nc_users SET phone = NULL WHERE id = 1");

        $rows = $this->ztdQuery(
            "SELECT name FROM sl_nc_users WHERE phone IS NULL ORDER BY id"
        );
        ...
        $rows = $this->ztdQuery(
            "SELECT name FROM sl_nc_users WHERE phone IS NOT NULL ORDER BY id"
        );
    ...
    {
        $this->pdo->exec("UPDATE sl_nc_users SET phone = NULL WHERE id = 1");
        $this->pdo->exec("INSERT INTO sl_nc_users VALUES (5, 'Eve', NULL, NULL, NULL)");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_nc_users");
        $this->assertEquals(5, (int) $rows[0]['cnt']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_nc_users')->fetchAll(PDO::FETCH_ASSOC);
