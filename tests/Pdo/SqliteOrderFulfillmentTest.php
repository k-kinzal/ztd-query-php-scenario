...
/**
 * Tests a multi-stage order fulfillment workflow through ZTD shadow store (SQLite PDO).
 * Covers line item fulfillment, partial/complete status transitions,
 * order summary reporting, and physical isolation.
 * @spec SPEC-10.2.73
 */
class SqliteOrderFulfillmentTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_of_orders (
                id INTEGER PRIMARY KEY,
                customer_name TEXT,
                status TEXT,
                order_date TEXT
            )',
            'CREATE TABLE sl_of_items (
                id INTEGER PRIMARY KEY,
                order_id INTEGER,
                product_name TEXT,
                quantity INTEGER,
            ...
            'CREATE TABLE sl_of_fulfillments (
                id INTEGER PRIMARY KEY,
                item_id INTEGER,
    ...
    {
        return ['sl_of_fulfillments', 'sl_of_items', 'sl_of_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 orders
        $this->pdo->exec("INSERT INTO sl_of_orders VALUES (1, 'Alice', 'processing', '2026-03-01 09:00:00')");
        $this->pdo->exec("INSERT INTO sl_of_orders VALUES (2, 'Bob', 'processing', '2026-03-02 10:00:00')");

        // Order 1: 3 items
        $this->pdo->exec("INSERT INTO sl_of_items VALUES (1, 1, 'Laptop', 1, 999.99, 'pending')");
        $this->pdo->exec("INSERT INTO sl_of_items VALUES (2, 1, 'Mouse', 2, 29.99, 'pending')");
        $this->pdo->exec("INSERT INTO sl_of_items VALUES (3, 1, 'Keyboard', 1, 79.99, 'pending')");

        // Order 2: 2 items
        $this->pdo->exec("INSERT INTO sl_of_items VALUES (4, 2, 'Monitor', 2, 349.99, 'pending')");
        $this->pdo->exec("INSERT INTO sl_of_items VALUES (5, 2, 'Webcam', 1, 89.99, 'pending')");
    }
    ...
    public function testOrderSummaryList(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.id, o.customer_name, o.status,
             ...
             FROM sl_of_orders o
             LEFT JOIN sl_of_items i ON i.order_id = o.id
    ...
    {
        // Ship the laptop
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (1, 1, 1, '2026-03-05 14:00:00', 'TRACK-001')");
        $affected = $this->pdo->exec("UPDATE sl_of_items SET status = 'shipped' WHERE id = 1 AND status = 'pending'");
        ...
        $rows = $this->ztdQuery(
            "SELECT i.product_name, i.status, f.tracking_code
             FROM sl_of_items i
             JOIN sl_of_fulfillments f ON f.item_id = i.id
             WHERE i.id = 1"
        );
    ...
    public function testPartialFulfillment(): void
    {
        // Ship laptop only
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (1, 1, 1, '2026-03-05 14:00:00', 'TRACK-001')");
        $this->pdo->exec("UPDATE sl_of_items SET status = 'shipped' WHERE id = 1");
        ...
        $rows = $this->ztdQuery(
            "SELECT o.id,
             ...
             FROM sl_of_orders o
             JOIN sl_of_items i ON i.order_id = o.id
             WHERE o.id = 1
             GROUP BY o.id"
        );
    ...
    public function testCompleteFulfillment(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (1, 1, 1, '2026-03-05 14:00:00', 'TRACK-001')");
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (2, 2, 2, '2026-03-05 14:00:00', 'TRACK-001')");
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (3, 3, 1, '2026-03-05 14:00:00', 'TRACK-002')");
        $this->pdo->exec("UPDATE sl_of_items SET status = 'shipped' WHERE order_id = 1");
        ...
        // Update order status
        $this->pdo->exec("UPDATE sl_of_orders SET status = 'shipped' WHERE id = 1");

        $rows = $this->ztdQuery("SELECT status FROM sl_of_orders WHERE id = 1");
        $this->assertSame('shipped', $rows[0]['status']);
    }
    ...
    public function testFulfillmentReport(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (1, 1, 1, '2026-03-05 14:00:00', 'TRACK-001')");
        $this->pdo->exec("UPDATE sl_of_items SET status = 'shipped' WHERE id = 1");
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (2, 4, 2, '2026-03-06 09:00:00', 'TRACK-003')");
        $this->pdo->exec("INSERT INTO sl_of_fulfillments VALUES (3, 5, 1, '2026-03-06 09:00:00', 'TRACK-003')");
        $this->pdo->exec("UPDATE sl_of_items SET status = 'shipped' WHERE order_id = 2");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_of_fulfillments");
        $this->assertEquals(1, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_of_fulfillments")->fetchAll(PDO::FETCH_ASSOC);
