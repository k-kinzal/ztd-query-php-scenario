 ...
 * @spec SPEC-3.3
 */
class SqlitePivotReportTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_pr_sales (
    ...
    protected function setUp(): void
    {
        ...
        // January sales
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (1, 'Laptop', 'Electronics', 999.00, '2024-01', 'North')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (2, 'Phone', 'Electronics', 699.00, '2024-01', 'South')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (3, 'Shirt', 'Clothing', 29.00, '2024-01', 'North')");
        // February sales
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (4, 'Laptop', 'Electronics', 999.00, '2024-02', 'South')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (5, 'Book', 'Books', 15.00, '2024-02', 'North')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (6, 'Pants', 'Clothing', 49.00, '2024-02', 'North')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (7, 'Phone', 'Electronics', 699.00, '2024-02', 'North')");
        // March sales
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (8, 'Tablet', 'Electronics', 399.00, '2024-03', 'South')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (9, 'Jacket', 'Clothing', 89.00, '2024-03', 'North')");
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (10, 'Book', 'Books', 25.00, '2024-03', 'South')");
    }
    ...
    /**
     * Scalar subquery referencing same shadow table returns empty.
     * The CTE rewriter does not rewrite table references inside scalar subqueries
     * in the SELECT expression list; the inner query reads from the physical table (empty).
     * @spec SPEC-11.SCALAR-SUBQUERY-SELECT
     */
    public function testScalarSubqueryInSelectReturnsEmpty(): void
    {
        $rows = $this->ztdQuery(
            "SELECT category,
                    SUM(amount) AS cat_revenue,
                    ROUND(SUM(amount) * 100.0 / (SELECT SUM(amount) FROM sl_pr_sales), 1) AS pct
             FROM sl_pr_sales
             GROUP BY category
             ORDER BY category"
        );

        // Known issue: scalar subquery on same table produces empty result
        $this->assertCount(0, $rows);
    }

    /**
     * Workaround: use CROSS JOIN with aggregate subquery instead of scalar subquery.
    ...
    public function testRevenueMixPercentageWorkaround(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.category,
             ...
             FROM sl_pr_sales s
             CROSS JOIN (SELECT SUM(amount) AS total FROM sl_pr_sales) t
        ...
        $this->assertCount(3, $rows);
        $this->assertSame('Electronics', $rows[0]['category']);
        // Electronics: 3795 / 4002 * 100 ≈ 94.8%
        $this->assertGreaterThan(90.0, (float) $rows[0]['pct']);
    }
    ...
    public function testPivotAfterInsert(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_pr_sales VALUES (11, 'Server', 'Electronics', 5000.00, '2024-03', 'North')");

        $rows = $this->ztdQuery(
            "SELECT SUM(CASE WHEN sale_month = '2024-03' THEN amount ELSE 0 END) AS mar_total
             FROM sl_pr_sales WHERE category = 'Electronics'"
        );
    ...
    public function testPivotAfterUpdate(): void
    {
        ...
        $this->pdo->exec("UPDATE sl_pr_sales SET amount = 799.00 WHERE id = 1");
    ...
    {
        $this->pdo->exec("DELETE FROM sl_pr_sales WHERE id = 2"); // Remove Jan phone sale (699)
