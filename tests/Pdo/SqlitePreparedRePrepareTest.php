...
/**
 * Tests re-preparing different SQL on the same PDOStatement variable.
 *
 ...
 * @spec SPEC-4.2
 */
class SqlitePreparedRePrepareTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_rp_users (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                status TEXT NOT NULL DEFAULT \'active\'
            )',
            'CREATE TABLE sl_rp_logs (
                id INTEGER PRIMARY KEY,
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_rp_users VALUES (1, 'Alice', 'active')");
        $this->pdo->exec("INSERT INTO sl_rp_users VALUES (2, 'Bob', 'active')");
        $this->pdo->exec("INSERT INTO sl_rp_users VALUES (3, 'Carol', 'inactive')");
    }
    ...
    public function testRePrepareInsertThenSelect(): void
    {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO sl_rp_users VALUES (?, ?, ?)");
            $stmt->execute([4, 'Dave', 'active']);

            // Re-prepare same variable: SELECT
            $stmt = $this->pdo->prepare("SELECT name FROM sl_rp_users WHERE status = ? ORDER BY name");
            $stmt->execute(['active']);
    ...
    public function testRePrepareSelectUpdateSelect(): void
    {
        try {
            // First: SELECT
            $stmt = $this->pdo->prepare("SELECT COUNT(*) AS cnt FROM sl_rp_users WHERE status = ?");
            $stmt->execute(['active']);
            $before = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $this->assertEquals(2, (int) $before[0]['cnt']);

            // Re-prepare: UPDATE
            $stmt = $this->pdo->prepare("UPDATE sl_rp_users SET status = ? WHERE id = ?");
    ...
    {
        try {
            $stmt = $this->pdo->prepare("DELETE FROM sl_rp_users WHERE id = ?");
            $stmt->execute([3]);

            $stmt = $this->pdo->prepare("SELECT name FROM sl_rp_users ORDER BY name");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testRePrepareDifferentTables(): void
    {
        try {
            // INSERT into logs
            $stmt = $this->pdo->prepare("INSERT INTO sl_rp_logs VALUES (?, ?, ?)");
            ...
            $stmt = $this->pdo->prepare(
                "SELECT u.name, l.action
                 FROM sl_rp_users u
                 INNER JOIN sl_rp_logs l ON u.id = l.user_id
                 WHERE l.action = ?
                 ORDER BY u.name"
            );
            $stmt->execute(['login']);
    ...
    public function testRePrepareDifferentParamCount(): void
    {
        try {
            ...
            // 1 param
            $stmt = $this->pdo->prepare("SELECT name FROM sl_rp_users WHERE id = ?");
            $stmt->execute([4]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            // 2 params
            $stmt = $this->pdo->prepare("UPDATE sl_rp_users SET name = ? WHERE id = ?");
            $stmt->execute(['David', 4]);

            // 0 params
            $stmt = $this->pdo->prepare("SELECT name FROM sl_rp_users WHERE id = 4");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testRapidRePrepare(): void
    {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT COUNT(*) AS cnt FROM sl_rp_users");
            $stmt->execute([]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $sql = "SELECT name FROM sl_rp_users WHERE id = ?";
            ...
            // Mutate
            $this->pdo->exec("UPDATE sl_rp_users SET name = 'Alicia' WHERE id = 1");
