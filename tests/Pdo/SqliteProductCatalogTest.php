...
/**
 * Tests a product catalog workflow through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.78
 */
class SqliteProductCatalogTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_pc_categories (
                id INTEGER PRIMARY KEY,
                name TEXT,
                parent_id INTEGER
            )',
            'CREATE TABLE sl_pc_products (
                id INTEGER PRIMARY KEY,
                name TEXT,
                category_id INTEGER,
                price REAL,
    ...
    {
        return ['sl_pc_products', 'sl_pc_categories'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_pc_categories VALUES (1, 'Electronics', NULL)");
        $this->pdo->exec("INSERT INTO sl_pc_categories VALUES (2, 'Phones', 1)");
        $this->pdo->exec("INSERT INTO sl_pc_categories VALUES (3, 'Laptops', 1)");
        $this->pdo->exec("INSERT INTO sl_pc_categories VALUES (4, 'Clothing', NULL)");
        $this->pdo->exec("INSERT INTO sl_pc_categories VALUES (5, 'Shoes', 4)");

        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (1, 'iPhone 15', 2, 999.99, 50, 'active')");
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (2, 'Galaxy S24', 2, 849.99, 3, 'active')");
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (3, 'MacBook Pro', 3, 2499.99, 20, 'active')");
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (4, 'ThinkPad X1', 3, 1599.99, 2, 'active')");
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (5, 'Running Shoes', 5, 129.99, 100, 'active')");
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (6, 'Boots', 5, 199.99, 0, 'discontinued')");
    }
    ...
    public function testCategoryTreeSelfJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c1.id, c1.name AS parent_name, COUNT(c2.id) AS children_count
             FROM sl_pc_categories c1
             LEFT JOIN sl_pc_categories c2 ON c1.id = c2.parent_id
    ...
    public function testProductsInCategory(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT p.name, p.price, c.name AS category_name
             FROM sl_pc_products p
             JOIN sl_pc_categories c ON c.id = p.category_id
             WHERE p.category_id = ?
             ORDER BY p.name",
            [2]
        );
    ...
    public function testFacetedCountsByCategory(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.name AS category_name, COUNT(p.id) AS product_count
             FROM sl_pc_categories c
             LEFT JOIN sl_pc_products p ON p.category_id = c.id
    ...
    {
        $affected = $this->pdo->exec("UPDATE sl_pc_categories SET name = 'Smartphones' WHERE id = 2");
    ...
    {
        $this->pdo->exec("INSERT INTO sl_pc_products VALUES (7, 'New Product', 2, 599.99, 10, 'active')");
        $this->pdo->exec("UPDATE sl_pc_categories SET name = 'Mobile Phones' WHERE id = 2");

        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_pc_products");
        $this->assertSame(7, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT name FROM sl_pc_categories WHERE id = 2");
        $this->assertSame('Mobile Phones', $rows[0]['name']);

        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_pc_products")->fetchAll(PDO::FETCH_ASSOC);
