...
/**
 * Tests a project milestone tracking workflow through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.87
 */
class SqliteProjectMilestoneTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_pm_projects (
    ...
    {
        return ['sl_pm_milestones', 'sl_pm_projects'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 projects
        $this->pdo->exec("INSERT INTO sl_pm_projects VALUES (1, 'Website Redesign', '2026-01-01 00:00:00', '2026-06-30 00:00:00', 'active')");
        $this->pdo->exec("INSERT INTO sl_pm_projects VALUES (2, 'Mobile App', '2026-02-01 00:00:00', '2026-09-30 00:00:00', 'active')");

        // Milestones for Website Redesign (project 1)
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (1, 1, 'Wireframes', '2026-02-01 00:00:00', '2026-01-28 00:00:00', 'completed')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (2, 1, 'Design Mockups', '2026-03-01 00:00:00', '2026-02-25 00:00:00', 'completed')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (3, 1, 'Frontend Dev', '2026-03-05 00:00:00', NULL, 'overdue')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (4, 1, 'Backend Dev', '2026-04-15 00:00:00', NULL, 'pending')");

        // Milestones for Mobile App (project 2)
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (5, 2, 'Requirements', '2026-02-15 00:00:00', '2026-02-14 00:00:00', 'completed')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (6, 2, 'Prototype', '2026-03-01 00:00:00', NULL, 'overdue')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (7, 2, 'Alpha Release', '2026-05-01 00:00:00', NULL, 'pending')");
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (8, 2, 'Beta Release', '2026-07-01 00:00:00', NULL, 'pending')");
    }
    ...
    public function testProjectOverview(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name,
             ...
             FROM sl_pm_projects p
             JOIN sl_pm_milestones m ON m.project_id = p.id
    ...
    public function testOverdueMilestones(): void
    {
        $rows = $this->ztdQuery(
            "SELECT m.name AS milestone_name, p.name AS project_name, m.due_date, m.status
             FROM sl_pm_milestones m
             JOIN sl_pm_projects p ON p.id = m.project_id
    ...
    public function testCompleteMilestone(): void
    {
        ...
        $affected = $this->pdo->exec("UPDATE sl_pm_milestones SET status = 'completed', completed_date = '2026-03-09 10:00:00' WHERE id = 3");
        $this->assertSame(1, $affected);

        // Verify the milestone is now completed
        $rows = $this->ztdQuery("SELECT status, completed_date FROM sl_pm_milestones WHERE id = 3");
    ...
    {
        $this->pdo->exec("INSERT INTO sl_pm_milestones VALUES (9, 1, 'Launch', '2026-06-30 00:00:00', NULL, 'pending')");
        $this->pdo->exec("UPDATE sl_pm_projects SET status = 'completed' WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_pm_milestones");
        $this->assertSame(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT status FROM sl_pm_projects WHERE id = 1");
        $this->assertSame('completed', $rows[0]['status']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_pm_milestones")->fetchAll(PDO::FETCH_ASSOC);
