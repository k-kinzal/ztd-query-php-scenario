 ...
 * and percentage calculations (SQLite PDO).
 * @spec SPEC-10.2.131
 */
class SqliteQuotaManagementTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_qm_plans (
            ...
            'CREATE TABLE sl_qm_accounts (
                id INTEGER PRIMARY KEY,
                account_name TEXT,
                plan_id INTEGER,
                created_at TEXT
            )',
            'CREATE TABLE sl_qm_usage_log (
                id INTEGER PRIMARY KEY,
                account_id INTEGER,
                usage_date TEXT,
    ...
    {
        return ['sl_qm_usage_log', 'sl_qm_accounts', 'sl_qm_plans'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Plans
        $this->pdo->exec("INSERT INTO sl_qm_plans VALUES (1, 'starter', 10, 1000, 5)");
        $this->pdo->exec("INSERT INTO sl_qm_plans VALUES (2, 'pro', 100, 10000, 25)");
        $this->pdo->exec("INSERT INTO sl_qm_plans VALUES (3, 'enterprise', 1000, 100000, 500)");

        // Accounts
        $this->pdo->exec("INSERT INTO sl_qm_accounts VALUES (1, 'Acme Corp', 2, '2025-01-01')");
        $this->pdo->exec("INSERT INTO sl_qm_accounts VALUES (2, 'Widgets Inc', 1, '2025-03-15')");
        $this->pdo->exec("INSERT INTO sl_qm_accounts VALUES (3, 'Big Bank', 3, '2025-02-01')");
        $this->pdo->exec("INSERT INTO sl_qm_accounts VALUES (4, 'Startup X', 1, '2025-06-01')");

        // Usage Log — Acme Corp (pro plan)
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (1, 1, '2025-10-01', 45.50, 3200, 18)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (2, 1, '2025-10-02', 46.00, 2800, 18)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (3, 1, '2025-10-03', 48.20, 4100, 20)");

        // Usage Log — Widgets Inc (starter plan — near limit)
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (4, 2, '2025-10-01', 8.50, 800, 4)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (5, 2, '2025-10-02', 9.10, 950, 5)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (6, 2, '2025-10-03', 9.80, 1100, 5)");

        // Usage Log — Big Bank (enterprise plan)
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (7, 3, '2025-10-01', 250.00, 45000, 180)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (8, 3, '2025-10-02', 252.00, 48000, 185)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (9, 3, '2025-10-03', 255.00, 52000, 190)");

        // Usage Log — Startup X (starter plan — low usage)
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (10, 4, '2025-10-01', 1.20, 100, 2)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (11, 4, '2025-10-02', 1.50, 150, 2)");
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (12, 4, '2025-10-03', 1.80, 120, 3)");
    }
    ...
    public function testLatestUsagePerAccount(): void
    {
        $rows = $this->ztdQuery(
            "SELECT a.account_name, p.plan_name,
             ...
             FROM sl_qm_accounts a
             JOIN sl_qm_plans p ON p.id = a.plan_id
             JOIN sl_qm_usage_log u ON u.account_id = a.id
             WHERE u.usage_date = (
                 SELECT MAX(u2.usage_date)
                 FROM sl_qm_usage_log u2
    ...
    {
        $this->pdo->exec("INSERT INTO sl_qm_usage_log VALUES (13, 1, '2025-10-04', 50.00, 5000, 22)");
        $this->pdo->exec("UPDATE sl_qm_accounts SET account_name = 'Acme Holdings' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_qm_usage_log");
        $this->assertEquals(13, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT account_name FROM sl_qm_accounts WHERE id = 1");
        $this->assertSame('Acme Holdings', $rows[0]['account_name']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_qm_usage_log")->fetchAll(PDO::FETCH_ASSOC);
