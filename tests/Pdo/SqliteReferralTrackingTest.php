...
/**
 * Tests a referral/affiliate tracking system through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.109
 */
class SqliteReferralTrackingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_rt_users (
    ...
    {
        return ['sl_rt_purchases', 'sl_rt_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // charlie(3) -> frank(6)
        // diana(4) -> grace(7)
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (1, 'alice', 'alice@example.com', NULL, '2026-01-01')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (2, 'bob', 'bob@example.com', 1, '2026-01-05')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (3, 'charlie', 'charlie@example.com', 1, '2026-01-10')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (4, 'diana', 'diana@example.com', 2, '2026-01-15')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (5, 'eve', 'eve@example.com', 2, '2026-01-20')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (6, 'frank', 'frank@example.com', 3, '2026-01-25')");
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (7, 'grace', 'grace@example.com', 4, '2026-02-01')");

        // Purchases across users
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (1, 2, 150.00, '2026-02-01')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (2, 3, 200.00, '2026-02-05')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (3, 4, 75.00, '2026-02-10')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (4, 4, 125.00, '2026-02-15')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (5, 5, 300.00, '2026-02-20')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (6, 6, 50.00, '2026-02-25')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (7, 7, 100.00, '2026-03-01')");
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (8, 1, 250.00, '2026-03-05')");
    }
    ...
    public function testDirectReferralCount(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.id, r.username, COUNT(u.id) AS referral_count
             FROM sl_rt_users r
             LEFT JOIN sl_rt_users u ON u.referred_by = r.id
    ...
    public function testReferralChainDepthTwo(): void
    {
        ...
        $indirect = $this->ztdPrepareAndExecute(
            "SELECT l2.id, l2.username, l1.username AS referred_via
             FROM sl_rt_users l2
             JOIN sl_rt_users l1 ON l2.referred_by = l1.id
        ...
        $allReferrals = $this->ztdPrepareAndExecute(
            "SELECT u.id, u.username
             ...
             WHERE u.referred_by = ?
             OR u.referred_by IN (
                SELECT id FROM sl_rt_users WHERE referred_by = ?
             )
             ORDER BY u.id",
            [1, 1]
        );
    ...
    public function testReferralRevenueAttribution(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.id AS referrer_id, r.username AS referrer,
             ...
             FROM sl_rt_users r
             JOIN sl_rt_users u ON u.referred_by = r.id
             JOIN sl_rt_purchases p ON p.user_id = u.id
             GROUP BY r.id, r.username
    ...
    public function testAddNewReferralAndVerify(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_rt_users VALUES (8, 'heidi', 'heidi@example.com', 5, '2026-03-06')");

        // Verify the referral chain: heidi -> eve -> bob -> alice
        $rows = $this->ztdPrepareAndExecute(
            "SELECT u.username, r.username AS referrer
             FROM sl_rt_users u
             JOIN sl_rt_users r ON u.referred_by = r.id
             WHERE u.id = ?",
            [8]
        );
        ...
        $this->pdo->exec("INSERT INTO sl_rt_purchases VALUES (9, 8, 175.00, '2026-03-07')");
    ...
    public function testUsersWithNoReferrals(): void
    {
        $rows = $this->ztdQuery(
            "SELECT u.id, u.username
             FROM sl_rt_users u
             LEFT JOIN sl_rt_users r ON r.referred_by = u.id
             WHERE r.id IS NULL
             ORDER BY u.id"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rt_users");
        $this->assertEquals(8, (int) $rows[0]['cnt']);
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rt_purchases");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_rt_users")->fetchAll(PDO::FETCH_ASSOC);
