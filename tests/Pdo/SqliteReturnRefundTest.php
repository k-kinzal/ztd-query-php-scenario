...
/**
 * Tests return authorization and refund processing workflows through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.133
 */
class SqliteReturnRefundTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_rr_orders (
            ...
            'CREATE TABLE sl_rr_order_items (
                id INTEGER PRIMARY KEY,
                order_id INTEGER,
                product_name TEXT,
                quantity INTEGER,
    ...
    {
        return ['sl_rr_returns', 'sl_rr_order_items', 'sl_rr_orders'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 orders
        $this->pdo->exec("INSERT INTO sl_rr_orders VALUES (1, 'Alice', '2025-08-01', 250.00, 'partially_returned')");
        $this->pdo->exec("INSERT INTO sl_rr_orders VALUES (2, 'Bob', '2025-08-10', 180.00, 'completed')");
        $this->pdo->exec("INSERT INTO sl_rr_orders VALUES (3, 'Carol', '2025-08-15', 95.00, 'fully_returned')");

        // Order items
        $this->pdo->exec("INSERT INTO sl_rr_order_items VALUES (1, 1, 'Widget', 2, 50.00)");
        $this->pdo->exec("INSERT INTO sl_rr_order_items VALUES (2, 1, 'Gadget', 1, 150.00)");
        $this->pdo->exec("INSERT INTO sl_rr_order_items VALUES (3, 2, 'Widget', 1, 50.00)");
        $this->pdo->exec("INSERT INTO sl_rr_order_items VALUES (4, 2, 'Doohickey', 2, 65.00)");
        $this->pdo->exec("INSERT INTO sl_rr_order_items VALUES (5, 3, 'Thingamajig', 1, 95.00)");

        // Returns
        $this->pdo->exec("INSERT INTO sl_rr_returns VALUES (1, 1, '2025-08-10', 1, 'defective', 50.00, 'approved')");
        $this->pdo->exec("INSERT INTO sl_rr_returns VALUES (2, 2, '2025-08-12', 1, 'wrong_item', 150.00, 'pending')");
        $this->pdo->exec("INSERT INTO sl_rr_returns VALUES (3, 5, '2025-08-20', 1, 'changed_mind', 85.50, 'approved')");
    }
    ...
    public function testOrderReturnSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT o.customer_name,
             ...
             FROM sl_rr_orders o
             LEFT JOIN sl_rr_order_items oi ON oi.order_id = o.id
             LEFT JOIN sl_rr_returns r ON r.order_item_id = oi.id
    ...
    public function testOrdersWithPendingReturns(): void
    {
        $rows = $this->ztdQuery(
            "SELECT DISTINCT o.id, o.customer_name
             FROM sl_rr_orders o
             JOIN sl_rr_order_items oi ON oi.order_id = o.id
             JOIN sl_rr_returns r ON r.order_item_id = oi.id
    ...
    public function testRestockingFeeCalculation(): void
    {
        $rows = $this->ztdQuery(
            "SELECT oi.product_name,
             ...
             FROM sl_rr_returns r
             JOIN sl_rr_order_items oi ON oi.id = r.order_item_id
    ...
    {
        // Insert a new return
        $this->pdo->exec("INSERT INTO sl_rr_returns VALUES (4, 3, '2025-08-25', 1, 'defective', 50.00, 'pending')");

        // Visible through ZTD: now 4 returns
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rr_returns");
        $this->assertSame(4, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_rr_returns")->fetchAll(PDO::FETCH_ASSOC);
