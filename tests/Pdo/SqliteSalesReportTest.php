...
/**
 * Tests a sales reporting workflow through ZTD shadow store (SQLite PDO).
 * Covers pivot aggregation, regional breakdown, product metrics,
 * net sales with returns, and physical isolation.
 * @spec SPEC-10.2.74
 */
class SqliteSalesReportTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_sr_regions (
                id INTEGER PRIMARY KEY,
                name TEXT
            )',
            'CREATE TABLE sl_sr_products (
                id INTEGER PRIMARY KEY,
                name TEXT,
                category TEXT
            )',
            'CREATE TABLE sl_sr_sales (
                id INTEGER PRIMARY KEY,
                product_id INTEGER,
                region_id INTEGER,
                quantity INTEGER,
    ...
    {
        return ['sl_sr_sales', 'sl_sr_products', 'sl_sr_regions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Regions
        $this->pdo->exec("INSERT INTO sl_sr_regions VALUES (1, 'North')");
        $this->pdo->exec("INSERT INTO sl_sr_regions VALUES (2, 'South')");
        $this->pdo->exec("INSERT INTO sl_sr_regions VALUES (3, 'West')");

        // Products
        $this->pdo->exec("INSERT INTO sl_sr_products VALUES (1, 'Alpha Widget', 'Electronics')");
        $this->pdo->exec("INSERT INTO sl_sr_products VALUES (2, 'Beta Gadget', 'Electronics')");
        $this->pdo->exec("INSERT INTO sl_sr_products VALUES (3, 'Gamma Tool', 'Hardware')");

        // Sales data (mix of sales and returns across regions and quarters)
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (1, 1, 1, 10, 49.99, 'sale', 'Q1', '2026-01-15')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (2, 1, 1, 5, 49.99, 'sale', 'Q2', '2026-04-10')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (3, 1, 2, 8, 49.99, 'sale', 'Q1', '2026-02-20')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (4, 2, 1, 15, 29.99, 'sale', 'Q1', '2026-01-25')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (5, 2, 3, 20, 29.99, 'sale', 'Q2', '2026-05-05')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (6, 3, 2, 12, 19.99, 'sale', 'Q1', '2026-03-01')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (7, 1, 1, 2, 49.99, 'return', 'Q1', '2026-02-01')");
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (8, 3, 2, 3, 19.99, 'return', 'Q1', '2026-03-15')");
    }
    ...
    public function testSalesByRegion(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name AS region,
             ...
             FROM sl_sr_regions r
             LEFT JOIN sl_sr_sales s ON s.region_id = r.id
    ...
    public function testSalesByProduct(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS product, p.category,
             ...
             FROM sl_sr_products p
             LEFT JOIN sl_sr_sales s ON s.product_id = p.id
    ...
    public function testRegionFilterWithHaving(): void
    {
        $rows = $this->ztdQuery(
            "SELECT r.name AS region,
                    SUM(CASE WHEN s.sale_type = 'sale' THEN s.quantity * s.unit_price ELSE 0 END) AS revenue
             FROM sl_sr_regions r
             JOIN sl_sr_sales s ON s.region_id = r.id
             GROUP BY r.id, r.name
    ...
    public function testRegionSalesPrepared(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT p.name AS product, s.quantity, s.unit_price, s.sale_type
             FROM sl_sr_sales s
             JOIN sl_sr_products p ON p.id = s.product_id
    ...
    {
        $this->pdo->exec("INSERT INTO sl_sr_sales VALUES (9, 1, 3, 100, 49.99, 'sale', 'Q3', '2026-07-01')");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_sr_sales");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_sr_sales")->fetchAll(PDO::FETCH_ASSOC);
