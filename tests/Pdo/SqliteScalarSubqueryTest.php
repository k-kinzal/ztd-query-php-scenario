...
/**
 * Tests scalar subqueries in SELECT lists through CTE shadow store (SQLite).
 *
 ...
 * @spec SPEC-3.3
 */
class SqliteScalarSubqueryTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ssq_departments (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL
            )',
            'CREATE TABLE sl_ssq_employees (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                department_id INTEGER NOT NULL,
                salary REAL NOT NULL
            )',
            'CREATE TABLE sl_ssq_projects (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL,
                department_id INTEGER NOT NULL,
    ...
    {
        return ['sl_ssq_projects', 'sl_ssq_employees', 'sl_ssq_departments'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 departments
        $this->pdo->exec("INSERT INTO sl_ssq_departments VALUES (1, 'Engineering')");
        $this->pdo->exec("INSERT INTO sl_ssq_departments VALUES (2, 'Marketing')");
        $this->pdo->exec("INSERT INTO sl_ssq_departments VALUES (3, 'Sales')");

        // 6 employees, 2 per department
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (1, 'Alice',   1, 90000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (2, 'Bob',     1, 85000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (3, 'Carol',   2, 70000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (4, 'Dave',    2, 65000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (5, 'Eve',     3, 60000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (6, 'Frank',   3, 55000.00)");

        // 4 projects spread across departments (dept 3 has none)
        $this->pdo->exec("INSERT INTO sl_ssq_projects VALUES (1, 'Alpha',   1, 500000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_projects VALUES (2, 'Beta',    1, 300000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_projects VALUES (3, 'Gamma',   2, 200000.00)");
        $this->pdo->exec("INSERT INTO sl_ssq_projects VALUES (4, 'Delta',   2, 150000.00)");
    }
    ...
    public function testSingleScalarSubquery(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM sl_ssq_employees e WHERE e.department_id = d.id) AS emp_count
             FROM sl_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testMultipleScalarSubqueries(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM sl_ssq_employees e WHERE e.department_id = d.id) AS emp_count,
                    (SELECT COALESCE(SUM(p.budget), 0) FROM sl_ssq_projects p WHERE p.department_id = d.id) AS total_budget
             FROM sl_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testCorrelatedScalarSubqueryWithAggregate(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT MAX(e.salary) FROM sl_ssq_employees e WHERE e.department_id = d.id) AS max_salary
             FROM sl_ssq_departments d
             ORDER BY d.id"
        );
    ...
    public function testScalarSubqueryWithWhere(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT COUNT(*) FROM sl_ssq_employees e WHERE e.department_id = d.id) AS emp_count
             FROM sl_ssq_departments d
             WHERE (SELECT SUM(p.budget) FROM sl_ssq_projects p WHERE p.department_id = d.id) > 300000
             ORDER BY d.id"
        );
     ...
     * For each department, find the average salary of employees who work on
    ...
    public function testScalarSubqueryReferencingMultipleTables(): void
    {
        $rows = $this->ztdQuery(
            "SELECT d.name,
                    (SELECT AVG(e.salary)
                     FROM sl_ssq_employees e
                     WHERE e.department_id = d.id
                       AND EXISTS (
                           SELECT 1 FROM sl_ssq_projects p
     ...
     * Add a department with no employees to verify NULL handling.
     */
    public function testScalarSubqueryReturnsNull(): void
    {
        $this->pdo->exec("INSERT INTO sl_ssq_departments VALUES (4, 'Research')");
    ...
    public function testScalarSubqueryAfterInsert(): void
    {
        ...
        $before = $this->ztdQuery(
            "SELECT (SELECT COUNT(*) FROM sl_ssq_employees e WHERE e.department_id = 3) AS cnt"
        );
        $this->assertEquals(2, (int) $before[0]['cnt']);

        // Mutate: add a new employee to Sales
        $this->pdo->exec("INSERT INTO sl_ssq_employees VALUES (7, 'Grace', 3, 58000.00)");
    ...
    public function testPreparedStatementWithScalarSubquery(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT d.name,
                    (SELECT SUM(e.salary) FROM sl_ssq_employees e WHERE e.department_id = d.id) AS total_salary
             FROM sl_ssq_departments d
             WHERE d.id = ?",
            [1]
        );
     ...
     * zero rows (SQLite in-memory starts empty; only shadow data was inserted).
     */
    public function testPhysicalIsolation(): void
    {
        $this->pdo->disableZtd();

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ssq_employees")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ssq_departments")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);

        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_ssq_projects")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);
    }
}
