...
/**
 * Tests an employee skill matrix scenario through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.153
 */
class SqliteSkillMatrixTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_sm_employees (
                id INTEGER PRIMARY KEY,
                name TEXT,
                department TEXT
            )',
            'CREATE TABLE sl_sm_skills (
                id INTEGER PRIMARY KEY,
                name TEXT,
                category TEXT
            )',
            'CREATE TABLE sl_sm_employee_skills (
                id INTEGER PRIMARY KEY,
                employee_id INTEGER,
                skill_id INTEGER,
                level INTEGER
            )',
            'CREATE TABLE sl_sm_project_requirements (
                id INTEGER PRIMARY KEY,
                project_name TEXT,
                skill_id INTEGER,
                min_level INTEGER
    ...
    {
        return ['sl_sm_project_requirements', 'sl_sm_employee_skills', 'sl_sm_skills', 'sl_sm_employees'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Skills (6)
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (1, 'PHP', 'language')");
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (2, 'JavaScript', 'language')");
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (3, 'React', 'framework')");
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (4, 'Laravel', 'framework')");
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (5, 'Docker', 'tool')");
        $this->pdo->exec("INSERT INTO sl_sm_skills VALUES (6, 'Git', 'tool')");

        // Employees (5)
        $this->pdo->exec("INSERT INTO sl_sm_employees VALUES (1, 'Alice', 'engineering')");
        $this->pdo->exec("INSERT INTO sl_sm_employees VALUES (2, 'Bob', 'engineering')");
        $this->pdo->exec("INSERT INTO sl_sm_employees VALUES (3, 'Carol', 'engineering')");
        $this->pdo->exec("INSERT INTO sl_sm_employees VALUES (4, 'Dave', 'design')");
        $this->pdo->exec("INSERT INTO sl_sm_employees VALUES (5, 'Eve', 'engineering')");

        // Employee skills
        // Alice: PHP=5, JavaScript=4, React=3, Laravel=5, Docker=4, Git=5 (full-stack senior)
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (1, 1, 1, 5)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (2, 1, 2, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (3, 1, 3, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (4, 1, 4, 5)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (5, 1, 5, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (6, 1, 6, 5)");
        // Bob: PHP=4, JavaScript=3, Laravel=4, Git=3 (backend mid)
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (7, 2, 1, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (8, 2, 2, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (9, 2, 4, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (10, 2, 6, 3)");
        // Carol: JavaScript=5, React=5, Git=4 (frontend senior)
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (11, 3, 2, 5)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (12, 3, 3, 5)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (13, 3, 6, 4)");
        // Dave: JavaScript=2, React=2, Git=3 (junior designer)
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (14, 4, 2, 2)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (15, 4, 3, 2)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (16, 4, 6, 3)");
        // Eve: PHP=3, JavaScript=3, React=2, Laravel=2, Docker=3, Git=4 (full-stack junior)
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (17, 5, 1, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (18, 5, 2, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (19, 5, 3, 2)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (20, 5, 4, 2)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (21, 5, 5, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_employee_skills VALUES (22, 5, 6, 4)");

        // Project requirements
        // Alpha: PHP>=4, Laravel>=3, Docker>=3
        $this->pdo->exec("INSERT INTO sl_sm_project_requirements VALUES (1, 'Alpha', 1, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_project_requirements VALUES (2, 'Alpha', 4, 3)");
        $this->pdo->exec("INSERT INTO sl_sm_project_requirements VALUES (3, 'Alpha', 5, 3)");
        // Beta: JavaScript>=4, React>=4
        $this->pdo->exec("INSERT INTO sl_sm_project_requirements VALUES (4, 'Beta', 2, 4)");
        $this->pdo->exec("INSERT INTO sl_sm_project_requirements VALUES (5, 'Beta', 3, 4)");
    }
    ...
    public function testEmployeeSkillSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name,
             ...
             FROM sl_sm_employees e
             JOIN sl_sm_employee_skills es ON es.employee_id = e.id
    ...
    public function testFullyQualifiedForProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT e.name
             FROM sl_sm_employees e
             JOIN sl_sm_employee_skills es ON es.employee_id = e.id
             JOIN sl_sm_project_requirements pr ON pr.skill_id = es.skill_id AND pr.project_name = 'Alpha'
             WHERE es.level >= pr.min_level
             GROUP BY e.id, e.name
             HAVING COUNT(DISTINCT pr.skill_id) = (SELECT COUNT(*) FROM sl_sm_project_requirements WHERE project_name = 'Alpha')"
        );
    ...
    public function testSkillGapForProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name AS skill_name, pr.min_level,
             ...
             FROM sl_sm_project_requirements pr
             JOIN sl_sm_skills s ON s.id = pr.skill_id
             LEFT JOIN sl_sm_employee_skills es ON es.skill_id = pr.skill_id AND es.employee_id = 2
    ...
    public function testSkillLevelDistribution(): void
    {
        $rows = $this->ztdQuery(
            "SELECT s.name AS skill_name,
             ...
             FROM sl_sm_skills s
             JOIN sl_sm_employee_skills es ON es.skill_id = s.id
    ...
    public function testPreparedEmployeeSkills(): void
    {
        $rows = $this->ztdPrepareAndExecute(
            "SELECT s.name AS skill_name, es.level
             FROM sl_sm_employee_skills es
             JOIN sl_sm_skills s ON s.id = es.skill_id
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec(
            "INSERT INTO sl_sm_employee_skills VALUES (23, 2, 5, 2)"
        );

        // ZTD sees the new record (22 original + 1 new = 23)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_sm_employee_skills");
        $this->assertEquals(23, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_sm_employee_skills')->fetchAll(PDO::FETCH_ASSOC);
