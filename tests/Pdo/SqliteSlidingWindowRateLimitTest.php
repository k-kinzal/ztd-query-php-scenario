...
/**
 * Tests rolling time-window rate limiting with token bucket patterns through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.119
 */
class SqliteSlidingWindowRateLimitTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_rl_api_clients (
                id INTEGER PRIMARY KEY,
                client_name TEXT,
                tier TEXT,
                requests_per_hour INTEGER
            )',
            'CREATE TABLE sl_rl_api_requests (
                id INTEGER PRIMARY KEY,
                client_id INTEGER,
                endpoint TEXT,
                requested_at TEXT,
            ...
            'CREATE TABLE sl_rl_rate_limit_overrides (
                id INTEGER PRIMARY KEY,
                client_id INTEGER,
                endpoint TEXT,
    ...
    {
        return ['sl_rl_rate_limit_overrides', 'sl_rl_api_requests', 'sl_rl_api_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 3 api clients
        $this->pdo->exec("INSERT INTO sl_rl_api_clients VALUES (1, 'acme-api', 'basic', 100)");
        $this->pdo->exec("INSERT INTO sl_rl_api_clients VALUES (2, 'bigcorp', 'pro', 500)");
        $this->pdo->exec("INSERT INTO sl_rl_api_clients VALUES (3, 'enterprise-co', 'enterprise', 2000)");

        // 10 api requests spanning a 1-hour window
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (1, 1, '/users', '2026-03-09 10:05:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (2, 1, '/users', '2026-03-09 10:10:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (3, 1, '/upload', '2026-03-09 10:12:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (4, 2, '/export', '2026-03-09 10:01:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (5, 2, '/export', '2026-03-09 10:02:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (6, 2, '/users', '2026-03-09 10:15:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (7, 3, '/users', '2026-03-09 10:20:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (8, 3, '/users', '2026-03-09 10:21:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (9, 3, '/data', '2026-03-09 10:22:00', 200)");
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (10, 1, '/users', '2026-03-09 10:03:00', 429)");

        // 2 rate limit overrides
        $this->pdo->exec("INSERT INTO sl_rl_rate_limit_overrides VALUES (1, 1, '/upload', 50)");
        $this->pdo->exec("INSERT INTO sl_rl_rate_limit_overrides VALUES (2, 2, '/export', 200)");
    }
    ...
    public function testRateLimitCheck(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, COUNT(*) AS req_count, c.requests_per_hour
             FROM sl_rl_api_clients c
             JOIN sl_rl_api_requests r ON r.client_id = c.id
    ...
    public function testOverrideApplied(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.client_name, r.endpoint,
             ...
             FROM sl_rl_api_clients c
             JOIN sl_rl_api_requests r ON r.client_id = c.id
             LEFT JOIN sl_rl_rate_limit_overrides o
    ...
    {
        $this->pdo->exec("INSERT INTO sl_rl_api_requests VALUES (11, 1, '/health', '2026-03-09 10:30:00', 200)");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_rl_api_requests");
        $this->assertSame(11, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT endpoint FROM sl_rl_api_requests WHERE id = 11");
        $this->assertSame('/health', $rows[0]['endpoint']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_rl_api_requests")->fetchAll(PDO::FETCH_ASSOC);
