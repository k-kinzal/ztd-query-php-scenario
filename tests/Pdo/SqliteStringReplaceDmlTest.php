...
/**
 * Tests UPDATE SET with replace() string function on SQLite via PDO.
 *
 * @spec SPEC-4.2
 */
class SqliteStringReplaceDmlTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return 'CREATE TABLE sl_srd_pages (
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_srd_pages VALUES (1, 'http://old.example.com/page1', 'Page One', 'Visit http://old.example.com for details')");
        $this->pdo->exec("INSERT INTO sl_srd_pages VALUES (2, 'http://old.example.com/page2', 'Page Two', 'Contact us at http://old.example.com/contact')");
        $this->pdo->exec("INSERT INTO sl_srd_pages VALUES (3, 'http://other.com/page3', 'Page Three', 'No old links here')");
    }
    ...
    public function testUpdateSetReplace(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE sl_srd_pages SET url = replace(url, 'old.example.com', 'new.example.com')"
            );

            $rows = $this->ztdQuery("SELECT id, url FROM sl_srd_pages ORDER BY id");
    ...
    public function testUpdateSetReplaceWithWhere(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE sl_srd_pages SET body = replace(body, 'http://old.example.com', 'https://new.example.com') WHERE id <= 2"
            );

            $rows = $this->ztdQuery("SELECT id, body FROM sl_srd_pages ORDER BY id");
            $this->assertCount(3, $rows);
    ...
    public function testUpdateSetNestedReplace(): void
    {
        try {
            $this->pdo->exec(
                "UPDATE sl_srd_pages SET url = replace(replace(url, 'http://', 'https://'), 'old.example.com', 'new.example.com') WHERE id = 1"
            );

            $rows = $this->ztdQuery("SELECT url FROM sl_srd_pages WHERE id = 1");
    ...
    public function testPreparedUpdateSetReplace(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "UPDATE sl_srd_pages SET url = replace(url, ?, ?) WHERE id = ?"
            );
