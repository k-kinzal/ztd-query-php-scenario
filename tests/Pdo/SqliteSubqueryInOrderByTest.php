...
/**
 * Tests subqueries in ORDER BY clause via SQLite PDO.
 *
 ...
 * @spec SPEC-3.1
 */
class SqliteSubqueryInOrderByTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_soob_authors (
                id INTEGER PRIMARY KEY,
                name TEXT NOT NULL
            )',
            'CREATE TABLE sl_soob_books (
                id INTEGER PRIMARY KEY,
    ...
    {
        return ['sl_soob_books', 'sl_soob_authors'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_soob_authors VALUES (1, 'Alice')");
        $this->pdo->exec("INSERT INTO sl_soob_authors VALUES (2, 'Bob')");
        $this->pdo->exec("INSERT INTO sl_soob_authors VALUES (3, 'Charlie')");

        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (1, 1, 'Book A1', 5)");
        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (2, 1, 'Book A2', 3)");
        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (3, 2, 'Book B1', 4)");
        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (4, 3, 'Book C1', 2)");
        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (5, 3, 'Book C2', 5)");
        $this->pdo->exec("INSERT INTO sl_soob_books VALUES (6, 3, 'Book C3', 1)");
    }
    ...
    public function testOrderByCorrelatedSubqueryCount(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT a.id, a.name
                 FROM sl_soob_authors a
                 ORDER BY (SELECT COUNT(*) FROM sl_soob_books b WHERE b.author_id = a.id) DESC, a.id"
    ...
    public function testOrderBySubqueryMaxRating(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT a.id, a.name
                 FROM sl_soob_authors a
                 ORDER BY (SELECT MAX(b.rating) FROM sl_soob_books b WHERE b.author_id = a.id) DESC, a.id"
            )->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        try {
            $this->pdo->exec("INSERT INTO sl_soob_authors VALUES (4, 'Dave')");
            $this->pdo->exec("INSERT INTO sl_soob_books VALUES (7, 4, 'Book D1', 5)");
            $this->pdo->exec("INSERT INTO sl_soob_books VALUES (8, 4, 'Book D2', 5)");
            $this->pdo->exec("INSERT INTO sl_soob_books VALUES (9, 4, 'Book D3', 5)");
            $this->pdo->exec("INSERT INTO sl_soob_books VALUES (10, 4, 'Book D4', 5)");
    ...
    public function testOrderBySelfReferenceSubquery(): void
    {
        try {
            $rows = $this->pdo->query(
                "SELECT b.id, b.title
                 FROM sl_soob_books b
                 ORDER BY (SELECT COUNT(*) FROM sl_soob_books b2 WHERE b2.author_id = b.author_id) DESC, b.id"
            )->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testPreparedOrderBySubquery(): void
    {
        try {
            $stmt = $this->pdo->prepare(
                "SELECT a.id, a.name
                 FROM sl_soob_authors a
                 ORDER BY (SELECT COUNT(*) FROM sl_soob_books b WHERE b.author_id = a.id AND b.rating >= ?) DESC, a.id"
            );
            $stmt->execute([4]);
    ...
    {
        $this->pdo->query(
            "SELECT a.id FROM sl_soob_authors a
             ORDER BY (SELECT COUNT(*) FROM sl_soob_books b WHERE b.author_id = a.id) DESC"
        )->fetchAll(PDO::FETCH_ASSOC);

        $this->pdo->disableZtd();
        $rawCount = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_soob_authors")
            ->fetchAll(PDO::FETCH_ASSOC);
