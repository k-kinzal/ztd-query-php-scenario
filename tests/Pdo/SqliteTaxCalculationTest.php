 ...
 * using BETWEEN, CASE, JOIN, SUM, and GROUP BY (SQLite PDO).
 * @spec SPEC-10.2.118
 */
class SqliteTaxCalculationTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_tc_tax_jurisdictions (
                id INTEGER PRIMARY KEY,
                name TEXT,
                country TEXT
            )',
            'CREATE TABLE sl_tc_tax_rules (
                id INTEGER PRIMARY KEY,
                jurisdiction_id INTEGER,
                category TEXT,
                rate_percent REAL
            )',
            'CREATE TABLE sl_tc_order_items (
                id INTEGER PRIMARY KEY,
                jurisdiction_id INTEGER,
                category TEXT,
                item_name TEXT,
    ...
    {
        return ['sl_tc_order_items', 'sl_tc_tax_rules', 'sl_tc_tax_jurisdictions'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Jurisdictions (3)
        $this->pdo->exec("INSERT INTO sl_tc_tax_jurisdictions VALUES (1, 'California', 'US')");
        $this->pdo->exec("INSERT INTO sl_tc_tax_jurisdictions VALUES (2, 'Ontario', 'CA')");
        $this->pdo->exec("INSERT INTO sl_tc_tax_jurisdictions VALUES (3, 'London', 'UK')");

        // Tax rules (12: 4 categories x 3 jurisdictions)
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (1,  1, 'electronics', 8.25)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (2,  1, 'food',        0.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (3,  1, 'clothing',    8.25)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (4,  1, 'services',    7.50)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (5,  2, 'electronics', 13.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (6,  2, 'food',        0.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (7,  2, 'clothing',    13.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (8,  2, 'services',    13.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (9,  3, 'electronics', 20.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (10, 3, 'food',        0.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (11, 3, 'clothing',    20.00)");
        $this->pdo->exec("INSERT INTO sl_tc_tax_rules VALUES (12, 3, 'services',    20.00)");

        // Order items (8)
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (1, 1, 'electronics', 'Laptop',      999.99)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (2, 1, 'food',        'Groceries',   45.50)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (3, 1, 'clothing',    'Jacket',      120.00)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (4, 2, 'electronics', 'Phone',       799.00)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (5, 2, 'food',        'Maple Syrup', 15.00)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (6, 3, 'electronics', 'Tablet',      500.00)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (7, 3, 'clothing',    'Coat',        200.00)");
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (8, 3, 'services',    'Consulting',  1000.00)");
    }
    ...
    public function testTaxPerItem(): void
    {
        $rows = $this->ztdQuery(
            "SELECT oi.item_name,
             ...
             FROM sl_tc_order_items oi
             JOIN sl_tc_tax_rules tr
        ...
        $rows = $this->ztdQuery(
            "SELECT j.name,
             ...
             JOIN sl_tc_tax_rules tr
               ON tr.jurisdiction_id = oi.jurisdiction_id
              AND tr.category = oi.category
             JOIN sl_tc_tax_jurisdictions j
    ...
    public function testAverageTaxRateByCountry(): void
    {
        $rows = $this->ztdQuery(
            "SELECT j.country,
             ...
             FROM sl_tc_tax_jurisdictions j
             JOIN sl_tc_tax_rules tr
    ...
    {
        $this->pdo->exec("INSERT INTO sl_tc_order_items VALUES (9, 1, 'services', 'Repair', 250.00)");
        $this->pdo->exec("UPDATE sl_tc_tax_rules SET rate_percent = 10.00 WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_tc_order_items");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT rate_percent FROM sl_tc_tax_rules WHERE id = 1");
        $this->assertEqualsWithDelta(10.00, (float) $rows[0]['rate_percent'], 0.01);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_tc_order_items")->fetchAll(PDO::FETCH_ASSOC);
