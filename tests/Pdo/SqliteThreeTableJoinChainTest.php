 ...
 * @spec SPEC-3.3
 */
class SqliteThreeTableJoinChainTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_ttj_authors (id INTEGER PRIMARY KEY, name TEXT)',
            'CREATE TABLE sl_ttj_books (id INTEGER PRIMARY KEY, author_id INTEGER, title TEXT, genre TEXT)',
            'CREATE TABLE sl_ttj_reviews (id INTEGER PRIMARY KEY, book_id INTEGER, rating INTEGER, reviewer TEXT)',
        ];
    }

    protected function getTableNames(): array
    {
        return ['sl_ttj_reviews', 'sl_ttj_books', 'sl_ttj_authors'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_ttj_authors (id, name) VALUES (1, 'Tolkien'), (2, 'Asimov'), (3, 'Bradbury')");
        $this->pdo->exec("INSERT INTO sl_ttj_books (id, author_id, title, genre) VALUES
            (1, 1, 'The Hobbit', 'Fantasy'), (2, 1, 'LOTR', 'Fantasy'),
            (3, 2, 'Foundation', 'SciFi'), (4, 3, 'Fahrenheit 451', 'SciFi')");
        $this->pdo->exec("INSERT INTO sl_ttj_reviews (id, book_id, rating, reviewer) VALUES
    ...
    public function testThreeTableInnerJoin(): void
    {
        $rows = $this->ztdQuery(
            'SELECT a.name AS author, b.title, r.rating
             FROM sl_ttj_authors a
             JOIN sl_ttj_books b ON b.author_id = a.id
             JOIN sl_ttj_reviews r ON r.book_id = b.id
    ...
    public function testThreeTableLeftJoin(): void
    {
        $rows = $this->ztdQuery(
            'SELECT a.name, b.title, COUNT(r.id) AS reviews
             FROM sl_ttj_authors a
             LEFT JOIN sl_ttj_books b ON b.author_id = a.id
             LEFT JOIN sl_ttj_reviews r ON r.book_id = b.id
    ...
    public function testThreeTableAfterMutations(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_ttj_books (id, author_id, title, genre) VALUES (5, 3, 'The Martian Chronicles', 'SciFi')");
        $this->pdo->exec("INSERT INTO sl_ttj_reviews (id, book_id, rating, reviewer) VALUES (7, 5, 4, 'Grace')");
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM sl_ttj_authors');
        $this->assertSame(0, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM sl_ttj_books');
        $this->assertSame(0, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery('SELECT COUNT(*) AS cnt FROM sl_ttj_reviews');
        $this->assertSame(0, (int) $rows[0]['cnt']);
    }
}
