...
/**
 * Tests a time-tracking workflow through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.80
 */
class SqliteTimeTrackingTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_tt_clients (
                id INTEGER PRIMARY KEY,
                name TEXT,
                rate_per_hour REAL
            )',
            'CREATE TABLE sl_tt_projects (
                id INTEGER PRIMARY KEY,
                client_id INTEGER,
                name TEXT,
                budget_hours REAL
            )',
            'CREATE TABLE sl_tt_time_entries (
                id INTEGER PRIMARY KEY,
                project_id INTEGER,
                employee_name TEXT,
                hours REAL,
    ...
    {
        return ['sl_tt_time_entries', 'sl_tt_projects', 'sl_tt_clients'];
    }
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_tt_clients VALUES (1, 'Acme Corp', 150.00)");
        $this->pdo->exec("INSERT INTO sl_tt_clients VALUES (2, 'Globex Inc', 200.00)");

        $this->pdo->exec("INSERT INTO sl_tt_projects VALUES (1, 1, 'Website Redesign', 40.00)");
        $this->pdo->exec("INSERT INTO sl_tt_projects VALUES (2, 1, 'Mobile App', 100.00)");
        $this->pdo->exec("INSERT INTO sl_tt_projects VALUES (3, 2, 'Data Migration', 20.00)");

        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (1, 1, 'Alice', 8.00, '2026-03-02 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (2, 1, 'Alice', 6.50, '2026-03-03 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (3, 1, 'Bob', 7.00, '2026-03-02 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (4, 1, 'Bob', 2.00, '2026-03-04 09:00:00', 0)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (5, 2, 'Alice', 5.00, '2026-03-02 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (6, 2, 'Charlie', 8.00, '2026-03-03 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (7, 3, 'Charlie', 10.00, '2026-03-02 09:00:00', 1)");
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (8, 3, 'Charlie', 12.00, '2026-03-03 09:00:00', 1)");
    }
    ...
    public function testBillableHoursByProject(): void
    {
        $rows = $this->ztdQuery(
            "SELECT p.name AS project_name, SUM(te.hours) AS total_billable
             FROM sl_tt_time_entries te
             JOIN sl_tt_projects p ON p.id = te.project_id
             WHERE te.billable = 1
    ...
    public function testClientInvoiceSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT c.name AS client_name,
             ...
             FROM sl_tt_time_entries te
             JOIN sl_tt_projects p ON p.id = te.project_id
             JOIN sl_tt_clients c ON c.id = p.client_id
             WHERE te.billable = 1
    ...
    public function testAddTimeEntry(): void
    {
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM sl_tt_time_entries WHERE project_id = 2 AND billable = 1"
        );
        $this->assertEquals(13.00, (float) $before[0]['total']);

        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (9, 2, 'Diana', 4.00, '2026-03-04 09:00:00', 1)");
    ...
    {
        $before = $this->ztdQuery(
            "SELECT SUM(hours) AS total FROM sl_tt_time_entries WHERE project_id = 1 AND billable = 1"
        );
        $this->assertEquals(21.50, (float) $before[0]['total']);

        $affected = $this->pdo->exec("UPDATE sl_tt_time_entries SET billable = 0 WHERE id = 1");
    ...
    {
        $this->pdo->exec("INSERT INTO sl_tt_time_entries VALUES (9, 1, 'Eve', 3.00, '2026-03-05 09:00:00', 1)");
        $this->pdo->exec("UPDATE sl_tt_time_entries SET billable = 0 WHERE id = 2");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_tt_time_entries");
        $this->assertEquals(9, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_tt_time_entries')->fetchAll(PDO::FETCH_ASSOC);
