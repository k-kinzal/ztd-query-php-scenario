...
/**
 * Tests a tenant usage metering scenario through ZTD shadow store (SQLite PDO).
 ...
 * @spec SPEC-10.2.156
 */
class SqliteUsageMeteringTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_um_tenants (
                id INTEGER PRIMARY KEY,
                name TEXT,
                plan_name TEXT,
                monthly_quota INTEGER
            )',
            'CREATE TABLE sl_um_usage_records (
                id INTEGER PRIMARY KEY,
                tenant_id INTEGER,
                endpoint TEXT,
                recorded_at TEXT,
            ...
            'CREATE TABLE sl_um_overage_charges (
                id INTEGER PRIMARY KEY,
                tenant_id INTEGER,
                month TEXT,
    ...
    {
        return ['sl_um_overage_charges', 'sl_um_usage_records', 'sl_um_tenants'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Tenants
        $this->pdo->exec("INSERT INTO sl_um_tenants VALUES (1, 'Acme Corp', 'pro', 10000)");
        $this->pdo->exec("INSERT INTO sl_um_tenants VALUES (2, 'Beta Inc', 'starter', 1000)");
        $this->pdo->exec("INSERT INTO sl_um_tenants VALUES (3, 'Gamma LLC', 'pro', 10000)");
        $this->pdo->exec("INSERT INTO sl_um_tenants VALUES (4, 'Delta Co', 'enterprise', 50000)");

        // Usage records
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (1, 1, '/api/users', '2026-01-15', 1500)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (2, 1, '/api/users', '2026-01-20', 1200)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (3, 1, '/api/orders', '2026-01-18', 800)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (4, 1, '/api/orders', '2026-01-25', 900)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (5, 2, '/api/users', '2026-01-10', 600)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (6, 2, '/api/users', '2026-01-22', 500)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (7, 3, '/api/users', '2026-01-12', 2000)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (8, 3, '/api/users', '2026-01-28', 1800)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (9, 3, '/api/orders', '2026-01-16', 1500)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (10, 3, '/api/orders', '2026-01-30', 1200)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (11, 4, '/api/users', '2026-01-05', 5000)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (12, 4, '/api/users', '2026-01-19', 4500)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (13, 4, '/api/orders', '2026-01-10', 3000)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (14, 4, '/api/orders', '2026-01-26', 2500)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (15, 1, '/api/users', '2026-02-05', 1600)");
        $this->pdo->exec("INSERT INTO sl_um_usage_records VALUES (16, 2, '/api/users', '2026-02-12', 700)");

        // Overage charges
        $this->pdo->exec("INSERT INTO sl_um_overage_charges VALUES (1, 2, '2026-01', 100, 10.00)");
    }
    ...
    public function testMonthlyUsageSummary(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name, SUBSTR(u.recorded_at, 1, 7) AS month,
             ...
             FROM sl_um_tenants t
             JOIN sl_um_usage_records u ON u.tenant_id = t.id
     ...
     * total_used, quota, utilization_pct (ROUND with CAST to REAL for division),
    ...
    public function testOverageChargeJoin(): void
    {
        $rows = $this->ztdQuery(
            "SELECT t.name,
             ...
             FROM sl_um_tenants t
             LEFT JOIN sl_um_overage_charges oc ON oc.tenant_id = t.id AND oc.month = '2026-01'
             ORDER BY t.name"
        );
    ...
    public function testPhysicalIsolation(): void
    {
        ...
        $this->pdo->exec(
            "INSERT INTO sl_um_usage_records VALUES (17, 1, '/api/users', '2026-02-10', 500)"
        );

        // ZTD sees the new record (16 original + 1 new = 17)
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_um_usage_records");
        $this->assertEquals(17, (int) $rows[0]['cnt']);

        // Physical table untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_um_usage_records')->fetchAll(PDO::FETCH_ASSOC);
