 ...
 * and date gap analysis (SQLite PDO).
 * @spec SPEC-10.2.116
 */
class SqliteUserActivityStreakTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_uas_users (
                id INTEGER PRIMARY KEY,
                username TEXT
            )',
            'CREATE TABLE sl_uas_activity_log (
                id INTEGER PRIMARY KEY,
                user_id INTEGER,
                activity_date TEXT,
                activity_type TEXT
    ...
    {
        return ['sl_uas_activity_log', 'sl_uas_users'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // Users
        $this->pdo->exec("INSERT INTO sl_uas_users VALUES (1, 'alice')");
        $this->pdo->exec("INSERT INTO sl_uas_users VALUES (2, 'bob')");

        // Activity log: alice has a 4-day streak, a gap, then a 2-day streak
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (1, 1, '2026-03-01', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (2, 1, '2026-03-02', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (3, 1, '2026-03-03', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (4, 1, '2026-03-04', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (5, 1, '2026-03-07', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (6, 1, '2026-03-08', 'login')");

        // Activity log: bob has a 3-day streak
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (7, 2, '2026-03-02', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (8, 2, '2026-03-03', 'login')");
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (9, 2, '2026-03-04', 'login')");
    }
    ...
    {
        $this->pdo->exec("INSERT INTO sl_uas_activity_log VALUES (10, 1, '2026-03-09', 'login')");
        $this->pdo->exec("UPDATE sl_uas_users SET username = 'alice_updated' WHERE id = 1");

        // ZTD sees changes
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_uas_activity_log");
        $this->assertEquals(10, (int) $rows[0]['cnt']);

        $rows = $this->ztdQuery("SELECT username FROM sl_uas_users WHERE id = 1");
        $this->assertSame('alice_updated', $rows[0]['username']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query("SELECT COUNT(*) AS cnt FROM sl_uas_activity_log")->fetchAll(PDO::FETCH_ASSOC);
