...
/**
 * Tests a waitlist queue management workflow through ZTD shadow store (SQLite PDO).
 * Covers priority ordering, promotion, capacity limits, position calculations
 * via window functions, and physical isolation.
 * @spec SPEC-10.2.68
 */
class SqliteWaitlistQueueTest extends AbstractSqlitePdoTestCase
{
    protected function getTableDDL(): string|array
    {
        return [
            'CREATE TABLE sl_wq_events (
                id INTEGER PRIMARY KEY,
                name TEXT,
                capacity INTEGER,
                enrolled_count INTEGER
            )',
            'CREATE TABLE sl_wq_waitlist (
                id INTEGER PRIMARY KEY,
                event_id INTEGER,
                person_name TEXT,
                priority INTEGER,
    ...
    {
        return ['sl_wq_waitlist', 'sl_wq_events'];
    }
    ...
    protected function setUp(): void
    {
        ...
        // 2 events
        $this->pdo->exec("INSERT INTO sl_wq_events VALUES (1, 'Workshop A', 3, 2)");
        $this->pdo->exec("INSERT INTO sl_wq_events VALUES (2, 'Workshop B', 5, 0)");

        // 6 waitlist entries for event 1
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (1, 1, 'Alice',   2, 'waiting',   '2026-03-10 10:00:00')");
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (2, 1, 'Bob',     1, 'waiting',   '2026-03-10 10:30:00')");
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (3, 1, 'Charlie', 2, 'waiting',   '2026-03-10 11:00:00')");
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (4, 1, 'Diana',   3, 'waiting',   '2026-03-10 09:00:00')");
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (5, 1, 'Eve',     1, 'waiting',   '2026-03-10 11:30:00')");
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (6, 1, 'Frank',   2, 'cancelled', '2026-03-10 10:15:00')");
    }
    ...
    public function testJoinWaitlist(): void
    {
        ...
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (7, 1, 'Grace', 2, 'waiting', '2026-03-10 12:00:00')");
    ...
    public function testPromoteFromWaitlist(): void
    {
        ...
        // Promote Bob
        $affected = $this->pdo->exec("UPDATE sl_wq_waitlist SET status = 'promoted' WHERE id = " . (int) $top[0]['id']);
        $this->assertSame(1, $affected);

        // Increment enrolled count
        $affected = $this->pdo->exec("UPDATE sl_wq_events SET enrolled_count = enrolled_count + 1 WHERE id = 1");
        $this->assertSame(1, $affected);

        // Verify enrolled count is now 3 (was 2)
        $event = $this->ztdQuery("SELECT enrolled_count, capacity FROM sl_wq_events WHERE id = 1");
        $this->assertEquals(3, (int) $event[0]['enrolled_count']);
        $this->assertEquals(3, (int) $event[0]['capacity']);

        // Verify Bob is promoted
        $rows = $this->ztdQuery("SELECT status FROM sl_wq_waitlist WHERE id = 2");
        $this->assertSame('promoted', $rows[0]['status']);

        // Remaining waiting list should not include Bob
        $waiting = $this->ztdQuery(
            "SELECT person_name FROM sl_wq_waitlist WHERE event_id = 1 AND status = 'waiting' ORDER BY priority, joined_at"
        );
    ...
    public function testCancelAndShiftPositions(): void
    {
        ...
        $affected = $this->pdo->exec("UPDATE sl_wq_waitlist SET status = 'cancelled' WHERE id = 1");
        $this->assertSame(1, $affected);
    ...
    public function testCapacityGuard(): void
    {
        ...
        $this->pdo->exec("UPDATE sl_wq_events SET enrolled_count = 3 WHERE id = 1");
        ...
        $waiting = $this->ztdQuery(
            "SELECT COUNT(*) AS cnt FROM sl_wq_waitlist WHERE event_id = 1 AND status = 'waiting'"
        );
        ...
        $promotable = $this->ztdQuery(
            "SELECT w.person_name
             FROM sl_wq_waitlist w
             JOIN sl_wq_events e ON e.id = w.event_id
             WHERE w.event_id = 1
    ...
    public function testPhysicalIsolation(): void
    {
        // Mutate through ZTD
        $this->pdo->exec("INSERT INTO sl_wq_waitlist VALUES (7, 1, 'Grace', 2, 'waiting', '2026-03-10 12:00:00')");
        $this->pdo->exec("UPDATE sl_wq_waitlist SET status = 'promoted' WHERE id = 2");
        $this->pdo->exec("UPDATE sl_wq_events SET enrolled_count = enrolled_count + 1 WHERE id = 1");

        // Visible through ZTD
        $rows = $this->ztdQuery("SELECT COUNT(*) AS cnt FROM sl_wq_waitlist");
        $this->assertSame(7, (int) $rows[0]['cnt']);

        $event = $this->ztdQuery("SELECT enrolled_count FROM sl_wq_events WHERE id = 1");
        $this->assertEquals(3, (int) $event[0]['enrolled_count']);

        // Physical tables untouched
        $this->pdo->disableZtd();
        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_wq_waitlist')->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);

        $rows = $this->pdo->query('SELECT COUNT(*) AS cnt FROM sl_wq_events')->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEquals(0, (int) $rows[0]['cnt']);
    }
}
